...
uses
  ...
  ImgList, JvComponent, JvAppEvent, JvMenus, Mask, JvMaskEdit,
  JvSpin, JvCombobox, JvColorCombo, JvSpeedBar, JvRichEdit, JvClipboardMonitor;
...
uses
  JvGIF, JvJVCLUtils, ExtDlgs, {$IFNDEF CBUILDER} Jpeg, {$ENDIF}
  RichEdit, ShellAPI, Math, ParaFmt;
...
function TMainForm.CurrText: TJvTextAttributes;
begin
...
begin
//  WinAbout('RichEdit 2.0 Control Demo', 'JVCL Library Demo Program');
  FocusEditor;
end;
...
begin
  ...
  begin
    StartWait;
    try
      Res := TResourceStream.Create(HInstance, 'RX_RTF', 'RTF');
      try
...
begin
  ...
  with TJvPopupMenu(Sender).Canvas do begin
