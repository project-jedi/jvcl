...
begin
  inherited Create; 
  for t := 0 to DEFAULT_PEN_COUNT do
...
{ SLOW Version }
  for I := 0 to Self.FValueCount - 2 do
  begin
    FData[I] := FData[I+1]; 
  ...
  {for I := 0 to Length(FData[FValueCount-1])-1 do
  begin
...
begin
  Result := psSolid; 
  if (Index < 0) or (Index >= Length(FPenStyles)) then
    Exit; 
...
begin
  ...
  begin
    ...
    Y := Round(yOrigin - (Options.YPixelGap * ((I) - Options.YOrigin)));
    if (i > 0) and (i < (Options.YValueCount)) and  Options.YAxisDivisionMarkers then
    begin
      ChartCanvas.Pen.Color :=  Options.GetPenColor(jvChartDivisionLineColorIndex);

      MyDrawDotLine(Options.XStartOffset,
        Y,
        Round(Options.XStartOffset + Options.XPixelGap * (Options.XValueCount + 1))-1,
        Y
       );
    end;
    if I > 0 then
...
var
  ...
  begin
    ...
    if (Options.YGap < 0.0000001) then
    begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          if NanFlag then // resume, valid value.
          begin
            NanFlag := False;
            Y := Round(V);
            // pick up the pen and slide forward
            ChartCanvas.MoveTo(Round(xOrigin + J * Options.XPixelGap), Y);
          end
          else
          begin
            Y := Round(V);
            ChartCanvas.Pen.Style := psSolid;
            MyPenLineTo(Round(xOrigin + J * Options.XPixelGap), Y);
...
//   DoSeparate := False; // not used (ahuser)
  ...
  if Options.XAxisDateTimeMode then { if DateTime mode then legends are painted where the division markers are painted }
  begin
    // if not Options.XAxisDivisionMarkers then Exit;
    ...
    for I := 1 to ((Options.XValueCount div Options.XAxisValuesPerDivision))-1 do begin
        ...
        if Options.FXAxisDateTimeFormat = '' then // not specified, means use Locale defaults
        ...
        if (XLegendHoriz-(ChartCanvas.TextWidth(timestampStr) div 2))> XOverlap then
        begin
  ...
  else if Options.XValueCount > 0 then // is there data to plot?
  begin
  ...
  else if Options.Legend = clChartLegendBelow then // space-saving legend below chart
  begin
    MySmallGraphFont;
...
begin
  if (Y > Options.YStartOffset) and (Y < Options.YStartOffset + Options.YEnd) and
     ...
     (Options.ChartKind <> JvChartPieChart) then
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          FOnChartClick(Self, Button, Shift, X, Y, nMouseValue, nMousePen, 
            FMouseDownShowHint, FMouseDownHintBold, FMouseDownHintStrs); 
        end
        else
        begin
...
begin
  ...
  begin
    if Options.DateTimeFormat = '' then
  ...
  else if Options.XLegends.Count > nMouseValue then
