...
{$ENDIF COMPLIB_CLX}
  public
    constructor Create(AOwner: TComponent); override;
    property PlainItems[Index: Integer]: string read GetPlainItems;
    procedure SetItemHeight(Value: integer);{$IFDEF COMPILER6_UP}override;{$ENDIF} // Kaczkowski
    function GetItemHeight: integer; // Kaczkowski
  published
...
{$ENDIF COMPLIB_VCL}
  ...
  CustomColors = array[1..3] of TColor;

procedure ItemHTDrawEx(Canvas: TCanvas; Rect: TRect;
  const State: TOwnerDrawState; const Text: string;
  const HideSelColor: Boolean; var Width: Integer; CalcWidth: Boolean;
  Highlight: TColor = clHighlight;
  HighlightText: TColor = clHighlightText);
...
var
  W: Integer;
begin
  ItemHTDrawEx(Canvas, Rect, State, Text, HideSelColor, W, True);
  Result := W;
end;
...
begin
  ...
  ItemHTDrawEx(Canvas, Rect, State, Items[Index], FHideSel, W, False,
    FSelectedColor, FSelectedTextColor);
//  ItemHTDraw(Canvas, Rect, State, Items[Index], FHideSel);
  SendMessage(Handle, LB_SETITEMHEIGHT, Index, ItemHTHeight(Canvas, Items[Index])); // Kaczkowski
end;
{$ENDIF COMPLIB_VCL}
{$IFDEF COMPLIB_CLX}
...
begin
  ItemHTDrawEx(Canvas, Rect, State, Items[Index], FHideSel, W, False,
    FSelectedColor, FSelectedTextColor);
  SendMessage(Handle, CB_SETITEMHEIGHT, Index, ItemHTHeight(Canvas, Items[Index])); // Kaczkowski
end;
{$ENDIF COMPLIB_VCL}
...
function TJvHTComboBox.GetItemHeight: integer;
begin
  Result := SendMessage(Handle, CB_GETITEMHEIGHT, -1, 0)
end;

procedure TJvHTComboBox.SetItemHeight(Value: integer);
begin
