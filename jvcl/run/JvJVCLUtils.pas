...
{pecifies the screen coordinates of a control's windows area is generated by GetControlRect.}
...
begin
  ...
  for I := Low(Conversions) to High(Conversions) do
    with Conversions[I] do
      Result := StringReplace(Result, Html, Utf8ToAnsi(Text), [rfReplaceAll, rfIgnoreCase]);
...
begin
  if AControl = nil then
    raise EInvalidOperation.Create('Control is nil.');
