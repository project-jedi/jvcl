...
{$ENDIF}
   ...
   private
    ...
    // Property Access Methods
      ...
      procedure SetLinearName(Value : string);
...
{$IFDEF TIMEBLOCKS}
  ...
  // ok
   ...
   public
      ...
      function BlockByName(BlockName : string) : TJvTFDaysTimeBlock;
      function FindBlock(BlockName : string) : TJvTFDaysTimeBlock;
   end;
...
{$ENDIF}
   ...
   private
      ...
      procedure SetSchedName(Value : string);
      procedure SetGroupTitle(Value : string);
      procedure SetTitle(Value : string);
...
{$ENDIF}
      ...
      procedure SetDateFormat(Value : string); override;
...
{$ENDIF}
    ...
    // Fancy painting routines
      ...
      procedure DrawMinor(aCanvas : TCanvas; aRect : TRect; RowNum : Integer;
         LabelStr : string; TickLength : Integer; Selected : Boolean);
...
{$IFDEF TIMEBLOCKS}
    // ok
      ...
      property TimeBlockProps : TJvTFDaysBlockProps read FTimeBlockProps
         write SetTimeBlockProps;
{$ENDIF}
...
{$ENDIF}
   ...
   protected
    ...
    //procedure DrawColHdr(aCanvas: TCanvas; Index: Integer;
      ...
      procedure DrawMinor(aCanvas : TCanvas; aRect : TRect; RowNum : Integer;
         LabelStr : string; TickLength : Integer);
...
procedure DrawTxt(aCanvas : TCanvas; aRect : TRect;
   Txt : string; HAlign : TAlignment; VAlign : TJvTFVAlignment);
var
   TxtWidth,
      TxtHeight,
      TxtLeft,
...
procedure TJvTFDaysTemplate.SetLinearName(Value : string);
var
   I : Integer;
begin
...
begin
   if Assigned(FApptGrid) and not (csLoading in FApptGrid.ComponentState) and
      (Value >= EndTime) then
      raise EJvTFDaysError.Create('Invalid PrimeTime StartTime');
...
begin
   if Assigned(FApptGrid) and (Value <= StartTime) and
      ...
      raise EJvTFDaysError.Create('Invalid PrimeTime EndTime');
...
procedure TJvTFDaysCol.SetSchedName(Value : string);
begin
...
procedure TJvTFDaysCol.SetTitle(Value : string);
begin
...
procedure TJvTFDaysCol.SetSchedule(NewSchedName : string; NewSchedDate : TDate);
begin
   Disconnect;
...
procedure TJvTFDaysCol.SetGroupTitle(Value : string);
begin
...
begin
   if (Index < 0) or (Index > Count - 1) then
      raise EJvTFDaysError.Create('Column index out of bounds');
end;
...
begin
   ...
   if Value <> FTopRow then
      ...
      begin
         ...
         raise EJvTFDaysError.Create('Row index out of bounds');
end;
...
procedure TJvTFDays.SetDateFormat(Value : string);
begin
...
begin
  ...
  // let the 'Map col not found' catch the error.
   ...
   begin
      //Cols[Col].DumpMap;
      raise EJvTFDaysError.Create('Map col not found for appointment');
   end;
   ...
   begin
      ...
      //Cols[Col].DumpMap;
      raise EJvTFDaysError.Create('Corrupt appointment map');
   end;
...
procedure TJvTFDays.ReleaseSchedule(SchedName : string;
   SchedDate : TDate);
var
   Used : Boolean;
   I : Integer;
...
begin
   ...
   begin
      if GridGran > BlockGran then
         raise EJvTFBlockGranError.Create('Grid granularity cannot be greater ' +
            'then the time block granularity');

      if (BlockGran mod GridGran) <> 0 then
         raise EJvTFBlockGranError.Create('Time block granularity must be evenly ' +
      ...
      if (BlockHrs <> GridHrs) or (BlockMins <> GridMins) then
         raise EJvTFBlockGranError.Create('Time blocks must begin exactly on ' +
...
begin
   ...
   if not (csLoading in ComponentState) and (WorkEnd <= GridStartTime) then
      raise EJvTFDaysError.Create('GridEndTime cannot be prior to GridStartTime');
...
begin
   ...
   if not (csLoading in ComponentState) and (Value >= WorkEnd) then
      raise EJvTFDaysError.Create('GridStartTime cannot be after GridEndTime');
...
procedure TJvTFDaysPrinter.DrawMinor(aCanvas : TCanvas; aRect : TRect;
   RowNum : Integer; LabelStr : string; TickLength : Integer);
var
   MinorRect,
      TxtRect : TRect;
   PTxt : PChar;
...
begin
   if RowNum >= RowCount then
      raise EJvTFPrinterError.Create('Invalid row (' + IntToStr(RowNum) + ')');
end;
...
begin
  ...
  // Printer bug, fixed
   ...
   begin
      //Cols[Col].FMap.Dump('corrupt dump.txt');  !!! FOR DEBUGGING ONLY !!!!
      raise EJvTFPrinterError.Create('Corrupt appointment map');
   end;
...
begin
   ...
   try
      ...
      if FPageInfoList.Count = 0 then
         raise EJvTFPrinterError.Create('There is no data to print');
...
begin
   if not FValidPageInfo then
      raise EJvTFPrinterError.Create('No page info exists.  ' +
...
begin
   if Value = '' then
      raise EJvTFDaysError.Create('A time block name cannot be null');
   ...
   if Value <> FName then
      ...
      begin
         ...
         raise EJvTFDaysError.Create('Another time block with the name ' +
...
function TJvTFDaysTimeBlocks.BlockByName(BlockName : string) : TJvTFDaysTimeBlock;
begin
   Result := FindBlock(BlockName);
   if not Assigned(Result) then
      raise EJvTFDaysError.Create('A time block with the name "' + BlockName +
         ' does not exist');
end;
...
function TJvTFDaysTimeBlocks.FindBlock(BlockName : string) : TJvTFDaysTimeBlock;
var
   I : Integer;
begin
   Result := nil;
