...
//      Dec(ParserIndex, LineClean(EndifToken.StartIndex - 1));
        ...
        Dec(ParserIndex, Cond.ElseEndIndex - Cond.ElseStartIndex + 1);
        // replace {$ELSE}..{$ENDIF} with ''
        Parser.ReplaceParseNext(Cond.ElseStartIndex, Cond.ElseEndIndex - Cond.ElseStartIndex + 1, '');
  ...
  begin
    ...
    begin
      // {$IFDEF x} blabla {$ENDIF}  or
      // {$IFNDEF x} blabla {$ENDIF}
      ...
      begin
        ...
        // replace {$IFDEF}/{$IFNDEF}..{$ENDIF} with ''
    ...
    begin
      ...
      begin
        ...
        // replace {$IFDEF}/{$IFNDEF}..{$ELSE} with ''
...
begin
  ...
  if FRemoveConditions.Find('~' + Cond.Condition, Index) then
  begin
    RemoveDefines;
  end
  else if FRemoveConditions.Find(Cond.Condition, Index) then  // "Condition"
