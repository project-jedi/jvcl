...
var
  GDotImageList: TImageList;

//=== Local procedures =======================================================

function DotImageList: TImageList;
var
  Bmp: TBitmap;
begin
  if GDotImageList = nil then
  begin
    GDotImageList:= TImageList.Create(nil);
      Bmp := TBitmap.Create;
      try
        Bmp.LoadFromResourceName(HInstance, 'DOT');
        GDotImageList.AddMasked(Bmp, clFuchsia);
      finally
        Bmp.Free;
      end;
  end;
  Result := GDotImageList;
end;
...
begin
  ...
  ListView.SmallImages := DotImageList;
