...
begin
  ...
  Header.Add('  for (var i = 0;i < document.all.length;i++) {');
  Header.Add('    if (document.all(i).id.indexOf(''FAQ'') == 0) {');
  Header.Add('	    document.all(i).style.display = allvisible ? ''none'':''block'';');
  Header.Add('   }');
  Header.Add(' }');
