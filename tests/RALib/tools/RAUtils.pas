...
uses
  ...
  StdCtrls, ExtCtrls, Dialogs, Menus {$IFDEF Delphi3_Up}, ShlObj{$ENDIF},
  Clipbrd,
  {$IFDEF Delphi3_Up} ActiveX {$ELSE} Ole2 {$ENDIF},
...
const
  Separators : set of char = [#00,' ','-',#13, #10,'.',',','/','\', '#', '"', '''',
    ':','+','%','*','(',')',';','=','{','}','[',']', '{', '}', '<', '>'];
 {const Separators is used in GetWordOnPos, RAUtils.ReplaceSokr and SubWord}
...
begin
  ...
  if not (PropInf^.PropType^.Kind in [tkString, tkLString {$IFDEF Delphi3_Up}, tkWString {$ENDIF Delphi3_Up}]) then
