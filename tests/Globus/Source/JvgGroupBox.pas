      ...
      for i := 0 to Owner.ComponentCount - 1 do
        with TControl(Owner) do
          if (Components[i] is TJvgGroupBox) and
            ...
            if
              (not TJvgGroupBox(Components[i]).Collapsed) and (Components[i] <> self) then
            begin
    ...
    for i := 0 to Owner.ComponentCount - 1 do
      with TControl(Owner) do
        if (Components[i] is TJvgGroupBox) and
          ...
          (Components[i] <> self) then TJvgGroupBox(Components[i]).Collapsed := true;
...
begin
  ...
  begin
    ...
    if (fgoHideChildrenWhenCollapsed in Options) or (fgoSaveChildFocus in Options) then
      for i := 0 to Owner.ComponentCount - 1 do
        with TControl(Owner) do
          if (Components[i] is TControl) and (TControl(Components[i]).Parent = self) then
          begin
...
begin
  if FCaptionTextStyle = Value then exit;
...
begin
  if FTransparentCaption = Value then exit;
