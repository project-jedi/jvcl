...
uses
  ...
  JvgScrollBox, StdCtrls, JvgGroupBox, ExtCtrls, ComCtrls, dsgnintf, ToolWin, ImgList, richEdit,
  JvgPage, Tabs;
...
implementation
uses JvgTypes, JvgUtils, JvgLogicItemEditor, clipbrd;
{$R *.DFM}
...
procedure TJvgLogicsEditor.OnTraceMessage(Sender: TJvgLogics; fStepResult: boolean; const StepResult, ParsedResult, Msg: string);
begin
  mLog.Lines.Add(Msg);
...
begin
  ...
  begin
    FActiveBox.LogicElement.NextElement := TJvgLogicElement(cbNext.items.Objects[cbNext.ItemIndex]);
  end;
...
begin
  ...
  begin
    FActiveBox.LogicElement.NextFalseElement := TJvgLogicElement(cbNextFalse.items.Objects[cbNextFalse.ItemIndex]);
  end;
  UpdateView;
end;
...
procedure TJvgLogicsEditor.pLeftDockOver(Sender: TObject; Source: TDragDockObject; X, Y: Integer; State: TDragState; var Accept: Boolean);
begin
  Accept := true;
end;

procedure TJvgLogicsEditor.pLeftUnDock(Sender: TObject; Client: TControl; NewTarget: TWinControl; var Allow: Boolean);
begin
  (Sender as TWinControl).Height := 6;
end;

procedure TJvgLogicsEditor.pLeftDockDrop(Sender: TObject; Source: TDragDockObject; X, Y: Integer);
begin
...
procedure TJvgLogicsEditor.TabSet1Change(Sender: TObject; NewTab: Integer; var AllowChange: Boolean);
begin
