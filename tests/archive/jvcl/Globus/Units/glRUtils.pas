unit glRUtils;//  TRichEdit Wrapping functuins
...
const
  ...
  begin
    ...
    if memoText[Pos]=' ' then
      repeat inc(Pos); until (Pos>length(memoText))or(memoText[Pos]<>' ');
    Pos2:=Pos; if fTrimleft then Pos1:=Pos;
    repeat inc(Pos2); until (Pos2>length(memoText))or(memoText[Pos2]=' ')or(memoText[Pos2]='''')or(memoText[Pos2]='"')or(memoText[Pos2] = chr($0D));
...
begin
  ...
  begin
    ...
    if ((memoText[Pos1] = '''')or(memoText[Pos1] = '"')) and (LastLexem.Comma._start <> -1) then
    begin
      New(PL);
      Lexems.Add(PL);
      PL^._type := ltComma;
    ...
    if (memoText[Pos1] = '<') then
      LastLexem.Tag._start := pos1-1 else
  ...
  for i := 0 to Lexems.Count-1 do with PLexem(Lexems[i])^ do
  if _type = ltTag then
  begin
  ...
  memo.SelLength := 0;
    ...
    for i := 0 to Lexems.Count-1 do Dispose(Lexems[i]);
    Lexems.Free;
  end;
end;
