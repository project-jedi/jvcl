...
uses
  ...
  glUtils, ExtCtrls, grids, stdctrls, forms;
...
TglGridCellStyle = record
  ...
  TglOnGetCellGradientParamsEvent = procedure (Sender: TObject; ACol, ARow: longint;
                                               var CellRect: TRect;
                                               var Gradient: TGradient) of object;

  TglStringGrid = class(TStringGrid)
  private
...
//    procedure DblClick; dynamic;
    ...
    procedure GetCellGradientParams(Sender: TObject; ACol, ARow: longint; var CellRect: TRect; var Gradient: TGradient); virtual;
  public
//    property InplaceEditor;
  ...
  published
    ...
    property OnGetCellGradientParams: TglOnGetCellGradientParamsEvent read FOnGetCellGradientParams write FOnGetCellGradientParams;
  end;
...
begin
  ...
  if Assigned(Gradient) then Gradient.Free;
  inherited;
end;

procedure TglStringGrid.Loaded;
begin
  inherited;
...
procedure TglStringGrid.DrawCell(ACol, ARow: Longint; ARect: TRect; AState: TGridDrawState);
const
...
//        dec(R.Left);
    ...
    begin
      if Gradient = nil then Gradient := TGradient.Create;
...
procedure TglStringGrid.SetCaptionTextAlignment(Value: TAlignment);
begin FCaptionTextAlignment := Value; Repaint; end;

procedure TglStringGrid.SetCaptFont(Value: TFont);
begin FCaptFont.Assign(Value); Repaint; end;

function  TglStringGrid.GetBitmap: TBitmap;
begin
  if not Assigned(FBitmap) then FBitmap := TBitmap.Create;
  Result := FBitmap;
end;

procedure TglStringGrid.SetBitmap(Value: TBitmap);
begin
  if Assigned(FBitmap) then FBitmap.Free;
...
procedure TglStringGrid.SetImage(Value: TImage);
begin
  FImage := Value;
  if Assigned(FImage)and Assigned(FImage.Picture)and Assigned(FImage.Picture.Bitmap) then
...
procedure TglStringGrid.SetCaptions(Value: TStringList);
begin
...
//-------------------------------------------------------------------------------
procedure TglStringGrid.MouseMove(Shift: TShiftState; X, Y: Integer);
var
  R: TRect;
  ACol, ARow: longint;
begin
...
procedure TglStringGrid.CMMouseLeave(var Message: TMessage);
var
  R: TRect;
begin
  inherited;
...
procedure TglStringGrid.MouseDown(Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
var
...
procedure TglStringGrid.ShowEditorAtCell(X, Y: Longint);
var
  R: TRect;
  GridRect: TGridRect;
begin
...
procedure TglStringGrid.OnMemoChange(Sender: TObject);
var
  R: TRect;
...
/// need handle it -> if Assigned(OnSetEditText) then ...
  ...
  begin
    ...
    begin
      ...
      DrawText(Canvas.Handle, PChar(Cells[i, MemoCell.Y]), length(Cells[i, MemoCell.Y]), R, DT_WORDBREAK or DT_CALCRECT);
      h := R.Bottom - R.Top;
      if h > maxHeight then
...
procedure TglStringGrid.OnMemoExit(Sender: TObject);
begin
//  Cells[MemoCell.X, MemoCell.Y] := Memo.Text;
  Memo.Visible := false;
end;

procedure TglStringGrid.ClearSelection;
var
  GR: TGridRect;
begin
...
procedure TglStringGrid.CMChildKey(var Message: TMessage);
begin
...
procedure TglStringGrid.GetCellGradientParams(Sender: TObject; ACol,
...
procedure TglStringGrid.GetCellStyle(Sender: TObject; var ACol, ARow: Integer; var Style: TglGridCellStyle);
begin
  if Assigned(OnGetCellStyle) then
    OnGetCellStyle(self, ACol, ARow, Style);
end;

procedure TglStringGrid.UpdateCaptions(Sender: TObject);
begin
  SetVertCaptions(fsgVertCaptions in FExtOptions);
end;

procedure TglStringGrid.SetHottrack(const Value: boolean);
begin
  if Value then
...
procedure TglStringGrid.SetMemoEditor(const Value: boolean);
begin
  if Value then
...
procedure TglStringGrid.SetWordWrap(const Value: boolean);
begin
  if Value then
...
procedure TglStringGrid.SetVertCaptions(Value: boolean);
var i: integer;
begin
  if Value then
...
procedure TglStringGrid.SetExtOptions(const Value: TglStringGridExtOptions);
begin
  FExtOptions := Value;
end;

procedure TglStringGrid.SetTextAlignment(const Value: TAlignment);
begin
...
function TglStringGrid.CanEditShow: Boolean;
begin
