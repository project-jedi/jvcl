{ 
 ...
      			  ' GLOBUS LIB '	                   
  ...
  
 ===================================================================
 glGrBox Unit 11.1998				  component TglGrBox
 ===================================================================
}
unit glGrBox;
...
uses
  ...
  StdCtrls, ExtCtrls, glTypes, glUtils, glCommCl;

type
  TCaptionAlignment = ( fcaNone, fcaLeft, fcaRight, fcaCenter, fcaWidth );
  TglGroupBox = class(TCustomGroupBox)
  private
    FEnabled            : boolean;
    FBorder	        : TglBevel;
    FCaptionBorder      : TglBevel;
    FGradient           : TGradient;
    FCaptionGradient    : TGradient;
    FCaptionShift       : TPointClass;
    FCaptionTextStyle   : TglTextStyle;
    FCaptionAlignment   : TCaptionAlignment;
    FColors             : TglGrBoxColors;
...
//    procedure WMLButtonDown(var Message: TWMLButtonDown); message WM_LBUTTONDOWN;
  ...
  {$ENDIF}
    ...
    property Border: TglBevel read FBorder write FBorder;
    property CaptionAlignment: TCaptionAlignment
     read FCaptionAlignment write SetCaptionAlignment default fcaNone;
    property CaptionBorder: TglBevel read FCaptionBorder write FCaptionBorder;
    property CaptionGradient: TGradient read FCaptionGradient write FCaptionGradient;
    property CaptionShift: TPointClass read FCaptionShift write FCaptionShift;
    property CaptionTextStyle: TglTextStyle
     read FCaptionTextStyle write SetCaptionTextStyle default fstNone;
    property Collapsed: boolean read FCollapsed write SetCollapsed default false;
    property Colors: TglGrBoxColors read FColors write FColors;
    property Gradient: TGradient read FGradient write FGradient;
    property Illumination: TIllumination read FIllumination write FIllumination stored false;
...
procedure Register;
begin
  RegisterComponents('Proba', [TglGroupBox]);
end;
//____________________________________________________ Methods _
constructor TglGroupBox.Create( AOwner : TComponent );
begin
  inherited;
//  ControlStyle := ControlStyle + [csOpaque];
  FEnabled := true;
  FBorder:=TglBevel.Create;
  FCaptionBorder:=TglBevel.Create;
  FGradient:=TGradient.Create;
  FCaptionGradient:=TGradient.Create;
  FColors := TglGrBoxColors.Create;
  FIllumination := TIllumination.Create;
  FCaptionShift:=TPointClass.Create;
  //...defaults
  ...
  FCaptionShift.x:=8; FCaptionShift.y:=0;
  FCaptionTextStyle:=fstNone;
  FCaptionAlignment:=fcaNone;
  FOptions := [fgoCanCollapse, fgoFilledCaption, fgoFluentlyCollapse,  fgoFluentlyExpand, fgoHideChildrenWhenCollapsed, fgoSaveChildFocus];
  {$IFDEF GL_RUS}
...
destructor TglGroupBox.Destroy;
begin
  FBorder.Free;
  FCaptionBorder.Free;
  FGradient.Free;
...
procedure TglGroupBox.DefineProperties( Filer: TFiler);
begin
  inherited;
  Filer.DefineProperty( 'FullHeight', ReadFullHeight, WriteFullHeight, true );
end;

procedure TglGroupBox.CreateParams(var Params: TCreateParams);
begin
...
{$IFDEF GLVER_D5}
procedure TglGroupBox.AdjustClientRect(var Rect: TRect);
begin
  inherited AdjustClientRect(Rect);
  Inc(Rect.Top, 1);
end;
{$ENDIF}
procedure TglGroupBox.WMLButtonDown(var Message: TWMLButtonDown);
var pt: TPoint;
begin
  inherited;
  if fgoIgnoreMouse in Options then exit;
  pt.x := Message.Pos.x; pt.y := Message.Pos.y;
...
procedure TglGroupBox.WMMouseMove(var Message: TWMMouseMove);
begin
  if fScrolling and (Parent is TScrollBox) then
    (Parent as TScrollBox).VertScrollBar.Position := (Parent as TScrollBox).VertScrollBar.Position + ptScroll.y - Message.Pos.y;
  inherited;
end;

procedure TglGroupBox.WMLButtonUp(var Message: TWMLButtonUp);
begin
  inherited;
  if fgoIgnoreMouse in Options then exit;
  fScrolling := false;
...
procedure TglGroupBox.ReadFullHeight( Reader: TReader );
begin FullHeight := Reader.ReadInteger; end;

procedure TglGroupBox.WriteFullHeight( Writer: TWriter );
begin Writer.writeInteger(FullHeight); end;

procedure TglGroupBox.Paint;
type
  TgbColor = record
...
//                         else H := CaptionHeight - FCaptionShift.y
    ...
    if FGradient.Active then GradientBox( Handle, r, FGradient, PS_SOLID, 1 );
    ...
    if Text <> '' then
    begin
      if Assigned(FGlyphExpanded) then GlyphWidth := FGlyphExpanded.Width
                                  else GlyphWidth := 0;
      if Assigned(FGlyphCollapsed) then GlyphWidth := max( FGlyphCollapsed.Width, GlyphWidth );

      R := Rect(FCaptionShift.x, 0, 0, H);

      if Collapsed then Glyph := FGlyphCollapsed
                   else Glyph := FGlyphExpanded;

      DrawText(Handle, PChar(Text), Length(Text), R, DT_LEFT or DT_SINGLELINE or DT_CALCRECT);

      inc(R.right,Interspace+GlyphWidth);
      with CaptionBorder do begin
	i:=0;
	if Inner<>bvNone then begin inc(i,2); if Bold then inc(i); end;
	if Outer<>bvNone then begin inc(i,2); if Bold then inc(i); end;

	inc(R.Right,i); inc(R.Bottom,i);
      end;

      RW:=R.Right-R.Left;
      case FCaptionAlignment of
	fcaLeft: begin R.Right:=RW; R.Left:=0; end;
	fcaRight: begin R.Left:=Width-RW-1; R.Right:=R.Left+RW; end;
	fcaCenter: begin R.Left:=(Width-RW)div 2; R.Right:=R.Left+RW; end;
	fcaWidth: begin R.Left:=0; R.Right:=width-1; end;
      end;
      if fgoDelineatedText in Options then inc( r.bottom,2 );
      if CaptionTextStyle = fstShadow then
      begin
	if fgoDelineatedText in Options then
	  inc( r.bottom, FIllumination.ShadowDepth-2 ) else inc( r.bottom, FIllumination.ShadowDepth )
      end
      else if CaptionTextStyle <> fstNone then inc( r.bottom, 2 );
...
//      Brush.Color := Color;
      ...
      begin
        if Enabled then DrawState := fdsDefault else DrawState := fdsDisabled;
      	CreateBitmapExt( Handle, Glyph, NewR, 0, max( 0, (NewR.bottom-NewR.top - Glyph.Height)div 2),
			 fwoNone, DrawState, true,
			 GetPixel( Glyph.Canvas.Handle, 0, Glyph.Height-1 ){TransparentColor},
			 {DisabledMaskColor}0 );
      end;


      ExtTextOutExt( (Handle), (NewR.Left+GlyphWidth+Interspace), (NewR.Top), NewR, Caption,
		     IIF(Enabled, FCaptionTextStyle, fstPushed), fgoDelineatedText in Options,
...
//    TControl(Components[i]).Repaint;
  if Assigned(AfterPaint) then AfterPaint(self);
end;

procedure TglGroupBox.Collapse( fCollapse: boolean );
var
  i: integer;
...
begin
  ...
  begin
    ...
    begin//...One Stay Always Expanded in group
      ...
      for i:=0 to Owner.ComponentCount-1 do with TControl(Owner) do
        if (Components[i] is TglGroupBox)and
           (TglGroupBox(Components[i]).GroupIndex = FGroupIndex )then
           if
           (not TglGroupBox(Components[i]).Collapsed)and(Components[i] <> self )
        then begin fAnotherExpandedWasFound := true; break; end;//...are another expanded controls in group
        if not fAnotherExpandedWasFound then exit;//..i'm last- can't collapse
    end;
  end else
    if (FGroupIndex<>0)and(fgoCollapseOther in Options) then
      for i:=0 to Owner.ComponentCount-1 do with TControl(Owner) do
        if (Components[i] is TglGroupBox)and
           (TglGroupBox(Components[i]).GroupIndex = FGroupIndex )and
           (Components[i] <> self )
        then TglGroupBox(Components[i]).Collapsed := true;

  Collapse_( fCollapse );
  if fCollapse and Assigned(FOnCollapsed) then FOnCollapsed(self);
  if not fCollapse and Assigned(FOnExpanded) then FOnExpanded(self);

end;

procedure TglGroupBox.Collapse_( fCollapse: boolean );
var
  i{, Step}: integer;
{  DC: HDC;
  pt: TPoint;
...
begin
  ...
  begin
    ...
    for i:=0 to Owner.ComponentCount-1 do with TControl(Owner) do
      if (Components[i] is TControl)and(TControl(Components[i]).Parent=self)
      then
      begin
        if (fgoSaveChildFocus in Options)and(Components[i] is TWinControl)
           and TWinControl(Components[i]).Focused
        then
  ...
  begin
    ...
    for i:=0 to Owner.ComponentCount-1 do with TControl(Owner) do
     ...
     try ChildFocusedControl.SetFocus; except end;
...
procedure TglGroupBox.SmthChanged(Sender: TObject);
begin Invalidate; end;
//____________________________________________________ Properties Methods _
procedure TglGroupBox.SetEnabled(Value: boolean);
var i: integer;
begin
  FEnabled := Value; inherited Enabled := FEnabled;
  for i:=0 to ControlCount-1 do
    Controls[i].Enabled := FEnabled;
end;

procedure TglGroupBox.SetCaptionAlignment( Value: TCaptionAlignment );
begin
  if FCaptionAlignment = Value then exit; FCaptionAlignment := Value;
  Invalidate;
end;

procedure TglGroupBox.SetCaptionTextStyle( Value: TglTextStyle );
begin
  if FCaptionTextStyle = Value then exit; FCaptionTextStyle := Value;
  Invalidate;
end;

procedure TglGroupBox.SetCollapsed( Value: boolean );
begin
  if FCollapsed = Value  then exit;
  if not(fgoCanCollapse in Options)and Value then exit;
  FCollapsed := Value;
  if csLoading in ComponentState then exit;
  Collapse_(Value);
end;

procedure TglGroupBox.SetOptions(Value: TglGrBoxOptions);
begin
  if FOptions = Value then exit;
  FOptions := Value;
//  if not(fgoCanCollapse in Options) then Collapsed := false;
  if Assigned(Parent) then
    CalcShadowAndHighlightColors( (Parent as TWinControl).Brush.Color, TglLabelColors(Colors) );
  Invalidate;
end;

procedure TglGroupBox.SetTransparent( Value: boolean );
begin
  if FTransparent = Value then exit; FTransparent := Value;
  RecreateWnd;
end;

procedure TglGroupBox.SetTransparentCaption( Value: boolean );
begin
  if FTransparentCaption = Value then exit; FTransparentCaption := Value;
  RecreateWnd;
end;

procedure TglGroupBox.SetGroupIndex( Value: integer );
var i: integer;
begin
  if FGroupIndex = Value then exit;  FGroupIndex := Value;
  if csLoading in ComponentState then exit;
...
function TglGroupBox.GetGlyphCollapsed: TBitmap;
begin
  if FGlyphCollapsed = nil then FGlyphCollapsed := TBitmap.Create;
  Result := FGlyphCollapsed;
end;

procedure TglGroupBox.SetGlyphCollapsed( Value: TBitmap);
begin
  if Assigned(FGlyphCollapsed) then FGlyphCollapsed.Free;
  FGlyphCollapsed := TBitmap.Create;
  FGlyphCollapsed.Assign(Value); Invalidate;
end;

function TglGroupBox.GetGlyphExpanded: TBitmap;
begin
  if FGlyphExpanded = nil then FGlyphExpanded := TBitmap.Create;
  Result := FGlyphExpanded;
end;

procedure TglGroupBox.SetGlyphExpanded( Value: TBitmap);
begin
  if Assigned(FGlyphExpanded) then FGlyphExpanded.Free;
  FGlyphExpanded := TBitmap.Create;
  FGlyphExpanded.Assign(Value); Invalidate;
end;
