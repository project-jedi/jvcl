{ 
 ...
 			  ' GLOBUS LIB '			   
  ...
  
 ===================================================================
 glTab Unit 0x.1998			     component TglTabControl
...
type
  ...
  TglTabControl = class (TTabControl)
   private
     FGlyphs		: TImageList;
...
//     property GlyphState[Index: Integer]: Integer read GetGlyphState write SetGlyphState;
  published
    ...
    property TabSelectedStyle: TTabStyle read FTabSelectedStyle write FTabSelectedStyle;
...
procedure Register;
begin
  RegisterComponents('Proba', [TglTabControl]);
end;
//*****************************************_____________LowLevel METHODS
constructor TglTabControl.Create (AOwner: TComponent);
begin
...
destructor TglTabControl.Destroy;
begin
  FTabStyle.Free;
...
procedure TglTabControl.SmthChanged;
begin Invalidate; end;

procedure TglTabControl.FontsChanged;
begin RemakeFonts; Invalidate; end;

//---------------------------------------------------special procs
procedure TglTabControl.CreateParams(var Params: TCreateParams);
...
procedure TglTabControl.Loaded;
begin
...
procedure TglTabControl.Notification(AComponent: TComponent; Operation: TOperation);
begin
  inherited Notification(AComponent, Operation);
  if Assigned(Wallpaper)and(AComponent = Wallpaper.Image) and (Operation = opRemove) then Wallpaper.Image := nil;
end;

procedure TglTabControl.CNDrawItem(var Message: TWMDrawItem);
begin
  DrawItem(Pointer(Message.DrawItemStruct));
end;

procedure TglTabControl.WndProc(var Message: TMessage);
var
  GlyphID:integer;
begin
...
procedure TglTabControl.GlyphsListChanged(Sender: TObject);
begin
  if HandleAllocated then SendMessage(Handle, TCM_SETIMAGELIST, 0,
				      Longint(TImageList(Sender).Handle));
end;

procedure TglTabControl.DrawItem(lpDrawItemStr:PDRAWITEMSTRUCT);
...
procedure TglTabControl.CMFontChanged(var Message: TMessage);
begin
  inherited;
...
procedure TglTabControl.RemakeFonts;
const
...
procedure TglTabControl.SetGlyphs(Value: TImageList);
var i:word;
...
procedure TglTabControl.SetGlyphIndex( Index: Integer; imgIndex: Integer );
var
  r	: TRect;
  Item	: TTCItem;
begin
...
function TglTabControl.GetGlyphIndex( Index: Integer ): Integer;
var
  imgItem: TTCItem;
begin
  if Assigned(FGlyphs) then
...
procedure TglTabControl.SetSingleGlyph(Value: boolean);
var i: word;
begin
  FSingleGlyph:=Value;
...
procedure TglTabControl.SetDrawGlyphsOption(Value: TglWallpaperOption);
begin
  if FDrawGlyphsOption = Value then exit;
  FDrawGlyphsOption := Value; Invalidate;
end;

procedure TglTabControl.SetLookLikeButtons(Value: boolean);
begin
...
procedure TglTabControl.SetTabsPosition(Value: TglSide);
begin
...
procedure TglTabControl.SetOptions(Value: TglTabOptions);
begin
...
procedure TglTabControl.SetFontDirection(Value: TgllabelDir);
begin
...
function TglTabControl.GetFont: TFont;
begin Result := inherited Font; end;

procedure TglTabControl.SetFont(Value: TFont);
begin
...
function TglTabControl.GetTabColor( Index: integer ): TColor;
begin
  if Index<100 then Result := aTabColors[Index] else Result := -1;
end;

procedure TglTabControl.SetTabColor(Index: integer; Value: TColor);
var
  TCItem: TTCItem;
begin
