...
//...calc and fill tabs background
      ...
      begin
	AddR:=ClientR; ItemR:=rcItem;
	AddR.Top:=0;AddR.Left:=ItemR.Right;
	AddR.Bottom:=ItemR.Bottom;
	if fButton then begin inc(AddR.Bottom,3); end
	else inc(AddR.Bottom,2);//if fSelected then AddR.Bottom:=AddR.Bottom-2;
...
//...calc client area exclude tabs and user-client area
	...
	fsdTop: begin
	  ...
	  if fButton then begin inc(r.top,4); end else
	  if fSelected then dec(r.top) else inc(r.top,5);
	  dec(r.right); dec(r.bottom); inc(r.left);
	end;
	fsdBottom: begin
	  r.bottom:=r.bottom-(rcItem.Bottom-rcItem.Top);
	  if fButton then begin dec(r.bottom,4); end else
	  if fSelected then dec(r.bottom) else dec(r.bottom,5);
	  dec(r.right); inc(r.top); inc(r.left);
	end;
	fsdLeft: begin
	  r.left:=r.left+(rcItem.Right-rcItem.Left);
	  if fButton then begin inc(r.left,3); end else
	  if fSelected then dec(r.left) else inc(r.left,5);
	  dec(r.right); dec(r.bottom); inc(r.top);
	end;
	fsdRight: begin
	  r.right:=r.right-(rcItem.Right-rcItem.Left);
	  if fButton then begin dec(r.right,4); end else
	  if fSelected then dec(r.right) else dec(r.right,5);
	  inc(r.left); dec(r.bottom); inc(r.top);
	end;
      end;
...
//      InflateRect(R, 4, 4);
	...
	case Position of
	  fsdTop:begin{
	    ...
	    r.Left := r.Left+3; r.right := r.right-4; r.top:=r.top+2;
	  ...
	  fsdBottom:begin
	    ...
	    r.Left := r.Left+3; r.right := r.right-4;
	    dec(r.bottom);
	    //if FInteriorOffset then InflateRect(r,1,0);
	  end;
	  fsdLeft:
	  begin
	    inc(r.Left,2); dec(r.right,2); inc(r.Top,2); dec(r.Bottom,2);
	  ...
	  begin
	    ...
	    inc(r.Left,0); dec(r.right,4); inc(r.Top,2); dec(r.Bottom,2);
	  //if FInteriorOffset then inc(r.Bottom);
	  end;
	end;
	...
	case Position of
	  fsdTop: begin
	    inc(r.Left); dec(r.right,2); OffsetRect(r,0,2);
	    //if FInteriorOffset then InflateRect(r,1,0);
	  end;
	  fsdBottom: begin
	  ...
	  fsdLeft:begin inc(r.Left,2);{if FInteriorOffset then inc(r.Bottom);} end;
	  fsdRight:begin dec(r.right,4);{if FInteriorOffset then inc(r.Bottom);} end;
	end;
      end;
      dec(r.bottom,2);
    end;
...
//      Gradient.FRGBToColor := GrToColor;
    ...
    r_:=r;
    fHor := (Position = fsdTop)or(Position = fsdBottom); fVert := not fHor;
    if IsItAFilledBitmap(Glyph)and(not(ftoHideGlyphs in Options)) then
    begin
      //if GlyphOption = fwoNone then ?
      case GlyphHAlign of
	fhaLeft:   begin x := 1; if fHor then inc(r_.left,Glyph.Width+1) end;
	fhaCenter: x := (r.Right-r.Left-Glyph.Width) div 2;
	else{fhaRight} begin x := r.Right-r.Left-Glyph.Width-1;
			if fHor then dec(r_.right,Glyph.Width+1); end;
      end;
      case GlyphVAlign of
	fvaTop: begin y := 0; if fVert then inc(r_.top,Glyph.Height+1); end;
	fvaCenter: y := (r.Bottom-r.Top-Glyph.Height) div 2;
	else{fvaBottom} begin y := r.Bottom-r.top-Glyph.Height; if fVert then dec(r_.bottom,Glyph.Height+1); end;
      end;
...
//    LineTo( hDC, r_.right, r_.bottom );
      ...
      if GlyphOption <> fwoNone then r_:=r else r_.Left := r_.Left+1;
    end;//...end draw glyph_
...
//    end;
    ...
    case FontDirection of
	  ...
	  begin
	    x := r_.right; y:=r_.top+4;
	    case CaptionHAlign of
	      fhaCenter: begin inc(y, (r_.bottom-r_.top-Size.cx) div 2);end;
	      fhaRight: y := r_.bottom - Size.cx-3;
	    end;
	    case CaptionVAlign of
	      fvaCenter: begin dec(x, (r_.right-r_.left-Size.cy) div 2);end;
