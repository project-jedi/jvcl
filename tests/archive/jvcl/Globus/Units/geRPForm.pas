...
uses
  ...
  Grids, glSGrid, stdctrls, dsgnintf, glReport, Buttons, ExtCtrls, mask;

type

  TglRepParams_Editor = class(TComponentEditor)
    procedure ExecuteVerb(Index: Integer); override;
    function GetVerb(Index: Integer): string; override;
    function GetVerbCount: Integer; override;
  end;

  TglReportParamsEditor = class(TComponent)
    FReport: TglReport;
  protected
...
procedure TglRepParams_Editor.ExecuteVerb(Index: Integer);
begin
  case Index of
    0: TglReportParamsEditor(Component).Edit;
  end;
end;

function TglRepParams_Editor.GetVerb(Index: Integer): string;
begin
  case Index of
...
function TglRepParams_Editor.GetVerbCount: Integer;
begin
  Result := 1;
end;
//----------- TglReportEditor
procedure TglReportParamsEditor.Notification(AComponent: TComponent; Operation: TOperation);
begin
  if (AComponent = Report)and(Operation = opRemove) then Report := nil;
  inherited;
end;

procedure TglReportParamsEditor.Edit;
var
  Form: TReportParamsForm;
  Label_: TLabel;
  Edit_: TCustomEdit;
  Check_: TCheckBox;
  Radio_: TRadioGroup;
...
begin//temporary commented
{
  ...
  Form := TReportParamsForm.Create(nil);
