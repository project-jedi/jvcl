...
type
  ...
  TglRep_Property = class( TPropertyEditor )
    function GetAttributes : TPropertyAttributes; override;
    function GetValue : string; override;
    procedure Edit; override;
  end;

  TglRep_Editor = class(TComponentEditor)
    procedure ExecuteVerb(Index: Integer); override;
    function GetVerb(Index: Integer): string; override;
    function GetVerbCount: Integer; override;
  end;

  TglReportEditor = class(TComponent)
    FReport: TglReport;
  protected
  ...
  public
    ...
    procedure Edit;
  published
    property Report: TglReport read FReport write FReport;
  end;

  TglRepEditor = class(TForm)
    ...
    procedure ScrollBox_MouseDown(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
  ...
  private
    ...
    procedure UpdateToolBar( Control: TglReportItem);
  public
    Component: TglReport;
    procedure Preview(glReport: TglReport);
    procedure Edit(glReport: TglReport);
  end;
...
const
  IGNORE_VALUE = 65536;
var
  glRepEditor: TglRepEditor;
  Form2: TComponent;

implementation
uses glTypes, glUtils, {PrintR,}{ ConfirmF, AboutF,} AlignFnc, AlignF;
{$R *.DFM}
//----------- common proc

procedure ShowReportEditor(glReport: TComponent; fEdit: boolean);
var
  Dialog : TglRepEditor;
  Report : TglReport;
begin
  Dialog := TglRepEditor.Create( Application );
  if glReport is TglReport then Report := TglReport(glReport) else
                                Report := TglReportEditor(glReport).Report;
...
function TglRep_Property.GetAttributes : TPropertyAttributes;
begin
  Result := [ paDialog ];
end;

function TglRep_Property.GetValue : string;
begin
  Result := Format( '(%s)', [ GetPropType^.Name ] );
end;

procedure TglRep_Property.Edit;
begin
  ShowReportEditor(TglReport(GetComponent(0)), true);
end;
//----------- TglRep_Editor
procedure TglRep_Editor.ExecuteVerb(Index: Integer);
begin
  case Index of
...
function TglRep_Editor.GetVerb(Index: Integer): string;
begin
  case Index of
...
function TglRep_Editor.GetVerbCount: Integer;
begin
  Result := 2;
end;
//----------- TglReportEditor
procedure TglReportEditor.Notification(AComponent: TComponent; Operation: TOperation);
begin
  if (AComponent = Report)and(Operation = opRemove) then Report := nil;
  inherited;
end;

procedure TglReportEditor.Preview;
begin
  ShowReportEditor( self, false );
end;

procedure TglReportEditor.Edit;
begin
  ShowReportEditor( self, true );
end;
//----------- TglRepEditor
procedure TglRepEditor.Preview(glReport: TglReport);
begin
  PC.Visible := false;
...
procedure TglRepEditor.Edit(glReport: TglReport);
begin
  PC.Visible := true;
...
procedure TglRepEditor.OnMouseDown_(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
var
  R: TRect;
  pt: TPoint;
  i: integer;
begin
  ...
  if Assigned(SelectedControl) then
    ...
    begin
      TglReportItem(Sender).Selected := not TglReportItem(Sender).Selected;
      fMouseDown := false;
      if SelectedControl = Sender then exit;
    end
    else
    begin
      if TglReportItem(Sender).Selected then exit;
      with ScrollBox do
        for i:=0 to ControlCount-1 do
          if (Controls[i] is TglReportItem)and TglReportItem(Controls[i]).Selected then
            TglReportItem(Controls[i]).Selected := false;
      SelectedControl.Selected := false;
      SelectedControl.Invalidate;
    end;

  SelectedControl := TglReportItem(Sender);
...
procedure TglRepEditor.OnMouseUp_(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
var R: TRect;
begin
...
{  DrawFocusRect( TControl(Sender).Parent.Handle, FocusRect );
  ...
  if TControl(Sender).Tag = 2 then//...was moved
    TglReportItem(Sender).RepaintBorder;}
  TControl(Sender).Tag := 0;
end;

procedure TglRepEditor.OnMouseMove_(Sender: TObject; Shift: TShiftState; X, Y: Integer);
var
  DC: HDC;
  i: integer;
begin
...
//  if TControl(Sender).Tag = 0 then exit;
  ...
  with ScrollBox do
  for i:=0 to ControlCount-1 do
    if (Controls[i] is TglReportItem) then with TglReportItem(Controls[i]) do
...
procedure TglRepEditor.OnResize_(Sender: TObject);
begin
  fSkipSizeUpdate := true;
  if Sender = SelectedControl then ShowComponentPos(TControl(Sender));
end;

procedure TglRepEditor.read( FileName: string; ParentWnd: TWinControl );
begin
...
procedure TglRepEditor.Save( FileName: string );
begin
  ScrollBox.HorzScrollBar.Position := 0;
  ScrollBox.VertScrollBar.Position := 0;
...
procedure TglRepEditor.OpenClick(Sender: TObject);
begin
...
procedure TglRepEditor.Save1Click(Sender: TObject);
begin
  SaveDialog1.InitialDir := ExtractFilePath(ParamStr(0));
  if SaveDialog1.Execute then Save( SaveDialog1.FileName );
end;

procedure TglRepEditor.ScrollBox_MouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
var
  l, Compon: TglReportItem;
  R: TRect;
  pt: TPoint;
begin
...
procedure TglRepEditor.ScrollBox_MouseMove(Sender: TObject; Shift: TShiftState;
  X, Y: Integer);
var
  DC: HDC;
  pt: TPoint;
...
procedure TglRepEditor.ScrollBox_MouseUp(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
var
  DC: HDC;
  R: TRect;
  i: integer;
...
procedure TglRepEditor.FormCreate(Sender: TObject);
var R: TRect;
begin
  R := Rect( 0, 0, Screen.Width, Screen.Height );
  ClipCursor( @R );
  ScrollBox := TlaScrollBox.Create(self);
...
procedure TglRepEditor.Edit1Change(Sender: TObject);
begin
...
procedure TglRepEditor.Memo1Change(Sender: TObject);
begin
...
procedure TglRepEditor.SB_LeftClick(Sender: TObject);
begin
...
procedure TglRepEditor.FontComboBox1Change(Sender: TObject);
begin
  if not Assigned(SelectedControl) then exit;
//-----!  SelectedControl.FName := TFontComboBox(Sender).FontName;
end;

procedure TglRepEditor.RxSpinEdit1Change(Sender: TObject);
begin
  if not Assigned(SelectedControl) then exit;
  SelectedControl.FSize := trunc(TSpinEdit(Sender).Value);
end;

procedure TglRepEditor.ColorComboBox1Change(Sender: TObject);
begin
  if not Assigned(SelectedControl) then exit;
//-----!  SelectedControl.FColor := TColorComboBox(Sender).ColorValue;
end;

procedure TglRepEditor.FormShow(Sender: TObject);
begin
...
procedure TglRepEditor.sb_BookClick(Sender: TObject);
begin
...
procedure TglRepEditor.N1Click(Sender: TObject);
begin
  ScrollBox.RemoveControl( SelectedControl );
  ScrollBox.InsertControl( SelectedControl );
end;

procedure TglRepEditor.sb_FontBoldClick(Sender: TObject);
begin
  if not Assigned(SelectedControl) then exit;
...
procedure TglRepEditor.sb_AlignLClick(Sender: TObject);
begin
  if not Assigned(SelectedControl) then exit;
  SelectedControl.Alignment := TControl(Sender).Tag;
end;

procedure TglRepEditor.sbFontColorClick(Sender: TObject);
var i: integer;
begin
  if not Assigned(SelectedControl) then exit;
  ...
  begin
    ...
    if Execute then
      for i:=0 to ScrollBox.ControlCount-1 do
        if ScrollBox.Controls[i] is TglReportItem then with TglReportItem(ScrollBox.Controls[i]) do
          if TglReportItem(ScrollBox.Controls[i]).Selected then
          case TControl(Sender).Tag of
             0: TglReportItem(ScrollBox.Controls[i]).FColor := Color;
             1: TglReportItem(ScrollBox.Controls[i]).BkColor := Color;
             else TglReportItem(ScrollBox.Controls[i]).BvColor := Color;
          end;
  end;
end;

procedure TglRepEditor.N_DeleteObjectClick(Sender: TObject);
begin
  if Assigned(SelectedControl) then
  begin
...
procedure TglRepEditor.OnDrawScrollBox(Sender: TObject);
begin
  VRuler.Top := ShapeSize.Top;
  HRuler.Left := ShapeSize.Left + P_VRuler.Width;
end;

procedure TglRepEditor.FormDestroy(Sender: TObject);
begin
//...
end;

procedure TglRepEditor.RemakeComponentsList;
var i: integer;
begin
  cb_Components.Items.Clear;
  for i := 0 to ScrollBox.ControlCount-1 do
    if ScrollBox.Controls[i] is TglReportItem then
      cb_Components.Items.Add( TglReportItem(ScrollBox.Controls[i]).CompName );
  cb_Components.Text := '';
...
procedure TglRepEditor.UpdatePageSize;
const
...
procedure TglRepEditor.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var
  l, t, w, h: integer;
begin
...
procedure TglRepEditor.sb_FixMovingClick(Sender: TObject);
  var i: integer;
begin
  with ScrollBox do
  for i:=0 to ControlCount-1 do
    if (Controls[i] is TglReportItem)and TglReportItem(Controls[i]).Selected then
      TglReportItem(Controls[i]).Fixed := integer(sb_FixMoving.Down);
end;

procedure TglRepEditor.N_AutoSizeClick(Sender: TObject);
begin
  SelectedControl.OLESizeMode := TMenuItem(Sender).Tag;
end;

procedure TglRepEditor.sb_SnapToGridClick(Sender: TObject);
begin
...
procedure TglRepEditor.se_SizeChange(Sender: TObject);
begin
...
procedure TglRepEditor.se_TopChange(Sender: TObject);
begin
  if (not fMouseDown)and not fSkipSizeUpdate then
...
procedure TglRepEditor.se_WidthChange(Sender: TObject);
begin
  if (not fMouseDown)and not fSkipSizeUpdate then
...
procedure TglRepEditor.se_HeightChange(Sender: TObject);
begin
  if (not fMouseDown)and not fSkipSizeUpdate then
    ResizeReportControls( IGNORE_VALUE, IGNORE_VALUE,
...
procedure TglRepEditor.ResizeReportControls( l, t, w, h: integer; fUseParamsAsShifts: boolean );
var i: integer;
begin
  with ScrollBox do
  for i:=0 to ControlCount-1 do
...
procedure TglRepEditor.cb_ComponentsChange(Sender: TObject);
var i: integer;
begin
  if Assigned(SelectedControl) then
  ...
  with ScrollBox do
  for i:=0 to ControlCount-1 do
    if (Controls[i] is TglReportItem) then
      if TglReportItem(Controls[i]).CompName = cb_Components.Text then
      begin
...
procedure TglRepEditor.ShowComponentPos(Control: TControl);
begin
...
procedure TglRepEditor.AssignEventsToAllComponents;
var i: integer;
begin
  with Component.ParentWnd do
  for i:=0 to ControlCount-1 do
...
begin
  Result := (Control is TglReportItem)and(bool(TglReportItem(Control).Selected))
            and(not bool(TglReportItem(Control).Fixed));
end;

procedure TglRepEditor.N4Click(Sender: TObject);
begin
...
procedure TglRepEditor.sb_BevelBoldClick(Sender: TObject);
var i: integer;
begin
  with Component.ParentWnd do
  for i:=0 to ControlCount-1 do
    if (Controls[i] is TglReportItem)and bool(TglReportItem(Controls[i]).Selected) then
      TglReportItem(Controls[i]).PenWidth := 1+integer(sb_BevelBold.Down);

end;

procedure TglRepEditor.UpdateToolBar( Control: TglReportItem);
begin
  with Control do
  begin
  {
...
procedure TglRepEditor.FormClose(Sender: TObject; var Action: TCloseAction);
...
procedure TglRepEditor.FE_OLEChange(Sender: TObject);
var
  str: string;
begin
...
procedure TglRepEditor.SpeedButton1Click(Sender: TObject);
begin
  if OpenOLEFile.Execute then FE_OLE.Text := OpenOLEFile.FileName;
end;

procedure TglRepEditor.sb_PrintClick(Sender: TObject);
begin
...
procedure TglRepEditor.sb_PreviewClick(Sender: TObject);
var
  Form: TForm;
  Image: TImage;
  bmp: TBitmap;
...
procedure TglRepEditor.SpeedButton2Click(Sender: TObject);
begin
...
procedure TglRepEditor.cb_ComponentsKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
...
procedure TglRepEditor.CheckBox1Click(Sender: TObject);
begin
  if Assigned(SelectedControl) then SelectedControl.Transparent := integer(TCheckBox(Sender).Checked);
end;
