{ 
 ...
 			  ' GLOBUS LIB '			   
  ...
  
 ===================================================================
 glLabel Unit 07.1998				component TglListBox
...
uses
  ...
  StdCtrls, commctrl, ExtCtrls, glTypes, glUtils, glCommCl, gl3DCol {$IFDEF GLVER_D4},Imglist{$ENDIF};
...
type
  ...
  TglLBChangeEvent = procedure (Sender: TObject; OldSelItemIndex, SelItemIndex: integer ) of object;
  TglLBOnDrawEvent = procedure (Sender: TObject; Message: TWMDrawItem ) of object;
  TglOnGetDragImageEvent = procedure (Sender: TObject; Bitmap: TBitmap; var TransparentColor: TColor;
                                      var HotSpotX, HotSpotY: integer ) of object;

  TglListBox = class(TCustomListBox)
  private
    ...
    FNumGlyphs		: word;
    FGlyphsAlign	: T2DAlign;
    FTextAlign		: T2DAlign;
    FTransparentColor	: TColor;
    FHotTrackColor	: TColor;
    FItemStyle		: TglListBoxItemStyle;
    FItemSelStyle	: TglListBoxItemStyle;
    FGlyphs		: TImageList;
    FItemHeight 	: word;
    FTextAlign_ 	: UINT;
    HotTrackingItemIndex: integer;
    FOptions		: TglListBoxOptions;
    FChangeGlyphColor   : TTwainColors;
...
//    property IntegralHeight;
    ...
    property GlyphsAlign: T2DAlign read FGlyphsAlign write FGlyphsAlign;
    property ItemStyle: TglListBoxItemStyle read FItemStyle write FItemStyle;
    property ItemSelStyle: TglListBoxItemStyle read FItemSelStyle write FItemSelStyle;
    property Glyphs: TImageList read FGlyphs write SetGlyphs;
    property TextAlign: T2DAlign read FTextAlign write FTextAlign;
    ...
    property ChangeGlyphColor: TTwainColors read FChangeGlyphColor write FChangeGlyphColor;
  ...
  TglCheckListBox = class(TglListBox)
  private
...
procedure Register;
begin
  RegisterComponents('Proba', [TglListBox]);
end;
...
begin
  ...
  FGlyphsAlign	:= T2DAlign.Create;
  FTextAlign	:= T2DAlign.Create;
  FItemStyle	:= TglListBoxItemStyle.Create;
  FItemSelStyle := TglListBoxItemStyle.Create;
  FChangeGlyphColor := TTwainColors.Create;
...
begin
  ...
  begin
    if Assigned(FWallpaper)and not FWallpaper.Empty then WallpaperBmp := FWallpaper;
...
//______________________________________________________________
procedure TglListBox.DestroyWnd;
var i: integer;
begin//...free all objects is assotiated with items
  if FreeObjectsOnDestroy then
    for i:=0 to Items.Count do
    try
      if Assigned(Items.Objects[i]) then Items.Objects[i].Free;
    except end;
  inherited;
end;
//______________________________________________________________
procedure TglListBox.CNMeasureItem(var Message: TWMMeasureItem);
var
  r: TRect;
  Shift: integer;
const
...
begin
  ...
  R := Rect( 0, 0, Width-FLeftIndent, 0 ); Shift := 0;
  if (fboExcludeGlyphs in Options)and Assigned(FGlyphs) then
    if FGlyphsAlign.Horizontal=fhaLeft then r.left:=FGlyphs.Width
    else if FGlyphsAlign.Horizontal=fhaRight then r.right:=r.right-FGlyphs.Width;
  with Message.MeasureItemStruct^ do
  begin
    DrawText( Canvas.Handle, PChar(Items[itemID]),
      Length( Items[itemID] ),r,DT_CALCRECT or WordBreak[fboWordWrap in Options] );
    if r.bottom = 0 then r.bottom := 14;
    ...
    if (ItemStyle.Bevel.Inner<>bvNone)or(ItemSelStyle.Bevel.Inner<>bvNone) then
      if (ItemStyle.Bevel.Bold)or(ItemSelStyle.Bevel.Bold) then inc(Shift,2) else inc(Shift);
    if (ItemStyle.Bevel.Outer<>bvNone)or(ItemSelStyle.Bevel.Outer<>bvNone) then
      if (ItemStyle.Bevel.Bold)or(ItemSelStyle.Bevel.Bold) then inc(Shift,2) else inc(Shift);
    if (ItemStyle.TextStyle<>fstNone)or(ItemSelStyle.TextStyle<>fstNone) then inc(Shift,2);
    ...
    if (FItemHeight>0){and(Message.MeasureItemStruct^.itemHeight<FItemHeight)} then
      Message.MeasureItemStruct^.itemHeight:=FItemHeight;
  end;
//  Message.MeasureItemStruct^.itemHeight:=13;
end;
//______________________________________________________________
procedure TglListBox.CNDrawItem(var Message: TWMDrawItem);
var
  Index                         : Integer;
  R, TxtRect                    : TRect;
  State                         : TOwnerDrawState;
  ItemStyle                     : TglListBoxItemStyle;
  ...
  begin
    ...
    if fboSingleGlyph in Options then i:=0
    else
      if Index < NumGlyphs then i := Index else i := -1;

    if i >= 0 then
    begin
      FGlyphs.GetBitmap(i, TmpBitmap);
      if fSelected and(fboChangeGlyphColor in Options) then
        ChangeBitmapColor( TmpBitmap, FChangeGlyphColor.FromColor, FChangeGlyphColor.ToColor );

      if FAutoTrColor = ftcUser then FTranspColor := FTransparentColor
                                else FTranspColor := GetTransparentColor( TmpBitmap, FAutoTrColor );
...
//      end;
  ...
  var
    ...
    begin
     ...
     x_ := r.Left; y_ := r.top; y := y_;
	    while x_ < r.right do
	    begin
	      if x_+IWidth > r.right then IWidth:=r.right-x_;
	      while y_ < r.bottom do begin
	        //if y_+IHeight > r.bottom then IHeight:=r.bottom-y_;
                BitBlt( DC, x_, y_, IWidth, IHeight, WallpaperBmp.Canvas.Handle, 0, yOffset, SRCCOPY );
	        Inc(y_, IHeight );  YOffset := 0;
	      end;
	      Inc(x_, IWidth); y_:=y;
	    end;
    end;
  begin
	...
	fwlStretch:
  	...
  	fwlGlobal:
       ...
       BitBlt( DC, r.left+1, r.top, r.right-r.left-1, r.bottom-r.top, WallpaperBmp.Canvas.Handle, 0, r.top, SRCCOPY );

       with Message.DrawItemStruct^ do
       if itemID = UINT(Items.Count-1) then
       begin
          BitBlt( DC, rcItem.left, rcItem.bottom, rcItem.right-rcItem.left, height, WallpaperBmp.Canvas.Handle, rcItem.left, rcItem.bottom, SRCCOPY );
       end;

        end;
  	else
  	  BitBlt( DC, r.left, r.top, min(WallpaperBmp.Width, r.right-r.left),
                    min(WallpaperBmp.Height, r.bottom-r.top), WallpaperBmp.Canvas.Handle, 0, 0, SRCCOPY );
        end;
    end;
  end;
begin
  ...
  if not FWallpaper.Empty then WallpaperBmp := FWallpaper
  else
    if Assigned(FWallpaperImage)and Assigned(FWallpaperImage.Picture)and Assigned(FWallpaperImage.Picture.Bitmap) then
  ...
  begin
    ...
    begin
      if IsItAFilledBitmap(WallpaperBmp) then
        BitBlt( hDC, 0, 0, Width, Height, WallpaperBmp.Canvas.Handle, 0, 0, SRCCOPY );
      exit;
    end;
  ...
  inc( R.Left, IndentLeft ); dec( R.Right, IndentRight );
  ...
  if fSelected then ItemStyle := FItemSelStyle
	       else ItemStyle := FItemStyle;

  fDrawWallpapper := (not (fSelected and (FItemStyle.Color<>FItemSelStyle.Color)))and fUseWallpaper;

  //...DrawLBItem
  inc(R.left); dec(R.right); dec(R.bottom);
  ItemColor := ItemStyle.Color;
  if Assigned(OnGetItemColor) then OnGetItemColor( self, Index, ItemColor );
  if fboAutoCtl3DColors in Options then
  begin
    ThreeDColors.CreateAuto3DColors(ItemColor);
    ThreeDColors.MakeGlobal;
  end;
  R := DrawBoxEx( DC, R, ItemStyle.Bevel.Sides, ItemStyle.Bevel.Inner, ItemStyle.Bevel.Outer,
		     ItemStyle.Bevel.Bold, ItemColor, fDrawWallpapper );
  if fboAutoCtl3DColors in Options then ThreeDColors.MakeLocal;

  dec(R.left); inc(R.right); inc(R.bottom);
  if ItemStyle.Gradient.Active then with ItemStyle do
  begin
    GrFromColor := Gradient.FRGBFromColor;
    GrToColor := Gradient.FRGBToColor;
    if ItemColor > 0 then begin
      if fboItemColorAsGradientFrom in Options then Gradient.FRGBFromColor := ItemColor;
      if fboItemColorAsGradientTo in Options then Gradient.FRGBToColor := ItemColor;
    end;
  ...
  if fDrawWallpapper then DrawWallpaper;

  if Assigned(FGlyphs) then
  begin
    DrawGlyph( R );
    if (fboExcludeGlyphs in Options) then
      if FGlyphsAlign.Horizontal=fhaLeft then R.left:=R.left+FGlyphs.Width
      else if FGlyphsAlign.Horizontal=fhaRight then R.right:=R.right-FGlyphs.Width
  end;
  inc(R.Left, FLeftIndent);
  SetBkMode(DC, TRANSPARENT);
  inc(R.Left); dec(R.right, 2);

  TxtRect := R; inc( TxtRect.Left, TextIndent );
...
//      if fSelected then ItemStyle.Font.Color := clWhite;
    ...
    DrawTextInRect( TargetCanvas.handle, TxtRect, Items[Index], ItemStyle.TextStyle, ItemStyle.Font, FTextAlign_ );
    ItemStyle.Font.Color := FontColor;
  end;
  if TargetCanvas <> Canvas then
    BitBlt( Message.DrawItemStruct^.hDC, Message.DrawItemStruct^.rcItem.Left, Message.DrawItemStruct^.rcItem.Top,
  ...
  if Assigned(FOnDrawItem) then FOnDrawItem(self, Message);
...
procedure TglListBox.CMMouseLeave(var Message: TMessage);
var R: TRect;
begin
  inherited;
...
procedure TglListBox.WMMouseMove(var Message: TMessage);
var
  pt: Tpoint;
  R: TRect;
  itemIndex: integer;
begin
  inherited;
  if not (fboHotTrack in Options)and not (fboHotTrackSelect in Options) then exit;
  pt.x := LOWORD(Message.lParam); pt.y := HIWORD(Message.lParam);
  itemIndex := ItemAtPos( pt, true );

  if itemIndex = HotTrackingItemIndex then exit;
...
{$IFDEF GLVER_D4}
function TglListBox.GetDragImages: TDragImageList;
{$ELSE}
function TglListBox.GetDragImages: TCustomImageList;
{$ENDIF}
begin
  if FDragImage.Count > 0 then Result := FDragImage else Result := nil;
end;

procedure TglListBox.CreateDragImage;
var
  ...
  R: TRect;
begin
  FDragImage.Clear; if ItemIndex = -1 then exit;
...
procedure TglListBox.DoStartDrag( var DragObject: TDragObject );
begin
  inherited;
  CreateDragImage;
end;
...
procedure TglListBox.SetAutoTrColor(Value: TglAutoTransparentColor);
begin
  if FAutoTrColor = Value then exit;
  FAutoTrColor := Value; Invalidate;
end;

function TglListBox.GetWallpaper: TBitmap;
begin
  if not Assigned(FWallpaper) then FWallpaper := TBitmap.Create;
  Result := FWallpaper;
end;

procedure TglListBox.SetWallpaper(Value: TBitmap);
begin
  FWallpaper.Assign(Value);
  Invalidate;
end;

procedure TglListBox.SetWallpaperImage(Value: TImage);
begin
  FWallpaperImage := Value;
  Invalidate;
end;

procedure TglListBox.SetWOpt(Value: TglLBWallpaperOption);
begin
  FWallpaperOption := Value; Invalidate;
end;

procedure TglListBox.SetNumGlyphs( Value: word );
begin
  if Value<1 then exit; FNumGlyphs := Value;
  Invalidate;
end;

procedure TglListBox.SetGlyphs( Value: TImageList );
begin
  FGlyphs := Value; Invalidate;
end;

procedure TglListBox.SetItemHeight(Value: word);
begin
  FItemHeight := Value;
  RecalcHeights;
end;

procedure TglListBox.SetAlign;
begin
  if fboWordWrap in Options then FTextAlign_:=DT_WORDBREAK or DT_NOPREFIX
                            else FTextAlign_:=DT_SINGLELINE or DT_NOPREFIX;
  case FTextAlign.Horizontal of
    fhaLeft:	 FTextAlign_:=FTextAlign_ or DT_LEFT;
    fhaCenter:	 FTextAlign_:=FTextAlign_ or DT_CENTER;
    else	 FTextAlign_:=FTextAlign_ or DT_RIGHT;
  end;
  case FTextAlign.Vertical of
    fvaTop:	 FTextAlign_:=FTextAlign_ or DT_TOP;
    fvaCenter:	 FTextAlign_:=FTextAlign_ or DT_VCENTER;
    else	 FTextAlign_:=FTextAlign_ or DT_BOTTOM;
  end;
end;

procedure TglListBox.SetTransparentColor(Value: TColor);
begin
  FTransparentColor := Value;
  if FAutoTrColor<>ftcUser then Invalidate;
end;

procedure TglListBox.SetHotTrackColor(Value: TColor);
var R: TRect;
begin
  if FHotTrackColor = Value then exit;
...
procedure TglListBox.SetOptions(Value: TglListBoxOptions);
begin
...
{  if (fboTransparent in Value) and not (fboTransparent in FOptions)then
  ...
  FOptions := Value; SetAlign; RecalcHeights; Invalidate;
end;

function TglListBox.GetSelectedObject: Pointer;
begin
  if ItemIndex >= 0 then Result := Items.Objects[ItemIndex]
                    else Result := nil;
end;

function TglListBox.GetSelCount: cardinal;
var i: integer;
begin
  Result := 0;
...
procedure TglListBox.RecalcHeights;
var i:integer;
begin
  Items.BeginUpdate;
  for i:=0 to Items.Count-1 do
...
procedure TglListBox.SmthChanged(Sender: TObject);
begin
  if (csLoading in ComponentState) then exit;
  RecalcHeights; SetAlign; Invalidate;
end;

//==============================================
constructor TglCheckListBox.Create( AOwner : TComponent );
begin
  inherited;
  FCheckWidth := 14; FCheckHeight := 14;
  FLeftIndent := 22;
end;

destructor TglCheckListBox.Destroy;
begin
  inherited;
end;

procedure TglCheckListBox.CNDrawItem(var Message: TWMDrawItem);
var
  R: TRect;
  Index                         : Integer;
  State                         : TOwnerDrawState;
...
function TglCheckListBox.GetState( Index: integer ): TCheckBoxState;
begin
  Result := TCheckBoxState(Items.Objects[Index]);
end;

procedure TglCheckListBox.DrawCheck( R: TRect; AState: TCheckBoxState );
var
  DrawState: Integer;
  DrawRect: TRect;
begin
...
procedure TglListBox.InitState(var State: TOwnerDrawState; ByteState: Byte);
begin
    State := [];
...
function TglCheckListBox.GetChecked(Index: Integer): TCheckBoxState;
begin
  Result := TCheckBoxState(Items.Objects[Index]);
end;

procedure TglCheckListBox.SetChecked(Index: Integer; State: TCheckBoxState);
begin
  Items.Objects[Index] := Pointer(State);
end;

procedure TglCheckListBox.MouseDown(Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
var
  APoint: TPoint;
  Index: integer;
begin
