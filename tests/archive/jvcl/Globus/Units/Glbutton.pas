{ 
 ...
 			  ' GLOBUS LIB '			   
  ...
  
 ===================================================================
 glButton Unit 08.1998				 component TglButton
 ===================================================================
}
unit glButton;
...
uses
  ...
  glTypes, glUtils, glCommCl{$IFDEF GLVER_D5},Imglist{$ENDIF};
...
//*************************************{ . TGlyphsIndexes . }
  TGlyphsIndexes = class(TPersistent)
  private
    FInactive	  :integer;
    FPushed	  :integer;
    FActive	  :integer;
  ...
  TglBtnGlyphs = class(TPersistent)
  private
...
//*************************************{ . TglButton . }
  TglButton = class(TGraphicControl)
  private
    FGlyph		: TBitmap;
    FGlyphs		: TglBtnGlyphs;
    FDrawMode		: TDrawMode;
    FGlyphsList 	: TImageList;
    FTransparentColor	: TColor;
    FNumGlyphs		: integer;
    FShiftMaskWhenPushed: TPointClass;
    FEnabled		: boolean;
    FOptions		: TglButtonOptions;
    FShadowDepth	: word;
    FGlyphsIndexes	: TGlyphsIndexes;
    FColorHighlight	: TColor;
    FColorShadow	: TColor;
    FColorDarkShadow	: TColor;
    FDisabledMaskColor	: TColor;
    FChangeColorOnActivate : TTwainColors;
    FChangeColorOnPush	: TTwainColors;
...
//    procedure CMMouseEnter(var Message: TMessage); message CM_MOUSEENTER;
  ...
  published
    ...
    property Glyphs: TglBtnGlyphs read FGlyphs write FGlyphs;
    ...
    property ShiftMaskWhenPushed: TPointClass read FShiftMaskWhenPushed write FShiftMaskWhenPushed;
...
//     read FRefreshOnPush write FRefreshOnPush default true;
    ...
    property GlyphsIndexes: TGlyphsIndexes
    ...
    property ChangeColorOnActivate : TTwainColors
     read FChangeColorOnActivate write FChangeColorOnActivate;
    property ChangeColorOnPush : TTwainColors
...
procedure Register;
begin
  RegisterComponents('Proba', [TglButton]);
end;

constructor TglBtnGlyphs.Create;
begin
  inherited;
...
destructor TglBtnGlyphs.Destroy;
begin
...
//procedure TglBtnGlyphs.SetGlyphInactive( Value: TBitmap );

procedure TglBtnGlyphs.SetGlyphInactive( Value: TBitmap );
begin if Assigned(GlyphInactive) then GlyphInactive.Free; GlyphInactive.Assign(value); end;
procedure TglBtnGlyphs.SetGlyphMask( Value: TBitmap );
begin if Assigned(GlyphMask) then GlyphMask.Free; GlyphMask.Assign(value); end;
procedure TglBtnGlyphs.SetGlyphPushed( Value: TBitmap );
begin if Assigned(GlyphPushed) then GlyphPushed.Free; GlyphPushed.Assign(value); end;
procedure TglBtnGlyphs.SetGlyphActive( Value: TBitmap );
begin if Assigned(GlyphActive) then GlyphActive.Free; GlyphActive.Assign(value); end;
procedure TglBtnGlyphs.SetGlyphDisabled( Value: TBitmap );
begin if Assigned(GlyphDisabled) then GlyphDisabled.Free; GlyphDisabled.Assign(value); end;
...
begin
  ...
  FGlyphs := TglBtnGlyphs.Create;
  DefaultGlyphsList:=TImageList.CreateSize(30,30);
  Img:=TBitmap.Create;
  TmpBMP:=TBitmap.Create;
  FShiftMaskWhenPushed:=TPointClass.Create;
  FGlyphsIndexes:=TGlyphsIndexes.Create;
  FChangeColorOnActivate:=TTwainColors.Create;
  FChangeColorOnPush:=TTwainColors.Create;
  FGlyphsIndexes.OnChanged:=SmthChanged;
  //...set defaults
  FShiftMaskWhenPushed.x:=0; FShiftMaskWhenPushed.y:=0;
  FEnabled:=true;
  FGlyphsList:=nil;
  FNumGlyphs:=3;
...
var DrawState: TglDrawState; i: word;
begin
  with Glyphs do begin
    if not fLoaded then begin fLoaded:=true; GetBitmaps; end;
//    if not fBimapsCreated then exit;
    Width := FGlyphInactive.Width+1; Height := FGlyphInactive.Height+1;
  ...
  if boDelicateInactive in FOptions then DrawState:=fdsDelicate else DrawState:=fdsDefault;
  ...
  begin
	...
	begin
	  if FDrawMode = dmAutoFlat then i:=2 else i:=0;
	...
	fActiveNow:=false;
	if (FDrawMode = dmAutoFlat) then i:=1 else i:=0;
...
//	  else
  ...
  else begin//..._____________DISABLED_
...
//Refresh;exit;
  ...
  if fPushed then
  begin
    if (boRefreshOnPush in FOptions)or(FDrawMode = dmAutoShadow)then Repaint else Paint;
  end
  else if boRefreshOnActivate in FOptions then Repaint else Paint;
end;

procedure TglButton.ApplicateGlyph( var TargetBMP: TBitmap;
				      State: TglBtnState;
...
begin
  with Glyphs do begin
  i:=1; j:=1;
  fChangeColor:=false; fCanBlink:=false;
  if DrawState = fdsDisabled then
  begin
    DrawState := fdsDefault;
    DrawState2 := fdsDisabled//DrawState;
  end else DrawState2 := DrawState;
  case FDrawMode of
    dmAutoCtrl3D:
      if State = fbsPushed then begin i:=2; j:=2; end;
    dmUseImageList: begin i:=0; j:=0; s:=0; end;
  end;
  ...
  case State of
	...
	CreateBitmapExt( TargetBMP.Canvas.Handle, FGlyphInactive, ClientRect, 0, 0,
      ...
      if State = fbsDisabled then begin i:=0; j:=0; end;
  ...
  if fCanBlink then fBlinked:=not fBlinked
  ...
  begin
   if fBlinked then
     if State = fbsPushed then
       with FChangeColorOnPush do ChangeBitmapColor( TmpBMP, FromColor, ToColor )
     else
       with FChangeColorOnActivate do ChangeBitmapColor( TmpBMP, FromColor, ToColor );
  end
...
//    SetBkColor(TmpBMP.Canvas.Handle, FTransparentColor);
    TmpBMP.Monochrome:=true;   TmpBMP.Monochrome:=false;
...
{procedure TglButton.WMSize(var Message: TWMSize);
begin
  Width := FGlyphInactive.Width; Height := FGlyphInactive.Height;
end;}

procedure TglButton.MouseMove(Shift: TShiftState; X, Y: Integer);
var
  pt: TPoint;
...
begin
  ...
  begin
    ...
    begin
      if fMouseInControl then if Assigned(FOnMouseEnter) then FOnMouseEnter(self)
			 else if Assigned(FOnMouseLeave) then FOnMouseLeave(self);
      fMouseInControl := fMouseInControl_; Paint_;
    end;
  end;
end;

{procedure TglButton.CMMouseEnter(var Message: TMessage);
begin
  fMouseInControl:=true; Paint_;
end;}

procedure TglButton.CMMouseLeave(var Message: TMessage);
begin
  inherited;
  //fMouseInControl:=IsMouseInControl;
  if Assigned(FOnMouseLeave) then FOnMouseLeave(self);
  fMouseInControl := false; Paint_;
end;
...
begin
  ...
  fPushed:=false; Paint_;
end;

procedure TglButton.GetBitmaps;
begin
  if not fLoaded then exit;
  with Glyphs do begin
    FGlyphInactive.Width:=0;FGlyphPushed.Width:=0;FGlyphActive.Width:=0;
    FGlyphDisabled.Width:=0;FGlyphMask.Width:=0;
	...
	begin MShift.x:=1; MShift.y:=1; end else
	begin MShift.x:=FShadowDepth-1; MShift.y:=FShadowDepth-1; end;
      dmAutoCtrl3D:
	begin MShift.x:=2; MShift.y:=2; end;
      dmAutoFlat:
	begin MShift.x:=1; MShift.y:=1; end;
      else
	begin MShift.x:=FShiftMaskWhenPushed.x; MShift.y:=FShiftMaskWhenPushed.y; end;
    end;
...
procedure TglButton.CreateBitmaps;//...based on Inactive Glyph
var
...
begin
  ...
  begin
    FInactive:=0;FPushed:=-1;FActive:=-1; FDisabled:=-1;FMask:=-1;
...
//  SetMapMode( MonoDC, GetMapMode(TmpBMP.Canvas.Handle) );
    ...
    begin
      with FGlyphInactive do begin
	Width:=TmpBMP.Width+FShadowDepth;  Height:=TmpBMP.Height+FShadowDepth;
	Canvas.Brush.Style:=bsSolid; Canvas.Brush.Color:=FTransparentColor;
	Canvas.FillRect(Rect(0,0,Width,Height));end;
      with FGlyphPushed do begin
	Width:=TmpBMP.Width+FShadowDepth;  Height:=TmpBMP.Height+FShadowDepth;
	Canvas.Brush.Style:=bsSolid; Canvas.Brush.Color:=FTransparentColor;
	Canvas.FillRect(Rect(0,0,Width,Height));end;
    ...
    if FDrawMode = dmAutoCtrl3D then i:=3 else i:=3;
    with FGlyphInactive do begin
      Width:=TmpBMP.Width+i;  Height:=TmpBMP.Height+i;
      Canvas.Brush.Style:=bsSolid; Canvas.Brush.Color:=FTransparentColor;
      Canvas.FillRect(Rect(0,0,Width,Height));end;
    if not (boDrawPushedAsFlat in FOptions) then with FGlyphPushed do begin
      Width:=TmpBMP.Width+i;  Height:=TmpBMP.Height+i;
      Canvas.Brush.Style:=bsSolid; Canvas.Brush.Color:=FTransparentColor;
      Canvas.FillRect(Rect(0,0,Width,Height));end;
    with FGlyphActive do begin
      Width:=TmpBMP.Width+i;  Height:=TmpBMP.Height+i;
      Canvas.Brush.Style:=bsSolid; Canvas.Brush.Color:=FTransparentColor;
      Canvas.FillRect(Rect(0,0,Width,Height));end;
...
//===========================-AUTOCTRL3D-=================================
	...
	ChangeBitmapColor( TmpBMP, clBlack, FColorDarkShadow );
      if boShadowSurround in FOptions then begin
	CreateBitmapExt( FGlyphInactive.Canvas.Handle, TmpBMP, Rect(0,0,TmpBMP.Width,TmpBMP.Height),
...
function TglButton.IsMouseInControl:boolean;
var
  pt: TPoint;
...
begin
  ...
  end else if FDrawMode = dmAutoShadow then
  begin
...
procedure TglButton.SmthChanged(Sender: TObject);
begin GetBitmaps; Invalidate; end;

procedure TglButton.GetBitmap_( Index: integer; var Bmp: TBitmap );
begin
  try
    if FDrawMode = dmUseImageList then FGlyphsList.GetBitmap( Index, Bmp )
    else
    begin
      if Assigned(Bmp) then begin Bmp.Free; Bmp:=TBitmap.Create; end;
      Bmp.Assign(Glyph);
    end;
  except
...
procedure TglButton.OnBlinkTimer(Sender: TObject);
var
  ParentForm: TForm;
  i: integer;
  procedure Blink( FreeButton: TglButton );
  begin
    with FreeButton do begin
      fNeedBlink:=false;
      if fShowingAsPushedNow then with FChangeColorOnPush do
	if (boBlinkWhenPushed in Options)and(FromColor <> ToColor) then
	begin fNeedBlink:=true; repaint; exit; end else exit;
      if fMouseInControl then with FChangeColorOnActivate do
	if (boBlinkWhenActive in Options)and(FromColor <> ToColor) then
	begin fNeedBlink:=true; repaint; exit; end else exit;
      if not fMouseInControl then with FChangeColorOnActivate do
...
begin//...main proc
  ...
  for i:=0 to ParentForm.ComponentCount-1 do
    if (ParentForm.Components[i] is TglButton)
       and(TglButton(ParentForm.Components[i]).BlinkTimer=FBlinkTimer) then
    with ParentForm.Components[i] as TglButton do
    begin
      Blink( TglButton(ParentForm.Components[i]) );
    end;

end;

//*************************************{ . TglButton properties methods. }
procedure TglButton.SetDrawMode( Value: TDrawMode );
begin
...
//   FGlyph.Assign(FGlyphInactive);
  ...
  GetBitmaps; Invalidate;
end;

procedure TglButton.SetGlyphsList( Value: TImageList );
begin
  if (not Assigned(Value)){or(Value.Count<2)} then exit;
  FGlyphsList := Value; GetBitmaps; Invalidate;
end;

procedure TglButton.SetGlyph( Value: TBitmap );
begin
//  if (not Assigned(Value)) then exit;
  if Assigned(FGlyph) then begin FGlyph.Free; FGlyph:=TBitmap.Create; end;
  FGlyph.Assign(Value);
  begin GetBitmaps; Invalidate; end;
...
procedure TglButton.SetNumGlyphs( Value: integer );
begin
  if (Value < 2) or (Value > 4) then exit;
  FNumGlyphs := Value;
end;

procedure TglButton.SetTransparentColor( Value: TColor );
begin
...
procedure TglButton.SetEnabled( Value: boolean );
begin
  if FEnabled = Value then exit;
  FEnabled := Value; Repaint;
end;

procedure TglButton.SetShadowDepth( Value: word );
begin
  if FShadowDepth = Value then exit;
  FShadowDepth := Value;
  if FDrawMode = dmAutoShadow then begin GetBitmaps; Invalidate; end;
end;

procedure TglButton.SetColorHighlight( Value: TColor );
begin
...
procedure TglButton.SetColorShadow( Value: TColor );
begin
...
procedure TglButton.SetColorDarkShadow( Value: TColor );
begin
...
procedure TglButton.SetDisabledMaskColor( Value: TColor );
begin
...
procedure TglButton.SetOptions( Value: TglButtonOptions );
begin
...
procedure TglButton.SetAutoTrColor(Value: TglAutoTransparentColor);
var x,y :integer;
    TmpBmp_: TBitmap;
begin
  FAutoTrColor := Value;
  ...
  try
    ...
    case FAutoTrColor of
      ...
      ftcLeftBottomPixel: begin x:=0; y:=Height-1; end;
      ftcRightTopPixel: begin x:=Width-1; y:=0; end;
      ftcRightBottomPixel: begin x:=Width-1; y:=Height-1; end;
      else exit;
    end;
...
procedure TglButton.SetBlinkTimer( Value: TTimer );
var
  ParentForm: TForm;
  i: integer;
  p1,p2: TNotifyEvent;
...
begin
  ...
  begin
    ...
    begin
      ...
      for i:=0 to ParentForm.ComponentCount-1 do
	if (ParentForm.Components[i] is TglButton)
	   and(TglButton(ParentForm.Components[i])<>self)
	   and(TglButton(ParentForm.Components[i]).BlinkTimer=FBlinkTimer) then
	begin
	  Timer := FBlinkTimer; FBlinkTimer := nil;
	  Timer.OnTimer := TglButton(ParentForm.Components[i]).OnBlinkTimer;
	  break;
	end;
      if Assigned(FBlinkTimer)and(@FBlinkTimer.OnTimer=@p2) then FBlinkTimer.OnTimer:=nil;
    end;
  end else begin FBlinkTimer := nil; end;

  FBlinkTimer := Value;
  if Assigned(FBlinkTimer) then FBlinkTimer.OnTimer := OnBlinkTimer;
end;

function TglButton.GetBlinkTimer: TTimer;
begin
  Result := nil;
  try
    if Assigned(FBlinkTimer) then
...
procedure TglButton.SetTestMode( Value: boolean );
var
  ParentForm: TForm;
  i: integer;
begin
  ParentForm := GetParentForm(self);
  for i:=0 to ParentForm.ComponentCount-1 do
    if (ParentForm.Components[i] is TglButton) then
      TglButton(ParentForm.Components[i]).FTestMode:=Value;
end;
//*************************************{ . TGlyphsIndexes methods. }

constructor TGlyphsIndexes.Create;
begin
  inherited Create;
  FInactive:=0; FPushed:=1; FActive:=2; FDisabled:=-1; FMask:=3;
end;

procedure TGlyphsIndexes.SetInactive( Value :integer );
begin FInactive := Value; if Assigned(OnChanged) then OnChanged(self); end;

procedure TGlyphsIndexes.SetPushed( Value :integer );
begin FPushed := Value; if Assigned(OnChanged) then OnChanged(self); end;

procedure TGlyphsIndexes.SetActive( Value :integer );
begin FActive := Value; if Assigned(OnChanged) then OnChanged(self); end;

procedure TGlyphsIndexes.SetDisabled( Value :integer );
begin FDisabled := Value; if Assigned(OnChanged) then OnChanged(self); end;

procedure TGlyphsIndexes.SetMask( Value :integer );
begin FMask := Value; if Assigned(OnChanged) then OnChanged(self); end;
