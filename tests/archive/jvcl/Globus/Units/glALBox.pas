{ 
 ...
 			  ' GLOBUS LIB '			   
  ...
  
 ===================================================================
 glALBox Unit 07.1998			     component TglAskListBox
 ===================================================================
...
uses
  ...
  StdCtrls, commctrl, ExtCtrls, glTypes, glCommCl;
...
type
  ...
  TglAskListBox = class(TCustomListBox)
  private
    ...
    FTextAlign		:T2DAlign;
    FCaptionsAlign	:T2DAlign;

    FTransparentColor	:TColor;
    FItemStyle		:TglAskListBoxItemStyle;
    FItemSelStyle	:TglAskListBoxItemStyle;
    FGlyphs		:TImageList;
    ...
    procedure SetAlign( Align:T2DAlign; var Align_:UINT);
...
//    property AfterItemWasDrown;
    ...
    property ItemStyle:TglAskListBoxItemStyle read FItemStyle write FItemStyle;
    property ItemSelStyle:TglAskListBoxItemStyle read FItemSelStyle write FItemSelStyle;
...
procedure Register;
begin
  RegisterComponents('Proba', [TglAskListBox]);
end;

//*****************************************_____________LowLevel METHODS

constructor TglAskListBox.Create( AOwner : TComponent );
begin
  inherited Create( AOwner );
  Style:=lbOwnerDrawVariable;
  FGlyphsAlign	 := T2DAlign.Create;
  FTextAlign	 := T2DAlign.Create;
  FCaptionsAlign := T2DAlign.Create;
  FButtons	 := TStringList.Create;
  FItemStyle	 := TglAskListBoxItemStyle.Create;
  FItemSelStyle  := TglAskListBoxItemStyle.Create;

  TmpBitmap := TBitmap.Create;
  FButtons.Add('yes'); FButtons.Add('no');
...
//------
destructor TglAskListBox.Destroy;
begin
...
procedure TglAskListBox.Loaded;
begin
  inherited;
  RecalcHeights;
end;
...
procedure TglAskListBox.CNMeasureItem(var Message: TWMMeasureItem);
var r: TRect;
begin
  r.left := 3; r.top := 0; r.bottom := 0;
...
//______________________________________________________________
procedure TglAskListBox.CNDrawItem(var Message: TWMDrawItem);
const w=1;
var
  Index: Integer;
  Rect: TRect;
  ...
  Rect1:TRect;
  ItemStyle: TglAskListBoxItemStyle;
//  TS:TglTextStyle;
//  TA:UINT;
  ...
  begin
    case ItemSt of
      idsRecessed:
      begin Shift:=0; Frame3D(Canvas, r,clBtnShadow,clBtnHighlight,1);end;
      idsRaised:
      begin Shift:=2; Frame3D(Canvas, r,clBtnHighlight,clBtnShadow,1);end;
    end;
  end;
  //~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
//    FillMemory(@szStr,255,0);
    if StrListNum = -1 then StrPCopy(szStr,Items[Index])
		       else StrPCopy(szStr,FButtons[StrListNum-1]);

    Len:=StrLen( szStr );

    if StrListNum = -1 then
    begin Canvas.Font := ItemStyle.Font; TextStyle := ItemStyle.TextStyle; end
    else
    begin
      Canvas.Font := ItemStyle.BtnFont;
      if FPushedButton[Index]=StrListNum then TextStyle := fstNone
                                         else TextStyle := ItemStyle.BtnTextStyle;
    end;
    ...
    case TextStyle of
      fstRaised:
      begin
	Canvas.Font.Color := clBtnHighlight; OffsetRect(Rect, -1, -1);
	DrawText( Canvas.Handle,szStr,Len,rect,Align);
	Canvas.Font.Color := clBtnShadow; OffsetRect(Rect, 2, 2);
	DrawText( Canvas.Handle,szStr,Len,rect,Align);
	Canvas.Font.Color := FontColor; OffsetRect(Rect, -1, -1);
	DrawText( Canvas.Handle,szStr,Len,rect,Align);
      end;
      fstRecessed:
      begin
	Canvas.Font.Color := clBtnShadow; OffsetRect(Rect, -1, -1);
	DrawText( Canvas.Handle,szStr,Len,rect,Align);
	Canvas.Font.Color := clBtnHighlight; OffsetRect(Rect, 2, 2);
	DrawText( Canvas.Handle,szStr,Len,rect,Align);
	Canvas.Font.Color := FontColor; OffsetRect(Rect, -1, -1);
	DrawText( Canvas.Handle,szStr,Len,rect,Align);
      end;
	...
	Canvas.Font.Color := clBtnShadow; OffsetRect(Rect, 2, 2);
	DrawText( Canvas.Handle,szStr,Len,rect,Align);
	Canvas.Font.Color := FontColor; OffsetRect(Rect, -2, -2);
	DrawText( Canvas.Handle,szStr,Len,rect,Align);
      end;
      else
      begin
...
//  if State = [odSelected,odFocused] then exit;
  ...
  Rect1:=rect; rect1.right:=FSegment1Width;

  if IsItAFilledBitmap(WallpaperBmp) then
  begin
    if aloTransparentButtons in Options then DrawWallpaper( rect )
					else DrawWallpaper( rect1 );
  end
  else Canvas.FillRect(Rect1);
  ...
  BtnRect:=Rect; BtnRect.Left:=Rect1.Right+2;
  ...
  begin
    ...
    begin
      if i=FPushedButton[Index] then FPushedButton[Index] := 0//...none pushed
				else FPushedButton[Index] := i;
    end;
    ...
    begin DrawLBItem( idsRecessed,BtnRect ); OffsetRect(BtnTxtRect, 1, 1);
    end else DrawLBItem( idsRaised,BtnRect );
    //...button text
    if FPushedButton[Index]=i then ItemStyle.BtnFont.Style := [fsBold]
                              else ItemStyle.BtnFont.Style := [];
    DrawTextInRect(BtnTxtRect,FCaptionsAlign_ or DT_SINGLELINE,i );
    inc( BtnRect.Left, FButtonWidth+1); inc( BtnRect.Right, FButtonWidth+1);
  end;//_________________________________________________DRAW BUTTONS END
  MouseClickPoint.x:=-1;MouseClickPoint.y:=0;
  //~~~~~~~~~
  rect1.left:=3;
  rect1.right:=FSegment1Width;

  inc(rect1.top);inc(rect.left);
  dec(rect1.bottom);dec(rect.right);
...
procedure TglAskListBox.DrawWallpaper( r:TRect );
var
...
begin
  ...
  case WallpaperOption of
	...
	while y < r.bottom-r.top do begin
	  Canvas.Draw(r.Left+x, r.top+y, WallpaperBmp);
	  Inc(y, WallpaperBmp.Height);
	end;
	Inc(x, WallpaperBmp.Width); y:=0;
      end;
    else Canvas.Draw(r.left,r.top,WallpaperBmp);
  end;
  DeleteObject(UpdateRgn);
  RestoreDC(Canvas.Handle, SaveIndex);
end;
//______________________________________________________________
procedure TglAskListBox.DrawGlyph( r:TRect; Index:word; Shift:word );
var
  i:integer;
  OldRect: TRect;
begin
  ...
  i:=-1;
  if NumGlyphs = 1 then i:=0 else if Index < NumGlyphs then i:=Index;
  if i>=0 then
  begin
...
procedure TglAskListBox.SetAutoTrColor(Value: TglAutoTransparentColor);
begin
  if FAutoTrColor = Value then exit;
  FAutoTrColor := Value; Invalidate;
end;

procedure TglAskListBox.SetWallpaper(Value: TBitmap);
begin
  if Assigned(FWallpaper) then FWallpaper.Free;
  ...
  if Assigned(FWallpaper) then WallpaperBmp := FWallpaper
    else if Assigned(FWallpaperImage) then WallpaperBmp := FWallpaperImage.Picture.Bitmap
      else WallpaperBmp := nil;

  if FShowWallpaper then Invalidate;
end;

function TglAskListBox.GetWallpaper: TBitmap;
begin
...
procedure TglAskListBox.SetWallpaperImage(Value: Timage);
begin
...
procedure TglAskListBox.SetWOpt(Value: TglWallpaperOption);
begin
  FWallpaperOption := Value;
  if FShowWallpaper then Invalidate;
end;

procedure TglAskListBox.SetNumGlyphs( Value: word );
begin
...
{procedure TglAskListBox.SetItemStyle( Value: TItemsDrawStyle );
begin
...
procedure TglAskListBox.SetSelItemStyle( Value: TItemsDrawStyle );
begin
...
procedure TglAskListBox.SetGlyphs( Value: TImageList );
begin
...
{procedure TglAskListBox.SetSelFont( Value: TFont );
begin
...
{
procedure TglAskListBox.SetColor(Value: TColor);
begin
  if FColor = Value then exit;
  FColor := Value; Canvas.Brush.Color:=Value; Invalidate;
end;

procedure TglAskListBox.SetSelColor(Value: TColor);
begin
  if FColor = Value then exit;
  FSelColor := Value; Canvas.Brush.Color:=Value; Invalidate;
end;
}
procedure TglAskListBox.SetItemHeight(Value: word);
begin
...
procedure TglAskListBox.SetAlign( Align:T2DAlign; var Align_:UINT);
begin
  case Align.Horizontal of
    fhaLeft:			Align_:=Align_ or DT_LEFT;
    fhaCenter:			Align_:=Align_ or DT_CENTER;
    else			Align_:=Align_ or DT_RIGHT;
  end;
  case Align.Vertical of
    fvaTop:			Align_:=Align_ or DT_TOP;
    fvaCenter:			Align_:=Align_ or DT_VCENTER;
    else			Align_:=Align_ or DT_BOTTOM;
  end;
end;
{
procedure TglAskListBox.SetTextStyle(Value: TglTextStyle);
begin
  if FTextStyle = Value then exit;
  FTextStyle := Value; if FShowText then Invalidate;
end;

procedure TglAskListBox.SetButtonsTextStyle(Value: TglTextStyle);
begin
  if FButtonsTextStyle = Value then exit;
  FButtonsTextStyle := Value; if FShowText then Invalidate;
end;
}
procedure TglAskListBox.SetShowText(Value: boolean);
begin
  if FShowText = Value then exit;
  FShowText := Value; Invalidate;
end;

procedure TglAskListBox.SetTransparentColor(Value: TColor);
begin
...
procedure TglAskListBox.SetButtonWidth(Value: word);
begin
...
procedure TglAskListBox.SetOptions(Value: TglAskLBOptions);
begin
...
function TglAskListBox.IsFilled:boolean;
var
  i:word;
begin
  Result:=false;
  for i:=0 to Items.Count-1 do if FPushedButton[i]=0 then exit;
  Result:=true;
end;

function TglAskListBox.CountPushedButtonsInColon( Colon: integer ):integer;
var i: word;
begin
  Result:=0;
  if Colon=0 then
  begin
    for i:=0 to Items.Count-1 do if FPushedButton[i]<>0 then inc( Result );
  end
  else
    for i:=0 to Items.Count-1 do if FPushedButton[i]=Colon then inc( Result );
end;

procedure TglAskListBox.SetSelectedItem(Value: word);
begin
  if Value>=Items.Count then exit;
  SendMessage( Handle, LB_SETCURSEL, Value, Longint(0));
end;

procedure TglAskListBox.SetButtons(Value: TStringList);
begin
  if (Value=nil)or(Value.Count=0)then exit;
  FButtons.Assign(Value); RecalcHeights; Invalidate;
end;

function TglAskListBox.GetPushedButtonInLine( Index:word ):integer;
begin
  if Index>=Items.Count then Result:=-1
			else Result := FPushedButton[ Index ];
end;

function TglAskListBox.SetPushedButtonInLine( Index:word; Value:word ):boolean;
var
  r:TRect;
begin
...
procedure TglAskListBox.WMLButtonDown(var Message: TWMLButtonDown);
var R: TRect; ItemN: integer;
begin
  inherited;
...
procedure TglAskListBox.WMSize(var Message: TWMSize);
begin inherited; RecalcHeights; end;

procedure TglAskListBox.ButtonClicked;
begin
  if Assigned(FOnButtonClicked) then FOnButtonClicked(self);
end;

procedure TglAskListBox.RecalcHeights;
...
procedure TglAskListBox.SmthChanged(Sender: TObject);
begin
  FTextAlign_:=DT_WORDBREAK; FCaptionsAlign_:=DT_SINGLELINE;
...
procedure TglAskListBox.InitState(var State: TOwnerDrawState; ByteState: Byte);
begin
    State := [];
