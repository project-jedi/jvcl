{ 
 ...
 			  ' GLOBUS LIB '			   
  ...
  
 ===================================================================
 glCBox Unit 11.1998			       component TglCheckBox
 ===================================================================
}
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
//...created: 11.1998
...
type
  ...
  TglCheckBox = class(TGraphicControl)
  private
    FChecked		: boolean;
  ...
  public
    ...
    function GetCheckedItemInGroup: TglCheckBox;
  ...
  published
    ...
    property GlyphShift: TPointClass read FGlyphShift write FGlyphShift;
...
procedure Register;
begin
  RegisterComponents('Proba', [TglCheckBox]);
end;
{~~~~~~~~~~~~~~~~~~~~~~~~~}
...
//  FGlyphOff := TBitmap.Create;
  ...
  FGlyphShift := TPointClass.Create;
...
//______________________________________________________________
procedure TglCheckBox.CMFontChanged(var Message: TMessage);
begin
  if fSuppressCMFontChanged then exit;
  Img.Canvas.Font.Assign( Font ); Repaint;
  inherited;
end;
//______________________________________________________________
procedure TglCheckBox.CMMouseEnter(var Message: TMessage);
begin
  if not Enabled or(fcoIgnoreMouse in Options)or fShowAsActiveWhileControlFocused then exit;
  if Assigned(FocusControl)and(FocusControlMethod = fcmOnMouseEnter) then FocusControl.SetFocus;
  fNeedRebuildBackground := true;
  fActiveNow := true;
  with TextStyles,Colors do
    if (Passive<>Active)or(fcoUnderlinedActive in Options) then Paint
    else
      if (fcoDelineatedText in Options)and(DelineateActive <> Delineate) then Paint
      else if (not Transparent)and(Colors.Background <> Colors.BackgroundActive) then Paint
      else if (TextActive <> Text)or(fcoUnderlinedActive in Options) then
	   begin fNeedUpdateOnlyMainText := true; Paint; end;
  if Assigned(FOnMouseEnter) then FOnMouseEnter(self);
end;
//______________________________________________________________
procedure TglCheckBox.CMMouseLeave(var Message: TMessage);
begin
  if not Enabled or(fcoIgnoreMouse in Options)or fShowAsActiveWhileControlFocused then exit;
  fNeedRebuildBackground := true;
  fActiveNow := false;
...
//______________________________________________________________
procedure TglCheckBox.OnLButtonUp(var Message: TMessage);
var pt: TPoint;
begin
  if not Enabled or(fcoIgnoreMouse in Options) then exit;
  GetCursorPos(pt);
  pt := ScreenToClient(pt);
  if PtInRect( ClientRect, pt ) then SetChecked( not FChecked );
...
//______________________________________________________________
procedure TglCheckBox.OnLButtonDown(var Message: TMessage);
begin
  if not Enabled or(fcoIgnoreMouse in Options) then exit;
  inherited;
...
//______________________________________________________________
procedure TglCheckBox.WMSize(var Message: TMessage);
begin
  inherited;
//  Img.Width := Width; Img.Height := Height;
end;
//______________________________________________________________
procedure TglCheckBox.CMTextChanged(var Message: TMessage);
begin
  Invalidate;
end;
//______________________________________________________________
...
//fNeedRebuildBackground := false;
  ...
  if fcoBoldChecked in Options then
    if Checked then Font.Style := Font.Style + [fsBold]
               else Font.Style := Font.Style - [fsBold];
  if Enabled then
  begin
    if Checked then Bitmap := FGlyphOn else Bitmap := FGlyphOff;
    DrawState := fdsDefault;
  end else
  begin
    if FGlyphDisabled.Handle<>0 then
    begin Bitmap := FGlyphDisabled; DrawState := fdsDefault; end
    else begin if Checked then Bitmap := FGlyphOn else Bitmap := FGlyphOff; DrawState := fdsDefault; end;
  end;
  ...
  begin
    ...
    if fcoUnderlinedActive in Options then
      if fActiveNow then Font.Style := Font.Style + [fsUnderline]
  ...
  X := 0;
  if Assigned(FGlyphOn) then X := max( X, FGlyphOn.Width );
  if Assigned(FGlyphOff) then X := max( X, FGlyphOff.Width );
  if Assigned(FGlyphDisabled) then X := max( X, FGlyphDisabled.Width );
  if Assigned(FGlyph) then X := max( X, FGlyph.Width );
  ...
  begin
    R := GetClientRect;
    if fActiveNow then BackBrush := CreateSolidBrush(ColorToRGB(Colors.BackgroundActive))
                  else BackBrush := CreateSolidBrush(ColorToRGB(Colors.Background));
    FillRect( Img.Canvas.Handle, R, BackBrush );
    DeleteObject( BackBrush );
  end;

  if FTransparent and (not fNeedUpdateOnlyMainText) then
    if (not(fcoFastDraw in Options))or fNeedRebuildBackground or (csDesigning in ComponentState)then
	GetParentImageRect( self, Bounds(Left,Top,Width,Height), Img.Canvas.Handle );

  if Alignment = taLeftJustify then
  begin
    X := 0;
    if FGlyph <> nil then inc(X, FGlyph.Width);
  end else
    inc(X, Interspace);
  ...
  begin
    ...
    begin
      ...
      begin
        if not Transparent then
          ChangeBitmapColor( FGlyph, GetPixel( FGlyph.Canvas.Handle, 0, FGlyph.Height-1 ), clBtnFace );

        // glyph always left
	CreateBitmapExt( Img.Canvas.Handle, FGlyph, ClientRect, 0, max( 0, (Height - FGlyph.Height)div 2),
			 fwoNone, DrawState, Transparent,
			 GetPixel( FGlyph.Canvas.Handle, 0, FGlyph.Height-1 ){TransparentColor},
			 DisabledMaskColor );
      end;
      fNeedRebuildBackground := false;
    end;
    if not Transparent then
      if fActiveNow then
        ChangeBitmapColor( Bitmap, GetPixel( Bitmap.Canvas.Handle, 0, Bitmap.Height-1 ), Colors.BackgroundActive )
      else
        ChangeBitmapColor( Bitmap, GetPixel( Bitmap.Canvas.Handle, 0, Bitmap.Height-1 ), Colors.Background );
    ...
    if Assigned(Bitmap) then
      CreateBitmapExt( Img.Canvas.Handle, Bitmap, ClientRect, X, integer(GlyphShift.Y+max( 0, (Height - Bitmap.Height)div 2)),
		       fwoNone, DrawState, Transparent,
  ...
  BitBlt( Canvas.Handle, 0, 0, Img.Width, Img.Height, Img.Canvas.Handle, 0, 0, SRCCOPY );
...
//______________________________________________________________
procedure TglCheckBox.Notification(AComponent: TComponent; Operation: TOperation);
begin
...
//______
procedure TglCheckBox.CallMouseEnter;
var EmptyMsg: TMessage;
begin CMMouseEnter(EmptyMsg); end;
//______
procedure TglCheckBox.CallMouseLeave;
var EmptyMsg: TMessage;
begin CMMouseLeave(EmptyMsg); end;
//______________________________________________________________
procedure TglCheckBox.HookFocusControlWndProc;
var
  P:Pointer;
begin
...
//______
procedure TglCheckBox.UnhookFocusControlWndProc;
begin
...
//______
procedure TglCheckBox.FocusControlWndHookProc(var Msg_: TMessage);
begin
  case Msg_.Msg of
    WM_SETFOCUS:
...
//______
procedure TglCheckBox.SetFocusControl(Value: TWinControl);
begin
...
procedure TglCheckBox.OnGradientChanged(Sender: TObject);
begin
  if not( csLoading in ComponentState) then fNeedUpdateOnlyMainText := true;
  Repaint;
end;
//______________________________________________________________
procedure TglCheckBox.OnIlluminationChanged(Sender: TObject);
begin
  CalcShadowAndHighlightColors( (Parent as TWinControl).Brush.Color, Colors );
  Repaint;
end;
//______
function TglCheckBox.IsCustomGlyph: Boolean;
begin
  Result := FGlyphKind = fgkCustom;
end;

function TglCheckBox.GetCheckedItemInGroup: TglCheckBox;
var i: integer;
begin
  if FChecked then begin Result := self; exit; end;
  Result := nil;
  if GroupIndex<>0 then
  begin
    for i:=0 to Owner.ComponentCount-1 do
      if (OWner.Components[i] is TglCheckBox)
	 and(TglCheckBox(OWner.Components[i]).GroupIndex = GroupIndex)
	 and(TglCheckBox(OWner.Components[i]).Checked) then
	 begin
	   Result := TglCheckBox(OWner.Components[i]);
           break;
         end;
  end;
end;

procedure TglCheckBox.SetCheckedItemInGroup(TagNo: integer);
var i: integer;
begin
  if GroupIndex<>0 then
  begin
    for i:=0 to Owner.ComponentCount-1 do
      if (OWner.Components[i] is TglCheckBox)
	 and(TglCheckBox(OWner.Components[i]).GroupIndex = GroupIndex)
	 and(TglCheckBox(OWner.Components[i]).Tag = TagNo) then
	 begin
	   TglCheckBox(OWner.Components[i]).Checked := true;
           break;
         end;
  end;
end;
...
begin
  ...
  begin
	...
	if (OWner.Components[i] is TglCheckBox)
	    and(TglCheckBox(OWner.Components[i]).GroupIndex = GroupIndex)
	    and(TglCheckBox(OWner.Components[i]).Checked)
	    and(OWner.Components[i] <> Self) then
	    begin
	      TglCheckBox(OWner.Components[i]).FChecked := false;
	      TglCheckBox(OWner.Components[i]).fNeedRebuildBackground := true;
	      TglCheckBox(OWner.Components[i]).Repaint;
	    end;
      FChecked := true;
    end;
  end else FChecked := Value;
  Repaint;
end;

procedure TglCheckBox.SetGlyph(Value: TBitmap);
begin
  if Assigned(FGlyph) then FGlyph.Free;
  FGlyph := TBitmap.Create;
  FGlyph.Assign( Value ); fNeedRebuildBackground := true; Invalidate;
end;

function TglCheckBox.GetGlyph: TBitmap;
begin
  if not Assigned(FGlyph) then FGlyph := TBitmap.Create;
  Result := FGlyph;
end;

procedure TglCheckBox.SetGlyphOn(Value: TBitmap);
begin
  if Assigned(FGlyphOn) then FGlyphOn.Free;
...
function TglCheckBox.GetGlyphOn: TBitmap;
begin
  if not Assigned(FGlyphOn) then FGlyphOn := TBitmap.Create;
  Result := FGlyphOn;
end;

procedure TglCheckBox.SetGlyphOff(Value: TBitmap);
begin
  if Assigned(FGlyphOff) then FGlyphOff.Free;
...
function TglCheckBox.GetGlyphOff: TBitmap;
begin
  if not Assigned(FGlyphOff) then FGlyphOff := TBitmap.Create;
  Result := FGlyphOff;
end;

procedure TglCheckBox.SetGlyphDisabled(Value: TBitmap);
begin
  if Assigned(FGlyphDisabled) then FGlyphDisabled.Free;
...
function TglCheckBox.GetGlyphDisabled: TBitmap;
begin
  if not Assigned(FGlyphDisabled) then FGlyphDisabled := TBitmap.Create;
  Result := FGlyphDisabled;
end;

procedure TglCheckBox.SetGroupIndex(Value: integer);
begin
...
procedure TglCheckBox.SetOptions(Value: TglCBoxOptions);
begin
...
procedure TglCheckBox.SetTransparent(Value: boolean);
begin
  if FTransparent = Value then exit;
  FTransparent := Value; Repaint;
end;

procedure TglCheckBox.SetDisabledMaskColor(Value: TColor);
begin
  if FDisabledMaskColor = Value then exit;
  FDisabledMaskColor := Value;
  fNeedRebuildBackground := true; Invalidate;
end;

procedure TglCheckBox.SetInterspace(Value: integer);
begin
...
procedure TglCheckBox.SetGlyphKind(Value: TglGlyphKind);
begin
...
procedure TglCheckBox.SetAlignment(const Value: TLeftRight);
begin
  FAlignment := Value;
  Invalidate;
end;
