{ 
 ...
 			  ' GLOBUS LIB '			   
  ...
  
 ===================================================================
 glFlyTxt Unit 07.1998			         component TglFlyTxt
 ===================================================================
}
unit glFlyTxt;
...
type
  ...
  TglFlyingText = class(TCustomPanel)
  private
    ...
    FBackgrColor		: TColor;
    FGradient			: TGradient;
    FThreeDGradient		: TThreeDGradient;
  ...
  {$ENDIF}  
    ...
    property Gradient3D: TThreeDGradient read FThreeDGradient write FThreeDGradient;
    ...
    property FastDraw: boolean read FFastDraw write SetFastDraw
     default true;
    property View3D: T2DAlign read FView3D write FView3D;
    ...
    property Visible: boolean read FVisible write SetVisible
     default true;
    property StepShift: TPointClass read FStepShift write FStepShift;
  end;
...
procedure Register;
begin
  RegisterComponents('Proba', [TglFlyingText]);
end;
//*****************************************_____________LowLevel METHODS
//________________________________________________________
constructor TglFlyingText.Create(AOwner: TComponent);
begin
  ...
  FThreeDGradient:=TThreeDGradient.Create;
  FText:=TStringList.Create;
  FView3D:=T2DAlign.Create;
  FStepShift:=TPointClass.Create;
  uCurTextLine:=0;
  ...
  FVisible:=true;
  FStepShift.x:=0; FStepShift.y:=0;
  Shift:=Point(0,0);
  FText.Add('Hello');FText.Add('World');
  ...
  fNeedRebuildBitmaps:=true; fNeedRemakeBackground:=false;
  fLoaded:=false;
end;
//________________________________________________________
destructor TglFlyingText.Destroy;
begin
  FResBitmap.Free;
  FBackgrBitmap.Free;
  FTxtBitmap.Free;
...
begin
  case FHorAlign of
    fhaLeft:  begin
      FR.Left:=0; FP.x:=0; end;
    fhaCenter:begin
      FR.Left:=max(0,(Width-FScaledWidth)div 2);
      FP.x:=max(0,-(Width-FScaledWidth)div 2); end;
    fhaRight: begin
      FR.Left:=max(0,Width-FScaledWidth);
      FP.x:=max(0,FScaledWidth-Width); end;
  end;
  case FVertAlign of
    fvaTop:    begin FR.Top:=0; FP.y:=0; end;
    fvaCenter: begin
      FR.Top:=max(0,(Height-FScaledHeight)div 2);
      FP.y:=max(0,-(Height-FScaledHeight)div 2); end;
    fvaBottom: begin
      FR.Top:=max(0,Height-FScaledHeight);
      FP.y:=max(0,FScaledHeight-Height); end;
  end;
  FR.Left:=FR.Left+Shift.x; FR.Top:=FR.Top+Shift.y;
  FR.Width:=min(Width,FScaledWidth); FR.Height:=min(Height,FScaledHeight);
end;
//~~~~~~~~~~~~~~~~~END LOCAL PROC
begin
  ...
  begin
    ...
    BuildBitmaps;  fNeedRebuildBitmaps:=false;Visible:=FVisible;
  end;
  ...
  begin
	...
	BitBlt( Canvas.Handle, 0, 0, Width, Height, FBackgrBitmap.canvas.Handle, 0, 0, SRCCOPY);
    end;
    fNeedRepaintBackground:=true;

    FPartTxtBitmap.Width:=FR.Width; FPartTxtBitmap.Height:=FR.Height;
    OldFR.Width:=OldFR.Width+abs(Shift.x-OldShift.x);
    OldFR.Height:=OldFR.Height+abs(Shift.y-OldShift.y);
    FResBitmap.Width:=OldFR.Width; FResBitmap.Height:=OldFR.Height;
  ...
  end else {SLOW DRAWING}
  begin
    FPartTxtBitmap.Width:=Width; FPartTxtBitmap.Height:=Height;
    FResBitmap.Width:=Width; FResBitmap.Height:=Height;
...
begin
  ...
  if	( FScaledWidth < (FTxtBitmap.Width*FScalePercent/100) )
     ...
     or ( FScaledHeight > FTxtBitmap.Height )
  then
  begin
    if Assigned(FOnTextLineChanging) then
      FOnTextLineChanging(self,uCurTextLine);
    if uCurTextLine < FText.Count-1 then inc(uCurTextLine)
    else begin
      uCurTextLine:=0;
      if not FCycled then
      begin Active:=false; BuildBitmaps; exit; end;
    end;
    BuildTxtBitmap;
  end
  else fNeedRepaintBackground:=false;

  Repaint;
end;
//________________________________________________________
procedure TglFlyingText.RepaintBackground; //...for users
begin
  fNeedRepaintBackground:=true; Repaint;
end;
procedure TglFlyingText.RemakeBackground; //...for users
begin
  fNeedRemakeBackground:=true; Repaint;
end;
//________________________________________________________
procedure TglFlyingText.CalcTxtBitmapWidth;
var Size: TSize;
begin
  ...
  begin
    ...
    Shift:=Point(0,0); OldShift:=Shift;
  end;
end;
//________________________________________________________
procedure TglFlyingText.BuildTxtBitmap;
var R: TRect; i,x,y:integer;
begin
  //if (not Assigned(FText))or(FText.Count=0) then exit;
  FTxtBitmap.Canvas.Font.Assign(FResultFont);
  if FText.Count<>0 then CalcTxtBitmapWidth
  else begin FTxtBitmap.Width:=0; FTxtBitmap.Height:=0; end;
  ...
  case FView3D.Horizontal of
    ...
    else{fhaRight}      x := FThreeDGradient.Depth;
  end;
  case FView3D.Vertical of
    fvaTop:             y := 0;
    fvaCenter:          y := FThreeDGradient.Depth div 2;
    else{fvaBottom}     y := FThreeDGradient.Depth;
  end;

  SetBkMode(FTxtBitmap.Canvas.Handle,integer(TRANSPARENT));
  for i:=0 to FThreeDGradient.Depth-1 do begin
   if FThreeDGradient.GType=fgtFlat then
     FThreeDGradient.TextOut( FTxtBitmap.Canvas.Handle, FText[uCurTextLine], r, x, y )
   else begin {fgt3D}
     FTxtBitmap.Canvas.Font.Color
      :=FThreeDGradient.GetColorFromGradientLine( FThreeDGradient.Depth, i );
     FTxtBitmap.Canvas.TextOut(x,y,FText[uCurTextLine]);
   end;
   if x < (FThreeDGradient.Depth div 2) then inc(x) else dec(x);
   if y < (FThreeDGradient.Depth div 2) then inc(y) else dec(y);
  end;
...
//________________________________________________________
procedure TglFlyingText.BuildBitmaps;
var R: TRect; fOldTimer: boolean;
begin
...
//    FPartTxtBitmap.Width:=Width; FPartTxtBitmap.Height:=Height;
  ...
  fOldTimer:=FTimer.Enabled; FTimer.Enabled:=false;
  if fNeedRemakeBackground then
  begin
    //________________________________PrepareBackground____begin
//    BringParentWindowToTop(self);
    FBackgrBitmap.Width:=Width; FBackgrBitmap.Height:=Height;

    //..prepare tabula rasa
    if FTransparent then FBackgrBitmap.Canvas.Brush.Color:=Parent.Brush.Color
		    else FBackgrBitmap.Canvas.Brush.Color:=FBackgrColor;
...
//________________________________________________________
procedure TglFlyingText.WMSize(var Message: TWMSize);
begin
  fNeedRemakeBackground:=true;
  BuildBitmaps;   
  Repaint_;
...
//________________________________________________________
procedure TglFlyingText.OnParamsChanged(Sender: TObject);
begin BuildBitmaps; Repaint_; end;
//________________________________________________________
procedure TglFlyingText.Repaint_;
//var R:TRect;
begin
...
//________________________________________________________
procedure TglFlyingText.SetHorAlign(Value: TglHorAlign);
begin FHorAlign:=Value; BuildBitmaps; Repaint_; end;
//________________________________________________________
procedure TglFlyingText.SetVertAlign(Value: TglVertAlign);
begin FVertAlign:=Value; BuildBitmaps; Repaint_; end;
//________________________________________________________
procedure TglFlyingText.SetTransparent(Value: boolean);
begin FTransparent:=Value; fNeedRemakeBackground:=true; BuildBitmaps; Repaint_; end;
//________________________________________________________
procedure TglFlyingText.SetBackgrColor(Value: TColor);
begin FBackgrColor:=Value; BuildBitmaps; Repaint_; end;
//________________________________________________________
procedure TglFlyingText.SetInteriorOffset(Value: word);
begin FInteriorOffset:=Value; end;
//________________________________________________________
procedure TglFlyingText.SetScalePercent(Value: TSpPercent);
begin FScalePercent:=Value; BuildBitmaps; Repaint_; end;
//________________________________________________________
procedure TglFlyingText.SetStepScaleFactor(Value: Single);
begin
  if Value<1 then Value:=1; if Value>2 then Value:=2;
  FStepScaleFactor:=Value; BuildBitmaps; Repaint_;
end;
//________________________________________________________
procedure TglFlyingText.SetResultFont(Value: TFont);
begin if Value<>nil then FResultFont.Assign(Value);BuildBitmaps;end;
//________________________________________________________
procedure TglFlyingText.SetTimerInterval(Value: word);
begin FTimerInterval:=Value; FTimer.Interval:=FTimerInterval; end;
//________________________________________________________
procedure TglFlyingText.SetActive(Value: boolean);
begin FActive:=Value; FTimer.Enabled:=FActive; end;
//________________________________________________________
procedure TglFlyingText.SetText(Value: TStringList);
var OldActive: boolean; i: integer;
begin
  OldActive:=FActive; Active:=false;
  if Assigned(Value) then FText.Assign(Value); uCurTextLine:=0;
  if FText.Count<>0 then
    for i:=0 to FText.Count-1 do FText[i]:=Trim(FText[i]);
  BuildBitmaps; Repaint_; Active:=OldActive;
end;
//________________________________________________________
procedure TglFlyingText.SetFastDraw(Value: boolean);
begin
  if FFastDraw = Value then exit;
  FFastDraw := Value;
  BuildBitmaps; Repaint_;
end;
//________________________________________________________
procedure TglFlyingText.SetDirection(Value: TglScalingDir);
begin
...
//________________________________________________________
procedure TglFlyingText.SetShowTextWhilePassive(Value: boolean);
begin
  if FShowTextWhilePassive = Value then exit;
  FShowTextWhilePassive := Value;
  BuildBitmaps; Repaint_;
end;
//________________________________________________________
procedure TglFlyingText.SetVisible(Value: boolean);
begin
  if FVisible = Value then exit;
  FVisible := Value; inherited Visible := FVisible;
  if FVisible then begin BuildBitmaps; Repaint_; end;
end;
//________________________________________________________
