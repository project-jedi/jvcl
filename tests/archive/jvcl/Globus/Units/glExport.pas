...
var
  ...
  var i: integer;
  begin
    for i:=1 to Length(str) do if (str[i] = #13)then str[i] := ' ';
    Result := str;
  end;
begin
...
//  Sheet.Cells[2, 1] := 'Globus'; Sheet.Cells[2, 1].Font.Bold := True; Sheet.Cells[2, 1].Font.Color := clWhite;
  ...
  begin
    ...
    if DataSet.Fields[i].DisplayLabel <> '' then Sheet.Cells[RecNo, 2+ColIndex] := DataSet.Fields[i].DisplayLabel
                                            else Sheet.Cells[RecNo, 2+ColIndex] := DataSet.Fields[i].FieldName;
