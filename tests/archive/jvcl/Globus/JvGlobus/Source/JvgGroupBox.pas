...
uses
  ...
  StdCtrls, ExtCtrls, JvgTypes, JvgUtils, JvgCommClasses;
...
//    procedure WMLButtonDown(var Message: TWMLButtonDown); message WM_LBUTTONDOWN;
  ...
  {$ENDIF}
    ...
    property CaptionBorder: TJvgBevel read FCaptionBorder write FCaptionBorder;
    ...
    begin//...One Stay Always Expanded in group
      ...
      for i:=0 to Owner.ComponentCount-1 do with TControl(Owner) do
        if (Components[i] is TJvgGroupBox)and
           ...
           if
           (not TJvgGroupBox(Components[i]).Collapsed)and(Components[i] <> self )
    ...
    if (FGroupIndex<>0)and(fgoCollapseOther in Options) then
      for i:=0 to Owner.ComponentCount-1 do with TControl(Owner) do
        if (Components[i] is TJvgGroupBox)and
           ...
           (Components[i] <> self )
        then TJvgGroupBox(Components[i]).Collapsed := true;
...
//  if not(fgoCanCollapse in Options) then Collapsed := false;
  if Assigned(Parent) then
    CalcShadowAndHighlightColors( (Parent as TWinControl).Brush.Color, TJvgLabelColors(Colors) );
  Invalidate;
end;
