...
{$EXTERNALSYM TJvDataType}
  ...
  private
    ...
    procedure SetMax(const Value: TMaxItems);
  ...
  published
    ...
    property MaxItems: TMaxItems read FMax write SetMax default 10;
...
const
  cRegKey: array[TJvHKey] of HKEY = (HKEY_CLASSES_ROOT, HKEY_CURRENT_USER,
...
begin
  if (FList <> 0) and (FType = dtBinary) then
...
begin
  if (FList <> 0) and (FType = dtString) then
...
function TJvMruList.Clear: boolean;
begin
  Result := RegDeleteKey(cRegKey[FKey], PChar(FSubKey)) = ERROR_SUCCESS;
  if Result then
    RecreateList;
end;
...
begin
  if FList <> 0 then
    Result := FindMruData(FList, Pointer(Value^), Size, nil)
  else
    Result := -1;
end;
...
procedure TJvMruList.SetMax(const Value: TMaxItems);
begin
  if Value <> FMax then
  begin
    FMax := Value;
