...
interface
uses
  Windows, Messages, SysUtils, Classes, JvComponent;
...
type
  ...
  protected
    procedure DoShellMessage(var Message: TMessage); dynamic;
    procedure ShellHookMethod(var Message: TMessage);
  public
...
procedure TJvShellHookComponent.DoShellMessage(var Message: TMessage);
begin
  if Assigned(FOnShellMessage) then
    FOnShellMessage(self, Message);
end;
  ...
  begin
    ...
    begin
      if not InitJvShellHooks then
        Exit; // raise ?
      FWndHandle := {$IFDEF COMPILER6_UP}Classes.{$ENDIF}AllocateHWnd(ShellHookMethod);
...
procedure TJvShellHookComponent.ShellHookMethod(var Message: TMessage);
begin
  if Message.Msg = FHookMsg then
    DoShellMessage(Message)
  else
    inherited;
end;
