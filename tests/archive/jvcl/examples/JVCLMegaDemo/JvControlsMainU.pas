...
type
  TJvControlsMainForm = class(TForm)
...
procedure TJvControlsMainForm.FormCreate(Sender: TObject);
begin
  Timer1Timer(nil);
  JvTrackBar1.Position := 20;
end;

procedure TJvControlsMainForm.Timer1Timer(Sender: TObject);
begin
  JvStatusBar1.Panels[1].Text := TimeToStr(Now);
end;


procedure TJvControlsMainForm.JvTrackBar1ToolTip(Sender: TObject; var ToolTipText: String);
begin
  ToolTipText := Format('TJvListBox displays %d items now', [JvTrackBar1.Position]);
end;

procedure TJvControlsMainForm.JvTrackBar1Change(Sender: TObject);
begin
  JvListBox1.Count := JvTrackBar1.Position;
end;

procedure TJvControlsMainForm.JvListBox1GetText(Sender: TWinControl; Index: Integer;
  var Text: String);
begin
  Text := Format('Item no.: %d', [Index]);
end;

procedure TJvControlsMainForm.Button1Click(Sender: TObject);
begin
  JvPageControl1.ActivePageIndex := 1;
end;

procedure TJvControlsMainForm.Button2Click(Sender: TObject);
begin
  JvPageControl1.ActivePageIndex := 0;
end;

procedure TJvControlsMainForm.JvImgBtn6MouseEnter(Sender: TObject);
begin
...
procedure TJvControlsMainForm.JvImgBtn6MouseLeave(Sender: TObject);
begin
  with TJvImgBtn(Sender) do
  begin
    Color := clBtnFace;
...
procedure TJvControlsMainForm.JvImgBtn7GetAnimateIndex(Sender: TObject;
  CurrentAnimateFrame: Byte; var ImageIndex: Integer);
begin
  ImageIndex := CurrentAnimateFrame * 2 + 2;
end;

procedure TJvControlsMainForm.RadioGroup1Click(Sender: TObject);
begin
  JvImgBtn4.Alignment := TAlignment(RadioGroup1.ItemIndex);
end;

procedure TJvControlsMainForm.RadioGroup2Click(Sender: TObject);
begin
