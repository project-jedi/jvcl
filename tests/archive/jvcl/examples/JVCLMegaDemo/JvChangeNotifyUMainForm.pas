...
type
  TJvChangeNotifyMainForm = class(TForm)
    CN1: TJvChangeNotify;
    ListBox2: TListBox;
    Label3: TLabel;
    ListView1: TListView;
...
uses JvChangeNotifyaddDirDlgU;

{$R *.DFM}

procedure TJvChangeNotifyMainForm.ResetCaptions(Invert: boolean);
const
...
procedure TJvChangeNotifyMainForm.FormCloseQuery(Sender: TObject; var CanClose: Boolean);
begin
  CN1.Active := false;
end;

procedure TJvChangeNotifyMainForm.btnDeleteClick(Sender: TObject);
begin
...
procedure TJvChangeNotifyMainForm.btnAddClick(Sender: TObject);
begin
  EditItem(nil);
end;

procedure TJvChangeNotifyMainForm.btnStartClick(Sender: TObject);
var b: boolean;
begin
...
procedure TJvChangeNotifyMainForm.btnClearClick(Sender: TObject);
begin
  ListBox2.Clear;
  ResetCaptions(false);
end;

procedure TJvChangeNotifyMainForm.CN1ChangeNotify(Sender: TObject; Dir: string;
  Actions: TJvChangeActions);
begin
...
procedure TJvChangeNotifyMainForm.WMGetMinMaxINfo(var Msg: TWMGetMinMaxInfo);
begin
...
procedure TJvChangeNotifyMainForm.EditItem(li: TListItem);
...
procedure TJvChangeNotifyMainForm.DeleteItem(li: TListItem);
begin
  if li = nil then
    Exit;
  if li.Data <> nil then
...
procedure TJvChangeNotifyMainForm.ListView1DblClick(Sender: TObject);
begin
