...
The Original Code is: JvFlashApplication.PAS, released on 2001-02-28.
...
type
  TJvFlashApplication = class(TJvComponent)
  private
    FEnabled: Boolean;
    FDelay: Integer;
    FTimer: TTimer;
...
constructor TJvFlashApplication.Create(AOwner: TComponent);
begin
  inherited;
  FDelay := 200;
  FFlashed := False;
...
destructor TJvFlashApplication.Destroy;
begin
...
procedure TJvFlashApplication.SetEnabled(En: Boolean);
begin
  FEnabled := En;
...
procedure TJvFlashApplication.SetDelay(De: Integer);
begin
  FDelay := De;
  FTimer.Interval := De;
end;
...
procedure TJvFlashApplication.OnFTimer(Sender: TObject);
begin
