...
uses
  ...
  StdCtrls, ExtCtrls, JvDiagramShape, ComCtrls, Psock, NMHttp, ImgList;
...
begin {ConnectShapes}
  with TJvSingleHeadArrow.Create(Self) do begin
  ...
  with ScrollBox1 do begin
    ...
    while i < ControlCount do begin
      if (Controls[i] is TJvCustomDiagramShape) and
         ...
         (Controls[i] <> ExceptShape.Caption) then begin
...
begin {ParseDoc}
  ...
  with TjimHtmlParser.Create do begin
    try
      if FCurrentUrl = '' then begin
        ClearAll(nil);
        Exit;
      end;
      ...
      if FCurrentShape = nil then begin
      ...
      if GetLastParentUrl > '' then begin
      ...
      for i := 0 to SymbolTable.Count - 1 do begin
        ...
        case SymbolTable.Items[i].SymbolType of
          ...
          stLink  : begin
            ...
            if StrLIComp('ftp://',PChar(TempStr),6) = 0 then begin
              TempIndex := 2;
            end else if StrLIComp('mailto:',PChar(TempStr),7) = 0 then begin
              TempIndex := 3;
            end else if StrLIComp('news:',PChar(TempStr),5) = 0 then begin
              TempIndex := 4;
            end else if StrLIComp('file://',PChar(TempStr),7) = 0 then begin
              TempIndex := 0;
            end else begin
              TempIndex := 0;
              IsLink    := True;

              if StrLIComp('http://',PChar(TempStr),7) <> 0 then begin
                ...
                if not ((FCurrentUrl[Length(FCurrentUrl)] in ['/','\']) or
                        ((Length(TempStr) > 0) and (TempStr[1] in ['/','\']))) then begin
                  TempStr := '/' + TempStr;
                end;
            ...
            if IsLink and Assigned(ChildShape) then begin
...
begin {GetLastParentUrl}
  ...
  if FParentUrlList.Count > 0 then begin
...
begin {ParentDblClick}
  if Sender is TJvBitmapShape then begin
