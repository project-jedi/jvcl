...
{$ENDIF}
  ...
  { TJvImageBox2 }
  TJvImageBox2 = class(TJvGraphicControl)
  private
    FHiColor,
      FBackColor,
      TmpColor: TColor;
...
constructor TJvImageBox2.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
...
procedure TJvImageBox2.ImageListChange(Sender: Tobject);
begin
  Repaint;
end;

destructor TJvImageBox2.Destroy;
begin
  FImageChangeLink.Free;
  inherited Destroy;
end;

procedure TJvImageBox2.Notification(AComponent: TComponent; Operation: TOperation);
begin
  if (aComponent = FImageList) and (Operation = opRemove) then
    FImageList := nil;
end;

procedure TJvImageBox2.PaintFrame;
var R: TRect;
begin
  R := GetClientRect;
...
procedure TJvImageBox2.SetBkColor(Value: TColor);
begin
  if FBackColor <> Value then
  begin
    FBackColor := Value;
...
procedure TJvImageBox2.SetHiColor(Value: TColor);
begin
...
procedure TJvImageBox2.SetBorderStyle(Value: TBorderStyle);
begin
...
procedure TJvImageBox2.SetIndex(Value: integer);
begin
...
procedure TJvImageBox2.SetImageList(Value: TImageList);
begin
  if ImageList <> nil then
    ImageList.UnRegisterChanges(FImageChangeLink);
  FImageList := Value;
...
procedure TJvImageBox2.MouseUp(Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  inherited MouseUp(Button, Shift, X, y);
  FDown := False;
  if FShowClick then
...
procedure TJvImageBox2.MouseDown(Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
...
procedure TJvImageBox2.CMMouseEnter(var Message: TMessage);
begin
  inherited;
...
procedure TJvImageBox2.CMMouseLeave(var Message: TMessage);
begin
  inherited;
  FDown := False;
