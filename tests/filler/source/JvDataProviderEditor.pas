...
begin
  if Index = 0 then
    ...
    Result := (Provider as IJvDataIDSearch).FindByID(FViewItems[Index].ID, True);
end;
...
begin
  ...
  begin
    if ID = (FRootItem as IJvDataItem).GetID then
      ...
      Item := (Provider as IJvDataIDSearch).FindByID(ID, True);
    if Item <> nil then
    begin
  ...
  begin
    ...
    begin
      if Index = 0 then
        ...
        Item := (Provider as IJvDataIDSearch).FindByID(Info.ID, True);
      if (Item <> nil) and Supports(Item, IJvDataItems, Items) then
      begin
        Inc(Index);
...
begin
  ...
  if ItemData.ID = (FRootItem as IJvDataItem).GetID then
    ...
    DataItem := (FProvider as IJvDataIDSearch).FindByID(ItemData.ID, True);
