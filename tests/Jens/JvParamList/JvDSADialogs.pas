...
procedure ShowMessageFmt(const Msg: string; const Params: array of const;
  const Center: TDlgCenterKind = dckScreen; const Timeout: Integer = 0; const ADynControlEngine : TJvDynControlEngine = NIL);
...
function MessageDlg(const Msg: string; const DlgType: TMsgDlgType;
  ...
  const HelpButton: TMsgDlgBtn = mbHelp; const ADynControlEngine : TJvDynControlEngine = NIL): TModalResult; overload;
...
function MessageDlgEx(const Msg: string; const DlgType: TMsgDlgType;
  ...
  const HelpButton: Integer = -1; const ADynControlEngine : TJvDynControlEngine = NIL): TModalResult; overload;
...
function DSAMessageDlgEx(const DlgID: Integer; const Msg: string; const DlgType: TMsgDlgType;
  ...
  const HelpCtx: Longint; const Center: TDlgCenterKind = dckScreen; const Timeout: Integer = 0;
  const DefaultButton: Integer = 0; const CancelButton: Integer = 1;
  const HelpButton: Integer = -1; const ADynControlEngine : TJvDynControlEngine = NIL): Integer; overload;
...
//        DynControlLabel.ControlSetWordWrap(True);
      ...
      for I := Low(Buttons) to High(Buttons) do
      begin
        with DynControlEngine.CreateButton(Result, Panel,
               'Button'+inttostr(I), Buttons[I], '', nil, False, False) do
        begin
      ...
      if ATimeout > 0 then
      begin
        CountDownlabel := DynControlEngine.CreateLabelControl(
                              Result, Panel, 'Countdown', Format(RsCntdownText, [Timeout, TimeoutUnit(Timeout)]), Nil);
...
procedure ShowMessageFmt(const Msg: string; const Params: array of const;
  const Center: TDlgCenterKind; const Timeout: Integer; const ADynControlEngine : TJvDynControlEngine);
begin
...
begin
  ...
  try
    Result := MessageDlg(DlgCaption(DlgType), Msg, TmpPic, Buttons, HelpCtx, Center, Timeout,
      DefaultButton, CancelButton, HelpButton, ADynControlEngine);
  finally
    TmpPic.Free;
  end;
end;
...
function MessageDlgEx(const Msg: string; const DlgType: TMsgDlgType;
  ...
  const CancelButton: Integer; const HelpButton: Integer; const ADynControlEngine : TJvDynControlEngine): TModalResult;
var
  TmpPic: TGraphic;
begin
  TmpPic := DlgPic(DlgType);
...
begin
  with CreateDSAMessageForm(Caption, Msg, Picture, Buttons, Results, HelpCtx, '', Center,
    Timeout, DefaultButton, CancelButton, HelpButton, ADynControlEngine) do
  try
    Result := ShowModal;
  finally
    Free;
...
function DSAMessageDlgEx(const DlgID: Integer; const Msg: string; const DlgType: TMsgDlgType;
  const Buttons: array of string; const Results: array of Integer; const HelpCtx: Longint;
  const Center: TDlgCenterKind; const Timeout: Integer; const DefaultButton: Integer;
  const CancelButton: Integer; const HelpButton: Integer; const ADynControlEngine : TJvDynControlEngine): Integer;
var
  TmpPic: TGraphic;
begin
  TmpPic := DlgPic(DlgType);
...
function DSAMessageDlgEx(const DlgID: Integer; const Caption, Msg: string;
  ...
  HelpButton: Integer; const ADynControlEngine : TJvDynControlEngine): TModalResult;
var
  TmpPic: TGraphic;
begin
  TmpPic := DlgPic(DlgType);
...
function DSAMessageDlgEx(const DlgID: Integer; const Caption, Msg: string; const Picture: TGraphic;
  ...
  HelpButton: Integer; const ADynControlEngine : TJvDynControlEngine): Integer;
var
  DSAItem: TDSARegItem;
  CheckCaption: string;
  Temp: string;
begin
  ...
  begin
    ...
    with CreateDSAMessageForm(Caption, Msg, Picture, Buttons, Results, HelpCtx,
      CheckCaption, Center, Timeout, DefaultButton, CancelButton, HelpButton, ADynControlEngine) do
    try
      Result := ShowModal;
