...
begin
  if (attributes.count>0) and (index<attributes.count) then begin
...
begin
  if (nodes.count>0) and (index<nodes.count) then begin
...
var aName,newpath,sindex:string;
...
//  result:=nil;
  ...
  doappend:=p>0;
  c:=nodes.Count;
  if (not doappend) and (c>0) then begin
    for i:=0 to c-1 do begin
      n:= TJvXMLNode(nodes[i]);
      if n.Name=aname then begin
        if newpath='' then begin
...
function TJvXMLNode.ForceNamePathNodeAttribute(aPath, aName: string;
  aValue: variant): TJvXMLAttribute;
var n:TJvXMLNode;a:TJvXMLAttribute;
begin
  result:=nil;
...
begin
  ...
  if n=nil then exit;
  pn:=n.parentNode;
  for i:=0 to pn.nodes.count-1 do begin
    if TJvXMLNode(pn.nodes[i])=n then begin
      pn.DeleteNode(i);
      exit;
    end;
  end;
...
begin
  ...
  if a=nil then exit;
  pn:=a.parent;
  for i:=0 to pn.attributes.count-1 do begin
    if TJvXMLAttribute(pn.attributes[i])=a then begin
