...
uses
  Windows, Messages, SysUtils, Classes, Graphics, Forms, Controls, jvComponent,StdCtrls;
...
begin
  ...
  if solText='' then
    ...
    s:=Eoltext;
  if Acanvas.TextWidth(s)<=available then begin
    soltext:=s;
    eoltext:='';
    exit;
  end;
...
procedure TJvHTMLElement.SetBounds(const Value: TRect);
begin
  FBounds := Value;
end;
...
var p:integer;
    ...
    begin
      ...
      havepar:=false;
      pp:=pos(' ',ss);
      if pp=0 then begin  // tag only
        atag:=ss;
      end
      else begin  // tag + atrributes
        atag:=copy(ss,1,pp-1);
      ...
      if atag='br' then
        fBreakLine:=true
      else if atag='b' then begin// bold
        pushtag;
        fstyle:=fstyle+[fsbold];
      end
      else if atag='/b' then begin // cancel bold
        fstyle:=fstyle-[fsbold];
        poptag;
      end
      else if atag='i' then begin // italic
        pushtag;
        fstyle:=fstyle+[fsitalic];
      end
      else if atag='/i' then begin // cancel italic
        fstyle:=fstyle-[fsitalic];
        poptag;
      end
      else if atag='u' then begin // underline
        pushtag;
        fstyle:=fstyle+[fsunderline];
      end
      else if atag='/u' then begin // cancel underline
        fstyle:=fstyle-[fsunderline];
        poptag;
      end
...
begin
  ...
  repeat
    ...
    repeat  // scan line
      el:=TJvHTMLElement(ElementStack.items[i]);
      if el.BreakLine then begin
        if not pendingBreak then begin
          pendingBreak:=true;
          eol:=true;
          ieol:=i-1;
        //  break;
      ...
      if not pendingBreak then begin
        ...
        el.Break(bm.canvas,xav);
        if el.soltext<>'' then begin
          xav:=xav-bm.canvas.TextWidth(el.Soltext);
          if el.EolText='' then begin
            if i>=c-1 then begin
              eol:=true;
              ieol:=i;
            end
