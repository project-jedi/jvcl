...
begin
 if assigned(FGridPrinter) then begin
...
//Alignment must be done another day
    ...
    begin
      ...
      if DoPaint then begin
        if GridPrinter.PrintOptions.BorderStyle =bssingle then begin
        ...
        if (GridPrinter.WordWrap and (iCol<>0) and (iRow<>0)) then begin
...
begin
  ...
  begin
    ...
    if GridPrinter.printoptions.Logo <>'' then
     if fileexists(GridPrinter.printoptions.logo) then begin
