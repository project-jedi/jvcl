...
begin
  ...
  for i:=0 to wc.ControlCount-1 do
    ...
    begin
      ...
      Rb.left:=Rb.right-32;
      if ptinrect(Rb,point(R.left,R.top)) then break
      else if ptinrect(Rb,point(R.right,R.top)) then break
      else if ptinrect(Rb,point(R.right,R.bottom)) then break
      else if ptinrect(Rb,point(R.left,R.bottom)) then break
      else keep:=true;
    end;
...
begin
  ...
  DButton:=false;
  DLight:=false;
  if ptinrect(RCon,p) then Dcon:=true
  else if ptinrect(RLogic,p) then DLogic:=true
  else if ptinrect(RButton,p) then DButton:=true
  else if ptinrect(RLight,p) then DLight:=true
  else if ptinrect(RRev,p) then DRev:=true;
  invalidate;
end;
