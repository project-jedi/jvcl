...
begin
  ...
  if i=0 then exit;
  while (i>0) and (sourceString[i] <> Findchar) do dec(i);
  result:=i;
end;
...
begin
  ...
  for index:=0 to c-1 do begin
     try
       ...
       p:=pos('..',s);
       if p=0 then begin
         if strtodate(aList[index])=d then begin
           result:=index;
           exit;
         end;
       end
         ...
         s1:=trim(copy(s,1,p-1));
         s2:=trim(copy(s,p+2,length(s)));
         if (d>=strtoDate(s1)) and (d<=strtoDate(s2)) then begin
           result:=index;
           exit;
         end;
       end;
...
var i,e,e2,c:integer;
begin
  result:=0;
  i:=0;
  c:=length(s);
  if c=0 then exit;
  while (i+1<=c) and (s[i+1] in ['0'..'9',',','.']) do inc(i);
  if (i+1<=c) and (s[i+1] in ['e','E']) then begin
    e:=i;
    inc(i);
...
//    var cc:integer;
    ...
    begin
      ...
      else if s[1]='[' then begin // get block
        p:=posstr(']',s,2);
        if p=0 then
...
begin
   ...
   while gettoken do begin
     if token='cstr' then opush(dfoCstr)
     else if token='seldir' then opush(dfoseldir)
     else if token='dsobase' then opush(dfodsobase)
     else if token='dup' then opush(dfoDup)
     else if token='drop' then opush(dfoDrop)
     else if token='swap' then opush(dfoSwap)
     ...
     else if token='until' then begin
       atomvalue:=rpop;
       pushatom(dfoUntil);
     end
     else if token='now' then opush(dfonow)
     else if token='datestr' then opush(dfodatestr)
     else if token='timestr' then opush(dfotimestr)
     else if token='shellexecute' then opush(dfoshellexecute)
     else if token=';' then opush(dfoEndSub)
     else if token='crlf' then opush(dfocrlf)
     else if token='--' then opush(dfoNegate)
     else if token='-' then opush(dfoSubtract)
     else if token='+' then opush(dfoAdd)
     else if token='*' then opush(dfoMultiply)
     else if token='/' then opush(dfoDivide)
     else if token='^' then opush(dfoPower)
     else if token='abs' then opush(dfoAbs)
     else if token='left' then opush(dfoleft)
     else if token='right' then opush(dforight)
     else if token='sqr' then opush(dfosqr)
     else if token='sqrt' then opush(dfosqrt)
     else if token='sin' then opush(dfosin)
     else if token='cos' then opush(dfocos)
     else if token='tan' then opush(dfotan)
     else if token='arcsin' then opush(dfoarcsin)
     else if token='arccos' then opush(dfoarccos)
     else if token='arctan' then opush(dfoarctan)
     else if token='arctan2' then opush(dfoarctan2)
     else if token='pi' then opush(dfopi)
     else if token='<>' then opush(dfoNe)
     else if token='>=' then opush(dfoGe)
     else if token='>' then opush(dfoGt)
     else if token='<=' then opush(dfoLe)
     else if token='<' then opush(dfoLt)
     else if token='=' then opush(dfoEq)
     else if token='or' then opush(dfoOr)
     else if token= 'and' then opush(dfoAnd)
     else if token='in' then opush(dfoIn)
     else if token='xor' then opush(dfoXor)
     else if token='not' then opush(dfoNot)
     else if token='like' then opush(dfoLike)
     else if token='unlike' then opush(dfoUnLike)
     // check for block
     ...
     else if (token[1]='_') and (length(token)>1)  then begin
       p:=pos('.',token);
       if (p=0) or (p<3) or (p=length(token)) then
...
begin
  ...
  if assigned(onSetVariable) then begin
...
begin
  ...
  if assigned(onSetSystem) then begin
...
begin
  ...
  else if aMethod='load' then begin
    table.LoadFromFile(FDSOBase+'\'+aName+'.txt');
  end
  else if aMethod='save' then begin
    table.SaveToFile(FDSOBase+'\'+aName+'.txt');
  end
  else if aMethod='clear' then begin
    table.Clear;
  end
  else if aMethod='count' then begin
    c:=table.Count;
    vpush(c);
  end;
end;
...
begin
  aName:=CurrentSymbol;
  aMethod:=CurrentValue;
  if aMethod='set' then procAssign
  else if aMethod='get' then procVariable
  else
...
begin
  aName:=CurrentSymbol;
  aMethod:=CurrentValue;
  if aMethod='set' then procVarSet
  else if aMethod='get' then procVarGet
  else if aMethod='1+' then  procVarInc
  else if aMethod='[1+]' then  procVarIncIndex
  else if aMethod='1-' then procVarDec
  else if aMethod='1-?0' then procVarDecTestZero
  else if aMethod='+' then procVarAdd
  else if aMethod='-' then procVarSub
  else if aMethod='*' then procVarMul
  else if aMethod='/' then procVarDiv
  else if aMethod='--' then procVarNeg
  else if aMethod='load' then procVarLoad
  else if aMethod='save' then procVarSave
  else
...
begin
  aName:=CurrentSymbol;
  aMethod:=CurrentValue;
  if aMethod='set' then procSysSet
  else if aMethod='get' then procSysGet
  else
...
begin
  ...
  else if aMethod='@set' then begin
    aPath:=vpop;
    atName:=vpop;
    avalue:=vpop;
  ...
  else if aMethod='astext' then begin
  ...
  else if aMethod='select' then begin
    apath:=vpop;
  ...
  else if aMethod='selectfirst' then begin
  ...
  else if aMethod='selectnext' then begin
    b:=FXMLSelect.Count<>0;
    if b then
      inc(FXMLSelectRecord)
    else
      FXMLSelectRecord:=-1;
    if FXMLSelectRecord>=FXMLSelect.count then begin
      b:=false;
      FXMLSelectRecord:=-1;
    end;
    avalue:=b;
  ...
  else if aMethod='selectget' then begin
