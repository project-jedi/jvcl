...
var x0,x1,y0,y1,a0,a1,da0,da1:real;
...
if (r0+r1)> xs then begin
  showmessage('sum of radi to large');
  exit;
  end;
if (r0<5) or (r1<5) then begin
...
d:=abs(y-y0);
damult:=1;
if not polygonchecked then begin
   case starpoints of
    5:damult:=2;
    7:damult:=3;
    9:damult:=4;
...
//   moveto(x,y);
   apoint:=point(x,y);
   for i:=1 to starpoints-1 do begin
...
var
  t,xcenter,a,ycenter,b:integer;r1,g1,b1,r2,g2,b2:byte;
...
var j:integer;
begin
for j:=YOrigin to YFinal do HorGradientLine(bitmap,XOrigin,XFinal,j,r1,g1,b1,r2,g2,b2,smooth);
end;

procedure TjvDrawImage.drawcolumn(x1,y1,x2,y2:integer);
var
  t:integer;r1,g1,b1,r2,g2,b2:byte;
  line:pbytearray;
begin
picture.bitmap.pixelformat:=pf24bit;
clip.assign(picture.bitmap);
...
da:=2*pi/starpoints;
apoint:=point(x,y);
for i:=0 to starpoints-1 do begin
  with canvas do begin
...
if style in [mbHorOval,mbVerOval] then begin
  src.width:=dst.width;
...
mypen:=canvas.Pen.Mode;
  if (myshape='rangemove')or (myshape='rangesmear')then begin
    clipcm:=cmSrcInvert;
    setclip(clwhite);
    copyclip;
    with canvas do begin
...
if myshape='smoothbrush' then clipall;
  ...
  if myshape='gradientbrush' then begin
    picture.bitmap.PixelFormat :=pf24bit;
    end;
  if myshape='mixbrush' then begin
    with canvas do begin
...
procedure TjvDrawImage.MouseMove(Shift: TShiftState;
  X, Y: Integer);
var xp,yp,i,j,x1,y1,x2,y2,h,w,pw,movex,movey:integer;
    myrect:trect;
...
if (myshape='zoombrush') or (myshape='zoomkeepbrush') then begin
  w:=canvas.pen.width;
  if w<5 then w:=50;
  drawplasma(x,y,w);
  end;
if myshape='fisheyebrush' then begin
  w:=canvas.pen.width;
  if w<5 then w:=50;
  draweffectbrush(x,y,w,0.9,lbfisheye);
  end;
if myshape='fisheyefixbrush' then begin
...
if myshape='twistbrush' then begin
  if ssalt in shift then moveorigin;
  dx:=x-myorigin.x;
  if dx=0 then dx:=0.01;
  dy:=y-myorigin.y;
...
if myshape='mbHorOval' then begin
  if ssalt in shift then moveorigin;
  dx:=x-myorigin.x;
  if dx=0 then dx:=0.01;
  dy:=y-myorigin.y;
...
if myshape='mbHorBox' then begin
  if ssalt in shift then moveorigin;
  dx:=x-myorigin.x;
  if dx=0 then dx:=0.01;
  dy:=y-myorigin.y;
...
if myshape='mbVerOval' then begin
  if ssalt in shift then moveorigin;
  dx:=x-myorigin.x;
  if dx=0 then dx:=0.01;
  dy:=y-myorigin.y;
...
if myshape='mbVerBox' then begin
  if ssalt in shift then moveorigin;
  dx:=x-myorigin.x;
  if dx=0 then dx:=0.01;
  dy:=y-myorigin.y;
...
if myshape='mbBottom' then begin
  if ssalt in shift then moveorigin;
  dx:=x-myorigin.x;
  if dx=0 then dx:=0.01;
  dy:=y-myorigin.y;
...
if myshape='mbRound2' then begin
  if ssalt in shift then moveorigin;
  dx:=x-myorigin.x;
  if dx=0 then dx:=0.01;
  dy:=y-myorigin.y;
...
if myshape='mbDiamond' then begin
  if ssalt in shift then moveorigin;
  dx:=x-myorigin.x;
  if dx=0 then dx:=0.01;
  dy:=y-myorigin.y;
...
if myshape='mbSplitRound' then begin
  if ssalt in shift then moveorigin;
  dx:=x-myorigin.x;
  if dx=0 then dx:=0.01;
  dy:=y-myorigin.y;
...
if myshape='mbSplitWaste' then begin
  if ssalt in shift then moveorigin;
  dx:=x-myorigin.x;
  if dx=0 then dx:=0.01;
  dy:=y-myorigin.y;
...
if myshape='rimplebrush' then begin
  if ssalt in shift then moveorigin;
  dx:=x-myorigin.x;
  if dx=0 then dx:=0.01;
  dy:=y-myorigin.y;
...
if myshape='rotatebrush' then begin
  if ssalt in shift then moveorigin;
   dx:=x-myorigin.x;
   if dx=0 then dx:=0.01;
   dy:=y-myorigin.y;
...
if myshape='brightnessbrush' then begin
  if ssalt in shift then moveorigin;
  dx:=x-myorigin.x;
  if dx=0 then dx:=0.01;
  dy:=y-myorigin.y;
...
if myshape='contrastbrush' then begin
  if ssalt in shift then moveorigin;
  dx:=x-myorigin.x;
  if dx=0 then dx:=0.01;
  dy:=y-myorigin.y;
...
if myshape='saturationbrush' then begin
  if ssalt in shift then moveorigin;
  dx:=x-myorigin.x;
  if dx=0 then dx:=0.01;
  dy:=y-myorigin.y;
...
if myshape='smoothbrush' then
 ...
 if (myshape='ellipse') or (myshape='globe')or (myshape='intersphere')
  ...
  if myshape='borderwaveline' then begin
  ...
  if myshape='decoline' then begin
   with canvas do begin
   pw:=pen.width;
   pen.color:=wavepen;
   pen.mode:=pmcopy;
  ...
  if (myshape='paste')and (ssshift in shift) then begin
    myrect:=rect(0,0,0,0);
    myrect.left:=x;
    myrect.top:=y;
  ...
  if myshape='sline' then begin
    ...
    if (myslinedir='v') and (abs(x-myprevpoint.x)>myslinetol) then
       myslinedir:='h';
    if myslinedir='h' then begin
       canvas.lineto(x,myprevpoint.y);
       myprevpoint.x:=x;
       end;
    if myslinedir='v' then begin
       canvas.lineto(myprevpoint.x,y);
       myprevpoint.y:=y;
       end;
  end;

  if myshape='vmirror'then begin
   x1:=myprevpoint.x;y1:=myprevpoint.y;
   x2:=width;y2:=height;
...
procedure TjvDrawImage.MouseUp(Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
var myrect:trect;xs,ys,xt,yt,i,tangle:integer;c:tcolor;drw:string;
    apoint:tpoint;bitmap:tbitmap;
    x1,y1,x2,y2:integer;
...
if myshape='transcopy' then begin
  clipcm:=cmSrcCopy;
  setclip(clwhite);
  copyclip;
...
if myshape='multisphere' then
  ...
  if ((myorigin.x<>x)and (myorigin.y<>y))then begin
   if ssalt in shift then
...
if myshape='spiral' then begin
   ...
   for i:=1 to variant(spirals*36) do begin
...
if myshape='pie'then
  ...
  if myshape='skewrect1' then begin
   if mypen<>pmnotxor then
    drawskew;
  myshape:='skewrect2';
  end;

if myshape='skewrect'then begin
  ...
  if myshape='triangle1' then begin
   if mypen<>pmnotxor then
    drawtriangle;
  myshape:='triangle2';
  end;
...
if myshape='triangle' then begin
  canvas.penpos:=point(myorigin.x,myorigin.y);
   if mypen<>pmnotxor then
   canvas.lineto(x,y);
   myskew[0]:=myorigin;
 ...
 if myshape='decobar' then begin
   ...
   db:=(b1-b2)/(pw/3);
     if pw<30 then
     pen.width:=30;
   for i:=1 to pen.width div 3 do begin
     r:=r-dr;
     g:=g-dg;
     b:=b-db;
 ...
 if myshape='decooval' then begin
   picture.bitmap.PixelFormat :=pf24bit;
   with canvas do begin
   pen.mode:=pmnotxor;
   ellipse(myorigin.x,myorigin.y,myprevpoint.x,myprevpoint.y);
...
if (myshape='polyline')or(myshape='polygon') then begin
  canvas.penpos:=point(myorigin.x,myorigin.y);
  if mypen<>pmnotxor then
   canvas.lineto(x,y);
  if freepolycount=0 then begin
    freepoly[0]:=myorigin;
    inc(freepolycount);
    end
    else begin
...
if myshape='line'then begin
  canvas.penpos:=point(myorigin.x,myorigin.y);
   if mypen<>pmnotxor then begin
   canvas.lineto(x,y);
   end;
  end;

if myshape='spiro' then begin
   canvas.penpos:=point(myorigin.x,myorigin.y);
   canvas.pen.mode:=pmnotxor;
   canvas.lineto(x,y);
   canvas.pen.mode:=mypen;
...
if myshape='zoomkeepbrush' then begin
  myshape:='freehand';
  canvas.pen.Width :=2;
  end;

if myshape='paintpick'then begin
  if ssalt in shift then
