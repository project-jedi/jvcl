...
Description:
 ...
 Fairly complete demo program for the JvStructStore unit.
...
uses
  ...
  ActnList, ImgList, Controls, Dialogs, ExtCtrls, Graphics, HexDump, JvStructStore;
...
type
  ...
  private
    ...
    function GetFolder(Node: TTreeNode): TJvStorageFolder;
    ...
    procedure AddFolder(ParentNode: TTreeNode; AName: string; Storage: TJvStorageFolder);
    // add a stream in Storage with name AName as a subnode to ParentNode using the name
    procedure AddFile(ParentNode: TTreeNode; AName: string; Storage: TJvStorageFolder);
    ...
    procedure UpdateFileData(Node: TTreeNode; const AName: string;
      Storage: TJvStorageFolder);
    // adds Storage to Node without creating a new node. Also adds new nodes for substorages
    // and substreams
    procedure UpdateFolderData(Node: TTreeNode; const AName: string; Storage: TJvStorageFolder);
...
implementation
uses
  ActiveX, ComObj, PropsFrm, JvFunctions;
...
// returns true if Node.Data contains a TJvStorageFolder instance
function IsFolder(Node: TTreeNode): boolean;
begin
  Result := (Node <> nil) and (Node.Data <> nil) and (TObject(Node.Data) is TJvStorageFolder);
end;
...
begin
  Result := MessageBox(0, PChar(Msg), PChar(Caption), MB_YESNO or MB_ICONQUESTION or MB_TASKMODAL) = IDYES;
end;
...
var
  Root: TJvStorageFolder;
  HR: HResult;
  AModes: TJvStorageAccessModes;
begin
  Screen.Cursor := crHourGlass;
  FUpdating := true;
  try
    if (AFilename <> '') and (Succeeded(TJvStorageFolder.IsStructured(AFilename)) or CreateNew) then
    begin
      FFilename := AFilename;
      ...
      try
        ...
        Root := TJvStorageFolder.Create(FFilename, AModes, CreateNew);
    ...
    begin
      HR := TJvStorageFolder.Convert(AFilename);
...
var Filename:string;F:TFileStream;
begin
  Filename := TJvStorageFolder(tvDocInfo.Items.getFirstNode.Data).Name;
...
function TfrmMain.GetFolder(Node: TTreeNode): TJvStorageFolder;
begin
  if (Node <> nil) and (Node.Data <> nil) and (TObject(Node.Data) is TJvStorageFolder) then
    Result := TJvStorageFolder(Node.Data)
  else
    Result := nil;
end;
...
var S: string; N: TTreeNode; SS, SS2: TJvStorageFolder;
begin
...
var S: string; N: TTreeNode; SS: TJvStorageFolder;
begin
  if not IsFolder(tvDocInfo.Selected) then
    N := tvDocInfo.Selected.Parent
  else
...
begin
  if YesNoDlg(SConfirm, SDeletePrompt) then
    if not TJvStorageFolder(tvDocInfo.Selected.Parent.Data).Delete(tvDocInfo.Selected.Text) then
      OleError(TJvStorageFolder(tvDocInfo.Selected.Parent.Data).LastError)
    else
    begin
...
var ParamsW: TMsgBoxParamsW; ParamsA: TMsgBoxParamsA;
begin
...
procedure TfrmMain.AddFolder(ParentNode: TTreeNode; AName: string;
  Storage: TJvStorageFolder);
var S: TStringlist; i: integer; N: TTreeNode; ST: TJvStorageFolder;
begin
  if ParentNode <> nil then
  with ParentNode do
  begin
...
procedure TfrmMain.UpdateFolderData(Node: TTreeNode; const AName: string; Storage: TJvStorageFolder);
var SS: TJvStorageFolder; S: TStringlist; i: integer;
begin
  TObject(Node.Data).Free;
  Node.Data := nil;
  if Storage <> nil then
...
procedure TfrmMain.UpdateFileData(Node: TTreeNode; const AName: string; Storage: TJvStorageFolder);
var SS: TStream;
begin
  TObject(Node.Data).Free;
  Node.Data := nil;
...
begin
  ...
  if IsFolder(tvDocInfo.Selected) then
    B := TJvStorageFolder(tvDocInfo.Selected.Data).GetStats(Stat, true)
  else if tvDocInfo.Selected <> nil then
    B := TJvStorageStream(tvDocInfo.Selected.Data).GetStats(Stat, true);
  if B then
  begin
    TfrmProps.ShowProperties(Stat);
    JvStructStore.CoMallocFree(Stat.pwcsName);
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      if IsFolder(N) then
      begin
        TJvStorageFolder(N.Data).Commit;
...
var AFile:TJvStorageFolder;
begin
  // I know: I could just as well have done a standard FileCopy, but that's not any fun!
  if SaveDialog.Execute then
  begin
    AFile := TJvStorageFolder.Create(SaveDialog.Filename,[smCreate],true);
    try
      AFile.Assign(TJvStorageFolder(tvDocInfo.Items.GetFirstNode.Data));
    finally
      AFile.Free;
    end;
