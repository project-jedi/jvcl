...
uses Windows, SysUtils, Messages, Classes, Graphics, Controls,
    Forms, StdCtrls, ExtCtrls, Menus, JvTimer, RTLConsts;
...
procedure TJvClock.CMCtl3DChanged(var Message: TMessage);
begin
  inherited;
  if ShowMode = scAnalog then Invalidate;
end;

procedure TJvClock.CMTextChanged(var Message: TMessage);
begin
  { Skip this message, no repaint }
end;

procedure TJvClock.CMFontChanged(var Message: TMessage);
begin
  inherited;
  Invalidate;
...
procedure TJvClock.WMTimeChange(var Message: TMessage);
begin
  inherited;
  Invalidate;
  CheckAlarm;
...
function TJvClock.FormatSettingsChange(var Message: TMessage): Boolean;
begin
  Result := False;
  case Message.Msg of
    WM_WININICHANGE:
