...
procedure TJvxStrEditDlg.FileOpen(Sender: TObject);
begin
  with OpenDialog do
    if Execute then Memo.Lines.LoadFromFile(FileName);
end;

procedure TJvxStrEditDlg.FileSave(Sender: TObject);
begin
...
procedure TJvxStrEditDlg.UpdateStatus(Sender: TObject);
var
  Count: Integer;
begin
...
procedure TJvxStrEditDlg.FormCreate(Sender: TObject);
begin
...
procedure TJvxStrEditDlg.MemoKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key = VK_ESCAPE then CancelBtn.Click;
end;

procedure TJvxStrEditDlg.HelpBtnClick(Sender: TObject);
begin
