...
The Original Code is: JvxCombos.PAS, released on 2002-07-04.
...
{.$DEFINE GXE}
{ Activate this define to use JvxCombos in the GXExplorer Open Source project }
...
{$ENDIF}
  ...
  TJvxColorComboBox = class(TOwnerDrawComboBox)
  private
    FColorValue: TColor;
...
{ TJvxFontComboBox }
  ...
  TJvxFontComboBox = class(TOwnerDrawComboBox)
  private
...
uses SysUtils, Consts, Printers {$IFNDEF GXE}, JvxVCLUtils {$ENDIF};
...
begin
  inherited Create(AOwner);
  FTrueTypeBMP := CreateBitmap('JvxTRUETYPE_FNT');
  FDeviceBMP := CreateBitmap('JvxDEVICE_FNT');
  FDevice := fdScreen;
  Sorted := True;
