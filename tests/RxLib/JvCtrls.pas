...
The Original Code is: JvCtrls.PAS, released on 2002-07-04.
...
{ TJvCustomListBox }
  ...
  TJvCustomListBox = class(TWinControl)
  private
    FItems: TStrings;
...
{ TJvCheckListBox }
  ...
  TJvCheckListBox = class(TJvCustomListBox)
  private
...
{ TJvSpeedButton }
  ...
  TJvSpeedButton = class(TGraphicControl)
  private
    FGroupIndex: Integer;
    FStyle: TButtonStyle;
    FGlyph: Pointer;
...
{ TJvCustomListBox implementation copied from STDCTRLS.PAS and modified }
...
function TJvCustomListBox.CreateItemList: TStrings;
begin
  Result := TJvListBoxStrings.Create;
end;

function TJvCustomListBox.GetItemData(Index: Integer): LongInt;
begin
  Result := SendMessage(Handle, LB_GETITEMDATA, Index, 0);
end;

procedure TJvCustomListBox.SetItemData(Index: Integer; AData: LongInt);
begin
...
procedure TJvCustomListBox.SetHorizontalExtent;
begin
  SendMessage(Handle, LB_SETHORIZONTALEXTENT, FMaxItemWidth, 0);
end;

function TJvCustomListBox.GetItemWidth(Index: Integer): Integer;
var
  ATabWidth: Longint;
  S: string;
begin
...
procedure TJvCustomListBox.ResetHorizontalExtent;
var
  I: Integer;
begin
  FMaxItemWidth := 0;
...
procedure TJvCustomListBox.Clear;
begin
  FItems.Clear;
end;

procedure TJvCustomListBox.SetColumnWidth;
begin
  if FColumns > 0 then
...
procedure TJvCustomListBox.SetColumns(Value: Integer);
begin
...
function TJvCustomListBox.GetItemIndex: Integer;
begin
  Result := SendMessage(Handle, LB_GETCURSEL, 0, 0);
end;

function TJvCustomListBox.GetSelCount: Integer;
begin
  Result := SendMessage(Handle, LB_GETSELCOUNT, 0, 0);
end;

procedure TJvCustomListBox.SetItemIndex(Value: Integer);
begin
  if GetItemIndex <> Value then
    SendMessage(Handle, LB_SETCURSEL, Value, 0);
end;

procedure TJvCustomListBox.SetExtendedSelect(Value: Boolean);
begin
...
procedure TJvCustomListBox.SetIntegralHeight(Value: Boolean);
begin
...
function TJvCustomListBox.GetAutoScroll: Boolean;
begin
  Result := FAutoScroll and (Columns = 0);
end;

procedure TJvCustomListBox.SetOnDrawItem(Value: TDrawItemEvent);
begin
...
procedure TJvCustomListBox.SetOnGetItemWidth(Value: TGetItemWidthEvent);
begin
...
procedure TJvCustomListBox.SetAutoScroll(Value: Boolean);
begin
...
function TJvCustomListBox.GetItemHeight: Integer;
var
  R: TRect;
begin
  Result := FItemHeight;
...
procedure TJvCustomListBox.SetItemHeight(Value: Integer);
begin
...
procedure TJvCustomListBox.SetTabWidth(Value: Integer);
begin
  if Value < 0 then Value := 0;
  if FTabWidth <> Value then begin
    FTabWidth := Value;
...
procedure TJvCustomListBox.SetMultiSelect(Value: Boolean);
begin
...
function TJvCustomListBox.GetSelected(Index: Integer): Boolean;
var
  R: Longint;
begin
...
procedure TJvCustomListBox.SetSelected(Index: Integer; Value: Boolean);
begin
...
procedure TJvCustomListBox.SetSorted(Value: Boolean);
begin
...
procedure TJvCustomListBox.SetStyle(Value: TListBoxStyle);
begin
...
function TJvCustomListBox.GetTopIndex: Integer;
begin
  Result := SendMessage(Handle, LB_GETTOPINDEX, 0, 0);
end;

procedure TJvCustomListBox.SetBorderStyle(Value: TBorderStyle);
begin
...
procedure TJvCustomListBox.SetTopIndex(Value: Integer);
begin
  if GetTopIndex <> Value then SendMessage(Handle, LB_SETTOPINDEX, Value, 0);
end;

procedure TJvCustomListBox.SetGraySelection(Value: Boolean);
begin
...
procedure TJvCustomListBox.SetItems(Value: TStrings);
begin
  Items.Assign(Value);
end;

function TJvCustomListBox.ItemAtPos(Pos: TPoint; Existing: Boolean): Integer;
var
  Count: Integer;
  ItemRect: TRect;
begin
...
procedure TJvCustomListBox.WndProc(var Message: TMessage);
begin
...
procedure TJvCustomListBox.WMLButtonDown(var Message: TWMLButtonDown);
var
  ItemNo: Integer;
...
procedure TJvCustomListBox.WMNCHitTest(var Msg: TWMNCHitTest);
begin
  if csDesigning in ComponentState then DefaultHandler(Msg)
  else inherited;
end;

procedure TJvCustomListBox.CNCommand(var Message: TWMCommand);
begin
...
procedure TJvCustomListBox.DragCanceled;
var
  M: TWMMouse;
{$IFDEF WIN32}
  MousePos: TPoint;
...
procedure TJvCustomListBox.DefaultDrawText(X, Y: Integer; const S: string);
var
  ATabWidth: Longint;
begin
{$IFDEF Delphi4_Up}
...
procedure TJvCustomListBox.MeasureItem(Index: Integer; var Height: Integer);
begin
  if Assigned(FOnMeasureItem) then FOnMeasureItem(Self, Index, Height)
end;

procedure TJvCustomListBox.CNDrawItem(var Message: TWMDrawItem);
var
  State: TOwnerDrawState;
begin
...
procedure TJvCustomListBox.CNMeasureItem(var Message: TWMMeasureItem);
begin
...
procedure TJvCustomListBox.WMKillFocus(var Msg: TWMKillFocus);
begin
  inherited;
  if FGraySelection and MultiSelect and (SelCount > 1) then Invalidate;
end;

procedure TJvCustomListBox.WMSetFocus(var Msg: TWMSetFocus);
begin
  inherited;
  if FGraySelection and MultiSelect and (SelCount > 1) then Invalidate;
end;
...
{$IFDEF WIN32}
procedure TJvCustomListBox.CMCtl3DChanged(var Message: TMessage);
begin
...
begin
  with TJvCheckListBox(ListBox) do begin
...
procedure TJvCheckListBox.Loaded;
begin
  inherited Loaded;
  UpdateCheckStates;
end;
...
procedure TJvCheckListBox.InternalSaveStates(IniFile: TObject;
...
procedure TJvCheckListBox.InternalRestoreStates(IniFile: TObject;
  const Section: string);
var
  I: Integer;
  ACount: Integer;
...
{$IFDEF WIN32}
procedure TJvCheckListBox.SaveStatesReg(IniFile: TRegIniFile);
begin
  InternalSaveStates(IniFile, GetDefaultSection(Self));
end;

procedure TJvCheckListBox.RestoreStatesReg(IniFile: TRegIniFile);
begin
  InternalRestoreStates(IniFile, GetDefaultSection(Self));
end;
{$ENDIF WIN32}

procedure TJvCheckListBox.SaveStates(IniFile: TIniFile);
begin
  InternalSaveStates(IniFile, GetDefaultSection(Self));
end;

procedure TJvCheckListBox.RestoreStates(IniFile: TIniFile);
begin
  InternalRestoreStates(IniFile, GetDefaultSection(Self));
end;

function TJvCheckListBox.GetStorage: TJvFormPlacement;
begin
  Result := FIniLink.Storage;
end;

procedure TJvCheckListBox.SetStorage(Value: TJvFormPlacement);
begin
  FIniLink.Storage := Value;
end;

procedure TJvCheckListBox.IniSave(Sender: TObject);
begin
...
procedure TJvCheckListBox.IniLoad(Sender: TObject);
begin
  if (Name <> '') and (FIniLink.IniObject <> nil) then
...
procedure TJvCheckListBox.ReadCheckData(Reader: TReader);
var
  I: Integer;
begin
  Items.BeginUpdate;
...
procedure TJvCheckListBox.WriteCheckData(Writer: TWriter);
var
  I: Integer;
begin
...
procedure TJvCheckListBox.ReadVersion(Reader: TReader);
begin
  FReserved := Reader.ReadInteger;
end;

procedure TJvCheckListBox.WriteVersion(Writer: TWriter);
begin
  Writer.WriteInteger(InternalVersion);
end;

procedure TJvCheckListBox.DefineProperties(Filer: TFiler);

{$IFDEF WIN32}
  function DoWrite: Boolean;
  var
    I: Integer;
    Ancestor: TJvCheckListBox;
  begin
    Result := False;
    Ancestor := TJvCheckListBox(Filer.Ancestor);
...
procedure TJvCheckListBox.CreateWnd;
begin
  inherited CreateWnd;
...
procedure TJvCheckListBox.DestroyWnd;
begin
  inherited DestroyWnd;
end;

procedure TJvCheckListBox.WMDestroy(var Msg: TWMDestroy);
var
  I: Integer;
begin
...
procedure TJvCheckListBox.SetItems(Value: TStrings);
var
  I: Integer;
begin
  Items.BeginUpdate;
  try
    ...
    if (Value <> nil) and (Value is TJvListBoxStrings) and
      ...
      (TJvListBoxStrings(Value).ListBox is TJvCheckListBox) then
    begin
      for I := 0 to Items.Count - 1 do
        if I < Value.Count then begin
          Self.State[I] := TJvCheckListBox(TJvListBoxStrings(Value).ListBox).State[I];
          EnabledItem[I] := TJvCheckListBox(TJvListBoxStrings(Value).ListBox).EnabledItem[I];
        end;
    end;
  finally
    Items.EndUpdate;
...
function TJvCheckListBox.GetItemWidth(Index: Integer): Integer;
begin
  Result := inherited GetItemWidth(Index) + GetCheckWidth;
end;

function TJvCheckListBox.GetCheckWidth: Integer;
begin
  Result := FCheckWidth + 2;
end;

function TJvCheckListBox.GetAllowGrayed: Boolean;
begin
  Result := FAllowGrayed and (FCheckKind in [ckCheckBoxes, ckCheckMarks]);
end;

procedure TJvCheckListBox.CMFontChanged(var Message: TMessage);
begin
  inherited;
  ResetItemHeight;
end;

function TJvCheckListBox.GetItemHeight: Integer;
var
  R: TRect;
begin
  Result := FItemHeight;
...
procedure TJvCheckListBox.ResetItemHeight;
var
  H: Integer;
begin
...
procedure TJvCheckListBox.DrawItem(Index: Integer; Rect: TRect;
  State: TOwnerDrawState);
var
  R: TRect;
...
procedure TJvCheckListBox.CNDrawItem(var Message: TWMDrawItem);
begin
...
procedure TJvCheckListBox.DrawCheck(R: TRect; AState: TCheckBoxState;
  Enabled: Boolean);
const
...
procedure TJvCheckListBox.ApplyState(AState: TCheckBoxState;
  EnabledOnly: Boolean);
var
  I: Integer;
begin
...
function TJvCheckListBox.GetCheckedIndex: Integer;
var
  I: Integer;
begin
  Result := -1;
...
procedure TJvCheckListBox.SetCheckedIndex(Value: Integer);
begin
  if (FCheckKind = ckRadioButtons) and (Items.Count > 0) then
    SetState(Max(Value, 0), cbChecked);
end;

procedure TJvCheckListBox.UpdateCheckStates;
begin
...
procedure TJvCheckListBox.SetCheckKind(Value: TCheckKind);
begin
...
procedure TJvCheckListBox.SetChecked(Index: Integer; AChecked: Boolean);
const
...
procedure TJvCheckListBox.SetState(Index: Integer; AState: TCheckBoxState);
var
  I: Integer;
begin
...
procedure TJvCheckListBox.InvalidateCheck(Index: Integer);
var
  R: TRect;
begin
  R := ItemRect(Index);
...
procedure TJvCheckListBox.InvalidateItem(Index: Integer);
var
  R: TRect;
begin
  R := ItemRect(Index);
...
procedure TJvCheckListBox.KeyPress(var Key: Char);
begin
...
procedure TJvCheckListBox.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
var
  Index: Integer;
begin
...
procedure TJvCheckListBox.ToggleClickCheck(Index: Integer);
var
...
procedure TJvCheckListBox.ChangeItemState(Index: Integer);
begin
...
function TJvCheckListBox.GetCheckObject(Index: Integer): TObject;
begin
  Result := FindCheckObject(Index);
  if Result = nil then Result := CreateCheckObject(Index);
end;

function TJvCheckListBox.FindCheckObject(Index: Integer): TObject;
var
  ItemData: Longint;
begin
  Result := nil;
...
function TJvCheckListBox.IsCheckObject(Index: Integer): Boolean;
begin
...
procedure TJvCheckListBox.ResetContent;
var
  I: Integer;
begin
  for I := Items.Count - 1 downto 0 do begin
...
procedure TJvCheckListBox.DeleteString(Index: Integer);
begin
...
procedure TJvSpeedButton.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
var
  P: TPoint;
  Msg: TMsg;
...
procedure TJvSpeedButton.WMRButtonDown(var Message: TWMRButtonDown);
begin
  inherited;
  UpdateTracking;
end;

procedure TJvSpeedButton.WMRButtonUp(var Message: TWMRButtonUp);
begin
  inherited;
  UpdateTracking;
end;

procedure TJvSpeedButton.WMLButtonDblClk(var Message: TWMLButtonDown);
begin
...
procedure TJvSpeedButton.CMEnabledChanged(var Message: TMessage);
var
  State: TJvButtonState;
begin
  inherited;
...
procedure TJvSpeedButton.CMVisibleChanged(var Message: TMessage);
begin
  inherited;
  if Visible then UpdateTracking;
end;

procedure TJvSpeedButton.CMMouseEnter(var Message: TMessage);
begin
  inherited;
  if (not FMouseInControl) and Enabled and IsForegroundTask then begin
    FMouseInControl := True;
...
procedure TJvSpeedButton.CMMouseLeave(var Message: TMessage);
begin
  inherited;
  if FMouseInControl and Enabled and not FDragging then begin
    FMouseInControl := False;
...
procedure TJvSpeedButton.WMMouseMove(var Message: TMessage);
begin
  inherited;
end;

procedure TJvSpeedButton.CMButtonPressed(var Message: TMessage);
var
  Sender: TControl;
begin
  if (Message.WParam = FGroupIndex) and Parent.HandleAllocated then begin
    ...
    if (Sender <> nil) and (Sender is TJvSpeedButton) then
      if Sender <> Self then begin
        if TJvSpeedButton(Sender).Down and FDown then begin
          FDown := False;
          FState := rbsUp;
          Repaint;
        end;
...
procedure TJvSpeedButton.CMDialogChar(var Message: TCMDialogChar);
begin
  with Message do
...
procedure TJvSpeedButton.CMFontChanged(var Message: TMessage);
begin
  Invalidate;
end;

procedure TJvSpeedButton.CMTextChanged(var Message: TMessage);
begin
  Invalidate;
end;

procedure TJvSpeedButton.CMSysColorChange(var Message: TMessage);
begin
  TJvButtonGlyph(FGlyph).Invalidate;
  Invalidate;
end;
