...
The Original Code is: JvxTimerLst.PAS, released on 2002-07-04.
...
type
  ...
  TJvxTimerEvent = class;

  TJvxTimerList = class(TComponent)
  private
    FEvents: TList;
    FWndHandle: hWnd;
    FActive: Boolean;
    ...
    function CreateNewEvent: TJvxTimerEvent;
    function GetCount: Integer;
    function GetEnabledCount: Integer;
    function ProcessEvents: Boolean;
    procedure RemoveItem(Item: TJvxTimerEvent);
...
{$ENDIF WIN32}
    procedure DoTimer(Event: TJvxTimerEvent); dynamic;
  ...
  public
    ...
    function AddItem(Item: TJvxTimerEvent): Longint;
    procedure Clear;
    procedure Delete(AHandle: Longint); virtual;
    procedure Activate;
    procedure Deactivate;
    function ItemByHandle(AHandle: Longint): TJvxTimerEvent;
  ...
  TJvxTimerEvent = class(TComponent)
  private
    FCycled: Boolean;
    FEnabled: Boolean;
    FExecCount: Integer;
...
{$IFNDEF WIN32}
    procedure SetParentList(Value: TJvxTimerList);
{$ENDIF WIN32}
  protected
{$IFDEF WIN32}
...
{$ENDIF}
    ...
    property TimerList: TJvxTimerList read FParentList;
  published
...
uses Consts, Controls, Forms, SysUtils, JvxVCLUtils, JvxMaxMin;
...
constructor TJvxTimerEvent.Create(AOwner: TComponent);
begin
...
destructor TJvxTimerEvent.Destroy;
begin
  FOnTimer := nil;
  inherited Destroy;
end;
...
{$IFNDEF WIN32}
procedure TJvxTimerEvent.SetParentList(Value: TJvxTimerList);
begin
...
function TJvxTimerEvent.HasParent: Boolean;
begin
  Result := True;
end;
...
function TJvxTimerEvent.GetParentComponent: TComponent;
begin
  Result := FParentList;
end;

procedure TJvxTimerEvent.SetParentComponent(Value: TComponent);
begin
  if FParentList <> nil then FParentList.RemoveItem(Self);
  if (Value <> nil) and (Value is TJvxTimerList) then
    TJvxTimerList(Value).AddItem(Self);
end;

{$ELSE}

procedure TJvxTimerEvent.ReadState(Reader: TReader);
begin
  inherited ReadState(Reader);
  if Reader.Parent is TJvxTimerList then
    SetParentList(TJvxTimerList(Reader.Parent));
end;

{$ENDIF WIN32}

procedure TJvxTimerEvent.SetEnabled(Value: Boolean);
begin
...
procedure TJvxTimerEvent.SetInterval(Value: Longint);
begin
...
procedure TJvxTimerEvent.SetRepeatCount(Value: Integer);
begin
...
function TJvxTimerEvent.GetAsSeconds: Cardinal;
begin
  Result := Interval div 1000;
end;

procedure TJvxTimerEvent.SetAsSeconds(Value: Cardinal);
begin
  Interval := Value * 1000;
end;
...
constructor TJvxTimerList.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  ...
  if not Registered then begin
    RegisterClasses([TJvxTimerEvent]);
    Registered := True;
  end;
end;

destructor TJvxTimerList.Destroy;
begin
  OnFinish := nil;
  OnTimers := nil;
  Deactivate;
...
procedure TJvxTimerList.Activate;
begin
  Active := True;
end;

procedure TJvxTimerList.Deactivate;
begin
  if not (csLoading in ComponentState) then Active := False;
end;

procedure TJvxTimerList.SetEvents(StartTicks: Longint);
var
  I: Integer;
begin
  for I := 0 to FEvents.Count - 1 do
    if TJvxTimerEvent(FEvents[I]).Enabled then
      TJvxTimerEvent(FEvents[I]).FLastExecute := StartTicks;
end;

procedure TJvxTimerList.SetActive(Value: Boolean);
var
  StartTicks: Longint;
begin
...
{$IFDEF WIN32}
procedure TJvxTimerList.GetChildren(Proc: TGetChildProc {$IFDEF Delphi3_Up};
...
begin
  ...
  for I := 0 to FEvents.Count - 1 do
    Proc(TJvxTimerEvent(FEvents[I]));
end;
{$ELSE}
procedure TJvxTimerList.WriteComponents(Writer: TWriter);
var
  I: Integer;
  Item: TJvxTimerEvent;
begin
  ...
  for I := 0 to FEvents.Count - 1 do begin
    Item := TJvxTimerEvent(FEvents[I]);
    if Item.Owner = Writer.Root then Writer.WriteComponent(Item);
  end;
end;
{$ENDIF WIN32}

procedure TJvxTimerList.Sort;
var
  I: Integer;
  ExitLoop: Boolean;
begin
  if not (csDesigning in ComponentState) then
    repeat
      ...
      for I := 0 to Count - 2 do begin
        if TJvxTimerEvent(FEvents[I]).Interval > TJvxTimerEvent(FEvents[I + 1]).Interval then
        begin
...
function TJvxTimerList.NextHandle: Longint;
begin
  Inc(FSequence);
  Result := FSequence;
end;

function TJvxTimerList.CreateNewEvent: TJvxTimerEvent;
begin
  Result := TJvxTimerEvent.Create(Owner);
end;

function TJvxTimerList.AddItem(Item: TJvxTimerEvent): Longint;
begin
...
function TJvxTimerList.Add(AOnTimer: TNotifyEvent; AInterval: Longint;
...
function TJvxTimerList.ItemIndexByHandle(AHandle: Longint): Integer;
begin
  for Result := 0 to FEvents.Count - 1 do
    if TJvxTimerEvent(FEvents[Result]).Handle = AHandle then Exit;
  Result := -1;
end;

function TJvxTimerList.ItemByHandle(AHandle: Longint): TJvxTimerEvent;
var
  I: Integer;
begin
  I := ItemIndexByHandle(AHandle);
  if I >= 0 then Result := TJvxTimerEvent(FEvents[I])
  else Result := nil;
end;

procedure TJvxTimerList.Delete(AHandle: Longint);
var
  I: Integer;
  Item: TJvxTimerEvent;
begin
...
function TJvxTimerList.GetCount: Integer;
begin
  Result := FEvents.Count;
end;

function TJvxTimerList.GetEnabledCount: Integer;
var
  I: Integer;
begin
  Result := 0;
  for I := 0 to Count - 1 do
    if TJvxTimerEvent(FEvents[I]).Enabled then Inc(Result);
end;

procedure TJvxTimerList.RemoveItem(Item: TJvxTimerEvent);
begin
  FEvents.Remove(Item);
  Item.FParentList := nil;
end;

procedure TJvxTimerList.Clear;
var
  I: Integer;
  Item: TJvxTimerEvent;
begin
...
procedure TJvxTimerList.DoTimer(Event: TJvxTimerEvent);
begin
  with Event do 
...
function TJvxTimerList.ProcessEvents: Boolean;
var
  I: Integer;
  Item: TJvxTimerEvent;
  StartTicks: Longint;
...
procedure TJvxTimerList.TimerWndProc(var Msg: TMessage);
begin
  if not (csDesigning in ComponentState) then begin
    with Msg do
...
procedure TJvxTimerList.CalculateInterval(StartTicks: Longint);
var
  I: Integer;
  ExitLoop: Boolean;
begin
  if not (csDesigning in ComponentState) then begin
    if Count = 0 then FInterval := 0
      ...
      for I := 0 to Count - 1 do
        with TJvxTimerEvent(FEvents[I]) do
...
procedure TJvxTimerList.UpdateTimer;
var
