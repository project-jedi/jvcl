...
The Original Code is: JvxDbPrgrss.PAS, released on 2002-07-04.
...
uses Classes, {$IFDEF WIN32} Bde, {$ELSE} DbiTypes, DbiProcs, {$ENDIF WIN32}
  Controls, DB, DBTables, JvxTimer{, JvxComponent};
...
{ TJvxDBProgress }
  ...
  TJvxDBProgress = class(TComponent)
  private
    FActive: Boolean;
    FStartTime: Longint;
    FTimer: TJvxTimer;
...
{ TJvxDBCallback - for internal use only }

type
  TJvxDBCallbackEvent = function(CBInfo: Pointer): CBRType of object;
  TJvxDBCallbackChain = (dcOnlyOnce, dcChain, dcReplace);

  TJvxDBCallback = class(TObject)
  private
    FOwner: TObject;
    FCBType: CBType;
    FCBBuf: Pointer;
    ...
    FOldCBFunc: Pointer;
    FInstalled: Boolean;
    FChain: TJvxDBCallbackChain;
    FCallbackEvent: TJvxDBCallbackEvent;
  protected
    function Invoke(CallType: CBType; var CBInfo: Pointer): CBRType;
  public
    constructor Create(AOwner: TObject; CBType: CBType;
      CBBufSize: Cardinal; CallbackEvent: TJvxDBCallbackEvent;
      Chain: TJvxDBCallbackChain);
    destructor Destroy; override;
  end;

implementation

uses {$IFDEF WIN32} Windows, {$ELSE} WinTypes, WinProcs, JvxStr16, {$ENDIF WIN32}
  Forms, SysUtils, StdCtrls, Dialogs, JvxMaxMin, JvxPrgrss, JvxBdeUtils;
...
begin
  if Data <> 0 then begin
    Result := TJvxDBCallback(Data).Invoke(CallType, CBInfo);
  end
  else Result := cbrUSEDEF;
end;

constructor TJvxDBCallback.Create(AOwner: TObject; CBType: CBType;
  CBBufSize: Cardinal; CallbackEvent: TJvxDBCallbackEvent;
  Chain: TJvxDBCallbackChain);
begin
  FOwner := AOwner;
...
destructor TJvxDBCallback.Destroy;
begin
...
function TJvxDBCallback.Invoke(CallType: CBType; var CBInfo: Pointer): CBRType;
begin
  Result := cbrUSEDEF;
...
constructor TJvxDBProgress.Create(AOwner: TComponent);
begin
...
destructor TJvxDBProgress.Destroy;
begin
{$IFDEF WIN32}
  FOnTrace := nil;
  Trace := False;
...
procedure TJvxDBProgress.Loaded;
begin
  inherited Loaded;
  FStreamedValue := True;
  try
...
procedure TJvxDBProgress.TimerExpired(Sender: TObject);
begin
  FreeTimer;
  SetPercent(0);
  SetMessage('');
...
procedure TJvxDBProgress.FreeTimer;
begin
...
procedure TJvxDBProgress.StartTimer;
begin
  if (FTimer = nil) then begin
...
procedure TJvxDBProgress.SetPercent(Value: Integer);
begin
...
procedure TJvxDBProgress.SetMessage(const Value: string);
begin
...
procedure TJvxDBProgress.SetActive(Value: Boolean);
begin
...
{$IFDEF WIN32}
        Activate;
{$ENDIF}
        FGenProgressCallback := TJvxDBCallback.Create(Self, cbGENPROGRESS,
          Max(SizeOf(CBPROGRESSDesc), SizeOf(DBIPATH) + SizeOf(Integer) * 4),
          GenProgressCallback, dcChain);
        FQryProgressCallback := TJvxDBCallback.Create(Self, cbQRYPROGRESS,
...
procedure TJvxDBProgress.Activate;
var
  S: TSession;
begin
  if FSessionLink = nil then begin
    ...
    try
      TJvxSessionLink(FSessionLink).Temporary := True;
      TJvxSessionLink(FSessionLink).KeepConnection := False;
      TJvxSessionLink(FSessionLink).FProgress := Self;
    except
      FSessionLink.Free;
      FSessionLink := nil;
      raise;
...
procedure TJvxDBProgress.Deactivate;
begin
  if FSessionLink <> nil then begin
    TJvxSessionLink(FSessionLink).FProgress := nil;
    FSessionLink.Free;
    FSessionLink := nil;
  end;
end;

function TJvxDBProgress.GetDBSession: TSession;
begin
...
procedure TJvxDBProgress.SetSessionName(const Value: string);
var
...
procedure TJvxDBProgress.SetTrace(Value: Boolean);
begin
  if (FTrace <> Value) or (FStreamedValue and Value) then begin
    if not (csDesigning in ComponentState) then begin
      if Value then begin
        ...
        FTraceCallback := TJvxDBCallback.Create(Self, cbTRACE,
...
procedure TJvxDBProgress.SetTraceFlags(Value: TTraceFlags);
begin
  FTraceFlags := Value;
  if Trace then GetDBSession.TraceFlags := FTraceFlags;
end;

function TJvxDBProgress.TraceCallBack(CBInfo: Pointer): CBRType;
var
  CurFlag: TTraceFlag;
begin
  Result := cbrUSEDEF;
...
procedure TJvxDBProgress.SetMessageControl(Value: TControl);
begin
...
procedure TJvxDBProgress.SetGauge(Value: TControl);
begin
  FGauge := Value;
{$IFDEF WIN32}
  if Value <> nil then Value.FreeNotification(Self);
...
procedure TJvxDBProgress.Notification(AComponent: TComponent; AOperation: TOperation);
begin
...
function TJvxDBProgress.GenProgressCallback(CBInfo: Pointer): CBRType;
var
...
function TJvxDBProgress.QryProgressCallback(CBInfo: Pointer): CBRType;
var
...
function TJvxDBProgress.ProgressMsgValue(const Msg: string): Longint;
begin
  if Msg <> '' then
