...
The Original Code is: JvxSpeedBar.PAS, released on 2002-07-04.
...
uses Windows, Registry, RTLConsts, SysUtils, Classes, Messages, Menus, Buttons, Controls, Graphics, Forms,
  ...
  JvxCtrls, JvxPlacemnt{, JvxComponent};
...
type
  TJvxSpeedItem = class;
  TJvxSpeedbarSection = class;
  EJvxSpeedbarError = class(Exception);

{ TJvxSpeedBar }

  TBarOrientation = (boHorizontal, boVertical);
  TBarPosition = (bpAuto, bpCustom);
  TJvxSpeedBarOption = (sbAllowDrag, sbAllowResize, sbFlatBtns, sbGrayedBtns,
    sbTransparentBtns, sbStretchBitmap);
  TJvxSpeedBarOptions = set of TJvxSpeedBarOption;
  ...
  TForEachItem = procedure (Item: TJvxSpeedItem; Data: Longint) of object;
  TApplyAlignEvent = procedure (Sender: TObject; Align: TAlign;
    var Apply: Boolean) of object;

  TJvxSpeedBar = class(TCustomPanel)
  private
    FSections: TList;
...
{$IFDEF WIN32}
    ...
    procedure InvalidateItem(Item: TJvxSpeedItem; Data: Longint);
{$ENDIF}
    ...
    procedure SetOptions(Value: TJvxSpeedBarOptions);
    ...
    procedure SetItemParams(Item: TJvxSpeedItem; InitBounds: Boolean);
    procedure SetItemVisible(Item: TJvxSpeedItem; Data: Longint);
    procedure SetItemEnabled(Item: TJvxSpeedItem; Data: Longint);
    procedure SetItemButtonSize(Item: TJvxSpeedItem; Data: Longint);
    procedure OffsetItem(Item: TJvxSpeedItem; Data: Longint);
    procedure ApplyItemSize(Item: TJvxSpeedItem; Data: Longint);
    procedure AlignItemToGrid(Item: TJvxSpeedItem; Data: Longint);
    procedure SwapItemBounds(Item: TJvxSpeedItem; Data: Longint);
    procedure SetItemEditing(Item: TJvxSpeedItem; Data: Longint);
    procedure HideItem(Item: TJvxSpeedItem; Data: Longint);
    procedure WriteItemLayout(Item: TJvxSpeedItem; Data: Longint);
    procedure FlatItem(Item: TJvxSpeedItem; Data: Longint);
    procedure TransparentItem(Item: TJvxSpeedItem; Data: Longint);
    function GetSection(Index: Integer): TJvxSpeedbarSection;
    function GetSectionCount: Integer;
    procedure GrayedItem(Item: TJvxSpeedItem; Data: Longint);
  ...
  protected
    ...
    function AppendSection(Value: TJvxSpeedbarSection): Integer; virtual;
...
{$ENDIF}
  ...
  public
    ...
    procedure RemoveItem(Item: TJvxSpeedItem);
    ...
    procedure AddItem(Section: Integer; Item: TJvxSpeedItem);
    function NewItem(AOwner: TComponent; Section: Integer;
      const AName: string): TJvxSpeedItem;
    function AcceptDropItem(Item: TJvxSpeedItem; X, Y: Integer): Boolean;
    procedure SetEditing(Win: HWnd);
    function GetEditing: Boolean;
    function SearchItem(const ItemName: string): TJvxSpeedItem;
    function FindItem(Item: TJvxSpeedItem; var Section, Index: Integer): Boolean;
...
{$ENDIF WIN32}
    ...
    function Items(Section, Index: Integer): TJvxSpeedItem;
    property EditMode: Boolean read GetEditing;
    property SectionCount: Integer read GetSectionCount;
    property Sections[Index: Integer]: TJvxSpeedbarSection read GetSection;
  ...
  published
    ...
    property Options: TJvxSpeedBarOptions read FOptions write SetOptions
...
{ TJvxSpeedItem }
  ...
  TJvxSpeedItem = class(TComponent)
  private
    FCaption: String;
    FEditing: Boolean;
    FEnabled: Boolean;
...
{$ENDIF}
    ...
    function GetNumGlyphs: TJvxNumGlyphs;
    procedure SetNumGlyphs(Value: TJvxNumGlyphs);
...
{$ENDIF}
    ...
    property JvxSpeedbar: TJvxSpeedBar read FParent;
...
{$ENDIF}
    ...
    property NumGlyphs: TJvxNumGlyphs read GetNumGlyphs write SetNumGlyphs default 1;
...
{ TJvxSpeedbarSection }
  ...
  TJvxSpeedbarSection = class(TComponent)
  private
    FList: TList;
    FTitle: String;
    FParent: TJvxSpeedBar;
    function Get(Index: Integer): TJvxSpeedItem;
    procedure Put(Index: Integer; Item: TJvxSpeedItem);
...
{$ENDIF}
    ...
    property Items[Index: Integer]: TJvxSpeedItem read Get write Put; default;
    property List: TList read FList; { for internal use only }
    property JvxSpeedbar: TJvxSpeedBar read FParent write SeTJvxSpeedBar stored False;
  published
...
{ TJvxBtnControl }
  ...
  TJvxBtnControl = class(TCustomControl)
  private
...
{$ENDIF}
  ...
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
    procedure AssignSpeedItem(Item: TJvxSpeedItem);
    procedure Activate(Rect: TRect);
    procedure ReleaseHandle;
    property Alignment: TAlignment read GetAlignment write SetAlignment;
    property Caption: TCaption read GetCaption write SetCaption;
    property Glyph: TBitmap read GetGlyph write SetGlyph;
    property NumGlyphs: TJvxNumGlyphs read GetNumGlyphs write SetNumGlyphs;
...
{ Values for WParam for CM_SPEEDBARCHANGED message }
  ...
  SBR_DESTROYED      = 1; { destroy JvxSpeedbar           }
  SBR_BTNSELECT      = 2; { select button in JvxSpeedbar  }
  SBR_BTNSIZECHANGED = 3; { button size changed        }

{ Utility routines for JvxSpeedbar Editors }

function FindSpeedBar(const Pos: TPoint): TJvxSpeedBar;
procedure DrawCellButton(Grid: TDrawGrid; R: TRect; Item: TJvxSpeedItem;
  Image: TJvxButtonImage {$IFDEF Delphi4_Up}; ARightToLeft: Boolean = False {$ENDIF});
function NewSpeedSection(ASpeedbar: TJvxSpeedBar; const ACaption: string): Integer;
function NewSpeedItem(AOwner: TComponent; ASpeedbar: TJvxSpeedBar; Section: Integer;
  const AName: string): TJvxSpeedItem;

implementation

uses Dialogs, JvxMaxMin, JvxVCLUtils, JvxAppUtils, JvxStrUtils, Consts, JvxConst, JvxSbSetup;

{ JvxSpeedbar exceptions }
{$IFDEF Delphi3_Up}
resourcestring
{$ELSE}
const
...
constructor TJvxSpeedbarSection.Create(AOwner: TComponent);
begin
...
destructor TJvxSpeedbarSection.Destroy;
begin
  Clear;
...
procedure TJvxSpeedbarSection.Clear;
begin
  while FList.Count > 0 do begin
    TJvxSpeedItem(FList[0]).Free;
    FList.Delete(0);
  end;
end;

function TJvxSpeedbarSection.Get(Index: Integer): TJvxSpeedItem;
begin
  Result := TJvxSpeedItem(FList[Index]);
end;

procedure TJvxSpeedbarSection.Put(Index: Integer; Item: TJvxSpeedItem);
begin
  FList[Index] := Item;
end;

function TJvxSpeedbarSection.GetCount: Integer;
begin
  Result := FList.Count;
end;

function TJvxSpeedbarSection.GetIndex: Integer;
begin
  if FParent <> nil then Result := FParent.FSections.IndexOf(Self)
  else Result := -1;
end;

procedure TJvxSpeedbarSection.SetIndex(Value: Integer);
var
...
function TJvxSpeedbarSection.HasParent: Boolean;
begin
  Result := True;
end;

procedure TJvxSpeedbarSection.SeTJvxSpeedBar(Value: TJvxSpeedBar);
var
  CurIndex: Integer;
begin
  CurIndex := GetIndex;
...
function TJvxSpeedbarSection.GetParentComponent: TComponent;
begin
  Result := FParent;
end;

procedure TJvxSpeedbarSection.SetParentComponent(Value: TComponent);
begin
  JvxSpeedbar := Value as TJvxSpeedBar;
end;

{$ELSE}

procedure TJvxSpeedbarSection.ReadState(Reader: TReader);
begin
  inherited ReadState(Reader);
  if Reader.Parent is TJvxSpeedBar then JvxSpeedbar := TJvxSpeedBar(Reader.Parent);
end;

{$ENDIF}

procedure TJvxSpeedbarSection.RemoveItem(Item: TJvxSpeedItem);
var
  I: Integer;
begin
...
procedure TJvxSpeedbarSection.ValidateCaption(const NewCaption: string);
var
  I: Integer;
begin
  if FParent <> nil then begin
    ...
    if (I <> Index) and (I >= 0) then
      raise EJvxSpeedbarError.Create(ResStr(SDuplicateString));
  end;
end;

procedure TJvxSpeedbarSection.SetTitle(const Value: string);
begin
  if not (csLoading in ComponentState) then ValidateCaption(Value);
  FTitle := Value;
end;

function TJvxSpeedbarSection.GetTitle: string;
begin
  Result := FTitle;
end;
...
type
  TJvxSpeedBarButton = class(TJvxSpeedButton)
  private
    FItem: TJvxSpeedItem;
    FBtn: TJvxBtnControl;
  ...
  protected
    ...
    procedure MouseUp(Button: TMouseButton; Shift: TShiftState;
      X, Y: Integer); override;
    procedure PaintGlyph(Canvas: TCanvas; ARect: TRect; AState: TJvxButtonState;
...
constructor TJvxSpeedBarButton.Create(AOwner: TComponent);
begin
  FItem := TJvxSpeedItem(AOwner);
...
destructor TJvxSpeedBarButton.Destroy;
begin
  FBtn.Free;
  inherited Destroy;
end;

procedure TJvxSpeedBarButton.SetBounds(ALeft, ATop, AWidth, AHeight: Integer);
begin
  if (FItem.JvxSpeedbar <> nil) then begin
    case FItem.JvxSpeedbar.Orientation of
      boHorizontal: ATop := Max(FItem.JvxSpeedbar.FOffset.Y, ATop);
      boVertical: ALeft := Max(FItem.JvxSpeedbar.FOffset.X, ALeft);
    end;
  end;
  inherited SetBounds(ALeft, ATop, AWidth, AHeight);
end;

procedure TJvxSpeedBarButton.CMVisibleChanged(var Message: TMessage);
begin
...
procedure TJvxSpeedBarButton.WndProc(var Message: TMessage);
begin
...
procedure TJvxSpeedBarButton.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
var
  P: TPoint;
begin
  if FItem.FEditing and Visible and (Button = mbLeft) and
    (FItem.JvxSpeedbar <> nil) then
  begin
    P := ClientToScreen(Point(FItem.JvxSpeedbar.BtnWidth {div 2},
      FItem.JvxSpeedbar.BtnHeight {div 2}));
    X := P.X; Y := P.Y;
    if FBtn = nil then begin
      SetCursorPos(X, Y);
      FBtn := TJvxBtnControl.Create(Self);
...
procedure TJvxSpeedBarButton.MouseMove(Shift: TShiftState; X, Y: Integer);
var
  P: TPoint;
  R: TRect;
begin
  if FItem.FEditing and (FBtn <> nil) then begin
    ...
    if FItem.JvxSpeedbar <> nil then begin
      ...
      if (csDesigning in ComponentState) then begin
        R := BoundsRect;
        InvalidateRect(FItem.JvxSpeedbar.Handle, @R, True);
      end;
      P := FItem.JvxSpeedbar.ScreenToClient(P);
      if PtInRect(FItem.JvxSpeedbar.ClientRect, P) then begin
...
procedure TJvxSpeedBarButton.MouseUp(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
var
  P: TPoint;
begin
  if FItem.FEditing and (FBtn <> nil) then begin
    ...
    if FItem.JvxSpeedbar <> nil then begin
      P := FItem.JvxSpeedbar.ScreenToClient(P);
      if PtInRect(FItem.JvxSpeedbar.ClientRect, P) then begin
        if not FItem.JvxSpeedbar.AcceptDropItem(FItem, P.X, P.Y) then begin
          SendMessage(FItem.JvxSpeedbar.FEditWin, CM_SPEEDBARCHANGED, SBR_CHANGED,
            Longint(FItem.JvxSpeedbar));
        end
        else begin
          SendMessage(FItem.JvxSpeedbar.FEditWin, CM_SPEEDBARCHANGED, SBR_BTNSELECT,
            Longint(FItem));
          Invalidate;
        end;
      end
...
procedure TJvxSpeedBarButton.InvalidateGlyph;
begin
  TJvxButtonGlyph(ButtonGlyph).Invalidate;
end;

procedure TJvxSpeedBarButton.PaintGlyph(Canvas: TCanvas; ARect: TRect;
  AState: TJvxButtonState; DrawMark: Boolean);
begin
{$IFDEF WIN32}
  if (FItem.JvxSpeedbar <> nil) then begin
    TJvxButtonGlyph(ButtonGlyph).DrawEx(Canvas, ARect, Caption, Layout,
      Margin, Spacing, DrawMark, FItem.JvxSpeedbar.Images, FItem.FImageIndex,
...
procedure TJvxSpeedBarButton.Paint;
begin
...
constructor TJvxSpeedItem.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  FButton := TJvxSpeedBarButton.Create(Self);
...
destructor TJvxSpeedItem.Destroy;
begin
  FVisible := False;
...
function TJvxSpeedItem.GetCaption: TCaption;
begin
  Result := TCaption(FCaption);
end;

procedure TJvxSpeedItem.SetCaption(const Value: TCaption);
var
  ChangeHint: Boolean;
begin
...
procedure TJvxSpeedItem.SetName(const Value: TComponentName);
var
  ChangeText: Boolean;
begin
...
procedure TJvxSpeedItem.SetEditing(Value: Boolean);
begin
  FEditing := Value;
  if FEditing then begin
    ...
    SetEnabled(FEnabled);
    if JvxSpeedbar <> nil then
      FButton.Flat := (sbFlatBtns in JvxSpeedbar.Options);
  end;
end;

function TJvxSpeedItem.HasParent: Boolean;
begin
  Result := True;
end;

procedure TJvxSpeedItem.DefineProperties(Filer: TFiler);

{$IFDEF WIN32}
  function DoWrite: Boolean;
  begin
    if Assigned(Filer.Ancestor) then
      Result := GetSectionName <> TJvxSpeedItem(Filer.Ancestor).GetSectionName
    else Result := True;
  end;
{$ENDIF}
...
procedure TJvxSpeedItem.ReadSectionName(Reader: TReader);
begin
  FSectionName := Reader.ReadString;
end;

procedure TJvxSpeedItem.WriteSectionName(Writer: TWriter);
begin
  Writer.WriteString(GetSectionName);
end;

procedure TJvxSpeedItem.ReadSection(Reader: TReader);
begin
  FSection := Reader.ReadInteger;
end;

procedure TJvxSpeedItem.WriteSection(Writer: TWriter);
begin
  UpdateSection;
...
function TJvxSpeedItem.GetParentComponent: TComponent;
begin
  Result := FParent;
end;

procedure TJvxSpeedItem.SetParentComponent(Value: TComponent);
var
  I: Integer;
begin
  if not (csLoading in ComponentState) then begin
    if FParent <> nil then FParent.RemoveItem(Self);
    if (Value <> nil) and (Value is TJvxSpeedBar) then begin
      I := TJvxSpeedBar(Value).SearchSection(FSectionName);
      if I >= 0 then FSection := I;
      TJvxSpeedBar(Value).AddItem(FSection, Self);
    end;
  end;
end;

procedure TJvxSpeedItem.SetImageIndex(Value: Integer);
begin
  if Value <> FImageIndex then begin
    FImageIndex := Value;
    TJvxSpeedBarButton(FButton).InvalidateGlyph;
    FButton.Invalidate;
  end;
end;
...
procedure TJvxSpeedItem.ReadState(Reader: TReader);
begin
  inherited ReadState(Reader);
  if Reader.Parent is TJvxSpeedBar then begin
    if FSectionName <> '' then
      FSection := TJvxSpeedBar(Reader.Parent).SearchSection(FSectionName);
    TJvxSpeedBar(Reader.Parent).AddItem(Max(FSection, 0), Self);
  end;
end;

function TJvxSpeedItem.GetSection: Integer;
begin
  UpdateSection;
  Result := FSection;
end;

procedure TJvxSpeedItem.SetSection(Value: Integer);
begin
  if JvxSpeedbar = nil then FSection := Value;
end;

function TJvxSpeedItem.GetSectionName: string;
begin
  UpdateSection;
...
{
procedure TJvxSpeedItem.SetSectionName(const Value: string);
begin
...
procedure TJvxSpeedItem.InvalidateItem;
begin
  FSection := -1;
end;

procedure TJvxSpeedItem.UpdateSection;
var
  I: Integer;
begin
...
procedure TJvxSpeedItem.SetEnabled(Value: Boolean);
begin
  if ((FButton.Enabled <> Value) or (FEnabled <> Value)) then begin
    FEnabled := Value;
    if not FEditing then begin
      if (JvxSpeedbar <> nil) and Value then
        FButton.Enabled := (Value and JvxSpeedbar.Enabled)
      else FButton.Enabled := Value;
    end;
  end;
end;

procedure TJvxSpeedItem.SetVisible(Value: Boolean);
begin
  ...
  begin
    FVisible := Value;
    if (JvxSpeedbar <> nil) and Value then
      FButton.Visible := Value and JvxSpeedbar.Visible
    else FButton.Visible := Value;
    if Value then FButton.Parent := JvxSpeedbar;
  end;
end;

function TJvxSpeedItem.GetAllowAllUp: Boolean;
begin
  Result := FButton.AllowAllUp;
end;

procedure TJvxSpeedItem.SetAllowAllUp(Value: Boolean);
begin
  FButton.AllowAllUp := Value;
end;

function TJvxSpeedItem.GetAllowTimer: Boolean;
begin
  Result := FButton.AllowTimer;
end;

procedure TJvxSpeedItem.SetAllowTimer(Value: Boolean);
begin
  FButton.AllowTimer := Value;
end;

function TJvxSpeedItem.GetBtnCaption: TCaption;
begin
  Result := FButton.Caption;
end;

procedure TJvxSpeedItem.SetBtnCaption(const Value: TCaption);
begin
  FButton.Caption := Value;
end;

function TJvxSpeedItem.GetGroupIndex: Integer;
begin
  Result := FButton.GroupIndex;
end;

procedure TJvxSpeedItem.SetGroupIndex(Value: Integer);
begin
  FButton.GroupIndex := Value;
end;

function TJvxSpeedItem.GetOnClick: TNotifyEvent;
begin
  Result := FButton.OnClick;
end;

procedure TJvxSpeedItem.SetOnClick(Value: TNotifyEvent);
begin
  FButton.OnClick := Value;
end;

function TJvxSpeedItem.GetOnDblClick: TNotifyEvent;
begin
  Result := FButton.OnDblClick;
end;

procedure TJvxSpeedItem.SetOnDblClick(Value: TNotifyEvent);
begin
  FButton.OnDblClick := Value;
end;

function TJvxSpeedItem.GetOnMouseDown: TMouseEvent;
begin
  Result := FButton.OnMouseDown;
end;

procedure TJvxSpeedItem.SetOnMouseDown(Value: TMouseEvent);
begin
  FButton.OnMouseDown := Value;
end;

function TJvxSpeedItem.GetOnMouseMove: TMouseMoveEvent;
begin
  Result := FButton.OnMouseMove;
end;

procedure TJvxSpeedItem.SetOnMouseMove(Value: TMouseMoveEvent);
begin
  FButton.OnMouseMove := Value;
end;

function TJvxSpeedItem.GetOnMouseUp: TMouseEvent;
begin
  Result := FButton.OnMouseUp;
end;

procedure TJvxSpeedItem.SetOnMouseUp(Value: TMouseEvent);
begin
  FButton.OnMouseUp := Value;
end;

function TJvxSpeedItem.GetOnMouseEnter: TNotifyEvent;
begin
  Result := FButton.OnMouseEnter;
end;

procedure TJvxSpeedItem.SetOnMouseEnter(Value: TNotifyEvent);
begin
  FButton.OnMouseEnter := Value;
end;

function TJvxSpeedItem.GetOnMouseLeave: TNotifyEvent;
begin
  Result := FButton.OnMouseLeave;
end;

procedure TJvxSpeedItem.SetOnMouseLeave(Value: TNotifyEvent);
begin
  FButton.OnMouseLeave := Value;
end;

function TJvxSpeedItem.GetDown: Boolean;
begin
  Result := FButton.Down;
end;

procedure TJvxSpeedItem.SetDown(Value: Boolean);
begin
  FButton.Down := Value;
end;

function TJvxSpeedItem.GetGlyph: TBitmap;
begin
  Result := FButton.Glyph;
end;

procedure TJvxSpeedItem.SetGlyph(Value: TBitmap);
begin
  FButton.Glyph := Value;
end;

function TJvxSpeedItem.GetLayout: TButtonLayout;
begin
  Result := FButton.Layout;
end;

procedure TJvxSpeedItem.SetLayout(Value: TButtonLayout);
begin
  FButton.Layout := Value;
end;

function TJvxSpeedItem.GetMargin: Integer;
begin
  Result := FButton.Margin;
end;

procedure TJvxSpeedItem.SetMargin(Value: Integer);
begin
  FButton.Margin := Value;
end;

function TJvxSpeedItem.GetNumGlyphs: TJvxNumGlyphs;
begin
  Result := FButton.NumGlyphs;
end;

procedure TJvxSpeedItem.SetNumGlyphs(Value: TJvxNumGlyphs);
begin
  FButton.NumGlyphs := Value;
end;

function TJvxSpeedItem.GetParentShowHint: Boolean;
begin
  Result := FButton.ParentShowHint;
end;

procedure TJvxSpeedItem.SetParentShowHint(Value: Boolean);
begin
  FButton.ParentShowHint := Value;
end;

function TJvxSpeedItem.GetShowHint: Boolean;
begin
  Result := FButton.ShowHint;
end;

procedure TJvxSpeedItem.SetShowHint(Value: Boolean);
begin
  FButton.ShowHint := Value;
end;

function TJvxSpeedItem.GetFont: TFont;
begin
  Result := FButton.Font;
end;

procedure TJvxSpeedItem.SetFont(Value: TFont);
begin
  FButton.Font := Value;
end;

function TJvxSpeedItem.GetParentFont: Boolean;
begin
  Result := FButton.ParentFont;
end;

procedure TJvxSpeedItem.SetParentFont(Value: Boolean);
begin
  FButton.ParentFont := Value;
end;

function TJvxSpeedItem.IsFontStored: Boolean;
begin
  Result := not ParentFont;
end;

function TJvxSpeedItem.IsShowHintStored: Boolean;
begin
  Result := not ParentShowHint;
end;

function TJvxSpeedItem.GetSpacing: Integer;
begin
  Result := FButton.Spacing;
end;

procedure TJvxSpeedItem.SetSpacing(Value: Integer);
begin
  FButton.Spacing := Value;
end;

function TJvxSpeedItem.GetCursor: TCursor;
begin
  Result := FButton.Cursor;
end;

procedure TJvxSpeedItem.SetCursor(Value: TCursor);
begin
  FButton.Cursor := Value;
end;

function TJvxSpeedItem.GetHint: string;
begin
  Result := FButton.Hint;
end;

procedure TJvxSpeedItem.SetHint(const Value: string);
begin
  FButton.Hint := Value;
end;

{$IFDEF Delphi4_Up}
function TJvxSpeedItem.GetAction: TBasicAction;
begin
  Result := FButton.Action;
end;

procedure TJvxSpeedItem.SetAction(Value: TBasicAction);
begin
  FButton.Action := Value;
end;
{$ENDIF}

procedure TJvxSpeedItem.ButtonClick;
begin
  FButton.ButtonClick;
end;

function TJvxSpeedItem.CheckBtnMenuDropDown: Boolean;
begin
  Result := FButton.CheckBtnMenuDropDown;
end;

procedure TJvxSpeedItem.Click;
begin
  FButton.Click;
end;

function TJvxSpeedItem.GetTag: Longint;
begin
  Result := inherited Tag;
end;

procedure TJvxSpeedItem.SetTag(Value: Longint);
begin
  inherited Tag := Value;
  FButton.Tag := Value;
end;

function TJvxSpeedItem.GetDropDownMenu: TPopupMenu;
begin
  Result := FButton.DropDownMenu;
end;

procedure TJvxSpeedItem.SetDropDownMenu(Value: TPopupMenu);
begin
  FButton.DropDownMenu := Value;
end;

function TJvxSpeedItem.GetMarkDropDown: Boolean;
begin
  Result := FButton.MarkDropDown;
end;

procedure TJvxSpeedItem.SetMarkDropDown(Value: Boolean);
begin
  FButton.MarkDropDown := Value;
end;

function TJvxSpeedItem.GetWordWrap: Boolean;
begin
  Result := FButton.WordWrap;
end;

procedure TJvxSpeedItem.SetWordWrap(Value: Boolean);
begin
  FButton.WordWrap := Value;
end;

function TJvxSpeedItem.GetLeft: Integer;
begin
  Result := FButton.Left;
end;

function TJvxSpeedItem.GetTop: Integer;
begin
  Result := FButton.Top;
end;

procedure TJvxSpeedItem.SetLeft(Value: Integer);
begin
  FButton.Left := Value;
end;

procedure TJvxSpeedItem.SetTop(Value: Integer);
begin
  FButton.Top := Value;
end;
...
constructor TJvxSpeedBar.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
...
{$ENDIF}
  if not Registered then begin
    RegisterClasses([TJvxSpeedItem, TJvxSpeedbarSection, TJvxSpeedBarButton]);
    Registered := True;
  end;
end;

destructor TJvxSpeedBar.Destroy;
begin
...
procedure TJvxSpeedBar.ReadData(Reader: TReader);
begin
  FReserved := Reader.ReadInteger;
end;

procedure TJvxSpeedBar.WriteData(Writer: TWriter);
begin
  Writer.WriteInteger(InternalVer);
end;

procedure TJvxSpeedBar.ReadAllowDrag(Reader: TReader);
begin
  if Reader.ReadBoolean then Options := Options + [sbAllowDrag]
  else Options := Options - [sbAllowDrag];
end;

procedure TJvxSpeedBar.ReadDesignStyle(Reader: TReader);
begin
  FDesignStyle := Reader.ReadBoolean;
end;

procedure TJvxSpeedBar.WriteDesignStyle(Writer: TWriter);
begin
  Writer.WriteBoolean(NewStyleControls);
end;

procedure TJvxSpeedBar.ReadSections(Reader: TReader);
var
{$IFDEF WIN32}
  TmpList: TStrings;
  I: Integer;
...
procedure TJvxSpeedBar.WriteSections(Writer: TWriter);
var
  I: Integer;
begin
...
procedure TJvxSpeedBar.DefineProperties(Filer: TFiler);
begin
  inherited DefineProperties(Filer);
...
function TJvxSpeedBar.GetSection(Index: Integer): TJvxSpeedbarSection;
begin
  Result := TJvxSpeedbarSection(FSections[Index]);
end;

function TJvxSpeedBar.GetSectionCount: Integer;
begin
  Result := FSections.Count;
end;

procedure TJvxSpeedBar.ForEachItem(Proc: TForEachItem; Data: Longint);
var
  I, Idx: Integer;
...
begin
  for I := 0 to FSections.Count - 1 do
    if FSections[I] <> nil then begin
      Sect := TJvxSpeedbarSection(FSections[I]);
      for Idx := 0 to Sect.Count - 1 do begin
        if (Sect[Idx] <> nil) and Assigned(Proc) then
          Proc(TJvxSpeedItem(Sect[Idx]), Data);
      end;
    end;
end;

function TJvxSpeedBar.MinButtonsOffset: Integer;
begin
...
procedure TJvxSpeedBar.SetItemVisible(Item: TJvxSpeedItem; Data: Longint);
var
  ItemVisible: Boolean;
begin
...
procedure TJvxSpeedBar.SetItemEnabled(Item: TJvxSpeedItem; Data: Longint);
begin
  Item.FButton.Enabled := Item.Enabled and Self.Enabled;
end;

procedure TJvxSpeedBar.SetItemButtonSize(Item: TJvxSpeedItem; Data: Longint);
begin
  ApplyItemSize(Item, Data);
  Item.Visible := Item.Visible; { update visible and parent after loading }
end;

procedure TJvxSpeedBar.SwapItemBounds(Item: TJvxSpeedItem; Data: Longint);
begin
  Item.FButton.SetBounds(Item.Top, Item.Left, FButtonSize.X, FButtonSize.Y);
end;

procedure TJvxSpeedBar.SetFontDefault;
{$IFDEF WIN32}
var
...
procedure TJvxSpeedBar.CMVisibleChanged(var Message: TMessage);
begin
  inherited;
...
procedure TJvxSpeedBar.CMEnabledChanged(var Message: TMessage);
begin
  inherited;
...
procedure TJvxSpeedBar.WallpaperChanged(Sender: TObject);
begin
  Invalidate;
end;

procedure TJvxSpeedBar.SetWallpaper(Value: TPicture);
begin
  FWallpaper.Assign(Value);
end;

procedure TJvxSpeedBar.ClearSections;
begin
  while FSections.Count > 0 do RemoveSection(FSections.Count - 1);
  FSections.Clear;
end;

function TJvxSpeedBar.Items(Section, Index: Integer): TJvxSpeedItem;
var
  List: TJvxSpeedbarSection;
begin
  Result := nil;
...
function TJvxSpeedBar.ItemsCount(Section: Integer): Integer;
begin
  Result := 0;
  if (Section >= 0) and (Section < FSections.Count) then begin
...
procedure TJvxSpeedBar.RemoveSection(Section: Integer);
var
  Sect: TJvxSpeedbarSection;
  Item: TJvxSpeedItem;
begin
...
procedure TJvxSpeedBar.DeleteSection(Section: Integer);
var
  Sect: TJvxSpeedbarSection;
  I: Integer;
begin
  Sect := Sections[Section];
  if Sect <> nil then begin
    for I := 0 to Sect.Count - 1 do RemoveItem(TJvxSpeedItem(Sect[I]));
    Sect.FParent := nil;
    FSections[Section] := nil;
  end;
  FSections.Delete(Section);
...
procedure TJvxSpeedBar.RemoveItem(Item: TJvxSpeedItem);
var
  I, Index: Integer;
begin
...
function TJvxSpeedBar.SearchSection(const ACaption: string): Integer;
var
  I: Integer;
begin
  Result := -1;
...
function TJvxSpeedBar.AppendSection(Value: TJvxSpeedbarSection): Integer;
var
  UniqueName: string;
  I: Integer;
begin
...
function TJvxSpeedBar.AddSection(const ACaption: string): Integer;
var
  Section: TJvxSpeedbarSection;
begin
  if Owner <> nil then Section := TJvxSpeedbarSection.Create(Owner)
  else Section := TJvxSpeedbarSection.Create(Self);
  Section.Caption := ACaption;
  Result := AppendSection(Section);
end;

procedure TJvxSpeedBar.SetItemParams(Item: TJvxSpeedItem; InitBounds: Boolean);
begin
  with Item do begin
    FParent := Self;
...
function TJvxSpeedBar.NewItem(AOwner: TComponent; Section: Integer;
  const AName: string): TJvxSpeedItem;
begin
  Result := nil;
  if (Section >= 0) and (Section < FSections.Count) then begin
    Result := TJvxSpeedItem.Create(AOwner);
    try
...
procedure TJvxSpeedBar.AddItem(Section: Integer; Item: TJvxSpeedItem);
var
  I, Index: Integer;
begin
  if FindItem(Item, I, Index) then begin
...
function TJvxSpeedBar.FindItem(Item: TJvxSpeedItem; var Section,
...
procedure TJvxSpeedBar.AlignItemsToGrid;
begin
  ForEachItem(AlignItemToGrid, 0);
end;

procedure TJvxSpeedBar.AlignItemToGrid(Item: TJvxSpeedItem; Data: Longint);
begin
...
function TJvxSpeedBar.AcceptDropItem(Item: TJvxSpeedItem; X, Y: Integer): Boolean;
var
  I, Sect: Integer;
begin
  Result := False;
...
procedure TJvxSpeedBar.SetItemEditing(Item: TJvxSpeedItem; Data: Longint);
begin
  Item.SetEditing(FEditWin <> 0);
end;

function TJvxSpeedBar.GetEditing: Boolean;
begin
  Result := (FEditWin <> 0);
end;

procedure TJvxSpeedBar.SetEditing(Win: HWnd);
begin
  FEditWin := Win;
...
procedure TJvxSpeedBar.ApplyOrientation(Value: TBarOrientation);
begin
...
procedure TJvxSpeedBar.SetOrientation(Value: TBarOrientation);
begin
  if GetOrientation <> Value then begin
    if (FPosition = bpAuto) then
      raise EJvxSpeedbarError.Create(SAutoSpeedbarMode);
    ApplyOrientation(Value);
  end;
end;

function TJvxSpeedBar.GetOrientation: TBarOrientation;
begin
...
function TJvxSpeedBar.GetAlign: TAlign;
begin
  Result := FAlign;
end;

procedure TJvxSpeedBar.SetAlign(Value: TAlign);
var
  X, Y: Integer;
begin
...
procedure TJvxSpeedBar.ChangeScale(M, D: Integer);
var
  Flags: TSbScaleFlags;
begin
  DisableAlign;
...
procedure TJvxSpeedBar.AlignControls(AControl: TControl; var Rect: TRect);
var
  P: TPoint;
  Min: Integer;
begin
...
procedure TJvxSpeedBar.FlatItem(Item: TJvxSpeedItem; Data: Longint);
begin
  Item.FButton.Flat := Boolean(Data);
end;

procedure TJvxSpeedBar.GrayedItem(Item: TJvxSpeedItem; Data: Longint);
begin
  Item.FButton.GrayedInactive := Boolean(Data);
end;

procedure TJvxSpeedBar.TransparentItem(Item: TJvxSpeedItem; Data: Longint);
begin
  Item.FButton.Transparent := Boolean(Data);
end;

procedure TJvxSpeedBar.SetBoundLines(Value: TBoundLines);
begin
...
procedure TJvxSpeedBar.SetOptions(Value: TJvxSpeedBarOptions);
var
  FlatChanged: Boolean;
begin
...
procedure TJvxSpeedBar.OffsetItem(Item: TJvxSpeedItem; Data: Longint);
var
  P: TPoint;
begin
  P := PPoint(Data)^;
...
function TJvxSpeedBar.GetButtonsOffset(Index: Integer): Integer;
begin
...
procedure TJvxSpeedBar.SetButtonsOffset(Index: Integer; Value: Integer);
var
  P: TPoint;
begin
...
procedure TJvxSpeedBar.UpdateGridSize;
var
  Base: Integer;
begin
  case Orientation of
...
procedure TJvxSpeedBar.ApplyItemSize(Item: TJvxSpeedItem; Data: Longint);
begin
  with Item do
    FButton.SetBounds(FButton.Left, FButton.Top, FButtonSize.X, FButtonSize.Y);
end;

procedure TJvxSpeedBar.ApplyButtonSize;
begin
  ForEachItem(ApplyItemSize, 0);
  if FEditWin <> 0 then { update JvxSpeedbar editor }
    SendMessage(FEditWin, CM_SPEEDBARCHANGED, SBR_BTNSIZECHANGED, Longint(Self));
end;

function TJvxSpeedBar.GetButtonSize(Index: Integer): Integer;
begin
...
procedure TJvxSpeedBar.SetButtonSize(Index, Value: Integer);
var
  NewSize: TPoint;
begin
...
procedure TJvxSpeedBar.GetChildren(Proc: TGetChildProc {$IFDEF Delphi3_Up};  Root: TComponent {$ENDIF});
var
  I, Idx: Integer;
  Sect: TJvxSpeedbarSection;
  Item: TJvxSpeedItem;
...
procedure TJvxSpeedBar.SetChildOrder(Component: TComponent; Order: Integer);
begin
  if FSections.IndexOf(Component) >= 0 then
    (Component as TJvxSpeedbarSection).Index := Order;
end;

procedure TJvxSpeedBar.Notification(AComponent: TComponent;
...
procedure TJvxSpeedBar.InvalidateItem(Item: TJvxSpeedItem; Data: Longint);
begin
  with Item do
    if (Button <> nil) then begin
      TJvxSpeedBarButton(Button).InvalidateGlyph;
      if FImageIndex >= 0 then Button.Invalidate;
    end;
end;

procedure TJvxSpeedBar.ImageListChange(Sender: TObject);
begin
  ForEachItem(InvalidateItem, 0);
end;

procedure TJvxSpeedBar.SetImages(Value: TImageList);
begin
...
procedure TJvxSpeedBar.WriteComponents(Writer: TWriter);
var
  I, Idx: Integer;
  Sect: TJvxSpeedbarSection;
  Item: TJvxSpeedItem;
...
function TJvxSpeedBar.SearchItem(const ItemName: string): TJvxSpeedItem;
var
  I, Idx: Integer;
  Sect: TJvxSpeedbarSection;
  Item: TJvxSpeedItem;
...
type
  TJvxSpeedBarPos = (bpTop, bpBottom, bpLeft, bpRight);
const
  PosToAlign: array[TJvxSpeedBarPos] of TAlign = (alTop, alBottom, alLeft, alRight);

function TJvxSpeedBar.GetFramePos(X, Y: Integer; var Apply: Boolean): Integer;
var
  P: TPoint;
  W, H: Double;
begin
  ...
  if Assigned(FOnApplyAlign) then
    FOnApplyAlign(Self, PosToAlign[TJvxSpeedBarPos(Result)], Apply);
end;

function TJvxSpeedBar.GetFrameRect(X, Y: Integer): TRect;
var
  Pos: TJvxSpeedBarPos;
  W: Integer;
  Apply: Boolean;
...
begin
  Apply := True;
  Pos := TJvxSpeedBarPos(GetFramePos(X, Y, Apply));
  if Apply then begin
    Result := MaxRect;
...
procedure TJvxSpeedBar.StartDragFrame;
var
  Rect: TRect;
begin
  with Rect do begin
...
procedure TJvxSpeedBar.DragFrame(X, Y: Integer);
var
  Rect: TRect;
begin
...
procedure TJvxSpeedBar.StopDragFrame(X, Y: Integer);
var
  Pos: TJvxSpeedBarPos;
  Apply: Boolean;
begin
...
function TJvxSpeedBar.CheckResize(Shift: TShiftState; X, Y: Integer): Boolean;
begin
  Result := False;
...
procedure TJvxSpeedBar.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
...
procedure TJvxSpeedBar.MouseMove(Shift: TShiftState; X, Y: Integer);
var
  Cnt: Integer;
  P: TPoint;
begin
...
procedure TJvxSpeedBar.MouseUp(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
...
procedure TJvxSpeedBar.PosChanged;
begin
  if Assigned(FOnPosChanged) then FOnPosChanged(Self);
end;

procedure TJvxSpeedBar.AfterCustomize;
begin
  if Assigned(FOnCustomize) then FOnCustomize(Self);
end;

function TJvxSpeedBar.GetStorage: TJvxFormPlacement;
begin
  Result := FIniLink.Storage;
end;

procedure TJvxSpeedBar.SetStorage(Value: TJvxFormPlacement);
begin
  FIniLink.Storage := Value;
end;

procedure TJvxSpeedBar.Customize(HelpCtx: THelpContext);
begin
  ShowSpeedbarSetupWindow(Self, HelpCtx);
end;

procedure TJvxSpeedBar.IniSave(Sender: TObject);
begin
...
procedure TJvxSpeedBar.IniLoad(Sender: TObject);
begin
  if (Name <> '') and (FIniLink.IniObject <> nil) then
...
procedure TJvxSpeedBar.HideItem(Item: TJvxSpeedItem; Data: Longint);
begin
  Item.Visible := False;
end;

procedure TJvxSpeedBar.WriteItemLayout(Item: TJvxSpeedItem; Data: Longint);
begin
...
procedure TJvxSpeedBar.InternalSaveLayout(IniFile: TObject;
...
procedure TJvxSpeedBar.InternalRestoreLayout(IniFile: TObject;
  const Section: string);
const
  Delims = [' ',','];
var
...
procedure TJvxSpeedBar.SaveLayout(IniFile: TIniFile);
begin
  InternalSaveLayout(IniFile, GetDefaultSection(Self));
end;

procedure TJvxSpeedBar.RestoreLayout(IniFile: TIniFile);
begin
  InternalRestoreLayout(IniFile, GetDefaultSection(Self));
end;

{$IFDEF WIN32}
procedure TJvxSpeedBar.SaveLayoutReg(IniFile: TRegIniFile);
begin
  InternalSaveLayout(IniFile, GetDefaultSection(Self));
end;

procedure TJvxSpeedBar.RestoreLayoutReg(IniFile: TRegIniFile);
begin
  InternalRestoreLayout(IniFile, GetDefaultSection(Self));
end;
{$ENDIF WIN32}
...
constructor TJvxBtnControl.Create(AOwner: TComponent);
begin
...
destructor TJvxBtnControl.Destroy;
begin
  FImage.Free;
  inherited Destroy;
end;

procedure TJvxBtnControl.CreateParams(var Params: TCreateParams);
begin
...
procedure TJvxBtnControl.AssignSpeedItem(Item: TJvxSpeedItem);
begin
...
{$IFDEF WIN32}
  ...
  if Item.JvxSpeedbar <> nil then Images := Item.JvxSpeedbar.Images
  else Images := nil;
{$ENDIF}
  Font := Item.Font;
{$IFDEF Delphi4_Up}
...
{$ENDIF}
  SetBounds(0, 0, Item.JvxSpeedbar.BtnWidth, Item.JvxSpeedbar.BtnHeight);
end;

function TJvxBtnControl.GetGlyph: TBitmap;
begin
  Result := FImage.Glyph;
end;

function TJvxBtnControl.GetNumGlyphs: TJvxNumGlyphs;
begin
  Result := FImage.NumGlyphs;
end;

function TJvxBtnControl.GetCaption: TCaption;
begin
  Result := FImage.Caption;
end;

procedure TJvxBtnControl.SetCaption(const Value: TCaption);
begin
  FImage.Caption := Value;
end;

procedure TJvxBtnControl.SetNumGlyphs(Value: TJvxNumGlyphs);
begin
  FImage.NumGlyphs := Value;
end;

procedure TJvxBtnControl.SetGlyph(Value: TBitmap);
begin
  FImage.Glyph := Value;
end;

function TJvxBtnControl.GetWordWrap: Boolean;
begin
  Result := FImage.WordWrap;
end;

procedure TJvxBtnControl.SetWordWrap(Value: Boolean);
begin
  FImage.WordWrap := Value;
end;

function TJvxBtnControl.GetAlignment: TAlignment;
begin
  Result := FImage.Alignment;
end;

procedure TJvxBtnControl.SetAlignment(Value: TAlignment);
begin
  FImage.Alignment := Value;
end;

procedure TJvxBtnControl.WMSize(var Message: TWMSize);
begin
  FImage.ButtonSize := Point(ClientWidth, ClientHeight);
end;

procedure TJvxBtnControl.Paint;
begin
{$IFDEF WIN32}
...
procedure TJvxBtnControl.Activate(Rect: TRect);
begin
...
procedure TJvxBtnControl.ReleaseHandle;
begin
  DestroyHandle;
end;
...
begin
  ...
  if Window <> nil then begin
    if Window is TJvxSpeedBar then Result := Window as TJvxSpeedBar;
  end;
end;
