...
function AppVerInfo: TJvxVersionInfo;
...
{$IFDEF WIN32}
uses SysUtils, JvxFileUtil, JvxDateUtil;
{$ELSE}
uses WinTypes, WinProcs, SysUtils, JvxFileUtil, JvxDateUtil, JvxVCLUtils;
{$ENDIF}
...
constructor TJvxVersionInfo.Create(const AFileName: string);
begin
  inherited Create;
...
destructor TJvxVersionInfo.Destroy;
begin
...
procedure TJvxVersionInfo.ReadVersionInfo;
begin
  FValid := False;
...
function TJvxVersionInfo.GetFileName: string;
begin
  Result := StrPas(FFileName);
end;

procedure TJvxVersionInfo.SetFileName(const Value: string);
begin
  if FBuffer <> nil then FreeMem(FBuffer, FSize);
  FBuffer := nil;
  StrDispose(FFileName);
...
function TJvxVersionInfo.GetTranslation: Pointer;
var
{$IFDEF WIN32}
  Len: UINT;
{$ELSE}
...
function TJvxVersionInfo.GetTranslationString: string;
var
  P: Pointer;
begin
  Result := '';
...
function TJvxVersionInfo.GetVersionLanguage: TVersionLanguage;
var
  P: Pointer;
begin
  P := GetTranslation;
...
function TJvxVersionInfo.GetVersionCharSet: TVersionCharSet;
var
  P: Pointer;
begin
  P := GetTranslation;
...
function TJvxVersionInfo.GetFixedFileInfo: PVSFixedFileInfo;
var
{$IFDEF WIN32}
  Len: UINT;
{$ELSE}
...
function TJvxVersionInfo.GetProductLongVersion: TLongVersion;
begin
  Result.MS := FixedFileInfo^.dwProductVersionMS;
  Result.LS := FixedFileInfo^.dwProductVersionLS;
end;

function TJvxVersionInfo.GetFileLongVersion: TLongVersion;
begin
  Result.MS := FixedFileInfo^.dwFileVersionMS;
  Result.LS := FixedFileInfo^.dwFileVersionLS;
end;

function TJvxVersionInfo.GetVersionNum: Longint;
begin
  if Valid then Result := FixedFileInfo^.dwFileVersionMS
  else Result := 0;
end;

function TJvxVersionInfo.GetVerValue(const VerName: string): string;
var
...
function TJvxVersionInfo.GetComments: string;
begin
  Result := GetVerValue('Comments');
end;

function TJvxVersionInfo.GetCompanyName: string;
begin
  Result := GetVerValue('CompanyName');
end;

function TJvxVersionInfo.GetFileDescription: string;
begin
  Result := GetVerValue('FileDescription');
end;

function TJvxVersionInfo.GetFileVersion: string;
begin
...
function TJvxVersionInfo.GetInternalName: string;
begin
  Result := GetVerValue('InternalName');
end;

function TJvxVersionInfo.GetLegalCopyright: string;
begin
  Result := GetVerValue('LegalCopyright');
end;

function TJvxVersionInfo.GetLegalTrademarks: string;
begin
  Result := GetVerValue('LegalTrademarks');
end;

function TJvxVersionInfo.GetOriginalFilename: string;
begin
  Result := GetVerValue('OriginalFilename');
end;

function TJvxVersionInfo.GetProductVersion: string;
begin
...
function TJvxVersionInfo.GetProductName: string;
begin
  Result := GetVerValue('ProductName');
end;

function TJvxVersionInfo.GetSpecialBuild: string;
begin
  Result := GetVerValue('SpecialBuild');
end;

function TJvxVersionInfo.GetPrivateBuild: string;
begin
  Result := GetVerValue('PrivateBuild');
end;

function TJvxVersionInfo.GetVerFileDate: TDateTime;
begin
...
function AppVerInfo: TJvxVersionInfo;
begin
  Result := TJvxVersionInfo.Create(AppFileName);
end;
...
begin
  ...
  with TJvxVersionInfo.Create(ModuleName) do begin
    try
