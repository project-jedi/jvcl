...
{ TJvxCustomSlider }
  ...
  TJvxCustomSlider = class(TCustomControl)
  private
  ...
  protected
    ...
    function GeTJvxSliderRect: TRect; virtual;
    function GeTJvxSliderValue: Longint; virtual;
    ...
    property SliderRect: TRect read GeTJvxSliderRect;
...
{ TJvxSlider }
  ...
  TJvxSlider = class(TJvxCustomSlider)
  published
    property Align;
    property BevelStyle;
    property Color;
...
{ TJvxCustomTrackBar }
  ...
  TJvxSliderImages = class;

  TJvxCustomTrackBar = class(TJvxCustomSlider)
  private
    FImages: TJvxSliderImages;
  protected
    property Images: TJvxSliderImages read FImages write FImages;
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
  end;

  TJvxSliderImages = class(TPersistent)
  private
    FSlider: TJvxCustomSlider;
    function GetNumThumbStates: TNumThumbStates;
    procedure SetNumThumbStates(Value: TNumThumbStates);
...
uses Consts, Forms, SysUtils, JvxVCLUtils, JvxMaxMin, JvxConst;
...
constructor TJvxCustomSlider.Create(AOwner: TComponent);
begin
...
destructor TJvxCustomSlider.Destroy;
var
  I: TSliderImage;
begin
  FOnChange := nil;
...
procedure TJvxCustomSlider.Loaded;
var
  I: TSliderImage;
begin
  inherited Loaded;
...
procedure TJvxCustomSlider.AlignControls(AControl: TControl; var Rect: TRect);
var
  BevelSize: Integer;
begin
  BevelSize := 0;
...
procedure TJvxCustomSlider.WMPaint(var Message: TWMPaint);
var
  DC, MemDC: HDC;
...
procedure TJvxCustomSlider.Paint;
var
  R: TRect;
...
{$IFDEF WIN32}
  if csPaintCopy in ControlState then begin
    Offset := GetOffsetByValue(GeTJvxSliderValue);
...
function TJvxCustomSlider.CanModify: Boolean;
begin
  Result := True;
end;

function TJvxCustomSlider.GeTJvxSliderValue: Longint;
begin
  Result := FValue;
end;

function TJvxCustomSlider.GeTJvxSliderRect: TRect;
begin
...
procedure TJvxCustomSlider.DrawThumb(Canvas: TCanvas; Origin: TPoint;
  Highlight: Boolean);
var
  R: TRect;
  Image: TBitmap;
...
procedure TJvxCustomSlider.InternalDrawPoints(ACanvas: TCanvas; PointsStep,
...
procedure TJvxCustomSlider.DefaultDrawPoints(PointsStep, PointsHeight,
  ExtremePointsHeight: Longint);
begin
  InternalDrawPoints(Canvas, PointsStep, PointsHeight, ExtremePointsHeight);
end;

procedure TJvxCustomSlider.CreateElements;
var
  I: TSliderImage;
begin
...
procedure TJvxCustomSlider.BuildRuler(R: TRect);
var
  DstR, BmpR: TRect;
...
procedure TJvxCustomSlider.AdjustElements;
var
  SaveValue: Longint;
  R: TRect;
begin
...
procedure TJvxCustomSlider.Sized;
begin
  AdjustElements;
end;

procedure TJvxCustomSlider.Change;
begin
  if Assigned(FOnChange) then FOnChange(Self);
end;

procedure TJvxCustomSlider.Changed;
begin
  if Assigned(FOnChanged) then FOnChanged(Self);
end;

procedure TJvxCustomSlider.RangeChanged;
begin
end;

procedure TJvxCustomSlider.DefineProperties(Filer: TFiler);

{$IFDEF WIN32}
  function DoWrite: Boolean;
  begin
    if Assigned(Filer.Ancestor) then
      Result := FUserImages <> TJvxCustomSlider(Filer.Ancestor).FUserImages
...
procedure TJvxCustomSlider.ReadUserImages(Stream: TStream);
begin
  Stream.ReadBuffer(FUserImages, SizeOf(FUserImages));
end;

procedure TJvxCustomSlider.WriteUserImages(Stream: TStream);
begin
  Stream.WriteBuffer(FUserImages, SizeOf(FUserImages));
end;

function TJvxCustomSlider.StoreImage(Index: Integer): Boolean;
begin
  Result := TSliderImage(Index) in FUserImages;
end;

function TJvxCustomSlider.GetImage(Index: Integer): TBitmap;
begin
  Result := FImages[TSliderImage(Index)];
end;

procedure TJvxCustomSlider.SliderImageChanged(Sender: TObject);
begin
  if not (csCreating in ControlState) then Sized;
end;

procedure TJvxCustomSlider.SetImage(Index: Integer; Value: TBitmap);
var
  Idx: TSliderImage;
begin
...
procedure TJvxCustomSlider.SetEdgeSize(Value: Integer);
var
  MaxSize: Integer;
begin
...
function TJvxCustomSlider.GetNumThumbStates: TNumThumbStates;
begin
  Result := FNumThumbStates;
end;

procedure TJvxCustomSlider.SetNumThumbStates(Value: TNumThumbStates);
begin
...
procedure TJvxCustomSlider.SetBevelStyle(Value: TPanelBevel);
begin
...
procedure TJvxCustomSlider.SetOrientation(Value: TSliderOrientation);
begin
...
procedure TJvxCustomSlider.SetOptions(Value: TSliderOptions);
begin
...
procedure TJvxCustomSlider.SetRange(Min, Max: Longint);
begin
...
procedure TJvxCustomSlider.SetMinValue(Value: Longint);
begin
  if FMinValue <> Value then SetRange(Value, MaxValue);
end;

procedure TJvxCustomSlider.SetMaxValue(Value: Longint);
begin
  if FMaxValue <> Value then SetRange(MinValue, Value);
end;

procedure TJvxCustomSlider.SetIncrement(Value: Longint);
begin
...
function TJvxCustomSlider.GetValueByOffset(Offset: Integer): Longint;
var
  Range: Double;
  R: TRect;
begin
...
function TJvxCustomSlider.GetOffsetByValue(Value: Longint): Integer;
var
  Range: Double;
  R: TRect;
  MinIndent: Integer;
...
function TJvxCustomSlider.GetThumbPosition(var Offset: Integer): TPoint;
var
  R: TRect;
  MinIndent: Integer;
begin
...
function TJvxCustomSlider.GetRulerLength: Integer;
begin
  if Orientation = soHorizontal then begin
    Result := FRuler.Width;
...
procedure TJvxCustomSlider.SetValue(Value: Longint);
var
...
procedure TJvxCustomSlider.SetReadOnly(Value: Boolean);
begin
...
procedure TJvxCustomSlider.ThumbJump(Jump: TJumpMode);
var
  NewValue: Longint;
begin
...
function TJvxCustomSlider.JumpTo(X, Y: Integer): TJumpMode;
begin
  Result := jmNone;
  if Orientation = soHorizontal then begin
...
procedure TJvxCustomSlider.WMTimer(var Message: TMessage);
begin
  TimerTrack;
end;

procedure TJvxCustomSlider.CMEnabledChanged(var Message: TMessage);
begin
  inherited;
  InvalidateThumb;
end;

procedure TJvxCustomSlider.CMFocusChanged(var Message: TCMFocusChanged);
var
  Active: Boolean;
begin
...
procedure TJvxCustomSlider.WMGetDlgCode(var Msg: TWMGetDlgCode);
begin
  Msg.Result := DLGC_WANTARROWS;
end;

procedure TJvxCustomSlider.WMSize(var Message: TWMSize);
begin
  inherited;
  if not (csReading in ComponentState) then Sized;
end;

procedure TJvxCustomSlider.WMSetCursor(var Message: TWMSetCursor);
var
  P: TPoint;
begin
  GetCursorPos(P);
...
procedure TJvxCustomSlider.StopTracking;
begin
...
procedure TJvxCustomSlider.TimerTrack;
var
  Jump: TJumpMode;
begin
...
procedure TJvxCustomSlider.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
var
  Rect: TRect;
  P: TPoint;
...
procedure TJvxCustomSlider.MouseMove(Shift: TShiftState; X, Y: Integer);
begin
...
procedure TJvxCustomSlider.MouseUp(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
  StopTracking;
...
procedure TJvxCustomSlider.KeyDown(var Key: Word; Shift: TShiftState);
var
  Jump: TJumpMode;
begin
  Jump := jmNone;
...
procedure TJvxCustomSlider.ThumbMouseDown(Button: TMouseButton;
...
procedure TJvxCustomSlider.ThumbMouseMove(Shift: TShiftState; X, Y: Integer);
begin
...
procedure TJvxCustomSlider.ThumbMouseUp(Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
constructor TJvxCustomTrackBar.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  FImages := TJvxSliderImages.Create;
  FImages.FSlider := Self;
end;

destructor TJvxCustomTrackBar.Destroy;
begin
  FImages.Free;
  inherited Destroy;
end;
...
function TJvxSliderImages.GetImage(Index: Integer): TBitmap;
begin
  Result := FSlider.GetImage(Index);
end;

procedure TJvxSliderImages.SetImage(Index: Integer; Value: TBitmap);
begin
  FSlider.SetImage(Index, Value);
end;

function TJvxSliderImages.StoreImage(Index: Integer): Boolean;
begin
  Result := FSlider.StoreImage(Index);
end;

function TJvxSliderImages.GetNumThumbStates: TNumThumbStates;
begin
  Result := FSlider.NumThumbStates;
end;

procedure TJvxSliderImages.SetNumThumbStates(Value: TNumThumbStates);
begin
  FSlider.NumThumbStates := Value;
end;

function TJvxSliderImages.GetEdgeSize: Integer;
begin
  Result := FSlider.EdgeSize;
end;

procedure TJvxSliderImages.SetEdgeSize(Value: Integer);
begin
