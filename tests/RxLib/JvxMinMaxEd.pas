...
The Original Code is: JvxMinMaxEd.PAS, released on 2002-07-04.
...
uses SysUtils, {$IFDEF WIN32} Windows, {$ELSE} WinTypes, WinProcs, {$ENDIF}
  ...
  Buttons, Mask, JvxCurrEdit, JvxVCLUtils, JvxPlacemnt, RTLConsts, DesignIntf, DesignEditors, VCLEditors, Consts;
...
type
  ...
  public
    { Public declarations }
    property WinMinMaxInfo: TJvxWinMinMaxInfo read FWinMinMaxInfo write SetWinMinMaxInfo;
  end;
...
function EditMinMaxInfo(AComponent: TJvxFormPlacement): Boolean;
...
var
  WinMinMaxInfo: TJvxWinMinMaxInfo;
begin
  WinMinMaxInfo := TJvxWinMinMaxInfo(GetOrdValue);
...
begin
  if EditMinMaxInfo(GetComponent(0) as TJvxFormPlacement) then Modified;
end;

{ TMinMaxInfoEditDialog }

procedure TMinMaxInfoEditDialog.SetWinMinMaxInfo(Value: TJvxWinMinMaxInfo);
begin
