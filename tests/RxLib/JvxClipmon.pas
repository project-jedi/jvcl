...
The Original Code is: JvxClipMon.PAS, released on 2002-07-04.
...
uses Messages, {$IFDEF WIN32} Windows, {$ELSE} WinTypes, WinProcs, {$ENDIF}
  SysUtils, Classes{, JvxComponent};

type
  TJvxClipboardMonitor = class(TComponent)
  private
    FWindowHandle: HWnd;
    FNextWindow: HWnd;
    FEnabled: Boolean;
...
{ TJvxClipboardMonitor }

constructor TJvxClipboardMonitor.Create(AOwner: TComponent);
begin
...
destructor TJvxClipboardMonitor.Destroy;
begin
  FOnChange := nil;
  SetEnabled(False);
...
procedure TJvxClipboardMonitor.ForwardMessage(var Msg: TMessage);
begin
  if FNextWindow <> 0 then
    with Msg do SendMessage(FNextWindow, Msg, WParam, LParam);
end;

procedure TJvxClipboardMonitor.WndProc(var AMsg: TMessage);
begin
  with AMsg do begin
    Result := 0;
    case Msg of
...
procedure TJvxClipboardMonitor.SetEnabled(Value: Boolean);
begin
...
procedure TJvxClipboardMonitor.ClipboardChanged;
begin
  try
    Change;
  except
...
procedure TJvxClipboardMonitor.Change;
begin
