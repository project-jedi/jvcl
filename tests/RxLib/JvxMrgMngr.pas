...
The Original Code is: JvxMrgMngr.PAS, released on 2002-07-04.
...
uses Classes, Controls, Forms, JvxVCLUtils{, JvxComponent};
...
type
  ...
  TJvxFormHistory = class;
  TFormHistoryCommand = (hcNone, hcAdd, hcBack, hcForward, hcGoto);

{ TJvxMergeManager }

  TJvxMergeManager = class(TComponent)
  private
    FMergeFrame: TWinControl;
    FFormHistory: TJvxFormHistory;
  ...
  public
    ...
    property FormHistory: TJvxFormHistory read FFormHistory;
...
constructor TJvxMergeManager.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  FFormHistory := TJvxFormHistory.Create;
  FHistoryCommand := hcAdd;
end;

destructor TJvxMergeManager.Destroy;
begin
  FFormHistory.Free;
  inherited Destroy;
end;

function TJvxMergeManager.NotIsForm: Boolean;
begin
  Result := (MergeFrame <> nil) and not (MergeFrame is TCustomForm);
end;

function TJvxMergeManager.IsForm: Boolean;
begin
  Result := (MergeFrame <> nil) and ((MergeFrame = Owner) and
    (Owner is TCustomForm));
end;

procedure TJvxMergeManager.ReadForm(Reader: TReader);
begin
  if Reader.ReadBoolean then
    if Owner is TCustomForm then MergeFrame := TWinControl(Owner);
end;

procedure TJvxMergeManager.WriteForm(Writer: TWriter);
begin
  Writer.WriteBoolean(IsForm);
end;

procedure TJvxMergeManager.DefineProperties(Filer: TFiler);
{$IFDEF WIN32}
  function DoWrite: Boolean;
  begin
    if Assigned(Filer.Ancestor) then
      Result := IsForm <> TJvxMergeManager(Filer.Ancestor).IsForm
    else Result := IsForm;
  end;
{$ENDIF}
begin
...
procedure TJvxMergeManager.SetMergeFrame(Value: TWinControl);
begin
...
function TJvxMergeManager.GetActiveForm: TCustomForm;
var
  I: Integer;
begin
...
procedure TJvxMergeManager.SetActiveForm(Value: TCustomForm);
begin
  GotoForm(Value);
end;

function TJvxMergeManager.GetBackForm: TCustomForm;
begin
...
function TJvxMergeManager.GetForwardForm: TCustomForm;
begin
...
procedure TJvxMergeManager.Notification(AComponent: TComponent;
...
procedure TJvxMergeManager.DoChange;
begin
  if Assigned(FOnChange) then FOnChange(Self);
end;

procedure TJvxMergeManager.DoReorder(Deactivated: TCustomForm);
begin
  if Assigned(FOnReorder) then FOnReorder(Self, ActiveForm, Deactivated);
end;

procedure TJvxMergeManager.Merge(AForm: TCustomForm; Show: Boolean);
begin
  MergeForm(MergeFrame, TForm(AForm), alClient, Show);
  GotoForm(AForm);
end;

function TJvxMergeManager.GotoForm(AForm: TCustomForm): Boolean;
var
  I: Integer;
...
function TJvxMergeManager.GotoFormClass(AFormClass: TFormClass): Boolean;
var
  I: Integer;
begin
  Result := False;
...
procedure TJvxMergeManager.GoBack;
begin
  HistoryCommand := hcBack;
  GotoForm(GetBackForm);
end;

procedure TJvxMergeManager.GoForward;
begin
  HistoryCommand := hcForward;
  GotoForm(GetForwardForm);
end;

procedure TJvxMergeManager.GotoHistoryIndex(HistoryIndex: Integer);
var
  SaveCurrent: Integer;
begin
...
constructor TJvxFormHistory.Create;
begin
  inherited Create;
  FCurrent := -1;
  FHistoryCapacity := 10;
...
destructor TJvxFormHistory.Destroy;
begin
  inherited Destroy;
end;

procedure TJvxFormHistory.SetCurrent(Value: Integer);
begin
...
procedure TJvxFormHistory.SetHistoryCapacity(Value: Integer);
var
  I: Integer;
begin
...
function TJvxFormHistory.GetForm(Index: Integer): TCustomForm;
begin
  Result := TCustomForm(Items[Index]);
end;

procedure TJvxFormHistory.AddForm(AForm: TCustomForm);
var
  I: Integer;
begin
...
procedure TJvxFormHistory.DeleteHistoryItem(Index: Integer);
begin
...
function TJvxFormHistory.RemoveItem(Item: TComponent): Boolean;
var
  I: Integer;
begin
  Result := False;
...
procedure TJvxFormHistory.ResetHistory;
begin
  Clear;
  Current := -1;
end;
