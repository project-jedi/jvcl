...
The Original Code is: JvxSplshWnd.PAS, released on 2002-07-04.
...
uses SysUtils, {$IFDEF WIN32} Windows, {$ELSE} WinTypes, WinProcs, {$ENDIF}
  ...
  JvxAnimate, JvxVCLUtils;

type
  TJvxSplashWindow = class(TForm)
  private
...
function ShowSplashWindow(Graphic: TGraphic; const MsgText: string;
  JvxAnimate: Boolean; AlignForm: TCustomForm): TJvxSplashWindow;
...
function CreateSplashWindow: TJvxSplashWindow;
begin
{$IFDEF CBUILDER}
  Result := TJvxSplashWindow.CreateNew(Application, 0);
{$ELSE}
  Result := TJvxSplashWindow.CreateNew(Application);
{$ENDIF}
  with Result do begin
    ...
    Animation := TJvxAnimatedImage.Create(Result);
...
procedure TJvxSplashWindow.CreateParams(var Params: TCreateParams);
begin
  inherited CreateParams(Params);
  Params.Style := Params.Style or WS_DLGFRAME;
end;

function TJvxSplashWindow.GetMessageText: string;
begin
  Result := FTextMessage.Caption;
end;

procedure TJvxSplashWindow.SetMessageText(const Value: string);
var
  TextRect: TRect;
{$IFNDEF WIN32}
...
procedure TJvxSplashWindow.CenterFor(Form: TCustomForm);
var
