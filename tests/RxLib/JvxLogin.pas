...
uses {$IFDEF WIN32} Windows, {$ELSE} WinTypes, WinProcs, {$ENDIF} SysUtils,
  ...
  Buttons{, JvxComponent};

type
  TUpdateCaption = (ucNoChange, ucAppTitle, ucFormCaption);
  TJvxLoginEvent = procedure(Sender: TObject; const UserName, Password: string;
...
{ TJvxCustomLogin }
  ...
  TJvxCustomLogin = class(TComponent)
  private
    FActive: Boolean;
  ...
  protected
    ...
    function CreateLoginForm(UnlockMode: Boolean): TJvxLoginForm; virtual;
...
{ TJvxLoginDialog }
  ...
  TJvxLoginDialog = class(TJvxCustomLogin)
  private
...
function CreateLoginDialog(UnlockMode, ASelectDatabase: Boolean;
  FormShowEvent, OkClickEvent: TNotifyEvent): TJvxLoginForm;

implementation

uses {$IFDEF WIN32} Registry, {$ENDIF} IniFiles, JvxAppUtils, JvxDConst,
  Consts, JvxVCLUtils, JvxConst;
...
function CreateLoginDialog(UnlockMode, ASelectDatabase: Boolean;
  FormShowEvent, OkClickEvent: TNotifyEvent): TJvxLoginForm;
begin
  Result := TJvxLoginForm.Create(Application);
...
constructor TJvxCustomLogin.Create(AOwner: TComponent);
begin
...
destructor TJvxCustomLogin.Destroy;
begin
...
function TJvxCustomLogin.GetIniFileName: string;
begin
  Result := FIniFileName;
...
procedure TJvxCustomLogin.SetIniFileName(const Value: string);
begin
  FIniFileName := Value;
end;

function TJvxCustomLogin.GetLoggedUser: string;
begin
  Result := FLoggedUser;
end;

procedure TJvxCustomLogin.SetLoggedUser(const Value: string);
begin
  FLoggedUser := Value;
end;

procedure TJvxCustomLogin.DoAfterLogin;
begin
  if Assigned(FAfterLogin) then FAfterLogin(Self);
end;

procedure TJvxCustomLogin.DoBeforeLogin;
begin
  if Assigned(FBeforeLogin) then FBeforeLogin(Self);
end;

procedure TJvxCustomLogin.DoIconDblCLick(Sender: TObject);
begin
  if Assigned(FOnIconDblClick) then FOnIconDblClick(Self);
end;

procedure TJvxCustomLogin.DoUpdateCaption;
var
  F: TForm;
begin
...
function TJvxCustomLogin.Login: Boolean;
var
  LoginName: string;
begin
  LoginName := EmptyStr;
...
procedure TJvxCustomLogin.Lock;
begin
...
procedure TJvxCustomLogin.TerminateApplication;
begin
...
procedure TJvxCustomLogin.UnlockOkClick(Sender: TObject);
var
  Ok: Boolean;
begin
...
function TJvxCustomLogin.CheckUnlock(const UserName, Password: string): Boolean;
begin
  Result := True;
...
function TJvxCustomLogin.CreateLoginForm(UnlockMode: Boolean): TJvxLoginForm;
begin
  Result := TJvxLoginForm.Create(Application);
  with Result do begin
    FUnlockMode := UnlockMode;
...
function TJvxCustomLogin.DoUnlockDialog: Boolean;
begin
...
function TJvxCustomLogin.UnlockHook(var Message: TMessage): Boolean;
...
{$ENDIF}
    if FUnlockDlgShowing then begin
      ...
      if (Popup <> 0) and IsWindowVisible(Popup) and
        (WindowClassName(Popup) = TJvxLoginForm.ClassName) then
      begin
{$IFDEF WIN32}
...
procedure TJvxLoginDialog.Loaded;
var
  Loading: Boolean;
begin
...
procedure TJvxLoginDialog.OkButtonClick(Sender: TObject);
var
  SetCursor: Boolean;
begin
  with TJvxLoginForm(Sender) do begin
...
function TJvxLoginDialog.DoCheckUser(const UserName, Password: string): Boolean;
begin
  Result := True;
...
procedure TJvxLoginDialog.WriteUserName(const UserName: string);
var
  Ini: TObject;
begin
  try
...
function TJvxLoginDialog.ReadUserName(const UserName: string): string;
var
  Ini: TObject;
begin
  try
...
function TJvxLoginDialog.DoLogin(var UserName: string): Boolean;
begin
  try
...
procedure TJvxLoginForm.FormCreate(Sender: TObject);
begin
...
procedure TJvxLoginForm.OkBtnClick(Sender: TObject);
begin
  Inc(FAttempt);
...
procedure TJvxLoginForm.FormShow(Sender: TObject);
var
  I: Integer;
  S: string;
begin
