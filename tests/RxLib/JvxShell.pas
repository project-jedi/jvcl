...
The Original Code is: JvxShell.PAS, released on 2002-07-04.
...
uses {$IFDEF WIN32} Windows, {$ELSE} WinTypes, WinProcs, {$ENDIF} Messages,
  ...
  {$IFDEF USE_TIMER} ExtCtrls, {$ENDIF} JvxIcoList{, JvxComponent};
...
{$ENDIF}
    ...
    procedure SetIconList(Value: TJvxIconList);
  ...
  published
    ...
    property Icons: TJvxIconList read FIconList write SetIconList;
...
uses JvxConst, JvxCConst, JvxVCLUtils, JvxMaxMin;
...
constructor TJvxTimerThread.Create(TrayIcon: TJvxTrayIcon; CreateSuspended: Boolean);
begin
  FOwnerTray := TrayIcon;
...
procedure TJvxTrayIcon.SetIconList(Value: TJvxIconList);
begin
...
{$ELSE}
      FTimer := TJvxTimerThread.Create(Self, not FAdded);
{$ENDIF}
      FAnimated := True;
    end
    else begin
...
{$IFDEF USE_TIMER}
      ...
      FTimer := nil;
{$ELSE}
      TJvxTimerThread(FTimer).FOwnerTray := nil;
