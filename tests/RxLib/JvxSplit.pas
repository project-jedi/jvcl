...
The Original Code is: JvxSplit.PAS, released on 2002-07-04.
...
uses Classes, {$IFDEF WIN32} Windows, {$ELSE} WinTypes, WinProcs, {$ENDIF}
  Controls, ExtCtrls, Forms, Graphics, JvxVCLUtils;
...
procedure TJvxSplitter.EndInverseRect(X, Y: Integer; AllowChange,
  Apply: Boolean);
const
  DecSize = 3;
var
...
begin
  if FSizing then begin
    ...
    if Assigned(FActiveControl) then begin
      TJvxHack(FActiveControl).OnKeyDown := FOldKeyDown;
      FActiveControl := nil;
    end;
  ...
  if not (csDesigning in ComponentState) and (Button = mbLeft) then begin
    ...
    if FStyle <> spUnknown then begin
      ...
      with ValidParentForm(Self) do begin
        ...
        FOldKeyDown := TJvxHack(FActiveControl).OnKeyDown;
        TJvxHack(FActiveControl).OnKeyDown := ControlKeyDown;
      end;
