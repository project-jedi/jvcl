...
The Original Code is: JvxQBndDlg.PAS, released on 2002-07-04.
...
type
  TJvxQueryParamsDialog = class(TForm)
    GroupBox1: TGroupBox;
    Label1: TLabel;
    ParamValue: TEdit;
    Label2: TLabel;
...
uses DbConsts, {$IFDEF Delphi3_Up} BdeConst, {$ENDIF} JvxVCLUtils;
...
function EditQueryParams(DataSet: TDataSet; List: TParams;
  AHelpContext: THelpContext {$IFDEF Delphi4_Up} = 0 {$ENDIF}): Boolean;
begin
  with TJvxQueryParamsDialog.Create(Application) do
  try
...
procedure TJvxQueryParamsDialog.Edit;
var
  I: Integer;
  J: TFieldType;
begin
...
procedure TJvxQueryParamsDialog.ParamListChange(Sender: TObject);
begin
  InParamChange := True;
  try
...
procedure TJvxQueryParamsDialog.TypeListChange(Sender: TObject);
begin
  with InitList.ParamByName(ParamList.Items[ParamList.ItemIndex]) do
  begin
...
procedure TJvxQueryParamsDialog.ParamValueExit(Sender: TObject);
begin
...
procedure TJvxQueryParamsDialog.CheckValue;
begin
  try
...
procedure TJvxQueryParamsDialog.Unbind;
begin
  with InitList.ParamByName(ParamList.Items[ParamList.ItemIndex]) do
  begin
    AsInteger := 1;
...
procedure TJvxQueryParamsDialog.NullValueClick(Sender: TObject);
begin
...
procedure TJvxQueryParamsDialog.OkBtnClick(Sender: TObject);
begin
...
procedure TJvxQueryParamsDialog.HelpBtnClick(Sender: TObject);
begin
  Application.HelpContext(HelpContext);
end;

procedure TJvxQueryParamsDialog.FormCreate(Sender: TObject);
begin
{$IFNDEF WIN32}
  Font.Style := [fsBold];
{$ENDIF}
