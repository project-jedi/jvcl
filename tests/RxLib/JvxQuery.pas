...
The Original Code is: JvxQuery.PAS, released on 2002-07-04.
...
uses Bde, Windows, RTLConsts,  Classes, SysUtils, DB, DBTables, JvxStrUtils, JvxBdeUtils{, JvxComponent};
...
{ TJvxSQLScript }
  ...
  TJvxSQLScript = class(TComponent)
  private
    FSQL: TStrings;
    FParams: TParams;
    FQuery: TJvxQuery;
...
uses JvxDBUtils, Consts, DBConsts, Forms {$IFDEF Delphi3_Up}, BDEConst {$ENDIF}
  {$IFNDEF WIN32}, JvxStr16 {$ENDIF}, JvxVCLUtils;
...
constructor TJvxQuery.Create(AOwner: TComponent);
begin
...
procedure TJvxQuery.InternalFirst;
begin
  if not (UniDirectional and BOF) then
    inherited InternalFirst;
end;

function TJvxQuery.GetRecord(Buffer: PChar; GetMode: TGetMode;
...
function TJvxQuery.CreateHandle: HDBICur;
begin
  FOpenStatus := qsFailed;
...
procedure TJvxQuery.OpenCursor;
begin
  ExpandMacros;
  inherited OpenCursor{$IFDEF Delphi3_Up}(InfoQuery){$ENDIF};
end;

procedure TJvxQuery.ExecSQL;
begin
  ExpandMacros;
  inherited ExecSQL;
end;

procedure TJvxQuery.Prepare;
begin
  ExpandMacros;
  inherited Prepare;
end;

procedure TJvxQuery.OpenOrExec(ChangeLive: Boolean);
...
procedure TJvxQuery.ExecDirect;
{$IFNDEF WIN32}
var
  P: PChar;
{$ENDIF}
...
procedure TJvxQuery.Disconnect;
var
  Strings: TStrings;
...
procedure TJvxQuery.SetMacroChar(Value: Char);
begin
...
function TJvxQuery.GetMacros: TParams;
begin
...
procedure TJvxQuery.SetMacros(Value: TParams);
begin
  FMacros.AssignValues(Value);
end;

procedure TJvxQuery.SetSQL(Value: TStrings);
begin
  inherited Disconnect;
...
procedure TJvxQuery.PatternChanged(Sender: TObject);
begin
...
procedure TJvxQuery.QueryChanged(Sender: TObject);
{$IFNDEF WIN32}
var
  List: TParams;
  SaveParams: Boolean;
...
procedure TJvxQuery.ExpandMacros;
var
...
procedure TJvxQuery.RecreateMacros;
var
  List: TParams;
{$IFNDEF WIN32}
  P: PChar;
...
procedure TJvxQuery.CreateMacros(List: TParams; const Value: PChar);
begin
  CreateQueryParams(List, Value, True, MacroChar, ['.']);
end;

procedure TJvxQuery.Expand(Query: TStrings);
...
function TJvxQuery.GetMacroCount: Word;
begin
  Result := FMacros.Count;
end;

function TJvxQuery.MacroByName(const Value: string): TParam;
begin
  Result := FMacros.ParamByName(Value);
end;

{$IFNDEF Delphi3_Up}
function TJvxQuery.IsEmpty: Boolean;
begin
  Result := IsDataSetEmpty(Self);
end;
{$ENDIF Delphi3_Up}

function TJvxQuery.GetRealSQL: TStrings;
begin
  try
    ExpandMacros;
  except
...
{ TJvxQuery.IProviderSupport }

function TJvxQuery.PSGetDefaultOrder: TIndexDef;
begin
  ExpandMacros;
  Result := inherited PSGetDefaultOrder;
end;

function TJvxQuery.PSGetTableName: string;
begin
  ExpandMacros;
  Result := inherited PSGetTableName;
end;

procedure TJvxQuery.PSExecute;
begin
  ExecSQL;
end;
...
{$IFDEF DEBUG}
procedure TJvxQuery.SetRealSQL(Value: TStrings);
begin
end;
{$ENDIF DEBUG}
...
constructor TJvxQueryThread.Create(Data: TBDEDataSet; RunMode: TRunQueryMode;
  Prepare, CreateSuspended: Boolean);
begin
...
procedure TJvxQueryThread.DoTerminate;
begin
  Synchronize(FData.EnableControls);
  inherited DoTerminate;
end;

procedure TJvxQueryThread.ModeError;
begin
  SysUtils.Abort;
end;

procedure TJvxQueryThread.DoHandleException;
begin
...
procedure TJvxQueryThread.HandleException;
begin
  FException := TObject(ExceptObject);
  Synchronize(DoHandleException);
end;

procedure TJvxQueryThread.Execute;
begin
  try
    if FPrepare and not (FMode in [rqExecDirect]) then begin
      if FData is TJvxQuery then TJvxQuery(FData).Prepare
    ...
    case FMode of
      ...
      rqExecute:
        begin
          if FData is TJvxQuery then TJvxQuery(FData).ExecSQL
      ...
      rqExecDirect:
        begin
          if FData is TJvxQuery then TJvxQuery(FData).ExecDirect
          else ModeError;
        end;
      rqOpenOrExec:
        begin
          if FData is TJvxQuery then TJvxQuery(FData).OpenOrExec(True)
          else FData.Open;
        end;
    end;
  except
...
constructor TJvxSQLScript.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  FSQL := TStringList.Create;
  TStringList(SQL).OnChange := QueryChanged;
  FParams := TParams.Create{$IFDEF Delphi4_Up}(Self){$ENDIF};
  FQuery := TJvxQuery.Create(Self);
  FSemicolonTerm := True;
  FTerm := DefaultTermChar;
end;

destructor TJvxSQLScript.Destroy;
begin
  FQuery.Free;
  FSQL.Free;
  FParams.Free;
...
function TJvxSQLScript.GetDatabase: TDatabase;
begin
  Result := FQuery.Database;
end;

function TJvxSQLScript.GetDatabaseName: string;
begin
  Result := FQuery.DatabaseName;
end;

procedure TJvxSQLScript.SetDatabaseName(const Value: string);
begin
  FQuery.DatabaseName := Value;
end;

{$IFDEF WIN32}
function TJvxSQLScript.GetSessionName: string;
begin
  Result := FQuery.SessionName;
end;

procedure TJvxSQLScript.SetSessionName(const Value: string);
begin
  FQuery.SessionName := Value;
end;

function TJvxSQLScript.GetDBSession: TSession;
begin
  Result := FQuery.DBSession;
end;
{$ENDIF WIN32}

procedure TJvxSQLScript.CheckExecQuery(LineNo, StatementNo: Integer);
var
  Done: Boolean;
...
procedure TJvxSQLScript.ExecuteScript(StatementNo: Integer);
var
  S, LastStr: string;
...
procedure TJvxSQLScript.ExecStatement(StatementNo: Integer);
begin
...
procedure TJvxSQLScript.ExecSQL;
begin
  ExecStatement(-1);
end;

procedure TJvxSQLScript.CreateParams(List: TParams; const Value: PChar);
begin
  CreateQueryParams(List, Value, False, ':', []);
end;

procedure TJvxSQLScript.SetQuery(Value: TStrings);
begin
...
function TJvxSQLScript.GetText: {$IFDEF WIN32} string {$ELSE} PChar {$ENDIF};
begin
{$IFDEF WIN32}
  Result := SQL.Text;
{$ELSE}
...
procedure TJvxSQLScript.QueryChanged(Sender: TObject);
var
  List: TParams;
{$IFNDEF WIN32}
  P: PChar;
...
function TJvxSQLScript.ParamByName(const Value: string): TParam;
begin
  Result := FParams.ParamByName(Value);
end;

procedure TJvxSQLScript.SetParamsList(Value: TParams);
begin
  FParams.AssignValues(Value);
end;

function TJvxSQLScript.GetParamsCount: Cardinal;
begin
  Result := FParams.Count;
end;

{$IFDEF Delphi4_Up}
procedure TJvxSQLScript.DefineProperties(Filer: TFiler);
begin
  inherited DefineProperties(Filer);
  Filer.DefineProperty('ParamData', ReadParamData, WriteParamData, True);
end;

procedure TJvxSQLScript.ReadParamData(Reader: TReader);
begin
  Reader.ReadValue;
  Reader.ReadCollection(FParams);
end;

procedure TJvxSQLScript.WriteParamData(Writer: TWriter);
begin
