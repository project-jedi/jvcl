...
The Original Code is: JvxPickDate.PAS, released on 2002-07-04.
...
uses Windows, Classes, Variants, Controls, SysUtils, Graphics, JvxDateUtil;
...
uses Messages, Consts, Forms, Buttons, StdCtrls, Grids, ExtCtrls, JvxCtrls,
  JvxCConst, JvxToolEdit, JvxVCLUtils, JvxMaxMin, JvxStrUtils;
...
{ TJvxTimerSpeedButton }

type
  TJvxTimerSpeedButton = class(TJvxSpeedButton)
  public
...
constructor TJvxTimerSpeedButton.Create(AOwner: TComponent);
begin
...
{ TJvxCalendar implementation copied from Borland CALENDAR.PAS sample unit
  and modified }

type
  TDayOfWeek = 0..6;

  TJvxCalendar = class(TCustomGrid)
  private
    FDate: TDateTime;
...
constructor TJvxCalendar.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
...
procedure TJvxCalendar.CreateParams(var Params: TCreateParams);
begin
...
procedure TJvxCalendar.Change;
begin
  if Assigned(FOnChange) then FOnChange(Self);
end;

procedure TJvxCalendar.Click;
var
  TheCellText: string;
begin
  inherited Click;
...
function TJvxCalendar.DaysThisMonth: Integer;
begin
  Result := DaysPerMonth(Year, Month);
end;

procedure TJvxCalendar.DrawCell(ACol, ARow: Longint; ARect: TRect; AState: TGridDrawState);
var
  TheText: string;
begin
...
function TJvxCalendar.GetCellText(ACol, ARow: Integer): string;
var
  DayNum: Integer;
begin
...
procedure TJvxCalendar.KeyDown(var Key: Word; Shift: TShiftState);
begin
  if Shift = [] then
    case Key of
      VK_LEFT, VK_SUBTRACT:
...
procedure TJvxCalendar.KeyPress(var Key: Char);
begin
...
function TJvxCalendar.SelectCell(ACol, ARow: Longint): Boolean;
begin
...
procedure TJvxCalendar.SetCalendarDate(Value: TDateTime);
begin
...
function TJvxCalendar.StoreCalendarDate: Boolean;
begin
  Result := not FUseCurrentDate;
end;

function TJvxCalendar.GetDateElement(Index: Integer): Integer;
var
...
procedure TJvxCalendar.SetDateElement(Index: Integer; Value: Integer);
var
  AYear, AMonth, ADay: Word;
begin
...
procedure TJvxCalendar.SetWeekendColor(Value: TColor);
begin
...
procedure TJvxCalendar.SetWeekends(Value: TDaysOfWeek);
begin
...
function TJvxCalendar.IsWeekend(ACol, ARow: Integer): Boolean;
begin
  Result := TDayOfWeekName((Integer(StartOfWeek) + ACol) mod 7) in FWeekends;
end;

procedure TJvxCalendar.SetStartOfWeek(Value: TDayOfWeekName);
begin
...
procedure TJvxCalendar.SetUseCurrentDate(Value: Boolean);
begin
...
procedure TJvxCalendar.ChangeMonth(Delta: Integer);
var
  AYear, AMonth, ADay: Word;
  NewDate: TDateTime;
  CurDay: Integer;
...
procedure TJvxCalendar.PrevMonth;
begin
  ChangeMonth(-1);
end;

procedure TJvxCalendar.NextMonth;
begin
  ChangeMonth(1);
end;

procedure TJvxCalendar.NextYear;
begin
  if IsLeapYear(Year) and (Month = 2) and (Day = 29) then Day := 28;
  Year := Year + 1;
end;

procedure TJvxCalendar.PrevYear;
begin
  if IsLeapYear(Year) and (Month = 2) and (Day = 29) then Day := 28;
  Year := Year - 1;
end;

procedure TJvxCalendar.CalendarUpdate(DayOnly: Boolean);
var
  AYear, AMonth, ADay: Word;
  FirstDate: TDateTime;
begin
...
procedure TJvxCalendar.UpdateCalendar;
begin
  CalendarUpdate(False);
end;

procedure TJvxCalendar.WMSize(var Message: TWMSize);
var
...
type
  TJvxLocCalendar = class(TJvxCalendar)
  private
...
constructor TJvxLocCalendar.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
...
procedure TJvxLocCalendar.CMParentColorChanged(var Message: TMessage);
begin
  inherited;
  if ParentColor then FixedColor := Self.Color;
end;

procedure TJvxLocCalendar.CMEnabledChanged(var Message: TMessage);
begin
  if HandleAllocated and not (csDesigning in ComponentState) then
    EnableWindow(Handle, True);
end;

procedure TJvxLocCalendar.CreateParams(var Params: TCreateParams);
begin
  inherited CreateParams(Params);
  with Params do
...
procedure TJvxLocCalendar.MouseToCell(X, Y: Integer; var ACol, ARow: Longint);
var
  Coord: TGridCoord;
begin
...
procedure TJvxLocCalendar.DrawCell(ACol, ARow: Longint; ARect: TRect;
...
type
  TJvxPopupCalendar = class(TJvxPopupWindow)
  private
    FCalendar: TJvxCalendar;
    FTitleLabel: TLabel;
...
function CreatePopupCalendar(AOwner: TComponent
  {$IFDEF Delphi4_Up}; ABiDiMode: TBiDiMode = bdLeftToRight {$ENDIF}): TWinControl;
begin
  Result := TJvxPopupCalendar.Create(AOwner);
  ...
  begin { scale to screen res }
    ...
    TJvxPopupCalendar(Result).FCalendar.ParentFont := True;
    FontSetDefault(TJvxPopupCalendar(Result).Font);
{$IFDEF Delphi4_Up}
...
var
  I: Integer;
begin
  if (PopupCalendar = nil) or not (PopupCalendar is TJvxPopupCalendar) then
    Exit;
  TJvxPopupCalendar(PopupCalendar).FFourDigitYear := FourDigitYear;
  if TJvxPopupCalendar(PopupCalendar).FCalendar <> nil then begin
    with TJvxPopupCalendar(PopupCalendar).FCalendar do begin
    ...
    if (BtnHints <> nil) then
      for I := 0 to Min(BtnHints.Count - 1, 3) do begin
        if BtnHints[I] <> '' then
          TJvxPopupCalendar(PopupCalendar).FBtns[I].Hint := BtnHints[I];
      end;
  end;
end;

constructor TJvxPopupCalendar.Create(AOwner: TComponent);
const
  BtnSide = 14;
var
...
{$ENDIF}
  ...
  FCalendar := TJvxLocCalendar.Create(Self);
  with TJvxLocCalendar(FCalendar) do begin
    Parent := BackPanel;
    Align := alClient;
  ...
  FBtns[0] := TJvxTimerSpeedButton.Create(Self);
  ...
  FBtns[1] := TJvxTimerSpeedButton.Create(Self);
...
{$ENDIF}
  ...
  FBtns[2] := TJvxTimerSpeedButton.Create(Self);
  ...
  FBtns[3] := TJvxTimerSpeedButton.Create(Self);
...
procedure TJvxPopupCalendar.CalendarMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
var
  Col, Row: Longint;
begin
  if (Button = mbLeft) and (Shift = []) then begin
    TJvxLocCalendar(FCalendar).MouseToCell(X, Y, Col, Row);
    if (Row > 0) and (FCalendar.CellText[Col, Row] <> '') then
      CloseUp(True);
  end;
end;

procedure TJvxPopupCalendar.TopPanelDblClick(Sender: TObject);
begin
  FCalendar.CalendarDate := Trunc(Now);
end;

procedure TJvxPopupCalendar.KeyDown(var Key: Word; Shift: TShiftState);
begin
  inherited KeyDown(Key, Shift);
  if FCalendar <> nil then
    case Key of
      ...
      else TJvxLocCalendar(FCalendar).KeyDown(Key, Shift);
    end;
end;

procedure TJvxPopupCalendar.KeyPress(var Key: Char);
begin
  inherited KeyPress(Key);
...
function TJvxPopupCalendar.GetValue: Variant;
begin
...
procedure TJvxPopupCalendar.SetValue(const Value: Variant);
begin
...
function TJvxPopupCalendar.GetValue: string;
begin
  if (csDesigning in ComponentState) then
...
procedure TJvxPopupCalendar.SetValue(const Value: string);
begin
  if not (csDesigning in ComponentState) then begin
...
procedure TJvxPopupCalendar.PrevYearBtnClick(Sender: TObject);
begin
  FCalendar.PrevYear;
end;

procedure TJvxPopupCalendar.NextYearBtnClick(Sender: TObject);
begin
  FCalendar.NextYear;
end;

procedure TJvxPopupCalendar.PrevMonthBtnClick(Sender: TObject);
begin
  FCalendar.PrevMonth;
end;

procedure TJvxPopupCalendar.NextMonthBtnClick(Sender: TObject);
begin
  FCalendar.NextMonth;
end;

procedure TJvxPopupCalendar.CalendarChange(Sender: TObject);
begin
...
type
  TJvxSelectDateDlg = class(TForm)
    Calendar: TJvxCalendar;
    TitleLabel: TLabel;
    procedure PrevMonthBtnClick(Sender: TObject);
    procedure NextMonthBtnClick(Sender: TObject);
    procedure PrevYearBtnClick(Sender: TObject);
...
constructor TJvxSelectDateDlg.Create(AOwner: TComponent);
var
  Control: TWinControl;
begin
{$IFDEF CBUILDER}
...
{$ENDIF}
  ...
  Calendar := TJvxCalendar.Create(Self);
...
procedure TJvxSelectDateDlg.SetDate(Date: TDateTime);
begin
...
function TJvxSelectDateDlg.GetDate: TDateTime;
begin
  Result := Calendar.CalendarDate;
end;

procedure TJvxSelectDateDlg.TopPanelDblClick(Sender: TObject);
begin
  SetDate(Trunc(Now));
end;

procedure TJvxSelectDateDlg.PrevYearBtnClick(Sender: TObject);
begin
  Calendar.PrevYear;
end;

procedure TJvxSelectDateDlg.NextYearBtnClick(Sender: TObject);
begin
  Calendar.NextYear;
end;

procedure TJvxSelectDateDlg.PrevMonthBtnClick(Sender: TObject);
begin
  Calendar.PrevMonth;
end;

procedure TJvxSelectDateDlg.NextMonthBtnClick(Sender: TObject);
begin
  Calendar.NextMonth;
end;

procedure TJvxSelectDateDlg.CalendarChange(Sender: TObject);
begin
  TitleLabel.Caption := FormatDateTime('MMMM, YYYY', Calendar.CalendarDate);
end;

procedure TJvxSelectDateDlg.CalendarDblClick(Sender: TObject);
begin
  ModalResult := mrOK;
end;

procedure TJvxSelectDateDlg.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  case Key of
...
function CreateDateDialog(const DlgCaption: TCaption): TJvxSelectDateDlg;
begin
  Result := TJvxSelectDateDlg.Create(Application);
  try
