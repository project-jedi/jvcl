...
The Original Code is: JvxResExp.PAS, released on 2002-07-04.
...
uses
  ...
  IniFiles, ComCtrls, EditIntf, ExptIntf, ToolIntf, Menus, StdCtrls, JvxPlacemnt;

type
  TJvxProjectResExpert = class;
  ...
  TJvxAddInNotifier = class(TIAddInNotifier)
  private
    FProjectResources: TJvxProjectResExpert;
  public
    constructor Create(AProjectResources: TJvxProjectResExpert);
...
{$ENDIF}
  ...
  TJvxProjectNotifier = class(TIModuleNotifier)
  private
    FProjectResources: TJvxProjectResExpert;
  public
    constructor Create(AProjectResources: TJvxProjectResExpert);
  ...
  TJvxResourceEntry = class(TObject)
  private
    FHandle: Pointer;
    FName: string;
    FType: string;
  ...
  TJvxProjectResExpert = class(TIExpert)
  private
    ProjectResourcesItem: TIMenuItemIntf;
    AddInNotifier: TJvxAddInNotifier;
    ProjectNotifier: TJvxProjectNotifier;
    ...
    FProjectName: string;
    FLockCount: Integer;
    procedure FindChildren(ResFile: TIResourceFile; Entry: TJvxResourceEntry);
...
{$ENDIF}
  ...
  public
    ...
    function DeleteEntry(ResFile: TIResourceFile; Entry: TJvxResourceEntry): Boolean;
    ...
    procedure EditBinaryRes(Entry: TJvxResourceEntry; Stream: TMemoryStream);
    procedure NewBitmapRes(ResFile: TIResourceFile; ResName: PChar;
      Bitmap: TBitmap);
    procedure EditBitmapRes(Entry: TJvxResourceEntry; Bitmap: TBitmap);
    procedure NewCursorIconRes(ResFile: TIResourceFile; ResName: PChar;
      IsIcon: Boolean; Stream: TStream);
    procedure EditCursorIconRes(Entry: TJvxResourceEntry; IsIcon: Boolean;
      Stream: TStream);
  end;

  TJvxResourceEditor = class(TForm)
...
var
  JvxResourceEditor: TJvxResourceEditor = nil;

procedure RegisterResourceExpert;

implementation

uses Consts, JvxVCLUtils, JvxStrUtils, JvxMaxMin, JvxPictEdit
...
procedure RegisterResourceExpert;
begin
  RegisterLibraryExpert(TJvxProjectResExpert.Create);
end;
...
function FindNode(TreeView: TCustomTreeView; Node: TTreeNode;
  ...
  var
    ChildNode: TTreeNode;
    Entry: TJvxResourceEntry;
  begin
    Result := nil;
    if Node = nil then Exit;
    Entry := TJvxResourceEntry(Node.Data);
...
type
  ...
  PIconList = ^TJvxIconList;
  TJvxIconList = array[0..64] of TIconRec;
...
procedure EnableMenuItem(Expert: TJvxProjectResExpert;
  AEnable: Boolean);
begin
...
constructor TJvxAddInNotifier.Create(AProjectResources: TJvxProjectResExpert);
begin
  inherited Create;
  FProjectResources := AProjectResources;
end;

procedure TJvxAddInNotifier.FileNotification(NotifyCode: TFileNotification;
...
{$IFDEF Delphi3_Up}
procedure TJvxAddInNotifier.EventNotification(NotifyCode: TEventNotification;
  var Cancel: Boolean);
begin
...
constructor TJvxProjectNotifier.Create(AProjectResources: TJvxProjectResExpert);
begin
  inherited Create;
  FProjectResources := AProjectResources;
end;

procedure TJvxProjectNotifier.Notify(NotifyCode: TNotifyCode);
begin
  if FProjectResources = nil then Exit;
  case NotifyCode of
    ncModuleDeleted:
      begin
        if JvxResourceEditor <> nil then JvxResourceEditor.Close;
...
procedure TJvxProjectNotifier.ComponentRenamed(const AComponent: TComponent;
...
constructor TJvxResourceEntry.Create(AEntry: TIResourceEntry);
var
  P: PChar;
begin
  inherited Create;
...
destructor TJvxResourceEntry.Destroy;
begin
  FChildren.Free;
  inherited Destroy;
end;

function TJvxResourceEntry.GetResourceName: PChar;
begin
  if FNameId > 0 then Result := MakeIntResource(FNameId)
  else Result := PChar(FName);
end;

function TJvxResourceEntry.GetResourceType: PChar;
begin
  if FTypeId > 0 then Result := MakeIntResource(FTypeId)
  else Result := PChar(FType);
end;

function TJvxResourceEntry.GetName: string;
begin
  Result := FName;
end;

function TJvxResourceEntry.GetTypeName: string;
begin
  Result := FType;
end;

function TJvxResourceEntry.EnableEdit: Boolean;
begin
  Result := FResType in [rtpGroupCursor, rtpBitmap, rtpGroupIcon, rtpRCData,
    rtpAniCursor, rtpCustom];
end;

function TJvxResourceEntry.EnableRenameDelete: Boolean;
begin
...
function TJvxResourceEntry.GetCursorOrIcon(ResFile: TIResourceFile;
...
procedure TJvxResourceEntry.GetIconData(ResFile: TIResourceFile; Stream: TStream);
var
  Data: TIconData;
...
function TJvxResourceEntry.GetBitmap(ResFile: TIResourceFile): TBitmap;
...
procedure TJvxResourceEntry.GetData(ResFile: TIResourceFile; Stream: TStream);
var
  Entry: TIResourceEntry;
begin
  Entry := ResFile.FindEntry(GetResourceType, GetResourceName);
...
function TJvxResourceEntry.GetGraphic(ResFile: TIResourceFile): TGraphic;
begin
  Result := nil;
  case FResType of
...
function TJvxResourceEntry.Rename(ResFile: TIResourceFile;
...
{ TJvxProjectResExpert }

constructor TJvxProjectResExpert.Create;
var
...
begin
  ...
  if Assigned(ToolServices) then begin
    ...
    AddInNotifier := TJvxAddInNotifier.Create(Self);
{$IFDEF Delphi4_Up}
...
destructor TJvxProjectResExpert.Destroy;
begin
  if JvxResourceEditor <> nil then JvxResourceEditor.Free;
...
function TJvxProjectResExpert.GetName: string;
begin
  Result := sExpertName;
end;

function TJvxProjectResExpert.GetAuthor: string;
begin
  Result := '';
end;

function TJvxProjectResExpert.GetComment: string;
begin
  Result := '';
end;

function TJvxProjectResExpert.GetPage: string;
begin
  Result := '';
end;

function TJvxProjectResExpert.GetGlyph: HICON;
begin
  Result := 0;
end;

function TJvxProjectResExpert.GetMenuText: string;
begin
  Result := sMenuItemCaption;
end;

function TJvxProjectResExpert.GetState: TExpertState;
begin
  Result := [esEnabled];
end;

function TJvxProjectResExpert.GetStyle: TExpertStyle;
begin
  Result := esAddIn;
end;

function TJvxProjectResExpert.GetIDString: string;
begin
  Result := sExpertID;
end;

procedure TJvxProjectResExpert.Execute;
begin
end;

procedure TJvxProjectResExpert.BeginUpdate;
begin
  Inc(FLockCount);
end;

procedure TJvxProjectResExpert.EndUpdate;
begin
  Dec(FLockCount);
  if FLockCount = 0 then UpdateProjectResInfo;
end;

function TJvxProjectResExpert.GetResFile: TIResourceFile;
begin
...
procedure TJvxProjectResExpert.FindChildren(ResFile: TIResourceFile;
  Entry: TJvxResourceEntry);
var
  I, Idx: Integer;
      ...
      if Data <> nil then begin
        ...
        for I := 0 to Header^.Count - 1 do begin
          for Idx := 0 to FResourceList.Count - 1 do begin
            Child := TJvxResourceEntry(FResourceList.Objects[Idx]);
...
procedure TJvxProjectResExpert.LoadProjectResInfo; //!!!!!
var
  I, Cnt: Integer;
...
{$ENDIF}
        for I := 0 to Cnt - 1 do begin
          ...
          if ResEntry <> nil then begin
            try
              Entry := TJvxResourceEntry.Create(ResEntry);
            finally
              ResEntry.Free;
            end;
        ...
        for I := 0 to FResourceList.Count - 1 do begin
          Entry := TJvxResourceEntry(FResourceList.Objects[I]);
          FindChildren(ResourceFile, Entry);
        end;
      end;
    if (JvxResourceEditor <> nil) and (ResourceFile <> nil) and (Cnt >= 0) then
    begin
      with JvxResourceEditor do begin
...
procedure TJvxProjectResExpert.ClearProjectResInfo;
var
  I: Integer;
begin
  FResFileName := '';
  if JvxResourceEditor <> nil then begin
    JvxResourceEditor.ResTree.Items.Clear;
    JvxResourceEditor.StatusBar.Panels[0].Text := '';
  end;
  for I := 0 to FResourceList.Count - 1 do
    TJvxResourceEntry(FResourceList.Objects[I]).Free;
  FResourceList.Clear;
end;

procedure TJvxProjectResExpert.UpdateProjectResInfo;
var
...
begin
  ...
  if JvxResourceEditor <> nil then
    JvxResourceEditor.ResTree.Items.BeginUpdate;
  try
    ...
    try
      if JvxResourceEditor <> nil then begin
        if FSelection.ResType = '' then begin
          { save selection }
          Node := JvxResourceEditor.ResTree.Selected;
          if Node <> nil then begin
            if (Node.Data <> nil) then begin
              FSelection.ResName := TJvxResourceEntry(Node.Data).GetName;
              FSelection.ResType := TJvxResourceEntry(Node.Data).GetTypeName;
            end
            else begin
              FSelection.ResName := '';
        ...
        { save tree state }
        Node := JvxResourceEditor.ResTree.Items.GetFirstNode;
      ...
      if (JvxResourceEditor <> nil) then begin
        ...
        begin { restore selection }
          with FSelection do
            Node := FindNode(JvxResourceEditor.ResTree, nil, ResName, ResType);
          if Node <> nil then begin
    ...
    if JvxResourceEditor <> nil then
      JvxResourceEditor.ResTree.Items.EndUpdate;
  end;
end;

procedure TJvxProjectResExpert.OpenProject(const FileName: string);
begin
  CloseProject;
  ProjectModule := ToolServices.GetModuleInterface(FileName);
  if ProjectModule <> nil then begin
    ProjectNotifier := TJvxProjectNotifier.Create(Self);
...
procedure TJvxProjectResExpert.CloseProject;
begin
  if ProjectModule <> nil then begin
    ClearProjectResInfo;
...
procedure TJvxProjectResExpert.LoadDesktop(const FileName: string);
var
  Desktop: TIniFile;
begin
  ...
  try
    ...
    else if JvxResourceEditor <> nil then JvxResourceEditor.Close;
  finally
    Desktop.Free;
  end;
end;

procedure TJvxProjectResExpert.SaveDesktop(const FileName: string);
var
  Desktop: TIniFile;
  Visible: Boolean;
begin
  Desktop := TIniFile.Create(FileName);
  try
    Visible := (JvxResourceEditor <> nil) and JvxResourceEditor.Visible;
...
procedure TJvxProjectResExpert.ProjectResourcesClick(Sender: TIMenuItemIntf);
var
  Reopen: Boolean;
  ProjectName: string;
  ResourceFile: TIResourceFile;
begin
  ...
  try
    if Assigned(ResourceFile) then begin
      Reopen := JvxResourceEditor = nil;
      CreateForm(TJvxResourceEditor, JvxResourceEditor);
      JvxResourceEditor.FExpert := Self;
...
procedure TJvxProjectResExpert.MarkModified;
var
...
procedure TJvxProjectResExpert.CheckRename(ResFile: TIResourceFile;
...
function TJvxProjectResExpert.UniqueName(ResFile: TIResourceFile;
...
function TJvxProjectResExpert.DeleteEntry(ResFile: TIResourceFile;
  Entry: TJvxResourceEntry): Boolean;
var
  I: Integer;
  P: Pointer;
  Child: TJvxResourceEntry;
      ...
      try
        ...
        try
          ...
          for I := 0 to Entry.FChildren.Count - 1 do begin
            Child := TJvxResourceEntry(Entry.FChildren[I]);
            if Child <> nil then
...
procedure TJvxProjectResExpert.CreateEntry(ResFile: TIResourceFile;
...
procedure TJvxProjectResExpert.NewCursorIconRes(ResFile: TIResourceFile;
...
procedure TJvxProjectResExpert.EditCursorIconRes(Entry: TJvxResourceEntry;
  IsIcon: Boolean; Stream: TStream);
var
...
procedure TJvxProjectResExpert.NewBitmapRes(ResFile: TIResourceFile;
...
procedure TJvxProjectResExpert.EditBitmapRes(Entry: TJvxResourceEntry;
  Bitmap: TBitmap);
var
  ResFile: TIResourceFile;
begin
...
procedure TJvxProjectResExpert.NewBinaryRes(ResFile: TIResourceFile;
...
procedure TJvxProjectResExpert.EditBinaryRes(Entry: TJvxResourceEntry;
  Stream: TMemoryStream);
var
  ResFile: TIResourceFile;
begin
...
procedure TJvxResourceEditor.FormCreate(Sender: TObject);
{$IFDEF Delphi4_Up}
var
  I: Integer;
{$ENDIF}
...
procedure TJvxResourceEditor.FormDestroy(Sender: TObject);
begin
  JvxResourceEditor := nil;
end;

procedure TJvxResourceEditor.ResTreeExpanded(Sender: TObject;
  Node: TTreeNode);
begin
...
procedure TJvxResourceEditor.ResTreeCollapsed(Sender: TObject;
  Node: TTreeNode);
begin
...
procedure TJvxResourceEditor.ResTreeEditing(Sender: TObject;
...
procedure TJvxResourceEditor.ResTreeEdited(Sender: TObject; Node: TTreeNode;
  var S: string);
var
  Entry: TJvxResourceEntry;
  RF: TIResourceFile;
...
procedure TJvxResourceEditor.PopupMenuPopup(Sender: TObject);
var
  Node: TTreeNode;
  Entry: TJvxResourceEntry;
begin
...
procedure TJvxResourceEditor.RenameItemClick(Sender: TObject);
var
  Node: TTreeNode;
begin
  Node := ResTree.Selected;
...
procedure TJvxResourceEditor.EditItemClick(Sender: TObject);
var
  Node: TTreeNode;
  ResFile: TIResourceFile;
  Entry: TJvxResourceEntry;
...
procedure TJvxResourceEditor.DeleteItemClick(Sender: TObject);
var
  Node: TTreeNode;
  Entry: TJvxResourceEntry;
begin
...
procedure TJvxResourceEditor.NewBitmapItemClick(Sender: TObject);
var
  Bitmap: TBitmap;
begin
...
procedure TJvxResourceEditor.NewIconItemClick(Sender: TObject);
var
  Stream: TStream;
begin
  OpenDlg.Filter := sIconFilesFilter + '|' + sCursorFilesFilter;
...
procedure TJvxResourceEditor.NewCursorItemClick(Sender: TObject);
var
  Stream: TStream;
begin
...
procedure TJvxResourceEditor.CheckResourceType(Sender: TObject;
...
function TJvxResourceEditor.GetResourceTypeName: string;
var
  I: Integer;
  Entry: TJvxResourceEntry;
begin
...
procedure TJvxResourceEditor.NewUserDataItemClick(Sender: TObject);
var
  Mem: TMemoryStream;
  TypeName: string;
  Code: Integer;
...
procedure TJvxResourceEditor.PreviewItemClick(Sender: TObject);
begin
  { not implemented yet, item is invisible }
end;

procedure TJvxResourceEditor.SaveItemClick(Sender: TObject);
var
  Node: TTreeNode;
  ResFile: TIResourceFile;
  Entry: TJvxResourceEntry;
...
procedure TJvxResourceEditor.ResTreeKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TJvxResourceEditor.ResTreeDblClick(Sender: TObject);
begin
  EditItemClick(Sender);
end;

procedure TJvxResourceEditor.ResTreeChange(Sender: TObject;
  Node: TTreeNode);
var
  Entry: TJvxResourceEntry;
  S: string;
...
procedure TJvxResourceEditor.StatusBarDrawPanel(StatusBar: TStatusBar;
  Panel: TStatusPanel; const Rect: TRect);
var
  Offset: Integer;
begin
