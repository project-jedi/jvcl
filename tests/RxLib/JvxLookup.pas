...
The Original Code is: JvxLookup.PAS, released on 2002-07-04.
...
uses SysUtils, Windows, DBCtrls, VDBConsts, Variants, Messages, Classes, Controls, Forms, Graphics, Menus, DB, Mask,
  {$IFNDEF Delphi3_Up} DBTables, {$ENDIF} Buttons, StdCtrls, JvxDBUtils, JvxToolEdit;
...
{ TJvxLookupControl }
  ...
  TJvxLookupControl = class(TCustomControl)
  private
    FLookupSource: TDataSource;
    FDataLink: TJvxDataSourceLink;
...
{ TJvxDBLookupList }
  ...
  TJvxDBLookupList = class(TJvxLookupControl)
  private
...
{ TJvxDBLookupCombo }
  ...
  TJvxPopupDataList = class(TJvxDBLookupList)
  private
    FCombo: TJvxLookupControl;
...
{$ENDIF}
  ...
  TJvxDBLookupCombo = class(TJvxLookupControl)
  private
    FDataList: TJvxPopupDataList;
...
{ TJvxPopupDataWindow }
  ...
  TJvxPopupDataWindow = class(TJvxPopupDataList)
  private
    FEditor: TWinControl;
...
{ TJvxLookupEdit }
  ...
  TJvxLookupEdit = class(TJvxCustomComboEdit)
  private
    FChanging: Boolean;
...
uses DBConsts, Dialogs, {$IFNDEF WIN32} JvxStr16, {$ENDIF} JvxVCLUtils, JvxStrUtils,
  {$IFNDEF Delphi3_Up} JvxBdeUtils, {$ENDIF} JvxMaxMin, JvxClipIcon;

{ TJvxDataSourceLink }

procedure TJvxDataSourceLink.ActiveChanged;
begin
  if FDataControl <> nil then FDataControl.DataLinkActiveChanged;
end;

procedure TJvxDataSourceLink.LayoutChanged;
begin
  if FDataControl <> nil then FDataControl.CheckDataLinkActiveChanged;
end;

procedure TJvxDataSourceLink.RecordChanged(Field: TField);
begin
  if FDataControl <> nil then FDataControl.DataLinkRecordChanged(Field);
end;

procedure TJvxDataSourceLink.FocusControl(Field: TFieldRef);
begin
...
{$IFNDEF WIN32}
procedure GeTJvxFieldList(DataSet: TDataSet; List: TList;
...
constructor TJvxLookupControl.Create(AOwner: TComponent);
begin
  ...
  FDataLink := TJvxDataSourceLink.Create;
...
{$ELSE}
  FLocate := TJvxDBLocate.Create;
{$ENDIF}
  FIndexSwitch := True;
  FIgnoreCase := True;
end;

destructor TJvxLookupControl.Destroy;
begin
  FListFields.Free;
  FListFields := nil;
...
function TJvxLookupControl.CanModify: Boolean;
begin
  Result := FListActive and not ReadOnly and ((FDataLink.DataSource = nil) or
    (FMasterField <> nil) and FMasterField.CanModify);
end;

procedure TJvxLookupControl.Change;
begin
  if Assigned(FOnChange) then FOnChange(Self);
end;

function TJvxLookupControl.ValueIsEmpty(const S: string): Boolean;
begin
  Result := (S = FEmptyValue);
end;

function TJvxLookupControl.StoreEmpty: Boolean;
begin
  Result := (FEmptyValue <> EmptyStr);
end;

{$IFDEF WIN32}
procedure TJvxLookupControl.CheckNotFixed;
begin
  if FLookupMode then _DBError(SPropDefByLookup);
  if FDataLink.DataSourceFixed then _DBError(SDataSourceFixed);
end;

procedure TJvxLookupControl.SetLookupMode(Value: Boolean);
begin
...
function TJvxLookupControl.GetKeyValue: Variant;
begin
  if ValueIsEmpty(Value) then Result := NULL
  else Result := Value;
end;

procedure TJvxLookupControl.SetKeyValue(const Value: Variant);
begin
  Self.Value := Value;
end;
{$ENDIF}

procedure TJvxLookupControl.CheckNotCircular;
begin
  {
...
procedure TJvxLookupControl.CheckDataLinkActiveChanged;
var
  TestField: TField;
begin
...
procedure TJvxLookupControl.DataLinkActiveChanged;
begin
  FDataField := nil;
  FMasterField := nil;
  if FDataLink.Active and (FDataFieldName <> '') then begin
...
procedure TJvxLookupControl.DataLinkRecordChanged(Field: TField);
begin
...
{$IFDEF Delphi4_Up}
function TJvxLookupControl.ExecuteAction(Action: TBasicAction): Boolean;
begin
  Result := inherited ExecuteAction(Action) or ((FDataLink <> nil) and
    FDataLink.ExecuteAction(Action));
end;

function TJvxLookupControl.UpdateAction(Action: TBasicAction): Boolean;
begin
  Result := inherited UpdateAction(Action) or ((FDataLink <> nil) and
    FDataLink.UpdateAction(Action));
end;

function TJvxLookupControl.UseRightToLeftAlignment: Boolean;
begin
  Result := DBUseRightToLeftAlignment(Self, Field);
end;
{$ENDIF}

function TJvxLookupControl.GetBorderSize: Integer;
var
...
function TJvxLookupControl.GetDataSource: TDataSource;
begin
  Result := FDataLink.DataSource;
end;

function TJvxLookupControl.GetLookupField: string;
begin
{$IFDEF WIN32}
...
function TJvxLookupControl.GetLookupSource: TDataSource;
begin
{$IFDEF WIN32}
...
function TJvxLookupControl.GetReadOnly: Boolean;
begin
  Result := FDataLink.ReadOnly;
end;

function TJvxLookupControl.GetField: TField;
begin
  if Assigned(FDataLink) then Result := FDataField
  else Result := nil;
end;

function TJvxLookupControl.DefaultTextHeight: Integer;
var
  DC: HDC;
  SaveFont: HFont;
  Metrics: TTextMetric;
...
function TJvxLookupControl.GetTextHeight: Integer;
begin
  Result := Max(DefaultTextHeight, FItemHeight);
end;

procedure TJvxLookupControl.KeyValueChanged;
begin
end;

procedure TJvxLookupControl.DisplayValueChanged;
begin
end;

procedure TJvxLookupControl.ListLinkActiveChanged;
var
  DataSet: TDataSet;
{$IFDEF WIN32}
  ResultField: TField;
...
procedure TJvxLookupControl.ListLinkDataChanged;
begin
end;

function TJvxLookupControl.LocateDisplay: Boolean;
begin
  Result := False;
  try
...
function TJvxLookupControl.LocateKey: Boolean;
begin
  Result := False;
  try
...
procedure TJvxLookupControl.Notification(AComponent: TComponent;
...
function TJvxLookupControl.SearchText(var AValue: string): Boolean;
begin
  Result := False;
...
procedure TJvxLookupControl.ProcessSearchKey(Key: Char);
var
  TickCount: Longint;
  S: string;
begin
...
procedure TJvxLookupControl.ResetField;
begin
...
procedure TJvxLookupControl.ClearValue;
begin
  SetValueKey(FEmptyValue);
end;

procedure TJvxLookupControl.SelectKeyValue(const Value: string);
begin
  if FMasterField <> nil then begin
...
procedure TJvxLookupControl.SetDataFieldName(const Value: string);
begin
...
procedure TJvxLookupControl.SetDataSource(Value: TDataSource);
begin
...
procedure TJvxLookupControl.SetListStyle(Value: TLookupListStyle);
begin
...
procedure TJvxLookupControl.SetFieldsDelim(Value: Char);
begin
...
procedure TJvxLookupControl.SetLookupField(const Value: string);
begin
{$IFDEF WIN32}
  CheckNotFixed;
{$ENDIF}
...
procedure TJvxLookupControl.SetDisplayEmpty(const Value: string);
begin
...
procedure TJvxLookupControl.SetEmptyValue(const Value: string);
begin
...
procedure TJvxLookupControl.SetEmptyItemColor(Value: TColor);
begin
...
procedure TJvxLookupControl.UpdateDisplayEmpty(const Value: string);
begin
end;

procedure TJvxLookupControl.SetDisplayValue(const Value: string);
var
  S: string;
begin
...
procedure TJvxLookupControl.UpdateKeyValue;
begin
...
procedure TJvxLookupControl.SetValueKey(const Value: string);
begin
...
procedure TJvxLookupControl.SetValue(const Value: string);
begin
...
procedure TJvxLookupControl.SetLookupDisplay(const Value: string);
begin
...
procedure TJvxLookupControl.SetLookupSource(Value: TDataSource);
begin
{$IFDEF WIN32}
  CheckNotFixed;
{$ENDIF}
...
procedure TJvxLookupControl.SetReadOnly(Value: Boolean);
begin
  FDataLink.ReadOnly := Value;
end;

function TJvxLookupControl.GetItemHeight: Integer;
begin
  Result := {Max(GetTextHeight, FItemHeight);}GetTextHeight;
end;

procedure TJvxLookupControl.SetItemHeight(Value: Integer);
begin
...
function TJvxLookupControl.ItemHeightStored: Boolean;
begin
  Result := FItemHeight > DefaultTextHeight;
end;

procedure TJvxLookupControl.DrawPicture(Canvas: TCanvas; Rect: TRect;
  Image: TGraphic);
var
...
function TJvxLookupControl.GetPicture(Current, Empty: Boolean;
...
procedure TJvxLookupControl.WMGetDlgCode(var Message: TMessage);
begin
  Message.Result := DLGC_WANTARROWS or DLGC_WANTCHARS;
end;

procedure TJvxLookupControl.WMKillFocus(var Message: TMessage);
begin
  FFocused := False;
  Invalidate;
end;

procedure TJvxLookupControl.WMSetFocus(var Message: TMessage);
begin
  FFocused := True;
  Invalidate;
end;

function TJvxLookupControl.Locate(const SearchField: TField;
...
function TJvxLookupControl.EmptyRowVisible: Boolean;
begin
  Result := DisplayEmpty <> EmptyStr;
end;

procedure TJvxLookupControl.UpdateDisplayValue;
begin
...
function TJvxLookupControl.GetWindowWidth: Integer;
var
  I: Integer;
begin
  Result := 0;
...
constructor TJvxDBLookupList.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  Width := 121;
  Ctl3D := True;
...
procedure TJvxDBLookupList.CreateParams(var Params: TCreateParams);
begin
...
procedure TJvxDBLookupList.CreateWnd;
begin
  inherited CreateWnd;
  UpdateScrollBar;
end;

procedure TJvxDBLookupList.Loaded;
begin
  inherited Loaded;
  Height := Height;
end;

function TJvxDBLookupList.GetKeyIndex: Integer;
var
  FieldValue: string;
begin
...
procedure TJvxDBLookupList.KeyDown(var Key: Word; Shift: TShiftState);
var
...
procedure TJvxDBLookupList.KeyPress(var Key: Char);
begin
  inherited KeyPress(Key);
  ProcessSearchKey(Key);
end;

procedure TJvxDBLookupList.KeyValueChanged;
begin
  if FListActive and not FLockPosition then
    if not LocateKey then FLookupLink.DataSet.First;
end;

procedure TJvxDBLookupList.DisplayValueChanged;
begin
  if FListActive and not FLockPosition then
    if not LocateDisplay then FLookupLink.DataSet.First;
end;

procedure TJvxDBLookupList.ListLinkActiveChanged;
begin
  try
...
procedure TJvxDBLookupList.ListDataChanged;
begin
...
procedure TJvxDBLookupList.ListLinkDataChanged;
begin
  ListDataChanged;
end;

procedure TJvxDBLookupList.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
...
procedure TJvxDBLookupList.MouseMove(Shift: TShiftState; X, Y: Integer);
begin
...
procedure TJvxDBLookupList.MouseUp(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
  if FTracking then begin
    StopTracking;
...
procedure TJvxDBLookupList.DrawItemText(Canvas: TCanvas; Rect: TRect;
...
procedure TJvxDBLookupList.Paint;
var
...
procedure TJvxDBLookupList.SelectCurrent;
begin
  FLockPosition := True;
  try
...
procedure TJvxDBLookupList.SelectItemAt(X, Y: Integer);
var
  Delta: Integer;
begin
  if Y < 0 then Y := 0;
...
procedure TJvxDBLookupList.SetBorderStyle(Value: TBorderStyle);
begin
...
procedure TJvxDBLookupList.UpdateDisplayEmpty(const Value: string);
begin
  UpdateBufferCount(RowCount - Ord(Value <> EmptyStr));
end;

procedure TJvxDBLookupList.UpdateBufferCount(Rows: Integer);
begin
...
procedure TJvxDBLookupList.SetBounds(ALeft, ATop, AWidth, AHeight: Integer);
var
...
procedure TJvxDBLookupList.SetRowCount(Value: Integer);
begin
...
procedure TJvxDBLookupList.StopTimer;
begin
...
procedure TJvxDBLookupList.StopTracking;
begin
  if FTracking then begin
    StopTimer;
    FTracking := False;
...
procedure TJvxDBLookupList.TimerScroll;
var
...
procedure TJvxDBLookupList.UpdateScrollBar;
(*
{$IFDEF Delphi3_Up}
var
...
procedure TJvxDBLookupList.CMCtl3DChanged(var Message: TMessage);
begin
{$IFDEF WIN32}
...
procedure TJvxDBLookupList.CMFontChanged(var Message: TMessage);
begin
  inherited;
  if not (csReading in ComponentState) then Height := Height;
end;

procedure TJvxDBLookupList.WMCancelMode(var Message: TMessage);
begin
  StopTracking;
  inherited;
end;

procedure TJvxDBLookupList.WMTimer(var Message: TMessage);
begin
  TimerScroll;
end;

procedure TJvxDBLookupList.WMNCHitTest(var Msg: TWMNCHitTest);
begin
...
{$IFDEF Delphi4_Up}
function TJvxDBLookupList.DoMouseWheelDown(Shift: TShiftState;
...
function TJvxDBLookupList.DoMouseWheelUp(Shift: TShiftState;
  MousePos: TPoint): Boolean;
begin
...
procedure TJvxDBLookupList.WMVScroll(var Message: TWMVScroll);
begin
  FSearchText := '';
...
constructor TJvxPopupDataList.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  if AOwner is TJvxLookupControl then FCombo := TJvxLookupControl(AOwner);
{$IFDEF WIN32}
...
procedure TJvxPopupDataList.CreateParams(var Params: TCreateParams);
begin
  inherited CreateParams(Params);
  with Params do begin
...
{$IFNDEF WIN32}
procedure TJvxPopupDataList.CreateWnd;
begin
  inherited CreateWnd;
...
procedure TJvxPopupDataList.WMMouseActivate(var Message: TMessage);
begin
  Message.Result := MA_NOACTIVATE;
end;

procedure TJvxPopupDataList.Click;
begin
  inherited Click;
  if Assigned(FCombo) and TJvxDBLookupCombo(FCombo).FListVisible then
    TJvxDBLookupCombo(FCombo).InvalidateText;
end;

procedure TJvxPopupDataList.KeyPress(var Key: Char);
begin
  inherited KeyPress(Key);
  if Assigned(FCombo) and TJvxDBLookupCombo(FCombo).FListVisible then
    TJvxDBLookupCombo(FCombo).InvalidateText;
...
constructor TJvxDBLookupCombo.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
{$IFDEF WIN32}
...
{$ENDIF}
  Width := 145;
  Height := 0;
  FDataList := TJvxPopupDataList.Create(Self);
...
destructor TJvxDBLookupCombo.Destroy;
begin
{$IFNDEF WIN32}
  FBtnDisabled.Free;
  FBtnGlyph.Free;
...
procedure TJvxDBLookupCombo.CreateParams(var Params: TCreateParams);
begin
  inherited CreateParams(Params);
  with Params do
{$IFDEF WIN32}
...
procedure TJvxDBLookupCombo.CloseUp(Accept: Boolean);
var
  ListValue: string;
begin
...
procedure TJvxDBLookupCombo.DropDown;
var
  P: TPoint;
  I, Y: Integer;
  S: string;
...
function TJvxDBLookupCombo.GetMinHeight: Integer;
begin
  Result := DefaultTextHeight + GetBorderSize + 3;
end;

procedure TJvxDBLookupCombo.UpdateFieldText;
var
  I: Integer;
  S: string;
begin
...
function TJvxDBLookupCombo.GetDisplayValues(Index: Integer): string; 
begin
...
function TJvxDBLookupCombo.GetText: string;
begin
  Result := inherited Text;
end;

procedure TJvxDBLookupCombo.InvalidateText;
var
  R: TRect;
begin
...
procedure TJvxDBLookupCombo.KeyDown(var Key: Word; Shift: TShiftState);
var
  Delta: Integer;
begin
...
procedure TJvxDBLookupCombo.KeyPress(var Key: Char);
begin
  if FListVisible then begin
...
procedure TJvxDBLookupCombo.DisplayValueChanged;
begin
...
procedure TJvxDBLookupCombo.KeyValueChanged;
begin
{$IFDEF WIN32}
  if FLookupMode then begin
    if FDisplayValues <> nil then FDisplayValues.Clear;
...
procedure TJvxDBLookupCombo.SetFieldsDelim(Value: Char);
begin
  if (FFieldsDelim <> Value) then begin
...
procedure TJvxDBLookupCombo.SetListStyle(Value: TLookupListStyle);
begin
  if (FListStyle <> Value) then begin
    FListStyle := Value;
...
function TJvxDBLookupCombo.GetDisplayAll: Boolean;
begin
{$IFDEF WIN32}
...
procedure TJvxDBLookupCombo.SetDisplayAll(Value: Boolean);
begin
...
procedure TJvxDBLookupCombo.ListLinkDataChanged;
begin
  if FDataLink.Active and FDataLink.DataSet.IsLinkedTo(LookupSource) then
    if FListActive then DataLinkRecordChanged(nil);
end;

procedure TJvxDBLookupCombo.ListLinkActiveChanged;
begin
  inherited ListLinkActiveChanged;
...
procedure TJvxDBLookupCombo.ListMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
procedure TJvxDBLookupCombo.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
  if Button = mbLeft then begin
    if CanFocus then SetFocus;
...
procedure TJvxDBLookupCombo.MouseMove(Shift: TShiftState; X, Y: Integer);
var
  ListPos: TPoint;
...
procedure TJvxDBLookupCombo.MouseUp(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
  StopTracking;
  inherited MouseUp(Button, Shift, X, Y);
...
procedure TJvxDBLookupCombo.UpdateCurrentImage;
begin
...
function TJvxDBLookupCombo.GetPicture(Current, Empty: Boolean;
  var TextMargin: Integer): TGraphic;
begin
...
procedure TJvxDBLookupCombo.PaintDisplayValues(Canvas: TCanvas; R: TRect;
  ALeft: Integer);
var
...
procedure TJvxDBLookupCombo.Paint;
const
...
procedure TJvxDBLookupCombo.ResetField;
begin
  if FListVisible then CloseUp(False);
...
procedure TJvxDBLookupCombo.StopTracking;
begin
  if FTracking then begin
    TrackButton(-1, -1);
    FTracking := False;
...
procedure TJvxDBLookupCombo.TrackButton(X, Y: Integer);
var
  NewState: Boolean;
begin
...
procedure TJvxDBLookupCombo.UpdateDisplayEmpty(const Value: string);
begin
  if Text = FDisplayEmpty then inherited Text := Value;
end;

procedure TJvxDBLookupCombo.Click;
begin
  inherited Click;
  Change;
end;

procedure TJvxDBLookupCombo.CMCancelMode(var Message: TCMCancelMode);
begin
...
{$IFDEF WIN32}
procedure TJvxDBLookupCombo.CMCtl3DChanged(var Message: TMessage);
begin
  if NewStyleControls then begin
    RecreateWnd;
...
procedure TJvxDBLookupCombo.CNKeyDown(var Message: TWMKeyDown);
begin
...
procedure TJvxDBLookupCombo.CMFontChanged(var Message: TMessage);
begin
  inherited;
  if not (csReading in ComponentState) then
...
procedure TJvxDBLookupCombo.CMEnabledChanged(var Message: TMessage);
begin
  inherited;
  Invalidate;
end;
...
{$IFDEF WIN32}
procedure TJvxDBLookupCombo.CMGetDataLink(var Message: TMessage);
begin
  Message.Result := Integer(FDataLink);
end;
{$ENDIF}

procedure TJvxDBLookupCombo.WMCancelMode(var Message: TMessage);
begin
  StopTracking;
  inherited;
end;

procedure TJvxDBLookupCombo.WMGetDlgCode(var Message: TMessage);
begin
  inherited;
...
procedure TJvxDBLookupCombo.WMKillFocus(var Message: TWMKillFocus);
begin
  inherited;
  CloseUp(False);
end;

procedure TJvxDBLookupCombo.WMSetCursor(var Message: TWMSetCursor);
var
  P: TPoint;
begin
  GetCursorPos(P);
...
procedure TJvxDBLookupCombo.WMSize(var Message: TWMSize);
begin
  inherited;
  if not (csReading in ComponentState) and (Height < GetMinHeight) then
    Height := GetMinHeight
...
{$IFDEF Delphi4_Up}
procedure TJvxDBLookupCombo.CMBiDiModeChanged(var Message: TMessage);
begin
  inherited;
...
constructor TJvxPopupDataWindow.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
...
procedure TJvxPopupDataWindow.InvalidateEditor;
var
  R: TRect;
begin
  if (FEditor is TJvxCustomComboEdit) then begin
    with TJvxComboEdit(FEditor) do
...
procedure TJvxPopupDataWindow.Click;
begin
  inherited Click;
  if Value <> '' then
    with TJvxLookupEdit(FEditor) do begin
...
procedure TJvxPopupDataWindow.DisplayValueChanged;
begin
...
procedure TJvxPopupDataWindow.KeyPress(var Key: Char);
begin
  inherited KeyPress(Key);
  InvalidateEditor;
end;

procedure TJvxPopupDataWindow.PopupMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  if Button = mbLeft then CloseUp(PtInRect(Self.ClientRect, Point(X, Y)));
end;

procedure TJvxPopupDataWindow.CloseUp(Accept: Boolean);
begin
  if Assigned(FCloseUp) then FCloseUp(Self, Accept);
end;

function TJvxPopupDataWindow.GetPicture(Current, Empty: Boolean;
  var TextMargin: Integer): TGraphic;
begin
  TextMargin := 0;
  Result := nil;
...
procedure TJvxPopupDataWindow.Hide;
begin
...
procedure TJvxPopupDataWindow.Show(Origin: TPoint);
begin
...
constructor TJvxLookupEdit.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  FDropDownCount := 8;
  ...
  try
    FPopup := TJvxPopupDataWindow.Create(Self);
    TJvxPopupDataWindow(FPopup).OnCloseUp := PopupCloseUp;
...
destructor TJvxLookupEdit.Destroy;
begin
  if FPopup <> nil then
    with TJvxPopupDataWindow(FPopup) do begin
      OnCloseUp := nil;
      OnGetImage := nil;
    end;
  FPopup.Free;
...
procedure TJvxLookupEdit.SetDropDownCount(Value: Integer);
begin
  if Value < 1 then Value := 1;
  if Value > 50 then Value := 50;
...
function TJvxLookupEdit.GetListStyle: TLookupListStyle;
begin
  Result := TJvxPopupDataWindow(FPopup).ListStyle;
end;

procedure TJvxLookupEdit.SetListStyle(Value: TLookupListStyle);
begin
  TJvxPopupDataWindow(FPopup).ListStyle := Value;
end;

function TJvxLookupEdit.GetFieldsDelim: Char;
begin
  Result := TJvxPopupDataWindow(FPopup).FieldsDelimiter;
end;

procedure TJvxLookupEdit.SetFieldsDelim(Value: Char);
begin
  TJvxPopupDataWindow(FPopup).FieldsDelimiter := Value;
end;

function TJvxLookupEdit.GetLookupDisplay: string;
begin
  Result := TJvxPopupDataWindow(FPopup).LookupDisplay;
end;

procedure TJvxLookupEdit.SetLookupDisplay(const Value: string);
begin
  TJvxPopupDataWindow(FPopup).LookupDisplay := Value;
end;

function TJvxLookupEdit.GetDisplayIndex: Integer;
begin
  Result := TJvxPopupDataWindow(FPopup).LookupDisplayIndex;
end;

procedure TJvxLookupEdit.SetDisplayIndex(Value: Integer);
begin
  TJvxPopupDataWindow(FPopup).LookupDisplayIndex := Value;
end;

function TJvxLookupEdit.GetLookupField: string;
begin
  Result := TJvxPopupDataWindow(FPopup).LookupField;
end;

procedure TJvxLookupEdit.SetLookupField(const Value: string);
begin
  TJvxPopupDataWindow(FPopup).LookupField := Value;
end;

function TJvxLookupEdit.GetLookupSource: TDataSource;
begin
  Result := TJvxPopupDataWindow(FPopup).LookupSource;
end;

procedure TJvxLookupEdit.SetLookupSource(Value: TDataSource);
begin
  TJvxPopupDataWindow(FPopup).LookupSource := Value;
end;

function TJvxLookupEdit.GetOnGetImage: TGetImageEvent;
begin
  Result := TJvxPopupDataWindow(FPopup).OnGetImage;
end;

procedure TJvxLookupEdit.SetOnGetImage(Value: TGetImageEvent);
begin
  TJvxPopupDataWindow(FPopup).OnGetImage := Value;
end;

function TJvxLookupEdit.GetLookupValue: string;
begin
  TJvxPopupDataWindow(FPopup).DisplayValue := Text;
  Result := TJvxPopupDataWindow(FPopup).Value;
end;

procedure TJvxLookupEdit.SetLookupValue(const Value: string);
begin
  TJvxPopupDataWindow(FPopup).Value := Value;
  Text := TJvxPopupDataWindow(FPopup).DisplayValue;
end;

procedure TJvxLookupEdit.ShowPopup(Origin: TPoint);
begin
  TJvxPopupDataWindow(FPopup).Show(Origin);
end;

procedure TJvxLookupEdit.HidePopup;
begin
  TJvxPopupDataWindow(FPopup).Hide;
end;

procedure TJvxLookupEdit.PopupDropDown(DisableEdit: Boolean);
begin
...
procedure TJvxLookupEdit.KeyDown(var Key: Word; Shift: TShiftState);
begin
  if (Key in [VK_PRIOR, VK_NEXT, VK_UP, VK_DOWN]) and
    PopupVisible then
  begin
    TJvxPopupDataWindow(FPopup).KeyDown(Key, Shift);
    Key := 0;
  end;
  inherited KeyDown(Key, Shift);
...
procedure TJvxLookupEdit.KeyPress(var Key: Char);
begin
  inherited KeyPress(Key);
  FIgnoreChange := (SelLength > 0) or (Key = Char(VK_BACK));
end;

procedure TJvxLookupEdit.Change;
begin
...
procedure TJvxLookupEdit.PopupChange;
var
  S: string;
  Len: Integer;
begin
  ...
  try
    S := Text;
    if TJvxPopupDataWindow(FPopup).SearchText(S) then begin
      Len := Length(Text);
      Text := TJvxPopupDataWindow(FPopup).DisplayValue;
      SelStart := Len;
      SelLength := Length(Text) - Len;
    end
    else with TJvxPopupDataWindow(FPopup) do Value := EmptyValue;
  finally
    FChanging := False;
  end;
end;
...
{$IFDEF WIN32}
procedure TJvxLookupEdit.SetPopupValue(const Value: Variant);
{$ELSE}
procedure TJvxLookupEdit.SetPopupValue(const Value: string);
{$ENDIF}
begin
{$IFDEF WIN32}
  if VarIsNull(Value) or VarIsEmpty(Value) then
    TJvxPopupDataWindow(FPopup).Value := TJvxPopupDataWindow(FPopup).EmptyValue
  else
{$ENDIF}
    TJvxPopupDataWindow(FPopup).DisplayValue := Value;
end;

{$IFDEF WIN32}
function TJvxLookupEdit.GetPopupValue: Variant;
{$ELSE}
function TJvxLookupEdit.GetPopupValue: string;
{$ENDIF}
begin
  with TJvxPopupDataWindow(FPopup) do
    if Value <> EmptyValue then Result := DisplayValue
    else Result := Self.Text;
end;

{$IFDEF WIN32}
function TJvxLookupEdit.AcceptPopup(var Value: Variant): Boolean;
{$ELSE}
function TJvxLookupEdit.AcceptPopup(var Value: string): Boolean;
{$ENDIF}
begin
  Result := True;
  if Assigned(FOnCloseUp) then FOnCloseUp(Self);
