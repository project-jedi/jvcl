...
The Original Code is: JvxSpin.PAS, released on 2002-07-04.
...
{ TJvxSpinButton }
  ...
  TJvxSpinButton = class(TGraphicControl)
  private
...
uses {$IFDEF WIN32} CommCtrl, {$ENDIF} JvxVCLUtils;
...
const
  sSpinUpBtn = 'JvxSPINUP';
  sSpinDownBtn = 'JvxSPINDOWN';
...
constructor TJvxSpinButton.Create(AOwner: TComponent);
begin
...
destructor TJvxSpinButton.Destroy;
begin
  FTopDownBtn.Free;
  FBottomDownBtn.Free;
  FNotDownBtn.Free;
...
procedure TJvxSpinButton.GlyphChanged(Sender: TObject);
begin
  FInvalidate := True;
  Invalidate;
end;

function TJvxSpinButton.GetUpGlyph: TBitmap;
begin
  Result := FUpBitmap;
end;

procedure TJvxSpinButton.SetUpGlyph(Value: TBitmap);
begin
  if Value <> nil then FUpBitmap.Assign(Value)
  else FUpBitmap.Handle := LoadBitmap(HInstance, sSpinUpBtn);
end;

function TJvxSpinButton.GetDownGlyph: TBitmap;
begin
  Result := FDownBitmap;
end;

procedure TJvxSpinButton.SetDownGlyph(Value: TBitmap);
begin
  if Value <> nil then FDownBitmap.Assign(Value)
  else FDownBitmap.Handle := LoadBitmap(HInstance, sSpinDownBtn);
end;

procedure TJvxSpinButton.SetDown(Value: TSpinButtonState);
var
...
procedure TJvxSpinButton.SetFocusControl(Value: TWinControl);
begin
  FFocusControl := Value;
{$IFDEF WIN32}
...
procedure TJvxSpinButton.Notification(AComponent: TComponent;
...
procedure TJvxSpinButton.Paint;
begin
...
procedure TJvxSpinButton.DrawAllBitmap;
begin
...
procedure TJvxSpinButton.DrawBitmap(ABitmap: TBitmap; ADownState: TSpinButtonState);
var
  R, RSrc: TRect;
  dRect: Integer;
  {Temp: TBitmap;}
...
procedure TJvxSpinButton.CMEnabledChanged(var Message: TMessage);
begin
  inherited;
  FInvalidate := True;
  Invalidate;
...
procedure TJvxSpinButton.TopClick;
begin
...
procedure TJvxSpinButton.BottomClick;
begin
...
procedure TJvxSpinButton.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
...
procedure TJvxSpinButton.MouseMove(Shift: TShiftState; X, Y: Integer);
var
...
procedure TJvxSpinButton.MouseUp(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
...
procedure TJvxSpinButton.TimerExpired(Sender: TObject);
begin
...
type
  TJvxUpDown = class(TCustomUpDown)
  private
    FChanging: Boolean;
...
constructor TJvxUpDown.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
...
destructor TJvxUpDown.Destroy;
begin
  OnClick := nil;
  inherited Destroy;
end;

procedure TJvxUpDown.ScrollMessage(var Message: TWMVScroll);
begin
...
procedure TJvxUpDown.WMHScroll(var Message: TWMHScroll);
begin
  ScrollMessage(TWMVScroll(Message));
end;

procedure TJvxUpDown.WMVScroll(var Message: TWMVScroll);
begin
  ScrollMessage(Message);
end;

procedure TJvxUpDown.WMSize(var Message: TWMSize);
begin
  inherited;
...
{$IFDEF WIN32}
  ...
  if GetButtonKind = bkStandard then begin
    FUpDown := TJvxUpDown.Create(Self);
    with TJvxUpDown(FUpDown) do begin
      Visible := True;
...
{$ENDIF}
    ...
    FButton := TJvxSpinButton.Create(Self);
