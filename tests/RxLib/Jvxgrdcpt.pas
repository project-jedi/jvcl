...
The Original Code is: JvxGrdCpt.PAS, released on 2002-07-04.
...
uses Windows, Messages, Classes, Graphics, Controls, Forms, Dialogs, Menus,
  JvxHook, JvxVCLUtils{, JvxComponent};

type
  THideDirection = (hdLeftToRight, hdRightToLeft);

  TJvxCaption = class;
  TJvxCaptionList = class;
...
{ TJvxGradientCaption }
  ...
  private
    ...
    procedure SetCaptions(Value: TJvxCaptionList);
...
{$ENDIF}
  ...
  published
    ...
    property Captions: TJvxCaptionList read FCaptions write SetCaptions;
...
{ TJvxCaptionList }
  ...
  TJvxCaptionList = class(TCollection)
  private
    FParent: TJvxGradientCaption;
    function GetCaption(Index: Integer): TJvxCaption;
    procedure SetCaption(Index: Integer; Value: TJvxCaption);
  protected
{$IFDEF Delphi3_Up}
...
{$ENDIF}
  ...
  public
    ...
    function Add: TJvxCaption;
    procedure RestoreDefaults;
    property Parent: TJvxGradientCaption read FParent;
    property Items[Index: Integer]: TJvxCaption read GetCaption write SetCaption; default;
  end;

{ TJvxCaption }

  TJvxCaption = class(TCollectionItem)
  private
    FCaption: string;
    FFont: TFont;
    FParentFont: Boolean;
  ...
  protected
    function GetParentCaption: TJvxGradientCaption;
    property TextWidth: Integer read GetTextWidth;
  public
    ...
    property GradientCaption: TJvxGradientCaption read GetParentCaption;
  published
...
constructor TJvxCaptionList.Create(AParent: TJvxGradientCaption);
begin
  inherited Create(TJvxCaption);
  FParent := AParent;
end;

function TJvxCaptionList.Add: TJvxCaption;
begin
  Result := TJvxCaption(inherited Add);
end;

function TJvxCaptionList.GetCaption(Index: Integer): TJvxCaption;
begin
  Result := TJvxCaption(inherited Items[Index]);
end;

{$IFDEF Delphi3_Up}
function TJvxCaptionList.GetOwner: TPersistent;
begin
  Result := FParent;
end;
{$ENDIF}

procedure TJvxCaptionList.RestoreDefaults;
var
  I: Integer;
begin
  BeginUpdate;
...
procedure TJvxCaptionList.SetCaption(Index: Integer; Value: TJvxCaption);
begin
  Items[Index].Assign(Value);
end;

procedure TJvxCaptionList.Update(Item: TCollectionItem);
begin
...
constructor TJvxCaption.Create(Collection: TCollection);
var
  Parent: TJvxGradientCaption;
begin
  Parent := nil;
  if Assigned(Collection) and (Collection is TJvxCaptionList) then
    Parent := TJvxCaptionList(Collection).Parent;
  try
...
destructor TJvxCaption.Destroy;
begin
  FFont.Free;
  FFont := nil;
  inherited Destroy;
...
procedure TJvxCaption.Assign(Source: TPersistent);
begin
  if Source is TJvxCaption then begin
    if Assigned(Collection) then Collection.BeginUpdate;
    try
      RestoreDefaults;
      Caption := TJvxCaption(Source).Caption;
      ParentFont := TJvxCaption(Source).ParentFont;
      if not ParentFont then
        Font.Assign(TJvxCaption(Source).Font);
      InactiveColor := TJvxCaption(Source).InactiveColor;
      GlueNext := TJvxCaption(Source).GlueNext;
      Visible := TJvxCaption(Source).Visible;
    finally
...
procedure TJvxCaption.RestoreDefaults;
begin
  FInactiveColor := clInactiveCaptionText;
  FVisible := True;
  ParentFont := True;
...
function TJvxCaption.GetParentCaption: TJvxGradientCaption;
begin
  if Assigned(Collection) and (Collection is TJvxCaptionList) then
    Result := TJvxCaptionList(Collection).Parent
  else
    Result := nil;
end;

procedure TJvxCaption.SetCaption(const Value: string);
begin
  FCaption := Value;
  Changed(False);
end;

procedure TJvxCaption.FontChanged(Sender: TObject);
begin
  FParentFont := False;
  Changed(False);
end;

procedure TJvxCaption.SetFont(Value: TFont);
begin
  FFont.Assign(Value);
end;

procedure TJvxCaption.SetParentFont(Value: Boolean);
begin
...
function TJvxCaption.IsFontStored: Boolean;
begin
  Result := not FParentFont;
end;

function TJvxCaption.GetTextWidth: Integer;
var
  Canvas: TCanvas;
  PS: TPaintStruct;
begin
...
procedure TJvxCaption.SetVisible(Value: Boolean);
begin
...
procedure TJvxCaption.SetInactiveColor(Value: TColor);
begin
...
procedure TJvxCaption.SetGlueNext(Value: Boolean);
begin
...
begin
  ...
  FActive := True;
  FCaptions := TJvxCaptionList.Create(Self);
...
destructor TJvxGradientCaption.Destroy;
begin
  FOnDeactivate := nil;
  FOnActivate := nil;
...
procedure TJvxGradientCaption.Loaded;
var
  Loading: Boolean;
begin
...
procedure TJvxGradientCaption.Notification(AComponent: TComponent;
...
procedure TJvxGradientCaption.SetCaptions(Value: TJvxCaptionList);
begin
...
procedure TJvxGradientCaption.SetFontDefault;
var
...
function TJvxGradientCaption.IsFontStored: Boolean;
begin
  Result := not DefaultFont;
end;

function TJvxGradientCaption.GetForm: TForm;
begin
...
function TJvxGradientCaption.GetFormCaption: string;
begin
...
procedure TJvxGradientCaption.SetHook;
begin
...
procedure TJvxGradientCaption.AfterMessage(Sender: TObject; var Msg: TMessage;
  var Handled: Boolean);
var
  DC: HDC;
  S: string;
...
function TJvxGradientCaption.GetActive: Boolean;
begin
  Result := FActive;
  if not (csDesigning in ComponentState) then
...
procedure TJvxGradientCaption.CalculateGradientParams(var R: TRect;
...
procedure TJvxGradientCaption.DrawGradientCaption(DC: HDC);
var
  R, DrawRect: TRect;
  Icons: TBorderIcons;
  C: TColor;
...
procedure TJvxGradientCaption.SetFont(Value: TFont);
begin
  FFont.Assign(Value);
end;

procedure TJvxGradientCaption.FontChanged(Sender: TObject);
var
  I: Integer;
begin
  FDefaultFont := False;
