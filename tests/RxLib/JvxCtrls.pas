...
uses Windows, Registry, RTLConsts,  Messages, Classes, Controls, Graphics, StdCtrls, ExtCtrls, Forms,
  Buttons, Menus, JvxTimer, JvxConst, IniFiles, JvxPlacemnt;
...
{ TJvxTextListBox }
  ...
  TJvxTextListBox = class(TCustomListBox)
  private
    FMaxWidth: Integer;
{$IFNDEF WIN32}
    FTabWidth: Integer;
...
{$ENDIF}
  ...
  TJvxCustomLabel = class(TGraphicControl)
  private
...
{ TJvxLabel }
  ...
  TJvxLabel = class(TJvxCustomLabel)
  published
    property Align;
    property Alignment;
    property AutoSize;
...
{ TJvxSecretPanel }
  ...
  TJvxSecretPanel = class(TCustomPanel)
  private
    FActive: Boolean;
    FAlignment: TAlignment;
    FLines: TStrings;
...
{ TJvxSpeedButton }
  ...
  TJvxNumGlyphs = 1..5;
  TJvxDropDownMenuPos = (dmpBottom, dmpRight);
  TJvxButtonState = (rbsUp, rbsDisabled, rbsDown, rbsExclusive, rbsInactive);
  ...
  private
    ...
    FMenuPosition: TJvxDropDownMenuPos;
...
{$ENDIF}
    ...
    property MenuPosition: TJvxDropDownMenuPos read FMenuPosition write FMenuPosition
      default dmpBottom;
    property Caption;
    property DragCursor;
    property DragMode;
...
{ TJvxButtonGlyph }
  ...
  private
    ...
    FIndexs: array[TJvxButtonState] of Integer;
    FTransparentColor: TColor;
    FNumGlyphs: TJvxNumGlyphs;
    FWordWrap: Boolean;
    FAlignment: TAlignment;
  ...
  protected
    ...
    function CreateButtonGlyph(State: TJvxButtonState): Integer;
{$IFDEF WIN32}
    function CreateImageGlyph(State: TJvxButtonState; Images: TImageList;
...
{$ENDIF}
  ...
  public
    ...
    function DrawButtonGlyph(Canvas: TCanvas; X, Y: Integer;
      State: TJvxButtonState): TPoint;
{$IFDEF WIN32}
    function DrawButtonImage(Canvas: TCanvas; X, Y: Integer; Images: TImageList;
      ImageIndex: Integer; State: TJvxButtonState): TPoint;
    function DrawEx(Canvas: TCanvas; const Client: TRect; const Caption: string;
      ...
      Images: TImageList; ImageIndex: Integer; State: TJvxButtonState;
      Flags: Word): TRect;
{$ENDIF}
    procedure DrawButtonText(Canvas: TCanvas; const Caption: string;
      TextBounds: TRect; State: TJvxButtonState; Flags: Word);
    procedure DrawPopupMark(Canvas: TCanvas; X, Y: Integer;
      State: TJvxButtonState);
    function Draw(Canvas: TCanvas; const Client: TRect; const Caption: string;
      Layout: TButtonLayout; Margin, Spacing: Integer; PopupMark: Boolean;
      State: TJvxButtonState; Flags: Word): TRect;
    property Alignment: TAlignment read FAlignment write FAlignment;
    property Glyph: TBitmap read FOriginal write SetGlyph;
    property NumGlyphs: TJvxNumGlyphs read FNumGlyphs write SetNumGlyphs;
...
uses SysUtils, Dialogs, {$IFDEF WIN32} CommCtrl, {$ELSE} JvxStr16, {$ENDIF}
  JvxVCLUtils, JvxMaxMin, Consts, JvxAppUtils {$IFDEF Delphi4_Up}, ImgList,
  ActnList {$ENDIF};
...
procedure TJvxTextListBox.SetHorizontalExtent;
begin
  SendMessage(Handle, LB_SETHORIZONTALEXTENT, FMaxWidth, 0);
end;

function TJvxTextListBox.GetItemWidth(Index: Integer): Integer;
var
  ATabWidth: Longint;
  S: string;
begin
...
procedure TJvxTextListBox.ResetHorizontalExtent;
var
  I: Integer;
begin
  FMaxWidth := 0;
...
procedure TJvxTextListBox.SetTabWidth(Value: Integer);
begin
...
procedure TJvxTextListBox.CreateParams(var Params: TCreateParams);
const
...
procedure TJvxTextListBox.CreateWnd;
begin
  inherited CreateWnd;
  if FTabWidth <> 0 then
...
procedure TJvxTextListBox.WndProc(var Message: TMessage);
begin
  case Message.Msg of
...
type
  TJvxListBoxStrings = class(TStrings)
  private
...
{$IFNDEF Delphi3_Up}
procedure TJvxListBoxStrings.Error(Msg: Word; Data: Integer);
...
function TJvxListBoxStrings.GetCount: Integer;
begin
  Result := SendMessage(ListBox.Handle, LB_GETCOUNT, 0, 0);
end;

function TJvxListBoxStrings.Get(Index: Integer): string;
var
  Len: Integer;
{$IFDEF WIN32}
...
function TJvxListBoxStrings.GetObject(Index: Integer): TObject;
begin
  Result := TObject(ListBox.GetItemData(Index));
  if Longint(Result) = LB_ERR then Error(SListIndexError, Index);
end;

procedure TJvxListBoxStrings.PutObject(Index: Integer; AObject: TObject);
begin
  ListBox.SetItemData(Index, LongInt(AObject));
end;

function TJvxListBoxStrings.Add(const S: string): Integer;
{$IFNDEF WIN32}
var
...
procedure TJvxListBoxStrings.Insert(Index: Integer; const S: string);
{$IFNDEF WIN32}
var
  Text: array[0..255] of Char;
{$ENDIF}
...
procedure TJvxListBoxStrings.Delete(Index: Integer);
begin
  ListBox.DeleteString(Index);
end;

procedure TJvxListBoxStrings.Clear;
begin
  ListBox.ResetContent;
end;

procedure TJvxListBoxStrings.SetUpdateState(Updating: Boolean);
begin
...
{$ENDIF}
  ...
  TJvxListBoxStrings(FItems).ListBox := Self;
...
begin
  Result := TJvxListBoxStrings.Create;
end;
...
{ TJvxCheckListBoxItem }

type
  TJvxCheckListBoxItem = class
  private
    FData: LongInt;
...
constructor TJvxCheckListBoxItem.Create;
begin
  inherited Create;
...
function TJvxCheckListBoxItem.GetChecked: Boolean;
begin
  Result := FState = cbChecked;
end;

{ TJvxCheckListBoxStrings }

type
  TJvxCheckListBoxStrings = class(TJvxListBoxStrings)
  public
    procedure Exchange(Index1, Index2: Integer); override;
    procedure Move(CurIndex, NewIndex: Integer); override;
  end;

procedure TJvxCheckListBoxStrings.Exchange(Index1, Index2: Integer);
var
...
procedure TJvxCheckListBoxStrings.Move(CurIndex, NewIndex: Integer);
var
  TempEnabled: Boolean;
...
begin
  Result := TJvxCheckListBoxStrings.Create;
end;
...
function TJvxCheckListBox.GetStorage: TJvxFormPlacement;
begin
  Result := FIniLink.Storage;
end;

procedure TJvxCheckListBox.SetStorage(Value: TJvxFormPlacement);
begin
...
begin
  ...
  try
    ...
    if (Value <> nil) and (Value is TJvxListBoxStrings) and
      (TJvxListBoxStrings(Value).ListBox <> nil) and
      (TJvxListBoxStrings(Value).ListBox is TJvxCheckListBox) then
    begin
      for I := 0 to Items.Count - 1 do
        if I < Value.Count then begin
          Self.State[I] := TJvxCheckListBox(TJvxListBoxStrings(Value).ListBox).State[I];
          EnabledItem[I] := TJvxCheckListBox(TJvxListBoxStrings(Value).ListBox).EnabledItem[I];
        end;
    end;
  finally
    Items.EndUpdate;
...
begin
  if (AState <> GetState(Index)) or FInUpdateStates then begin
    ...
    TJvxCheckListBoxItem(GetCheckObject(Index)).State := AState;
    if (FCheckKind = ckRadioButtons) and (AState = cbChecked) then
      for I := Items.Count - 1 downto 0 do begin
        if (I <> Index) and (GetState(I) = cbChecked) then begin
          TJvxCheckListBoxItem(GetCheckObject(I)).State := cbUnchecked;
          InvalidateCheck(I);
        end;
      end;
...
begin
  if Value <> GetItemEnabled(Index) then begin
    TJvxCheckListBoxItem(GetCheckObject(Index)).Enabled := Value;
...
begin
  if IsCheckObject(Index) then
    Result := TJvxCheckListBoxItem(GetCheckObject(Index)).GetChecked
  else Result := False;
end;
...
begin
  if IsCheckObject(Index) then
    Result := TJvxCheckListBoxItem(GetCheckObject(Index)).State
...
begin
  if IsCheckObject(Index) then
    Result := TJvxCheckListBoxItem(GetCheckObject(Index)).Enabled
...
var
  Item: TJvxCheckListBoxItem;
begin
  Result := 0;
  if IsCheckObject(Index) then begin
    Item := TJvxCheckListBoxItem(GetCheckObject(Index));
...
begin
  ...
  if ItemData = LB_ERR then ListIndexError(Index)
    ...
    Result := TJvxCheckListBoxItem(ItemData);
    if not (Result is TJvxCheckListBoxItem) then Result := nil;
  end;
end;

function TJvxCheckListBox.CreateCheckObject(Index: Integer): TObject;
begin
  Result := TJvxCheckListBoxItem.Create;
...
constructor TJvxCustomLabel.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
...
function TJvxCustomLabel.GetLabelCaption: string;
begin
  Result := Caption;
end;

function TJvxCustomLabel.GetDefaultFontColor: TColor;
begin
  Result := Font.Color;
end;

procedure TJvxCustomLabel.DoDrawText(var Rect: TRect; Flags: Word);
var
{$IFDEF WIN32}
  Text: string;
{$ELSE}
...
procedure TJvxCustomLabel.Paint;
var
  Rect: TRect;
  DrawStyle: Integer;
begin
...
procedure TJvxCustomLabel.AdjustBounds;
var
  DC: HDC;
  X: Integer;
  Rect: TRect;
...
procedure TJvxCustomLabel.SetAlignment(Value: TAlignment);
begin
...
procedure TJvxCustomLabel.SetAutoSize(Value: Boolean);
begin
...
procedure TJvxCustomLabel.SetLayout(Value: TTextLayout);
begin
...
procedure TJvxCustomLabel.SetLeftMargin(Value: Integer);
begin
...
procedure TJvxCustomLabel.SetRightMargin(Value: Integer);
begin
...
procedure TJvxCustomLabel.SetShadowColor(Value: TColor);
begin
...
procedure TJvxCustomLabel.SetShadowSize(Value: Byte);
begin
...
procedure TJvxCustomLabel.SetShadowPos(Value: TShadowPosition);
begin
...
function TJvxCustomLabel.GetTransparent: Boolean;
begin
  Result := not (csOpaque in ControlStyle);
end;

procedure TJvxCustomLabel.SetFocusControl(Value: TWinControl);
begin
  FFocusControl := Value;
{$IFDEF WIN32}
...
procedure TJvxCustomLabel.SetShowAccelChar(Value: Boolean);
begin
...
procedure TJvxCustomLabel.SetTransparent(Value: Boolean);
begin
...
procedure TJvxCustomLabel.SetShowFocus(Value: Boolean);
begin
...
procedure TJvxCustomLabel.SetWordWrap(Value: Boolean);
begin
...
procedure TJvxCustomLabel.Notification(AComponent: TComponent;
...
procedure TJvxCustomLabel.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
  inherited MouseDown(Button, Shift, X, Y);
...
procedure TJvxCustomLabel.MouseUp(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
...
procedure TJvxCustomLabel.MouseEnter;
begin
  if Assigned(FOnMouseEnter) then FOnMouseEnter(Self);
end;

procedure TJvxCustomLabel.MouseLeave;
begin
  if Assigned(FOnMouseLeave) then FOnMouseLeave(Self);
end;

procedure TJvxCustomLabel.UpdateTracking;
var
  P: TPoint;
  OldValue: Boolean;
begin
...
procedure TJvxCustomLabel.CMFocusChanged(var Message: TCMFocusChanged);
var
  Active: Boolean;
begin
...
procedure TJvxCustomLabel.CMTextChanged(var Message: TMessage);
begin
  Invalidate;
  AdjustBounds;
end;

procedure TJvxCustomLabel.CMFontChanged(var Message: TMessage);
begin
  inherited;
  AdjustBounds;
end;

procedure TJvxCustomLabel.CMDialogChar(var Message: TCMDialogChar);
begin
...
procedure TJvxCustomLabel.WMRButtonDown(var Message: TWMRButtonDown);
begin
  inherited;
  UpdateTracking;
end;

procedure TJvxCustomLabel.WMRButtonUp(var Message: TWMRButtonUp);
begin
  inherited;
  UpdateTracking;
end;

procedure TJvxCustomLabel.CMEnabledChanged(var Message: TMessage);
begin
  inherited;
  UpdateTracking;
end;

procedure TJvxCustomLabel.CMVisibleChanged(var Message: TMessage);
begin
  inherited;
  if Visible then UpdateTracking;
end;

procedure TJvxCustomLabel.CMMouseEnter(var Message: TMessage);
begin
  inherited;
...
procedure TJvxCustomLabel.CMMouseLeave(var Message: TMessage);
begin
  inherited;
...
constructor TJvxSecretPanel.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  FScrollCnt := 0;
  FAlignment := taCenter;
...
destructor TJvxSecretPanel.Destroy;
begin
  SetActive(False);
  FGlyph.OnChange := nil;
  FGlyph.Free;
...
procedure TJvxSecretPanel.GlyphChanged(Sender: TObject);
begin
...
procedure TJvxSecretPanel.LinesChanged(Sender: TObject);
begin
  if Active then begin
    FScrollCnt := 0;
    UpdateMemoryImage;
...
procedure TJvxSecretPanel.CMFontChanged(var Message: TMessage);
begin
  inherited;
  if Active then UpdateMemoryImage;
end;

procedure TJvxSecretPanel.CMColorChanged(var Message: TMessage);
begin
  inherited;
  if Active then UpdateMemoryImage;
end;

procedure TJvxSecretPanel.WMSize(var Message: TMessage);
begin
  inherited;
  if Active then begin
    UpdateMemoryImage;
...
{$IFDEF Delphi3_Up}
procedure TJvxSecretPanel.SetAsyncDrawing(Value: Boolean);
begin
...
procedure TJvxSecretPanel.AlignControls(AControl: TControl; var Rect: TRect);
begin
  inherited AlignControls(AControl, Rect);
  if (AControl = nil) and Active then UpdateMemoryImage;
end;

function TJvxSecretPanel.GetInflateWidth: Integer;
begin
  Result := BorderWidth;
...
procedure TJvxSecretPanel.RecalcDrawRect;
const
  MinOffset = 3;
var
...
procedure TJvxSecretPanel.PaintGlyph;
begin
...
procedure TJvxSecretPanel.PaintText;
var
...
procedure TJvxSecretPanel.PaintClient(Canvas: TCanvas; Rect: TRect);
begin
  if Assigned(FOnPaintClient) then FOnPaintClient(Self, Canvas, Rect)
  else Canvas.FillRect(Rect);
end;

procedure TJvxSecretPanel.Paint;
var
  Rect: TRect;
...
procedure TJvxSecretPanel.StartPlay;
begin
  if Assigned(FOnStartPlay) then FOnStartPlay(Self);
end;

procedure TJvxSecretPanel.StopPlay;
begin
  if Assigned(FOnStopPlay) then FOnStopPlay(Self);
end;

procedure TJvxSecretPanel.TimerExpired(Sender: TObject);
begin
...
procedure TJvxSecretPanel.UpdateMemoryImage;
var
  Metrics: TTextMetric;
  I: Integer;
begin
...
function TJvxSecretPanel.GetInterval: Cardinal;
begin
  Result := FTimer.Interval;
end;

procedure TJvxSecretPanel.SetInterval(Value: Cardinal);
begin
  FTimer.Interval := Value;
end;

procedure TJvxSecretPanel.Play;
begin
  SetActive(True);
end;

procedure TJvxSecretPanel.Stop;
begin
  SetActive(False);
end;

procedure TJvxSecretPanel.SetActive(Value: Boolean);
var
  I: Integer;
begin
...
procedure TJvxSecretPanel.SetAlignment(Value: TAlignment);
begin
  if FAlignment <> Value then begin
    FAlignment := Value;
...
procedure TJvxSecretPanel.SetGlyph(Value: TBitmap);
begin
  FGlyph.Assign(Value);
end;

procedure TJvxSecretPanel.SetDirection(Value: TScrollDirection);
begin
...
procedure TJvxSecretPanel.SetTextStyle(Value: TPanelBevel);
begin
...
procedure TJvxSecretPanel.SetGlyphLayout(Value: TGlyphLayout);
begin
...
procedure TJvxSecretPanel.SetLines(Value: TStrings);
begin
  FLines.Assign(Value);
end;
...
type
  TJvxGlyphList = class(TImageList)
  private
    FUsed: TBits;
    FCount: Integer;
...
{ TJvxGlyphCache }
  ...
  TJvxGlyphCache = class
  private
    FGlyphLists: TList;
  public
    constructor Create;
    destructor Destroy; override;
    function GetList(AWidth, AHeight: Integer): TJvxGlyphList;
    procedure ReturnList(List: TJvxGlyphList);
    function Empty: Boolean;
  end;

{ TJvxGlyphList }

constructor TJvxGlyphList.CreateSize(AWidth, AHeight: Integer);
begin
{$IFDEF WIN32}
...
destructor TJvxGlyphList.Destroy;
begin
  FUsed.Free;
  inherited Destroy;
end;

function TJvxGlyphList.AllocateIndex: Integer;
begin
...
{$IFDEF WIN32}
{$IFNDEF Delphi3_Up} { Delphi 2.0 bug fix }
procedure TJvxGlyphList.ReplaceMasked(Index: Integer; NewImage: TBitmap; MaskColor: TColor);
var
  TempIndex: Integer;
  Image, Mask: TBitmap;
begin
...
function TJvxGlyphList.Add(Image, Mask: TBitmap): Integer;
begin
...
function TJvxGlyphList.AddMasked(Image: TBitmap; MaskColor: TColor): Integer;
begin
  Result := AllocateIndex;
...
procedure TJvxGlyphList.Delete(Index: Integer);
begin
...
constructor TJvxGlyphCache.Create;
begin
  inherited Create;
  FGlyphLists := TList.Create;
end;

destructor TJvxGlyphCache.Destroy;
begin
  FGlyphLists.Free;
  inherited Destroy;
end;

function TJvxGlyphCache.GetList(AWidth, AHeight: Integer): TJvxGlyphList;
var
  I: Integer;
begin
  ...
  Result := TJvxGlyphList.CreateSize(AWidth, AHeight);
  FGlyphLists.Add(Result);
end;

procedure TJvxGlyphCache.ReturnList(List: TJvxGlyphList);
begin
...
function TJvxGlyphCache.Empty: Boolean;
begin
  Result := FGlyphLists.Count = 0;
end;

const
  GlyphCache: TJvxGlyphCache = nil;
...
begin
  ...
  if GlyphCache = nil then GlyphCache := TJvxGlyphCache.Create;
end;
...
begin
  for I := Low(I) to High(I) do begin
    if Assigned(FGlyphList) then
      if (FIndexs[I] <> -1) then TJvxGlyphList(FGlyphList).Delete(FIndexs[I]);
    FIndexs[I] := -1;
  end;
  GlyphCache.ReturnList(TJvxGlyphList(FGlyphList));
  FGlyphList := nil;
end;
  ...
  if Sender = FOriginal then begin
    ...
    if (FOriginal <> nil) and (FOriginal.Height > 0) then begin
      ...
      if FOriginal.Width mod FOriginal.Height = 0 then begin
        ...
        if Glyphs > (Ord(High(TJvxButtonState)) + 1) then Glyphs := 1;
        SetNumGlyphs(Glyphs);
      end;
    end;
...
procedure TJvxButtonGlyph.SetNumGlyphs(Value: TJvxNumGlyphs);
begin
...
{$IFDEF WIN32}
function TJvxButtonGlyph.CreateImageGlyph(State: TJvxButtonState;
    ...
    case State of
      rbsUp, rbsDown, rbsExclusive:
        begin
          ...
          try
            ...
            FIndexs[State] := TJvxGlyphList(FGlyphList).Add(TmpImage, Mask);
          finally
            Mask.Free;
          end;
        end;
      rbsDisabled:
        begin
          ...
          FIndexs[State] := TJvxGlyphList(FGlyphList).AddMasked(TmpImage,
...
function TJvxButtonGlyph.CreateButtonGlyph(State: TJvxButtonState): Integer;
var
    ...
    case State of
      rbsUp, rbsDown, rbsExclusive:
        begin
          ...
          FIndexs[State] := TJvxGlyphList(FGlyphList).AddMasked(TmpImage, FTransparentColor);
        end;
      rbsDisabled:
        if NumGlyphs > 1 then begin
          ...
          try
            FIndexs[State] := TJvxGlyphList(FGlyphList).AddMasked(MonoBmp,
              ColorToRGB(clBtnFace));
          finally
            MonoBmp.Free;
          end;
...
function TJvxButtonGlyph.DrawButtonImage(Canvas: TCanvas; X, Y: Integer;
  Images: TImageList; ImageIndex: Integer; State: TJvxButtonState): TPoint;
var
  Index: Integer;
begin
  Result := Point(0, 0);
...
function TJvxButtonGlyph.Draw(Canvas: TCanvas; const Client: TRect;
  ...
  PopupMark: Boolean; State: TJvxButtonState; Flags: Word): TRect;
begin
...
constructor TJvxButtonImage.Create;
begin
...
destructor TJvxButtonImage.Destroy;
begin
  FGlyph.Free;
  inherited Destroy;
end;

procedure TJvxButtonImage.Invalidate;
begin
  TJvxButtonGlyph(FGlyph).Invalidate;
end;

function TJvxButtonImage.GetNumGlyphs: TJvxNumGlyphs;
begin
  Result := TJvxButtonGlyph(FGlyph).NumGlyphs;
end;

procedure TJvxButtonImage.SetNumGlyphs(Value: TJvxNumGlyphs);
begin
  TJvxButtonGlyph(FGlyph).NumGlyphs := Value;
end;

function TJvxButtonImage.GetWordWrap: Boolean;
begin
  Result := TJvxButtonGlyph(FGlyph).WordWrap;
end;

procedure TJvxButtonImage.SetWordWrap(Value: Boolean);
begin
  TJvxButtonGlyph(FGlyph).WordWrap := Value;
end;

function TJvxButtonImage.GetGlyph: TBitmap;
begin
  Result := TJvxButtonGlyph(FGlyph).Glyph;
end;

procedure TJvxButtonImage.SetGlyph(Value: TBitmap);
begin
  TJvxButtonGlyph(FGlyph).Glyph := Value;
end;

function TJvxButtonImage.GetAlignment: TAlignment;
begin
  Result := TJvxButtonGlyph(FGlyph).Alignment;
end;

procedure TJvxButtonImage.SetAlignment(Value: TAlignment);
begin
  TJvxButtonGlyph(FGlyph).Alignment := Value;
end;

{$IFDEF WIN32}
procedure TJvxButtonImage.Draw(Canvas: TCanvas; X, Y, Margin, Spacing: Integer;
  Layout: TButtonLayout; AFont: TFont; Flags: Word);
begin
...
{$IFDEF WIN32}
procedure TJvxButtonImage.DrawEx(Canvas: TCanvas; X, Y, Margin, Spacing: Integer;
...
{$ELSE}
    TJvxButtonGlyph(FGlyph).Draw(Canvas, Target, Caption, Layout, Margin,
...
function TJvxSpeedButton.GetNumGlyphs: TJvxNumGlyphs;
begin
  Result := TJvxButtonGlyph(FGlyph).NumGlyphs;
end;

procedure TJvxSpeedButton.SetNumGlyphs(Value: TJvxNumGlyphs);
begin
  if Value < 0 then Value := 1
  else if Value > Ord(High(TJvxButtonState)) + 1 then
    Value := Ord(High(TJvxButtonState)) + 1;
  if Value <> TJvxButtonGlyph(FGlyph).NumGlyphs then begin
    TJvxButtonGlyph(FGlyph).NumGlyphs := Value;
    Invalidate;
  end;
end;
