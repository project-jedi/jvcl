...
The Original Code is: JvxFileUtil.PAS, released on 2002-07-04.
...
uses Windows, RTLConsts, Messages, SysUtils, Classes, Consts, Controls{, JvxComponent};
...
uses {$IFDEF WIN32} {$IFDEF Delphi3_Up} ActiveX, ComObj, ShlObj, {$ELSE} Ole2,
  OleAuto, {$ENDIF} {$ENDIF} JvxDateUtil, ShellAPI, FileCtrl, Forms, JvxVCLUtils,
  JvxPrgrss;
...
type
  ...
  TJvxBrowseFolderDlg = class(TComponent)
  private
    FDefWndProc: Pointer;
...
begin
  ...
  if Msg = BFFM_INITIALIZED then begin
    if TJvxBrowseFolderDlg(Data).Position = dpScreenCenter then
      CenterWindow(Wnd);
    TJvxBrowseFolderDlg(Data).FHandle := Wnd;
    TJvxBrowseFolderDlg(Data).FDefWndProc := Pointer(SetWindowLong(Wnd, GWL_WNDPROC,
      Longint(TJvxBrowseFolderDlg(Data).FObjectInstance)));
    TJvxBrowseFolderDlg(Data).DoInitialized;
  end
  else if Msg = BFFM_SELCHANGED then begin
    TJvxBrowseFolderDlg(Data).FHandle := Wnd;
    TJvxBrowseFolderDlg(Data).DoSelChanged(PItemIDList(LParam));
  end;
end;
...
constructor TJvxBrowseFolderDlg.Create(AOwner: TComponent);
begin
...
destructor TJvxBrowseFolderDlg.Destroy;
begin
  if FObjectInstance <> nil then Classes.FreeObjectInstance(FObjectInstance);
  inherited Destroy;
end;

procedure TJvxBrowseFolderDlg.DoInitialized;
const
  SBtn = 'BUTTON';
var
...
procedure TJvxBrowseFolderDlg.DoSelChanged(Param: PItemIDList);
var
...
procedure TJvxBrowseFolderDlg.SetSelPath(const Path: string);
begin
  if FHandle <> 0 then
    SendMessage(FHandle, BFFM_SETSELECTION, 1, Longint(PChar(Path)));
end;

procedure TJvxBrowseFolderDlg.SetOkEnable(Value: Boolean);
begin
  if FHandle <> 0 then SendMessage(FHandle, BFFM_ENABLEOK, 0, Ord(Value));
end;

procedure TJvxBrowseFolderDlg.DefaultHandler(var Message);
begin
  if FHandle <> 0 then
...
procedure TJvxBrowseFolderDlg.WndProc(var Message: TMessage);
begin
  Dispatch(Message);
end;

procedure TJvxBrowseFolderDlg.WMCommand(var Message: TMessage);
begin
...
procedure TJvxBrowseFolderDlg.WMNCDestroy(var Message: TWMNCDestroy);
begin
  inherited;
  FHandle := 0;
end;

function TJvxBrowseFolderDlg.Execute: Boolean;
var
...
function TJvxBrowseFolderDlg.TaskModalDialog(var Info: TBrowseInfo): PItemIDList;
var
  ActiveWindow: HWnd;
  WindowList: Pointer;
begin
...
begin
  if NewStyleControls then begin
    with TJvxBrowseFolderDlg.Create(Application) do
    try
      DialogText := DlgText;
...
type
  ...
  TJvxFileOperator = class(TComponent)
  private
    FAborted: Boolean;
...
constructor TJvxFileOperator.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  FOptions := [flAllowUndo, flNoConfirmMkDir];
end;

function TJvxFileOperator.TaskModalDialog(DialogFunc: Pointer; var DialogData): Boolean;
type
...
function TJvxFileOperator.Execute: Boolean;
const
...
begin
  with TJvxFileOperator.Create(nil) do
  try
    Source := FileName;
