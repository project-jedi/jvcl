...
The Original Code is: JvxAppUtils.PAS, released on 2002-07-04.
...
uses Windows, Registry, Classes, Controls, Forms, IniFiles, Grids, JvxVCLUtils{, JvxComponent}
{$IFDEF Delphi6}
, RTLConsts{, Variants}
{$ENDIF}
...
uses SysUtils, Messages, Consts, JvxStrUtils, JvxFileUtil, JvxPlacemnt;
  ...
  with Control do
    if Owner is TCustomForm then
      for I := 0 to Owner.ComponentCount - 1 do
        if (Owner.Components[I] is TJvxFormPlacement) then begin
          IniFileName := TJvxFormPlacement(Owner.Components[I]).IniFileName;
          Break;
        end;
...
{*******************************************************}
  ...
  TJvxNastyForm = class(TScrollingWinControl)
  private
...
{$ENDIF}
  ...
  TJvxHackComponent = class(TComponent);
{$IFDEF WIN32}
  {$HINTS ON}
{$ENDIF}
    ...
    if LoadPosition then begin
      ...
      if DataFound then begin
        ...
        if rcNormalPosition.Right > rcNormalPosition.Left then begin
          ...
          begin
            TJvxHackComponent(Form).SetDesigning(True);
            try
              Position := poDesigned;
            finally
              TJvxHackComponent(Form).SetDesigning(False);
            end;
          end;
...
{$IFDEF WIN32}
      ...
      begin
        TJvxNastyForm(Form).FWindowState := wsNormal;
        PostMessage(Application.Handle, WM_SYSCOMMAND, SC_MINIMIZE, 0);
        Exit;
      end;
{$ENDIF}
      if FormStyle in [fsMDIChild, fsMDIForm] then
        TJvxNastyForm(Form).FWindowState := WinState
