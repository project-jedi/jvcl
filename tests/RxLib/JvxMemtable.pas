...
The Original Code is: JvxMemTable.PAS, released on 2002-07-04.
...
{ TJvxMemoryTable }
  ...
  TJvxMemoryTable = class(TDBDataSet)
  private
    ...
    procedure EncodeFieldDesc(var FieldDesc: FLDDesc;
      ...
      {$IFDEF Delphi4_Up}, Precision {$ENDIF}: Word);
...
uses DBConsts, JvxDBUtils, JvxBdeUtils, {$IFDEF Delphi3_Up} BDEConst, {$ENDIF} 
  Forms, JvxMaxMin;
...
constructor TJvxMemoryTable.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  FEnableDelete := True;
end;

function TJvxMemoryTable.BatchMove(ASource: TDataSet; AMode: TBatchMode;
  ARecordCount: Longint): Longint;
var
...
procedure TJvxMemoryTable.CopyStructure(ASource: TDataSet);
...
procedure TJvxMemoryTable.DeleteCurrentRecord;
var
...
function TJvxMemoryTable.GetFieldData(Field: TField; Buffer: Pointer): Boolean;
var
  IsBlank: LongBool;
  RecBuf: PChar;
begin
...
procedure TJvxMemoryTable.InternalDelete;
begin
  if EnableDelete then DeleteCurrentRecord
  else inherited;
end;

function TJvxMemoryTable.Locate(const KeyFields: string;
...
function TJvxMemoryTable.Lookup(const KeyFields: string; const KeyValues: Variant;
...
procedure TJvxMemoryTable.DoBeforeDelete;
begin
...
procedure TJvxMemoryTable.DoAfterClose;
begin
  if not FDisableEvents then inherited DoAfterClose;
end;

procedure TJvxMemoryTable.DoAfterOpen;
begin
  if not FDisableEvents then inherited DoAfterOpen;
end;

procedure TJvxMemoryTable.DoBeforeClose;
begin
  if not FDisableEvents then inherited DoBeforeClose;
end;

procedure TJvxMemoryTable.DoBeforeOpen;
begin
...
procedure TJvxMemoryTable.DoBeforeScroll;
begin
  if not FDisableEvents then inherited DoBeforeScroll;
end;

procedure TJvxMemoryTable.DoAfterScroll;
begin
...
function TJvxMemoryTable.SupportedFieldType(AType: TFieldType): Boolean;
begin
  Result := not (AType in [ftUnknown {$IFDEF Delphi4_Up}, ftWideString {$ENDIF}
    {$IFDEF Delphi5_Up}, ftOraBlob, ftOraClob, ftVariant, ftInterface, 
    ftIDispatch, ftGuid {$ENDIF}] + ftNonTextTypes);
end;

function TJvxMemoryTable.CreateHandle: HDBICur;
var
  I: Integer;
{$IFDEF Delphi4_Up}
...
procedure TJvxMemoryTable.CreateTable;
begin
  CheckInactive;
  Open;
end;

procedure TJvxMemoryTable.DeleteTable;
begin
  CheckBrowseMode;
  Close;
end;

procedure TJvxMemoryTable.EmptyTable;
begin
...
procedure TJvxMemoryTable.EncodeFieldDesc(var FieldDesc: FLDDesc;
  const Name: string; DataType: TFieldType; Size
  {$IFDEF Delphi4_Up}, Precision {$ENDIF}: Word);
begin
...
{$IFDEF WIN32}
function TJvxMemoryTable.GetRecordCount: {$IFNDEF Delphi3_Up} Longint {$ELSE} Integer {$ENDIF};
begin
...
procedure TJvxMemoryTable.SetRecNo(Value: {$IFDEF Delphi3_Up} Integer {$ELSE} Longint {$ENDIF});
var
  Rslt: DBIResult;
begin
  CheckBrowseMode;
...
{$IFDEF Delphi3_Up}
function TJvxMemoryTable.GetRecNo: Integer;
{$ELSE}
function TJvxMemoryTable.GetRecordNumber: Longint;
{$ENDIF}
var
  Rslt: DBIResult;
begin
...
procedure TJvxMemoryTable.GotoRecord(RecordNo: Longint);
begin
  RecNo := RecordNo;
end;

{$IFDEF Delphi3_Up}
function TJvxMemoryTable.IsSequenced: Boolean;
begin
  Result := not Filtered;
end;
{$ENDIF Delphi3_Up}

procedure TJvxMemoryTable.SetFieldValues(const FieldNames: array of string;
  const Values: array of const);
var
  I: Integer;
  Pos: Longint;
...
procedure TJvxMemoryTable.SetTableName(const Value: TFileName);
begin
  CheckInactive;
  FTableName := Value;
