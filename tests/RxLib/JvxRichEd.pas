...
The Original Code is: JvxRichEd.PAS, released on 2002-07-04.
...
type
  TJvxCustomRichEdit = class;

{ TJvxTextAttributes }

  TJvxAttributeType = (atDefaultText, atSelected, atWord);
  TJvxConsistentAttribute = (caBold, caColor, caFace, caItalic, caSize,
    caStrikeOut, caUnderline, caProtected, caOffset, caHidden, caLink,
    caBackColor, caDisabled, caWeight, caSubscript, caRevAuthor);
  TJvxConsistentAttributes = set of TJvxConsistentAttribute;
  TSubscriptStyle = (ssNone, ssSubscript, ssSuperscript);
  TUnderlineType = (utNone, utSolid, utWord, utDouble, utDotted, utWave);

  TJvxTextAttributes = class(TPersistent)
  private
    RichEdit: TJvxCustomRichEdit;
    FType: TJvxAttributeType;
...
{$ENDIF}
    ...
    function GetConsistentAttributes: TJvxConsistentAttributes;
  ...
  public
    constructor Create(AOwner: TJvxCustomRichEdit; AttributeType: TJvxAttributeType);
...
{$ENDIF}
    ...
    property ConsistentAttributes: TJvxConsistentAttributes read GetConsistentAttributes;
...
{ TJvxParaAttributes }
  ...
  TJvxNumbering = (nsNone, nsBullet, nsArabicNumbers, nsLoCaseLetter,
    nsUpCaseLetter, nsLoCaseRoman, nsUpCaseRoman);
  TJvxNumberingStyle = (nsParenthesis, nsPeriod, nsEnclosed, nsSimple);
  ...
  TJvxParaAttributes = class(TPersistent)
  private
    RichEdit: TJvxCustomRichEdit;
    ...
    function GetNumbering: TJvxNumbering;
    function GetNumberingStyle: TJvxNumberingStyle;
    ...
    procedure SetNumbering(Value: TJvxNumbering);
    procedure SetNumberingStyle(Value: TJvxNumberingStyle);
  ...
  protected
    ...
    procedure AssignTo(Dest: TPersistent); override;
  public
    constructor Create(AOwner: TJvxCustomRichEdit);
    procedure Assign(Source: TPersistent); override;
    ...
    property Numbering: TJvxNumbering read GetNumbering write SetNumbering;
    property NumberingStyle: TJvxNumberingStyle read GetNumberingStyle write SetNumberingStyle;
...
{ TJvxOEMConversion }
  ...
  TJvxOEMConversion = class(TConversion)
  public
...
{$ENDIF}
  ...
  TJvxCustomRichEdit = class(TCustomMemo)
  private
    ...
    FPlainText: Boolean;
    FSelAttributes: TJvxTextAttributes;
    FDefAttributes: TJvxTextAttributes;
    FWordAttributes: TJvxTextAttributes;
    FParagraph: TJvxParaAttributes;
...
{$ENDIF}
    FDefaulTJvxConverter: TConversionClass;
...
{$ENDIF}
    ...
    procedure SetDefAttributes(Value: TJvxTextAttributes);
    procedure SetSelAttributes(Value: TJvxTextAttributes);
    procedure SetWordAttributes(Value: TJvxTextAttributes);
...
{$ENDIF}
    ...
    property DefaulTJvxConverter: TConversionClass read FDefaulTJvxConverter
      write FDefaulTJvxConverter;
    property DefAttributes: TJvxTextAttributes read FDefAttributes write SetDefAttributes;
    property SelAttributes: TJvxTextAttributes read FSelAttributes write SetSelAttributes;
    property WordAttributes: TJvxTextAttributes read FWordAttributes write SetWordAttributes;
    property PageRect: TRect read FPageRect write FPageRect;
    property Paragraph: TJvxParaAttributes read FParagraph;
    property SelectionType: TRichSelectionType read GetSelectionType;
  end;

  TJvxRichEdit = class(TJvxCustomRichEdit)
  published
    property Align;
    property Alignment;
...
const
  AttrFlags: array[TJvxAttributeType] of Word = (0, SCF_SELECTION,
    SCF_WORD or SCF_SELECTION);

constructor TJvxTextAttributes.Create(AOwner: TJvxCustomRichEdit;
  AttributeType: TJvxAttributeType);
begin
  inherited Create;
  RichEdit := AOwner;
  FType := AttributeType;
...
procedure TJvxTextAttributes.InitFormat(var Format: TCharFormat2);
begin
...
function TJvxTextAttributes.GetConsistentAttributes: TJvxConsistentAttributes;
var
  Format: TCharFormat2;
begin
  Result := [];
...
procedure TJvxTextAttributes.GetAttributes(var Format: TCharFormat2);
begin
  InitFormat(Format);
...
procedure TJvxTextAttributes.SetAttributes(var Format: TCharFormat2);
begin
  if RichEdit.HandleAllocated then
...
{$IFNDEF VER90}
function TJvxTextAttributes.GetCharset: TFontCharset;
var
  Format: TCharFormat2;
begin
...
procedure TJvxTextAttributes.SetCharset(Value: TFontCharset);
var
  Format: TCharFormat2;
begin
  InitFormat(Format);
...
function TJvxTextAttributes.GetProtected: Boolean;
var
  Format: TCharFormat2;
begin
  GetAttributes(Format);
...
procedure TJvxTextAttributes.SetProtected(Value: Boolean);
var
  Format: TCharFormat2;
begin
  InitFormat(Format);
...
function TJvxTextAttributes.GetLink: Boolean;
var
  Format: TCharFormat2;
begin
  Result := False;
...
procedure TJvxTextAttributes.SetLink(Value: Boolean);
var
  Format: TCharFormat2;
begin
  if RichEditVersion < 2 then Exit;
...
function TJvxTextAttributes.GetRevAuthorIndex: Byte;
var
  Format: TCharFormat2;
begin
  GetAttributes(Format);
...
procedure TJvxTextAttributes.SetRevAuthorIndex(Value: Byte);
var
  Format: TCharFormat2;
begin
  if RichEditVersion < 2 then Exit;
...
function TJvxTextAttributes.GetHidden: Boolean;
var
  Format: TCharFormat2;
begin
  Result := False;
...
procedure TJvxTextAttributes.SetHidden(Value: Boolean);
var
  Format: TCharFormat2;
begin
  if RichEditVersion < 2 then Exit;
...
function TJvxTextAttributes.GetDisabled: Boolean;
var
  Format: TCharFormat2;
begin
  Result := False;
...
procedure TJvxTextAttributes.SetDisabled(Value: Boolean);
var
  Format: TCharFormat2;
begin
  if RichEditVersion < 2 then Exit;
...
function TJvxTextAttributes.GetColor: TColor;
var
  Format: TCharFormat2;
begin
  GetAttributes(Format);
...
procedure TJvxTextAttributes.SetColor(Value: TColor);
var
  Format: TCharFormat2;
begin
  InitFormat(Format);
...
function TJvxTextAttributes.GetBackColor: TColor;
var
  Format: TCharFormat2;
begin
  if RichEditVersion < 2 then begin
...
procedure TJvxTextAttributes.SetBackColor(Value: TColor);
var
  Format: TCharFormat2;
begin
  if RichEditVersion < 2 then Exit;
...
function TJvxTextAttributes.GetName: TFontName;
var
  Format: TCharFormat2;
begin
  GetAttributes(Format);
...
procedure TJvxTextAttributes.SetName(Value: TFontName);
var
  Format: TCharFormat2;
begin
  InitFormat(Format);
...
function TJvxTextAttributes.GetStyle: TFontStyles;
var
  Format: TCharFormat2;
begin
  Result := [];
...
procedure TJvxTextAttributes.SetStyle(Value: TFontStyles);
var
  Format: TCharFormat2;
begin
  InitFormat(Format);
...
function TJvxTextAttributes.GetUnderlineType: TUnderlineType;
var
  Format: TCharFormat2;
begin
  Result := utNone;
...
procedure TJvxTextAttributes.SetUnderlineType(Value: TUnderlineType);
var
  Format: TCharFormat2;
begin
  if RichEditVersion < 2 then Exit;
...
function TJvxTextAttributes.GetOffset: Integer;
var
  Format: TCharFormat2;
begin
  GetAttributes(Format);
...
procedure TJvxTextAttributes.SetOffset(Value: Integer);
var
  Format: TCharFormat2;
begin
  InitFormat(Format);
...
function TJvxTextAttributes.GetSize: Integer;
var
  Format: TCharFormat2;
begin
  GetAttributes(Format);
...
procedure TJvxTextAttributes.SetSize(Value: Integer);
var
  Format: TCharFormat2;
begin
  InitFormat(Format);
...
function TJvxTextAttributes.GetHeight: Integer;
begin
  Result := MulDiv(Size, RichEdit.FScreenLogPixels, 72);
end;

procedure TJvxTextAttributes.SetHeight(Value: Integer);
begin
  Size := MulDiv(Value, 72, RichEdit.FScreenLogPixels);
end;

function TJvxTextAttributes.GetPitch: TFontPitch;
var
  Format: TCharFormat2;
begin
  GetAttributes(Format);
...
procedure TJvxTextAttributes.SetPitch(Value: TFontPitch);
var
  Format: TCharFormat2;
begin
  InitFormat(Format);
...
function TJvxTextAttributes.GetSubscriptStyle: TSubscriptStyle;
var
  Format: TCharFormat2;
begin
  Result := ssNone;
...
procedure TJvxTextAttributes.SetSubscriptStyle(Value: TSubscriptStyle);
var
  Format: TCharFormat2;
begin
  if RichEditVersion < 2 then Exit;
...
procedure TJvxTextAttributes.AssignFont(Font: TFont);
var
  LogFont: TLogFont;
  Format: TCharFormat2;
begin
...
procedure TJvxTextAttributes.Assign(Source: TPersistent);
var
  Format: TCharFormat2;
begin
...
{$ENDIF}
  ...
  else if Source is TJvxTextAttributes then begin
    TJvxTextAttributes(Source).GetAttributes(Format);
    SetAttributes(Format);
  end
  else inherited Assign(Source);
end;

procedure TJvxTextAttributes.AssignTo(Dest: TPersistent);
begin
...
constructor TJvxParaAttributes.Create(AOwner: TJvxCustomRichEdit);
begin
  inherited Create;
  RichEdit := AOwner;
end;

procedure TJvxParaAttributes.InitPara(var Paragraph: TParaFormat2);
begin
...
procedure TJvxParaAttributes.GetAttributes(var Paragraph: TParaFormat2);
begin
  InitPara(Paragraph);
  if RichEdit.HandleAllocated then
...
procedure TJvxParaAttributes.SetAttributes(var Paragraph: TParaFormat2);
begin
{$IFDEF Delphi4_Up}
...
function TJvxParaAttributes.GetAlignment: TParaAlignment;
var
...
procedure TJvxParaAttributes.SetAlignment(Value: TParaAlignment);
var
  Paragraph: TParaFormat2;
begin
  InitPara(Paragraph);
...
function TJvxParaAttributes.GetNumbering: TJvxNumbering;
var
  Paragraph: TParaFormat2;
begin
  GetAttributes(Paragraph);
  Result := TJvxNumbering(Paragraph.wNumbering);
  if RichEditVersion = 1 then
    if Result <> nsNone then Result := nsBullet;
end;

procedure TJvxParaAttributes.SetNumbering(Value: TJvxNumbering);
var
  Paragraph: TParaFormat2;
begin
  if RichEditVersion = 1 then
    if Value <> nsNone then Value := TJvxNumbering(PFN_BULLET);
  case Value of
    nsNone: LeftIndent := 0;
...
function TJvxParaAttributes.GetNumberingStyle: TJvxNumberingStyle;
var
  Paragraph: TParaFormat2;
begin
  if RichEditVersion < 2 then
    ...
    GetAttributes(Paragraph);
    Result := TJvxNumberingStyle(Paragraph.wNumberingStyle);
  end;
end;

procedure TJvxParaAttributes.SetNumberingStyle(Value: TJvxNumberingStyle);
var
  Paragraph: TParaFormat2;
begin
  if RichEditVersion < 2 then Exit;
...
function TJvxParaAttributes.GetNumberingTab: Word;
var
  Paragraph: TParaFormat2;
begin
  GetAttributes(Paragraph);
...
procedure TJvxParaAttributes.SetNumberingTab(Value: Word);
var
  Paragraph: TParaFormat2;
begin
  if RichEditVersion < 2 then Exit;
...
function TJvxParaAttributes.GetFirstIndent: Longint;
var
  Paragraph: TParaFormat2;
begin
  GetAttributes(Paragraph);
...
procedure TJvxParaAttributes.SetFirstIndent(Value: Longint);
var
  Paragraph: TParaFormat2;
begin
  InitPara(Paragraph);
...
function TJvxParaAttributes.GetHeadingStyle: THeadingStyle;
var
  Paragraph: TParaFormat2;
begin
...
procedure TJvxParaAttributes.SetHeadingStyle(Value: THeadingStyle);
var
  Paragraph: TParaFormat2;
begin
...
function TJvxParaAttributes.GetLeftIndent: Longint;
var
  Paragraph: TParaFormat2;
begin
  GetAttributes(Paragraph);
...
procedure TJvxParaAttributes.SetLeftIndent(Value: Longint);
var
  Paragraph: TParaFormat2;
begin
  InitPara(Paragraph);
...
function TJvxParaAttributes.GetRightIndent: Longint;
var
  Paragraph: TParaFormat2;
begin
  GetAttributes(Paragraph);
...
procedure TJvxParaAttributes.SetRightIndent(Value: Longint);
var
  Paragraph: TParaFormat2;
begin
  InitPara(Paragraph);
...
function TJvxParaAttributes.GetSpaceAfter: Longint;
var
  Paragraph: TParaFormat2;
begin
  GetAttributes(Paragraph);
...
procedure TJvxParaAttributes.SetSpaceAfter(Value: Longint);
var
  Paragraph: TParaFormat2;
begin
  if RichEditVersion < 2 then Exit;
...
function TJvxParaAttributes.GetSpaceBefore: Longint;
var
  Paragraph: TParaFormat2;
begin
  GetAttributes(Paragraph);
...
procedure TJvxParaAttributes.SetSpaceBefore(Value: Longint);
var
  Paragraph: TParaFormat2;
begin
  if RichEditVersion < 2 then Exit;
...
function TJvxParaAttributes.GetLineSpacing: Longint;
var
  Paragraph: TParaFormat2;
begin
  GetAttributes(Paragraph);
...
procedure TJvxParaAttributes.SetLineSpacing(Value: Longint);
var
  Paragraph: TParaFormat2;
begin
  if RichEditVersion < 2 then Exit;
...
function TJvxParaAttributes.GetLineSpacingRule: TLineSpacingRule;
var
  Paragraph: TParaFormat2;
begin
  GetAttributes(Paragraph);
...
procedure TJvxParaAttributes.SetLineSpacingRule(Value: TLineSpacingRule);
var
  Paragraph: TParaFormat2;
begin
  if RichEditVersion < 2 then Exit;
...
function TJvxParaAttributes.GetTab(Index: Byte): Longint;
var
  Paragraph: TParaFormat2;
begin
  GetAttributes(Paragraph);
...
procedure TJvxParaAttributes.SetTab(Index: Byte; Value: Longint);
var
  Paragraph: TParaFormat2;
begin
  GetAttributes(Paragraph);
...
function TJvxParaAttributes.GetTabCount: Integer;
var
  Paragraph: TParaFormat2;
begin
  GetAttributes(Paragraph);
...
procedure TJvxParaAttributes.SetTabCount(Value: Integer);
var
  Paragraph: TParaFormat2;
begin
  GetAttributes(Paragraph);
...
function TJvxParaAttributes.GetTableStyle: TParaTableStyle;
var
  Paragraph: TParaFormat2;
begin
  Result := tsNone;
...
procedure TJvxParaAttributes.SetTableStyle(Value: TParaTableStyle);
var
  Paragraph: TParaFormat2;
begin
  if RichEditVersion < 2 then Exit;
...
procedure TJvxParaAttributes.AssignTo(Dest: TPersistent);
var
  I: Integer;
begin
...
procedure TJvxParaAttributes.Assign(Source: TPersistent);
var
  I: Integer;
  Paragraph: TParaFormat2;
begin
  if Source is TParaAttributes then begin
    ...
    Numbering := TJvxNumbering(TParaAttributes(Source).Numbering);
    for I := 0 to MAX_TAB_STOPS - 1 do
      Tab[I] := TParaAttributes(Source).Tab[I];
  end
  else if Source is TJvxParaAttributes then begin
    TJvxParaAttributes(Source).GetAttributes(Paragraph);
    SetAttributes(Paragraph);
  end
  else inherited Assign(Source);
end;
...
{$ENDIF}
  private
    FRefCount: Longint;
    FRichEdit: TJvxCustomRichEdit;
  public
    constructor Create(RichEdit: TJvxCustomRichEdit);
    destructor Destroy; override;
{$IFDEF Delphi3_Up}
...
constructor TRichEditOleCallback.Create(RichEdit: TJvxCustomRichEdit);
begin
  inherited Create;
  FRichEdit := RichEdit;
end;
...
{$ENDIF}
  private
    ...
    FReObject: TReObject;
  public
    constructor Create(RichEdit: TJvxCustomRichEdit; ReObject: TReObject);
{$IFNDEF Delphi3_Up}
    function QueryInterface(const iid: TIID; var obj): HResult; override;
    function AddRef: Longint; override;
    function Release: Longint; override;
...
constructor TOleUIObjInfo.Create(RichEdit: TJvxCustomRichEdit;
  ReObject: TReObject);
begin
  inherited Create;
  FRichEdit := RichEdit;
...
constructor TOleUILinkInfo.Create(RichEdit: TJvxCustomRichEdit;
  ReObject: TReObject);
begin
  inherited Create;
  FReObject := ReObject;
...
type
  TJvxRichEditStrings = class(TStrings)
  private
    RichEdit: TJvxCustomRichEdit;
...
destructor TJvxRichEditStrings.Destroy;
begin
  FConverter.Free;
  inherited Destroy;
end;

procedure TJvxRichEditStrings.AddStrings(Strings: TStrings);
var
...
function TJvxRichEditStrings.GetCount: Integer;
begin
...
function TJvxRichEditStrings.Get(Index: Integer): string;
var
...
procedure TJvxRichEditStrings.Put(Index: Integer; const S: string);
var
...
procedure TJvxRichEditStrings.Insert(Index: Integer; const S: string);
var
  L: Integer;
  Selection: TCharRange;
  Fmt: PChar;
...
procedure TJvxRichEditStrings.Delete(Index: Integer);
const
  Empty: PChar = '';
var
  Selection: TCharRange;
...
procedure TJvxRichEditStrings.Clear;
begin
  RichEdit.Clear;
end;

procedure TJvxRichEditStrings.SetUpdateState(Updating: Boolean);
begin
...
procedure TJvxRichEditStrings.EnableChange(const Value: Boolean);
var
  EventMask: Longint;
begin
  with RichEdit do begin
...
procedure TJvxRichEditStrings.SetTextStr(const Value: string);
begin
  EnableChange(False);
  try
...
procedure TJvxRichEditStrings.LoadFromStream(Stream: TStream);
var
...
begin
  ...
  else Converter := RichEdit.DefaulTJvxConverter.Create;
...
procedure TJvxRichEditStrings.SaveToStream(Stream: TStream);
var
  EditStream: TEditStream;
  TextType: Longint;
  StreamInfo: TRichEditStreamInfo;
...
procedure TJvxRichEditStrings.LoadFromFile(const FileName: string);
var
  Ext: string;
...
procedure TJvxRichEditStrings.SaveToFile(const FileName: string);
var
  Ext: string;
  Convert: PRichConversionFormat;
  SaveFormat: TRichStreamFormat;
...
function TJvxOEMConversion.ConvertReadStream(Stream: TStream; Buffer: PChar;
...
function TJvxOEMConversion.ConvertWriteStream(Stream: TStream; Buffer: PChar;
  BufSize: Integer): Integer;
var
  Mem: TMemoryStream;
begin
...
constructor TJvxCustomRichEdit.Create(AOwner: TComponent);
var
  DC: HDC;
begin
  inherited Create(AOwner);
  ControlStyle := ControlStyle - [csSetCaption];
  FSelAttributes := TJvxTextAttributes.Create(Self, atSelected);
  FDefAttributes := TJvxTextAttributes.Create(Self, atDefaultText);
  FWordAttributes := TJvxTextAttributes.Create(Self, atWord);
  FParagraph := TJvxParaAttributes.Create(Self);
  FRichEditStrings := TJvxRichEditStrings.Create;
  TJvxRichEditStrings(FRichEditStrings).RichEdit := Self;
  TabStop := True;
  Width := 185;
  Height := 89;
  AutoSize := False;
...
{$ENDIF}
  ...
  ReleaseDC(0, DC);
  DefaulTJvxConverter := TConversion;
...
destructor TJvxCustomRichEdit.Destroy;
begin
  FLastFind := nil;
  FSelAttributes.Free;
  FDefAttributes.Free;
...
procedure TJvxCustomRichEdit.Clear;
begin
  CloseObjects;
  inherited Clear;
  Modified := False;
...
procedure TJvxCustomRichEdit.CreateParams(var Params: TCreateParams);
const
...
procedure TJvxCustomRichEdit.CreateWnd;
var
  ...
  Mask: Longint;
begin
  StreamFmt := TJvxRichEditStrings(Lines).Format;
  Mode := TJvxRichEditStrings(Lines).Mode;
  inherited CreateWnd;
{$IFNDEF VER90}
...
{$ENDIF}
  ...
  if FMemStream <> nil then begin
    ...
    if DesignMode then begin
      TJvxRichEditStrings(Lines).Format := sfPlainText;
      TJvxRichEditStrings(Lines).Mode := [];
    end;
    try
      ...
      TJvxRichEditStrings(Lines).Format := StreamFmt;
      TJvxRichEditStrings(Lines).Mode := Mode;
    end;
  end;
  if RichEditVersion < 2 then
...
procedure TJvxCustomRichEdit.DestroyWnd;
var
  StreamFmt: TRichStreamFormat;
  Mode: TRichStreamModes;
  DesignMode: Boolean;
...
procedure TJvxCustomRichEdit.SetAllowObjects(Value: Boolean);
begin
...
procedure TJvxCustomRichEdit.UpdateHostNames;
var
  AppName: string;
begin
...
procedure TJvxCustomRichEdit.SetTitle(const Value: string);
begin
...
function TJvxCustomRichEdit.GetPopupMenu: TPopupMenu;
var
...
procedure TJvxCustomRichEdit.PopupVerbClick(Sender: TObject);
var
  ReObject: TReObject;
begin
...
procedure TJvxCustomRichEdit.ObjectPropsClick(Sender: TObject);
begin
  ObjectPropertiesDialog;
end;

procedure TJvxCustomRichEdit.WMSetFont(var Message: TWMSetFont);
begin
  FDefAttributes.Assign(Font);
end;

procedure TJvxCustomRichEdit.CMFontChanged(var Message: TMessage);
begin
  inherited;
  FDefAttributes.Assign(Font);
end;

procedure TJvxCustomRichEdit.CreateWindowHandle(const Params: TCreateParams);
var
  Bounds: TRect;
begin
  Bounds := BoundsRect;
...
{$IFDEF Delphi3_Up}
procedure TJvxCustomRichEdit.DoSetMaxLength(Value: Integer);
begin
...
function TJvxCustomRichEdit.GetCaretPos: TPoint;
var
...
{$IFDEF Delphi3_Up}
function TJvxCustomRichEdit.GetSelLength: Integer;
begin
  with GetSelection do
    Result := cpMax - cpMin;
end;

function TJvxCustomRichEdit.GetSelStart: Integer;
begin
  Result := GetSelection.cpMin;
end;

function TJvxCustomRichEdit.GetSelText: string;
begin
  with GetSelection do
...
function TJvxCustomRichEdit.GetSelTextBuf(Buffer: PChar; BufSize: Integer): Integer;
var
  S: string;
begin
  S := SelText;
...
{$IFDEF Delphi4_Up}
procedure TJvxCustomRichEdit.CMBiDiModeChanged(var Message: TMessage);
var
...
procedure TJvxCustomRichEdit.SetHideScrollBars(Value: Boolean);
begin
...
procedure TJvxCustomRichEdit.SetSelectionBar(Value: Boolean);
begin
...
procedure TJvxCustomRichEdit.SetHideSelection(Value: Boolean);
begin
...
function TJvxCustomRichEdit.GetAutoURLDetect: Boolean;
begin
...
procedure TJvxCustomRichEdit.SetAutoURLDetect(Value: Boolean);
begin
...
function TJvxCustomRichEdit.GetWordSelection: Boolean;
begin
...
procedure TJvxCustomRichEdit.SetWordSelection(Value: Boolean);
var
  Options: LPARAM;
begin
...
function TJvxCustomRichEdit.GetLangOptions: TRichLangOptions;
var
  Flags: Longint;
  I: TRichLangOption;
begin
...
procedure TJvxCustomRichEdit.SetLangOptions(Value: TRichLangOptions);
var
  Flags: DWORD;
  I: TRichLangOption;
begin
...
procedure TJvxCustomRichEdit.SetSelAttributes(Value: TJvxTextAttributes);
begin
  FSelAttributes.Assign(Value);
end;

function TJvxCustomRichEdit.GetCanRedo: Boolean;
begin
  Result := False;
  if HandleAllocated and (RichEditVersion >= 2) then
...
function TJvxCustomRichEdit.GetCanPaste: Boolean;
begin
  Result := False;
  if HandleAllocated then
...
{$IFNDEF COMPILER35_UP}
function TJvxCustomRichEdit.GetCanUndo: Boolean;
begin
  Result := False;
  if HandleAllocated then
...
function TJvxCustomRichEdit.GetRedoName: TUndoName;
begin
  Result := unUnknown;
...
function TJvxCustomRichEdit.GetUndoName: TUndoName;
begin
  Result := unUnknown;
  if (RichEditVersion >= 2) and HandleAllocated then
...
function TJvxCustomRichEdit.GetSelectionType: TRichSelectionType;
const
...
function TJvxCustomRichEdit.GetSelection: TCharRange;
begin
  SendMessage(Handle, EM_EXGETSEL, 0, Longint(@Result));
end;

procedure TJvxCustomRichEdit.SetSelection(StartPos, EndPos: Longint;
  ScrollCaret: Boolean);
var
  CharRange: TCharRange;
begin
...
{$IFDEF Delphi3_Up}
procedure TJvxCustomRichEdit.SetSelLength(Value: Integer);
begin
  with GetSelection do SetSelection(cpMin, cpMin + Value, True);
end;

procedure TJvxCustomRichEdit.SetSelStart(Value: Integer);
begin
  SetSelection(Value, Value, False);
end;
{$ENDIF Delphi3_Up}

function TJvxCustomRichEdit.GetCharPos(CharIndex: Integer): TPoint;
var
  Res: Longint;
begin
...
function TJvxCustomRichEdit.GetTextRange(StartPos, EndPos: Longint): string;
var
...
function TJvxCustomRichEdit.WordAtCursor: string;
var
  Range: TCharRange;
begin
  Result := '';
...
function TJvxCustomRichEdit.LineFromChar(CharIndex: Integer): Integer;
begin
  Result := SendMessage(Handle, EM_EXLINEFROMCHAR, 0, CharIndex);
end;

function TJvxCustomRichEdit.GetLineIndex(LineNo: Integer): Integer;
begin
  Result := SendMessage(Handle, EM_LINEINDEX, LineNo, 0);
end;

function TJvxCustomRichEdit.GetLineLength(CharIndex: Integer): Integer;
begin
  Result := SendMessage(Handle, EM_LINELENGTH, CharIndex, 0);
end;

procedure TJvxCustomRichEdit.SetUndoLimit(Value: Integer);
begin
...
procedure TJvxCustomRichEdit.SetDefAttributes(Value: TJvxTextAttributes);
begin
  FDefAttributes.Assign(Value);
end;

procedure TJvxCustomRichEdit.SetWordAttributes(Value: TJvxTextAttributes);
begin
  FWordAttributes.Assign(Value);
end;

function TJvxCustomRichEdit.GetStreamFormat: TRichStreamFormat;
begin
  Result := TJvxRichEditStrings(Lines).Format;
end;

function TJvxCustomRichEdit.GetStreamMode: TRichStreamModes;
begin
  Result := TJvxRichEditStrings(Lines).Mode;
end;

procedure TJvxCustomRichEdit.SetStreamFormat(Value: TRichStreamFormat);
begin
  TJvxRichEditStrings(Lines).Format := Value;
end;

procedure TJvxCustomRichEdit.SetStreamMode(Value: TRichStreamModes);
begin
  TJvxRichEditStrings(Lines).Mode := Value;
end;

procedure TJvxCustomRichEdit.SetPlainText(Value: Boolean);
var
  MemStream: TStream;
  StreamFmt: TRichStreamFormat;
  Mode: TRichStreamModes;
      ...
      try
        ...
        try
          if (csDesigning in ComponentState) or Value then
            TJvxRichEditStrings(Lines).Format := sfPlainText
          else TJvxRichEditStrings(Lines).Format := sfRichText;
          TJvxRichEditStrings(Lines).Mode := [];
          Lines.SaveToStream(MemStream);
          MemStream.Position := 0;
          TJvxRichEditStrings(Lines).EnableChange(False);
          try
            SendMessage(Handle, WM_SETTEXT, 0, 0);
            UpdateTextModes(Value);
            FPlainText := Value;
          finally
            TJvxRichEditStrings(Lines).EnableChange(True);
          end;
...
procedure TJvxCustomRichEdit.UpdateTextModes(Plain: Boolean);
const
...
procedure TJvxCustomRichEdit.CMColorChanged(var Message: TMessage);
begin
  inherited;
  SendMessage(Handle, EM_SETBKGNDCOLOR, 0, ColorToRGB(Color))
end;

procedure TJvxCustomRichEdit.EMReplaceSel(var Message: TMessage);
var
  CharRange: TCharRange;
begin
...
procedure TJvxCustomRichEdit.SetRichEditStrings(Value: TStrings);
begin
  FRichEditStrings.Assign(Value);
end;

procedure TJvxCustomRichEdit.CloseObjects;
var
  I: Integer;
  ReObject: TReObject;
begin
...
function TJvxCustomRichEdit.PasteSpecialDialog: Boolean;
...
function TJvxCustomRichEdit.InsertObjectDialog: Boolean;
var
...
function TJvxCustomRichEdit.ObjectPropertiesDialog: Boolean;
var
...
procedure TJvxCustomRichEdit.Print(const Caption: string);
var
  Range: TFormatRange;
...
procedure TJvxCustomRichEdit.WMPaint(var Message: TWMPaint);
var
  R, R1: TRect;
begin
  if RichEditVersion >= 2 then
...
procedure TJvxCustomRichEdit.WMDestroy(var Msg: TWMDestroy);
begin
  CloseObjects;
...
procedure TJvxCustomRichEdit.WMMouseMove(var Message: TMessage);
begin
  inherited;
end;

procedure TJvxCustomRichEdit.WMSetCursor(var Message: TWMSetCursor);
begin
  inherited;
end;

{$IFDEF Delphi5_Up}
procedure TJvxCustomRichEdit.WMRButtonUp(var Message: TMessage);
begin
...
procedure TJvxCustomRichEdit.CNNotify(var Message: TWMNotify);
var
  AMsg: TMessage;
begin
  with Message do
...
function TJvxCustomRichEdit.SaveClipboard(NumObj, NumChars: Integer): Boolean;
begin
  Result := True;
...
function TJvxCustomRichEdit.ProtectChange(const Message: TMessage; StartPos,
...
procedure TJvxCustomRichEdit.SelectionChange;
begin
  if Assigned(OnSelectionChange) then OnSelectionChange(Self);
end;

procedure TJvxCustomRichEdit.RequestSize(const Rect: TRect);
begin
  if Assigned(OnResizeRequest) then OnResizeRequest(Self, Rect);
end;

procedure TJvxCustomRichEdit.URLClick(const URLText: string; Button: TMouseButton);
begin
  if Assigned(OnURLClick) then OnURLClick(Self, URLText, Button);
end;

function TJvxCustomRichEdit.FindText(const SearchStr: string;
  StartPos, Length: Integer; Options: TRichSearchTypes): Integer;
var
  Find: TFindTextEx;
  Flags: Integer;
...
procedure TJvxCustomRichEdit.ClearUndo;
begin
  SendMessage(Handle, EM_EMPTYUNDOBUFFER, 0, 0);
end;

procedure TJvxCustomRichEdit.Redo;
begin
  SendMessage(Handle, EM_REDO, 0, 0);
end;

{$IFNDEF COMPILER35_UP}
procedure TJvxCustomRichEdit.Undo;
begin
  SendMessage(Handle, WM_UNDO, 0, 0);
end;
{$ENDIF}

procedure TJvxCustomRichEdit.StopGroupTyping;
begin
  if (RichEditVersion >= 2) and HandleAllocated then
...
{$IFDEF Delphi3_Up}
procedure TJvxCustomRichEdit.SetUIActive(Active: Boolean);
var
  Form: TCustomForm;
begin
  try
...
procedure TJvxCustomRichEdit.CMDocWindowActivate(var Message: TMessage);
begin
...
procedure TJvxCustomRichEdit.CMUIDeactivate(var Message: TMessage);
begin
...
procedure TJvxCustomRichEdit.SetupFindDialog(Dialog: TFindDialog;
...
function TJvxCustomRichEdit.FindDialog(const SearchStr: string): TFindDialog;
begin
...
function TJvxCustomRichEdit.ReplaceDialog(const SearchStr,
...
function TJvxCustomRichEdit.GetCanFindNext: Boolean;
begin
  Result := HandleAllocated and (FLastFind <> nil) and
    (FLastFind.FindText <> '');
end;

function TJvxCustomRichEdit.FindNext: Boolean;
begin
  if CanFindNext then Result := FindEditText(FLastFind, False, True)
  else Result := False;
end;

procedure TJvxCustomRichEdit.AdjustFindDialogPosition(Dialog: TFindDialog);
var
  TextRect, R: TRect;
begin
...
function TJvxCustomRichEdit.FindEditText(Dialog: TFindDialog; AdjustPos, Events: Boolean): Boolean;
var
...
procedure TJvxCustomRichEdit.TextNotFound(Dialog: TFindDialog);
begin
  with Dialog do
    if Assigned(FOnTextNotFound) then FOnTextNotFound(Self, FindText);
end;

procedure TJvxCustomRichEdit.FindDialogFind(Sender: TObject);
begin
  FindEditText(TFindDialog(Sender), True, True);
end;

procedure TJvxCustomRichEdit.ReplaceDialogReplace(Sender: TObject);
var
  Cnt: Integer;
...
{$IFDEF Delphi3_Up}
procedure TJvxCustomRichEdit.FindDialogClose(Sender: TObject);
begin
  CloseFindDialog(Sender as TFindDialog);
end;

procedure TJvxCustomRichEdit.CloseFindDialog(Dialog: TFindDialog);
begin
...
class procedure TJvxCustomRichEdit.RegisterConversionFormat(const AExtension: string;
  APlainText: Boolean; AConversionClass: TConversionClass);
begin
