...
The Original Code is: JvxFDualLst.PAS, released on 2002-07-04.
...
type
  TJvxDualListForm = class(TForm)
    SrcList: TJvxTextListBox;
    DstList: TJvxTextListBox;
    SrcLabel: TLabel;
    DstLabel: TLabel;
    IncBtn: TButton;
    IncAllBtn: TButton;
...
uses Consts, JvxVCLUtils, JvxBoxProcs;

{$R *.DFM}

{ TJvxDualListForm }

procedure TJvxDualListForm.CreateParams(var Params: TCreateParams);
begin
  inherited CreateParams(Params);
end;

procedure TJvxDualListForm.SetButtons;
var
...
function TJvxDualListForm.GetShowHelp: Boolean;
begin
  Result := (HelpBtn.Enabled) and (HelpBtn.Visible);
end;

procedure TJvxDualListForm.SetShowHelp(Value: Boolean);
const
  x_FrmBtn = 16;
  x_GrpBtn = 15;
  x_BtnBtn = 8;
...
procedure TJvxDualListForm.IncBtnClick(Sender: TObject);
begin
  BoxMoveSelectedItems(SrcList, DstList);
  SetButtons;
end;

procedure TJvxDualListForm.IncAllBtnClick(Sender: TObject);
begin
  BoxMoveAllItems(SrcList, DstList);
  SetButtons;
end;

procedure TJvxDualListForm.ExclBtnClick(Sender: TObject);
begin
  BoxMoveSelectedItems(DstList, SrcList);
  SetButtons;
end;

procedure TJvxDualListForm.ExclAllBtnClick(Sender: TObject);
begin
  BoxMoveAllItems(DstList, SrcList);
  SetButtons;
end;

procedure TJvxDualListForm.SrcListDragOver(Sender, Source: TObject; X,
  Y: Integer; State: TDragState; var Accept: Boolean);
begin
  BoxDragOver(SrcList, Source, X, Y, State, Accept, SrcList.Sorted);
  if State = dsDragLeave then
    (Source as TJvxTextListBox).DragCursor := crDrag;
  if (State = dsDragEnter) and ((Source as TJvxTextListBox).SelCount > 1) then
    (Source as TJvxTextListBox).DragCursor := crMultiDrag;
end;

procedure TJvxDualListForm.DstListDragOver(Sender, Source: TObject; X,
  Y: Integer; State: TDragState; var Accept: Boolean);
begin
...
procedure TJvxDualListForm.SrcListDragDrop(Sender, Source: TObject; X,
  Y: Integer);
begin
...
procedure TJvxDualListForm.DstListDragDrop(Sender, Source: TObject; X,
  Y: Integer);
begin
...
procedure TJvxDualListForm.SrcListKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var
  Incr: Integer;
begin
...
procedure TJvxDualListForm.DstListKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var
  Incr: Integer;
begin
...
procedure TJvxDualListForm.HelpBtnClick(Sender: TObject);
begin
  Application.HelpContext(HelpContext);
end;

procedure TJvxDualListForm.FormCreate(Sender: TObject);
begin
...
procedure TJvxDualListForm.ListClick(Sender: TObject);
begin
  SetButtons;
end;
