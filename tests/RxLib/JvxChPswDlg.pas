...
The Original Code is: JvxChPswDlg.PAS, released on 2002-07-04.
...
uses Consts, JvxDConst, JvxVCLUtils;
...
begin
  ...
  try
    Form := TJvxChPswdForm.Create(Application);
    try
...
procedure TJvxChPswdForm.CreateParams(var Params: TCreateParams);
begin
  inherited CreateParams(Params);
end;

procedure TJvxChPswdForm.FormCreate(Sender: TObject);
begin
...
procedure TJvxChPswdForm.ClearEdits;
begin
  OldPswd.Text := '';
  NewPswd.Text := '';
  ConfirmNewPswd.Text := '';
...
procedure TJvxChPswdForm.OkEnabled;
begin
  OkBtn.Enabled := FEnableEmpty or ((OldPswd.Text <> '') and (NewPswd.Text <> '')
    and (ConfirmNewPswd.Text <> ''));
end;

procedure TJvxChPswdForm.OkBtnClick(Sender: TObject);
type
...
procedure TJvxChPswdForm.FormShow(Sender: TObject);
begin
  ClearEdits;
end;

procedure TJvxChPswdForm.PswdChange(Sender: TObject);
begin
  OkEnabled;
end;
