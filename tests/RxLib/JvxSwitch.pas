...
The Original Code is: JvxSwitch.PAS, released on 2002-07-04.
...
{ TJvxSwitch }
  ...
  TJvxSwitch = class(TCustomControl)
  private
    FActive: Boolean;
...
{ TJvxSwitch component }

constructor TJvxSwitch.Create(AOwner: TComponent);
var
  I: Byte;
begin
...
procedure TJvxSwitch.CreateParams(var Params: TCreateParams);
begin
...
procedure TJvxSwitch.DefineProperties(Filer: TFiler);

{$IFDEF WIN32}
  function DoWrite: Boolean;
  begin
    if Assigned(Filer.Ancestor) then
      Result := FUserBitmaps <> TJvxSwitch(Filer.Ancestor).FUserBitmaps
...
function TJvxSwitch.GetPalette: HPALETTE;
begin
  if Enabled then Result := FBitmaps[FStateOn].Palette else Result := 0;
end;

procedure TJvxSwitch.ReadBinaryData(Stream: TStream);
begin
  Stream.ReadBuffer(FUserBitmaps, SizeOf(FUserBitmaps));
end;

procedure TJvxSwitch.WriteBinaryData(Stream: TStream);
begin
  Stream.WriteBuffer(FUserBitmaps, SizeOf(FUserBitmaps));
end;

function TJvxSwitch.StoreBitmap(Index: Integer): Boolean;
begin
  Result := Boolean(Index) in FUserBitmaps;
end;

function TJvxSwitch.GetSwitchGlyph(Index: Integer): TBitmap;
begin
  if csLoading in ComponentState then Include(FUserBitmaps, Boolean(Index));
  Result := FBitmaps[Boolean(Index)]
end;

procedure TJvxSwitch.CreateDisabled(Index: Integer);
begin
...
procedure TJvxSwitch.GlyphChanged(Sender: TObject);
var
  I: Boolean;
begin
...
procedure TJvxSwitch.SetSwitchGlyph(Index: Integer; Value: TBitmap);
begin
...
procedure TJvxSwitch.CMFocusChanged(var Message: TCMFocusChanged);
var
  Active: Boolean;
begin
...
procedure TJvxSwitch.CMEnabledChanged(var Message: TMessage);
begin
  inherited;
  Invalidate;
end;

procedure TJvxSwitch.CMTextChanged(var Message: TMessage);
begin
  inherited;
  Invalidate;
end;

procedure TJvxSwitch.CMDialogChar(var Message: TCMDialogChar);
begin
...
procedure TJvxSwitch.MouseDown(Button: TMouseButton;
...
procedure TJvxSwitch.KeyDown(var Key: Word; Shift: TShiftState);
begin
...
procedure TJvxSwitch.DoOn;
begin
  if Assigned(FOnOn) then FOnOn(Self);
end;

procedure TJvxSwitch.DoOff;
begin
  if Assigned(FOnOff) then FOnOff(Self);
end;

procedure TJvxSwitch.ToggleSwitch;
begin
  StateOn := not StateOn;
end;

procedure TJvxSwitch.SetBorderStyle(Value: TBorderStyle);
begin
...
procedure TJvxSwitch.SetStateOn(Value: Boolean);
begin
...
procedure TJvxSwitch.SetTextPosition(Value: TTextPos);
begin
...
procedure TJvxSwitch.SetShowFocus(Value: Boolean);
begin
