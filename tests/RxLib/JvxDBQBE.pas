...
The Original Code is: JvxDBQBE.PAS, released on 2002-07-04.
...
uses DBConsts, {$IFDEF Delphi3_Up} BDEConst, {$ENDIF} JvxDBUtils, JvxBdeUtils;
...
constructor TJvxQBEQuery.Create(AOwner: TComponent);
begin
...
destructor TJvxQBEQuery.Destroy;
begin
  Destroying;
  Disconnect;
  QBE.Free;
...
procedure TJvxQBEQuery.Disconnect;
begin
  Close;
  UnPrepare;
end;

procedure TJvxQBEQuery.RefreshQuery;
var
  Bookmark: TBookmark;
begin
  DisableControls;
...
procedure TJvxQBEQuery.SetPrepare(Value: Boolean);
begin
  if Value then Prepare
  else UnPrepare;
end;

procedure TJvxQBEQuery.Prepare;
begin
  SetDBFlag(dbfPrepared, True);
  SetPrepared(True);
end;

procedure TJvxQBEQuery.UnPrepare;
begin
  SetPrepared(False);
  SetDBFlag(dbfPrepared, False);
end;

procedure TJvxQBEQuery.SetStartParam(Value: Char);
begin
...
procedure TJvxQBEQuery.SetQuery(Value: TStrings);
begin
{$IFDEF WIN32}
...
procedure TJvxQBEQuery.QueryChanged(Sender: TObject);
var
  List: TParams;
begin
{$IFDEF Delphi4_Up}
...
procedure TJvxQBEQuery.SetParamsList(Value: TParams);
begin
  FParams.AssignValues(Value);
end;

{$IFDEF Delphi4_Up}
procedure TJvxQBEQuery.DefineProperties(Filer: TFiler);
begin
  inherited DefineProperties(Filer);
  Filer.DefineProperty('ParamData', ReadParamData, WriteParamData, True);
end;

procedure TJvxQBEQuery.ReadParamData(Reader: TReader);
begin
  Reader.ReadValue;
  Reader.ReadCollection(FParams);
end;

procedure TJvxQBEQuery.WriteParamData(Writer: TWriter);
begin
  Writer.WriteCollection(Params);
end;
{$ENDIF}

function TJvxQBEQuery.GetParamsCount: Word;
begin
  Result := FParams.Count;
end;

procedure TJvxQBEQuery.ReplaceParams(QBEText: TStrings);
...
procedure TJvxQBEQuery.SetPrepared(Value: Boolean);
var
  TempQBE: TStrings;
  AText: PChar;
begin
...
procedure TJvxQBEQuery.FreeStatement;
begin
...
function TJvxQBEQuery.ParamByName(const Value: string): TParam;
begin
  Result := FParams.ParamByName(Value);
end;

procedure TJvxQBEQuery.CreateParams(List: TParams; const Value: PChar);
var
...
{$IFNDEF Delphi3_Up}
function TJvxQBEQuery.IsEmpty: Boolean;
begin
  Result := IsDataSetEmpty(Self);
end;
{$ENDIF}

function TJvxQBEQuery.CreateCursor(GenHandle: Boolean): HDBICur;
begin
...
function TJvxQBEQuery.CreateHandle: HDBICur;
begin
  Result := CreateCursor(True)
end;

procedure TJvxQBEQuery.ExecQBE;
begin
  CheckInActive;
...
function TJvxQBEQuery.GetQueryCursor(GenHandle: Boolean): HDBICur;
var
  PCursor: phDBICur;
begin
  Result := nil;
...
{$IFDEF COMPILER35_UP}
function TJvxQBEQuery.SetDBFlag(Flag: Integer; Value: Boolean): Boolean;
{$ELSE}
procedure TJvxQBEQuery.SetDBFlag(Flag: Integer; Value: Boolean);
{$ENDIF}
var
...
procedure TJvxQBEQuery.PrepareQBE(Value: PChar);
begin
  GetStatementHandle(Value);
end;

procedure TJvxQBEQuery.GetStatementHandle(QBEText: PChar);
const
...
function TJvxQBEQuery.GetQBEText: PChar;
var
  BufLen: Word;
  I: Integer;
  StrEnd: PChar;
...
{$IFDEF WIN32}
function TJvxQBEQuery.GetRowsAffected: Integer;
var
  Length: Word;
begin
  if Prepared then
...
{ TJvxQBEQuery.IProviderSupport }

function TJvxQBEQuery.PSGetParams: TParams;
begin
  Result := Params;
end;

procedure TJvxQBEQuery.PSSetParams(AParams: TParams);
begin
...
procedure TJvxQBEQuery.PSExecute;
begin
  ExecQBE;
end;

procedure TJvxQBEQuery.PSSetCommandText(const CommandText: string);
begin
  if CommandText <> '' then
