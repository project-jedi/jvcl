...
The Original Code is: JvxMenus.PAS, released on 2002-07-04.
...
type
  TJvxMenuStyle = (msStandard, msOwnerDraw {$IFDEF WIN32}, msBtnLowered,
    msBtnRaised {$ENDIF});
...
{ TJvxMainMenu }
  ...
  TJvxMainMenu = class(TMainMenu)
  private
    FStyle: TJvxMenuStyle;
    FCanvas: TCanvas;
    FHook: TJvxWindowHook;
...
{$ENDIF}
    procedure SetStyle(Value: TJvxMenuStyle);
...
{$ENDIF}
  ...
  published
    ...
    property Style: TJvxMenuStyle read FStyle write SetStyle default msStandard;
...
{ TJvxPopupMenu }
  ...
  TJvxPopupMenu = class(TPopupMenu)
  private
    FStyle: TJvxMenuStyle;
    FCanvas: TCanvas;
    FShowCheckMarks: Boolean;
...
uses {$IFDEF WIN32} CommCtrl, {$ENDIF} Forms, ExtCtrls, Consts, JvxConst,
  JvxMaxMin, JvxVCLUtils, JvxClipIcon, JvxStrUtils;
...
function BtnStyle(MenuStyle: TJvxMenuStyle): TBtnStyle;
begin
{$IFDEF WIN32}
  case MenuStyle of
...
constructor TJvxMainMenu.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  FCanvas := TControlCanvas.Create;
  FShowCheckMarks := True;
  FHook := TJvxWindowHook.Create(Self);
...
destructor TJvxMainMenu.Destroy;
begin
{$IFDEF WIN32}
...
procedure TJvxMainMenu.Loaded;
begin
  inherited Loaded;
  if IsOwnerDrawMenu then RefreshMenu(True);
end;

function TJvxMainMenu.IsOwnerDrawMenu: Boolean;
begin
...
{$IFDEF WIN32}
procedure TJvxMainMenu.Notification(AComponent: TComponent; Operation: TOperation);
begin
...
procedure TJvxMainMenu.ImageListChange(Sender: TObject);
begin
  if Sender = FImages then RefreshMenu(IsOwnerDrawMenu);
end;

procedure TJvxMainMenu.SetImages(Value: TImageList);
var
...
procedure TJvxMainMenu.SetStyle(Value: TJvxMenuStyle);
begin
...
function TJvxMainMenu.FindForm: TWinControl;
begin
...
procedure TJvxMainMenu.Refresh;
begin
  RefreshMenu(IsOwnerDrawMenu);
end;

procedure TJvxMainMenu.RefreshMenu(AOwnerDraw: Boolean);
{$IFDEF Delphi4_Up}
begin
...
procedure TJvxMainMenu.DefaultDrawItem(Item: TMenuItem; Rect: TRect;
  State: TMenuOwnerDrawState);
var
  Graphic: TGraphic;
  BackColor: TColor;
...
procedure TJvxMainMenu.DrawItem(Item: TMenuItem; Rect: TRect;
  State: TMenuOwnerDrawState);
var
  Graphic: TGraphic;
  BackColor: TColor;
...
procedure TJvxMainMenu.MeasureItem(Item: TMenuItem; var Width, Height: Integer);
begin
  if Assigned(FOnMeasureItem) then FOnMeasureItem(Self, Item, Width, Height)
end;

procedure TJvxMainMenu.WndMessage(Sender: TObject; var AMsg: TMessage;
  var Handled: Boolean);
begin
  if IsOwnerDrawMenu then MenuWndMessage(Self, AMsg, Handled);
end;

procedure TJvxMainMenu.GetItemParams(Item: TMenuItem; State: TMenuOwnerDrawState;
...
{$IFDEF WIN32}
procedure TJvxMainMenu.GetImageIndex(Item: TMenuItem; State: TMenuOwnerDrawState;
...
procedure TJvxMainMenu.CMMenuChanged(var Message: TMessage);
begin
{$IFNDEF Delphi4_Up}
  if IsOwnerDrawMenu then RefreshMenu(True);
{$ENDIF}
...
procedure TJvxMainMenu.WMDrawItem(var Message: TWMDrawItem);
var
  State: TMenuOwnerDrawState;
  SaveIndex: Integer;
  Item: TMenuItem;
...
procedure TJvxMainMenu.WMMeasureItem(var Message: TWMMeasureItem);
var
  Item: TMenuItem;
  Graphic: TGraphic;
  BackColor: TColor;
...
procedure TJvxMainMenu.WMMenuSelect(var Message: TWMMenuSelect);
var
  MenuItem: TMenuItem;
...
const
  PopupList: TJvxPopupList = nil;

procedure TJvxPopupList.WndProc(var Message: TMessage);
var
  I: Integer;
  MenuItem: TMenuItem;
  FindKind: TFindItemKind;
    ...
    case Message.Msg of
      WM_MEASUREITEM, WM_DRAWITEM:
        for I := 0 to Count - 1 do begin
          Handled := False;
          TJvxPopupMenu(Items[I]).WndMessage(nil, Message, Handled);
          if Handled then Exit;
        end;
      WM_COMMAND:
        for I := 0 to Count - 1 do
          if TJvxPopupMenu(Items[I]).DispatchCommand(Message.wParam) then Exit;
      WM_INITMENUPOPUP:
        for I := 0 to Count - 1 do
          with TWMInitMenuPopup(Message) do
            if TJvxPopupMenu(Items[I]).DispatchPopup(MenuPopup) then Exit;
      WM_MENUSELECT:
        with TWMMenuSelect(Message) do begin
          ...
          for I := 0 to Count - 1 do begin
            MenuItem := TJvxPopupMenu(Items[I]).FindItem(ContextId, FindKind);
            if MenuItem <> nil then begin
{$IFNDEF WIN32}
              FMenuHelp := MenuItem.HelpContext;
{$ENDIF}
              Application.Hint := MenuItem.Hint;
              with TJvxPopupMenu(Items[I]) do
...
{$ELSE}
              ProcessMenuChar(TJvxPopupMenu(Items[I]), TWMMenuChar(Message));
{$ENDIF}
              Exit;
            end;
{$IFDEF WIN32}
      WM_HELP:
        with PHelpInfo(Message.LParam)^ do begin
          for I := 0 to Count - 1 do
            if TJvxPopupMenu(Items[I]).Handle = hItemHandle then begin
...
procedure TJvxPopupList.Add(Popup: TPopupMenu);
begin
  if Count = 0 then Window := Classes.AllocateHWnd(WndProc);
  inherited Add(Popup);
end;

procedure TJvxPopupList.Remove(Popup: TPopupMenu);
begin
...
constructor TJvxPopupMenu.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  if PopupList = nil then
    PopupList := TJvxPopupList.Create;
  FShowCheckMarks := True;
  FCanvas := TControlCanvas.Create;
  FCursor := crDefault;
  PopupList.Add(Self);
...
destructor TJvxPopupMenu.Destroy;
begin
{$IFDEF WIN32}
  FImageChangeLink.Free;
{$ENDIF}
...
procedure TJvxPopupMenu.Loaded;
begin
  inherited Loaded;
  if IsOwnerDrawMenu then RefreshMenu(True);
end;
...
{$IFDEF WIN32}
procedure TJvxPopupMenu.Notification(AComponent: TComponent; Operation: TOperation);
begin
  inherited Notification(AComponent, Operation);
  if Operation = opRemove then begin
    if AComponent = FImages then SetImages(nil);
...
procedure TJvxPopupMenu.ImageListChange(Sender: TObject);
begin
  if Sender = FImages then RefreshMenu(IsOwnerDrawMenu);
end;

procedure TJvxPopupMenu.SetImages(Value: TImageList);
var
  OldOwnerDraw: Boolean;
begin
  OldOwnerDraw := IsOwnerDrawMenu;
...
procedure TJvxPopupMenu.SetBiDiModeFromPopupControl;
var
  AControl: TControl;
begin
...
function TJvxPopupMenu.UseRightToLeftAlignment: Boolean;
var
  AControl: TControl;
begin
  Result := False;
...
procedure TJvxPopupMenu.Popup(X, Y: Integer);
const
{$IFDEF Delphi4_Up}
...
procedure TJvxPopupMenu.Refresh;
begin
  RefreshMenu(IsOwnerDrawMenu);
end;

function TJvxPopupMenu.IsOwnerDrawMenu: Boolean;
begin
  Result := (FStyle <> msStandard)
    {$IFDEF WIN32} or (Assigned(FImages) and (FImages.Count > 0)) {$ENDIF};
end;

procedure TJvxPopupMenu.RefreshMenu(AOwnerDraw: Boolean);
{$IFDEF Delphi4_Up}
begin
...
procedure TJvxPopupMenu.SetStyle(Value: TJvxMenuStyle);
begin
  if FStyle <> Value then begin
    FStyle := Value;
    RefreshMenu(IsOwnerDrawMenu);
...
procedure TJvxPopupMenu.DefaultDrawItem(Item: TMenuItem; Rect: TRect;
  State: TMenuOwnerDrawState);
var
  Graphic: TGraphic;
  BackColor: TColor;
...
procedure TJvxPopupMenu.DrawItem(Item: TMenuItem; Rect: TRect;
  State: TMenuOwnerDrawState);
var
  Graphic: TGraphic;
  BackColor: TColor;
...
procedure TJvxPopupMenu.MeasureItem(Item: TMenuItem; var Width, Height: Integer);
begin
  if Assigned(FOnMeasureItem) then FOnMeasureItem(Self, Item, Width, Height)
end;

procedure TJvxPopupMenu.WndMessage(Sender: TObject; var AMsg: TMessage;
  var Handled: Boolean);
begin
  if IsOwnerDrawMenu then MenuWndMessage(Self, AMsg, Handled);
end;

procedure TJvxPopupMenu.GetItemParams(Item: TMenuItem; State: TMenuOwnerDrawState;
  AFont: TFont; var Color: TColor; var Graphic: TGraphic; var NumGlyphs: Integer);
begin
  if Assigned(FOnGetItemParams) then
    FOnGetItemParams(Self, Item, State, AFont, Color, Graphic, NumGlyphs);
...
{$IFDEF WIN32}
procedure TJvxPopupMenu.GetImageIndex(Item: TMenuItem; State: TMenuOwnerDrawState;
  var ImageIndex: Integer);
begin
  if Assigned(FImages) and (Item <> nil) and (Item.Caption <> Separator) and
    Assigned(FOnGetImageIndex) then
...
procedure TJvxPopupMenu.DefaultDrawMargin(ARect: TRect; StartColor,
  EndColor: TColor);
var
  R: Integer;
begin
...
procedure TJvxPopupMenu.DrawMargin(ARect: TRect);
begin
...
procedure TJvxPopupMenu.WMDrawItem(var Message: TWMDrawItem);
var
  State: TMenuOwnerDrawState;
  SaveIndex: Integer;
  Item: TMenuItem;
...
procedure TJvxPopupMenu.WMMeasureItem(var Message: TWMMeasureItem);
var
  Item: TMenuItem;
  Graphic: TGraphic;
  BackColor: TColor;
