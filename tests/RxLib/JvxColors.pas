...
The Original Code is: JvxColors.PAS, released on 2002-07-04.
...
uses Classes, Controls, Graphics, Forms, JvxVCLUtils;
...
type
  TJvxColorProperty = class(TColorProperty)
  public
...
function TJvxColorProperty.GetValue: string;
var
  Color: TColor;
begin
...
procedure TJvxColorProperty.GetValues(Proc: TGetStrProc);
begin
  RxGetColorValues(Proc);
end;

procedure TJvxColorProperty.SetValue(const Value: string);
begin
  SetOrdValue(RxStringToColor(Value));
end;

procedure TJvxColorProperty.ListDrawValue(const Value: string; ACanvas: TCanvas;
...
procedure RegisterRxColors;
begin
  RegisterPropertyEditor(TypeInfo(TColor), TPersistent, '', TJvxColorProperty);
end;
