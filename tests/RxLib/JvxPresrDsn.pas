...
The Original Code is: JvxPresrDsn.PAS, released on 2002-07-04.
...
uses
  ...
  Buttons, ExtCtrls, JvxCtrls, JvxPlacemnt, JvxProps, Consts, RTLConsts, DesignIntf, DesignEditors, 
  VCLEditors, JvxVCLUtils;
...
{ TJvxFormPropsDlg }
  ...
  TJvxFormPropsDlg = class(TForm)
    ...
    StoredList: TJvxTextListBox;
    PropertiesList: TJvxTextListBox;
    ComponentsList: TJvxTextListBox;
    FormBox: TGroupBox;
...
{ TJvxFormStorageEditor }

  TJvxFormStorageEditor = class(TComponentEditor)
...
{ TJvxStoredPropsProperty }
  ...
  TJvxStoredPropsProperty = class(TClassProperty)
  public
...
{$IFDEF WIN32}
uses Windows, JvxBoxProcs, TypInfo, JvxLConst;
{$ELSE}
uses WinTypes, WinProcs, JvxStr16, JvxBoxProcs, TypInfo, JvxLConst;
{$ENDIF}
...
procedure TJvxFormStorageEditor.ExecuteVerb(Index: Integer);
var
  Storage: TJvxFormStorage;
  Opt: TPlacementOptions;
begin
  Storage := Component as TJvxFormStorage;
...
function TJvxFormStorageEditor.GetVerb(Index: Integer): string;
begin
  case Index of
...
function TJvxFormStorageEditor.GetVerbCount: Integer;
begin
  Result := 1;
end;
...
function TJvxStoredPropsProperty.GetAttributes: TPropertyAttributes;
begin
  Result := inherited GetAttributes + [paDialog] - [paSubProperties];
end;

function TJvxStoredPropsProperty.GetValue: string;
begin
  if TStrings(GetOrdValue).Count > 0 then Result := inherited GetValue
  else Result := ResStr(srNone);
end;

procedure TJvxStoredPropsProperty.Edit;
var
  Storage: TJvxFormStorage;
  Opt: TPlacementOptions;
begin
  Storage := GetComponent(0) as TJvxFormStorage;
  Opt := Storage.Options;
...
function ShowStorageDesigner(ACompOwner: TComponent; ADesigner: IDesigner;
  AStoredList: TStrings; var Options: TPlacementOptions): Boolean;
begin
  with TJvxFormPropsDlg.Create(Application) do
  try
...
procedure TJvxFormPropsDlg.ListToIndex(List: TCustomListBox; Idx: Integer);

  procedure SetItemIndex(Index: Integer);
  begin
    if TJvxTextListBox(List).MultiSelect then
      TJvxTextListBox(List).Selected[Index] := True;
...
procedure TJvxFormPropsDlg.UpdateCurrent;
var
  IdxProp: Integer;
  List: TStrings;
begin
...
procedure TJvxFormPropsDlg.DeleteProp(I: Integer);
var
...
function TJvxFormPropsDlg.FindProp(const CompName, PropName: string; var IdxComp,
  IdxProp: Integer): Boolean;
begin
  Result := False;
...
procedure TJvxFormPropsDlg.ClearLists;
var
  I: Integer;
begin
...
procedure TJvxFormPropsDlg.AddItem(IdxComp, IdxProp: Integer; AUpdate: Boolean);
var
  Idx: Integer;
  StrList: TStringList;
  CompName, PropName: string;
...
procedure TJvxFormPropsDlg.CheckAddItem(const CompName, PropName: string);
var
...
procedure TJvxFormPropsDlg.BuildLists(StoredProps: TStrings);
var
  I, J: Integer;
  C: TComponent;
  List: TJvxPropInfoList;
  StrList: TStrings;
  CompName, PropName: string;
begin
  ClearLists;
  if FCompOwner <> nil then begin
    for I := 0 to FCompOwner.ComponentCount - 1 do begin
      ...
      if (C is TJvxFormPlacement) or (C.Name = '') then Continue;
      List := TJvxPropInfoList.Create(C, tkProperties);
      try
        StrList := TStringList.Create;
        try
...
procedure TJvxFormPropsDlg.SetStoredList(AList: TStrings);
begin
  BuildLists(AList);
  if ComponentsList.Items.Count > 0 then
    ComponentsList.ItemIndex := 0;
...
procedure TJvxFormPropsDlg.CheckButtons;
var
  Enable: Boolean;
begin
...
procedure TJvxFormPropsDlg.AddButtonClick(Sender: TObject);
var
  I: Integer;
begin
...
procedure TJvxFormPropsDlg.ClearButtonClick(Sender: TObject);
begin
...
procedure TJvxFormPropsDlg.DeleteButtonClick(Sender: TObject);
begin
  DeleteProp(StoredList.ItemIndex);
end;

procedure TJvxFormPropsDlg.ListClick(Sender: TObject);
begin
  if Sender = ComponentsList then UpdateCurrent
  else CheckButtons;
end;

procedure TJvxFormPropsDlg.FormDestroy(Sender: TObject);
begin
  ClearLists;
end;

procedure TJvxFormPropsDlg.StoredListClick(Sender: TObject);
begin
  CheckButtons;
end;

procedure TJvxFormPropsDlg.UpBtnClick(Sender: TObject);
begin
...
procedure TJvxFormPropsDlg.DownBtnClick(Sender: TObject);
begin
  BoxMoveFocusedItem(StoredList, StoredList.ItemIndex + 1);
  if FDesigner <> nil then FDesigner.Modified;
  CheckButtons;
...
procedure TJvxFormPropsDlg.StoredListDragOver(Sender, Source: TObject; X,
...
procedure TJvxFormPropsDlg.StoredListDragDrop(Sender, Source: TObject; X,
  Y: Integer);
begin
...
procedure TJvxFormPropsDlg.PropertiesListDblClick(Sender: TObject);
begin
