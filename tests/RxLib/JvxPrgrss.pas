...
The Original Code is: JvxPrgrss.PAS, released on 2002-07-04.
...
constructor TJvxProgressList.Create;
begin
  inherited Create;
{$IFDEF WIN32}
...
destructor TJvxProgressList.Destroy;
var
  I: Integer;
begin
...
procedure TJvxProgressList.Add(AClass: TControlClass; const MaxPropName,
  MinPropName, ProgressPropName: string);
var
...
function TJvxProgressList.FindClass(AClass: TControlClass): Integer;
var
  P: PProgressData;
begin
...
procedure TJvxProgressList.Remove(AClass: TControlClass);
var
  I: Integer;
  P: PProgressData;
begin
...
function TJvxProgressList.SetControlProperty(Control: TControl;
...
const
  ProgressList: TJvxProgressList = nil;

function GeTJvxProgressList: TJvxProgressList;
begin
  if ProgressList = nil then ProgressList := TJvxProgressList.Create;
  Result := ProgressList;
end;
...
begin
  if Control <> nil then
    Result := GeTJvxProgressList.FindClass(TControlClass(Control.ClassType)) >= 0
  else Result := False;
end;
...
procedure RegisterProgressControl(AClass: TControlClass; const MaxPropName,
  MinPropName, ProgressPropName: string);
begin
  GeTJvxProgressList.Add(AClass, MaxPropName, MinPropName, ProgressPropName);
end;
...
procedure SetProgressMax(Control: TControl; MaxValue: Longint);
begin
  GeTJvxProgressList.SetControlProperty(Control, ppMax, MaxValue);
end;

procedure SetProgressMin(Control: TControl; MinValue: Longint);
begin
  GeTJvxProgressList.SetControlProperty(Control, ppMin, MinValue);
end;

procedure SetProgressValue(Control: TControl; ProgressValue: Longint);
begin
  GeTJvxProgressList.SetControlProperty(Control, ppProgress, ProgressValue);
end;
