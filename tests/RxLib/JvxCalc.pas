...
The Original Code is: JvxCalc.PAS, released on 2002-07-04.
...
uses Windows, SysUtils, Variants,
  ...
  ExtCtrls, Buttons, JvxCtrls, Clipbrd{, JvxComponent};

const
  DefCalcPrecision = 15;

type
  TJvxCalcState = (csFirst, csValid, csError);
  TJvxCalculatorForm = class;
...
{ TJvxCalculatorForm }
  ...
  TJvxCalculatorForm = class(TForm)
  private
    FMainPanel: TPanel;
    FCalcPanel: TPanel;
...
function CreateCalculatorForm(AOwner: TComponent; AHelpContext: THelpContext): TJvxCalculatorForm;
...
uses {$IFNDEF WIN32} JvxStr16, {$ENDIF} JvxVCLUtils, JvxMaxMin, JvxStrUtils, JvxToolEdit;
...
function CreateCalculatorForm(AOwner: TComponent; AHelpContext: THelpContext): TJvxCalculatorForm;
begin
  Result := TJvxCalculatorForm.Create(AOwner);
  with Result do
  try
...
type
  TJvxCalcButton = class(TJvxSpeedButton)
  private
    FKind: TCalcBtnKind;
...
constructor TJvxCalcButton.CreateKind(AOwner: TComponent; AKind: TCalcBtnKind);
begin
...
procedure TJvxCalcButton.CMParentFontChanged(var Message: TMessage);
...
function CreateCalcBtn(AParent: TWinControl; AKind: TCalcBtnKind;
  AOnClick: TNotifyEvent; ALayout: TCalcPanelLayout): TJvxCalcButton;
const
  BtnCaptions: array[cbSgn..cbMC] of PChar =
   ('±', ',', '/', '*', '-', '+', 'sqrt', '%', '1/x', '=', '<-', 'C',
    'MP', 'MS', 'MR', 'MC');
begin
  Result := TJvxCalcButton.CreateKind(AParent, AKind);
  with Result do
  try
...
type
  TJvxCalculatorPanel = class(TPanel)
  private
    FText: string;
    FStatus: TJvxCalcState;
    FOperator: Char;
    FOperand: Double;
    FMemory: Double;
    FPrecision: Byte;
...
constructor TJvxCalculatorPanel.CreateLayout(AOwner: TComponent;
...
procedure TJvxCalculatorPanel.SetText(const Value: string);
begin
...
procedure TJvxCalculatorPanel.SetDisplay(R: Double);
var
  S: string;
begin
...
function TJvxCalculatorPanel.GetDisplay: Double;
begin
  if FStatus = csError then Result := 0.0
  else Result := StrToFloat(Trim(FText));
end;

procedure TJvxCalculatorPanel.CheckFirst;
begin
...
procedure TJvxCalculatorPanel.CMCtl3DChanged(var Message: TMessage);
const
...
procedure TJvxCalculatorPanel.UpdateMemoryLabel;
begin
...
procedure TJvxCalculatorPanel.CalcKey(Key: Char);
var
  R: Double;
begin
  Key := UpCase(Key);
...
procedure TJvxCalculatorPanel.Clear;
begin
  FStatus := csFirst;
  SetDisplay(0.0);
  FOperator := '=';
...
procedure TJvxCalculatorPanel.CalcKeyPress(Sender: TObject; var Key: Char);
var
  Btn: TJvxSpeedButton;
begin
  Btn := FindButton(Key);
...
function TJvxCalculatorPanel.FindButton(Key: Char): TJvxSpeedButton;
const
...
begin
  ...
  if BtnTag >= 0 then
    for I := 0 to ControlCount - 1 do begin
      if Controls[I] is TJvxSpeedButton then begin
...
procedure TJvxCalculatorPanel.BtnClick(Sender: TObject);
begin
  case TJvxCalcButton(Sender).Kind of
...
procedure TJvxCalculatorPanel.Copy;
begin
  Clipboard.AsText := FText;
end;

procedure TJvxCalculatorPanel.Paste;
begin
...
type
  TJvxLocCalculator = class(TJvxCalculatorPanel)
  private
...
constructor TJvxLocCalculator.Create(AOwner: TComponent);
begin
...
procedure TJvxLocCalculator.CMEnabledChanged(var Message: TMessage);
begin
  if HandleAllocated and not (csDesigning in ComponentState) then
    EnableWindow(Handle, True);
end;

procedure TJvxLocCalculator.CreateParams(var Params: TCreateParams);
begin
...
type
  TJvxPopupCalculator = class(TJvxPopupWindow)
  private
    FCalcPanel: TJvxLocCalculator;
...
function CreatePopupCalculator(AOwner: TComponent
  {$IFDEF Delphi4_Up}; ABiDiMode: TBiDiMode = bdLeftToRight {$ENDIF}): TWinControl;
begin
  Result := TJvxPopupCalculator.Create(AOwner);
  ...
  begin { scale to screen res }
    ...
    TJvxPopupCalculator(Result).FCalcPanel.ParentFont := True;
    SetDefaultFont(TJvxPopupCalculator(Result).Font, clPopup);
{$IFDEF Delphi4_Up}
...
procedure SetupPopupCalculator(PopupCalc: TWinControl; APrecision: Byte;
  ABeepOnError: Boolean);
begin
  if (PopupCalc = nil) or not (PopupCalc is TJvxPopupCalculator) then
    Exit;
  if TJvxPopupCalculator(PopupCalc).FCalcPanel <> nil then
    with TJvxPopupCalculator(PopupCalc).FCalcPanel do begin
      FPrecision := Max(2, APrecision);
      FBeepOnError := ABeepOnError;
    end;
end;

constructor TJvxPopupCalculator.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  Height := 127;
  Width := 104;
  ...
  FCalcPanel := TJvxLocCalculator.Create(Self);
...
procedure TJvxPopupCalculator.KeyPress(var Key: Char);
begin
...
function TJvxPopupCalculator.GetValue: Variant;
begin
...
procedure TJvxPopupCalculator.SetValue(const Value: Variant);
begin
...
function TJvxPopupCalculator.GetValue: string;
var
  D: Double;
begin
...
procedure TJvxPopupCalculator.SetValue(const Value: string);
begin
...
function TJvxPopupCalculator.GetPopupText: string;
begin
  Result := FCalcPanel.Text;
end;

procedure TJvxPopupCalculator.ResultClick(Sender: TObject);
begin
  if FCalcPanel.FStatus <> csError then begin
    FCalcPanel.DisplayValue := FCalcPanel.DisplayValue;
    CloseUp(True);
...
procedure TJvxPopupCalculator.TextChange(Sender: TObject);
begin
  InvalidateEditor;
end;
...
begin
  if Assigned(FCalc) then
    Result := TJvxCalculatorPanel(FCalc.FCalcPanel).GetDisplay
  else Result := FValue;
end;
...
begin
  ...
  try
    Ctl3D := FCtl3D;
    Caption := Self.Title;
    TJvxCalculatorPanel(FCalcPanel).FMemory := Self.FMemory;
    TJvxCalculatorPanel(FCalcPanel).UpdateMemoryLabel;
    TJvxCalculatorPanel(FCalcPanel).FPrecision := Max(2, Self.Precision);
    TJvxCalculatorPanel(FCalcPanel).FBeepOnError := Self.BeepOnError;
    if Self.FValue <> 0 then begin
      TJvxCalculatorPanel(FCalcPanel).DisplayValue := Self.FValue;
      TJvxCalculatorPanel(FCalcPanel).FStatus := csFirst;
      TJvxCalculatorPanel(FCalcPanel).FOperator := '=';
    end;
    Result := (ShowModal = mrOk);
    if Result then begin
      Self.FMemory := TJvxCalculatorPanel(FCalcPanel).FMemory;
      if (TJvxCalculatorPanel(FCalcPanel).DisplayValue <> Self.FValue) then begin
        Self.FValue := TJvxCalculatorPanel(FCalcPanel).DisplayValue;
        Change;
      end;
    end;
  finally
...
constructor TJvxCalculatorForm.Create(AOwner: TComponent);
var
  Control: TWinControl;
  Popup: TPopupMenu;
...
{$ENDIF}
  ...
  { CalcPanel }
  FCalcPanel := TJvxCalculatorPanel.CreateLayout(Self, clDialog);
  with TJvxCalculatorPanel(FCalcPanel) do begin
    Align := alBottom;
    Parent := FMainPanel;
...
procedure TJvxCalculatorForm.CMCtl3DChanged(var Message: TMessage);
const
  Ctl3DBevel: array[Boolean] of TPanelBevel = (bvNone, bvLowered);
begin
  inherited;
...
procedure TJvxCalculatorForm.FormKeyPress(Sender: TObject; var Key: Char);
begin
  TJvxCalculatorPanel(FCalcPanel).CalcKeyPress(Sender, Key);
end;

procedure TJvxCalculatorForm.CopyItemClick(Sender: TObject);
begin
  TJvxCalculatorPanel(FCalcPanel).Copy;
end;

procedure TJvxCalculatorForm.PasteItemClick(Sender: TObject);
begin
  TJvxCalculatorPanel(FCalcPanel).Paste;
end;

procedure TJvxCalculatorForm.OkClick(Sender: TObject);
begin
  ModalResult := mrOk;
end;

procedure TJvxCalculatorForm.CancelClick(Sender: TObject);
begin
  ModalResult := mrCancel;
end;

procedure TJvxCalculatorForm.CalcKey(Sender: TObject; var Key: Char);
begin
  if (Owner <> nil) and (Owner is TJvxCalculator) then
    TJvxCalculator(Owner).CalcKey(Key);
end;

procedure TJvxCalculatorForm.DisplayChange(Sender: TObject);
begin
  if (Owner <> nil) and (Owner is TJvxCalculator) then
    TJvxCalculator(Owner).DisplayChange;
end;

procedure TJvxCalculatorForm.PopupMenuPopup(Sender: TObject);
begin
