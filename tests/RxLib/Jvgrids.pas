...
begin
  with TJvDrawGrid(Grid) do begin
...
{$IFDEF Delphi3_Up}
  if not SysLocale.FarEast then inherited
    ...
    ImeMode := imDontCare;
    inherited;
    if THandle(Message.WParam) <> TJvDrawGrid(Grid).Handle then
...
procedure TJvDrawGrid.WMCommand(var Message: TWMCommand);
begin
...
{$IFDEF WIN32}
procedure TJvDrawGrid.WMRButtonUp(var Message: TWMMouse);
begin
...
procedure TJvDrawGrid.WMLButtonDblClk(var Message: TWMLButtonDblClk);
var
  Cell: TGridCoord;
{$IFNDEF WIN32}
  Form: TForm;
...
procedure TJvDrawGrid.WMCancelMode(var Message: TMessage);
begin
  StopTracking;
  inherited;
end;
