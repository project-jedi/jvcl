...
The Original Code is: JvxDUALLIST.PAS, released on 2002-07-04.
...
uses Classes, Controls{, JvxComponent};

type

{ TJvxDualListDialog }

  TJvxDualListDialog = class(TComponent)
  private
    FCtl3D: Boolean;
    FSorted: Boolean;
    FTitle: String;
...
uses SysUtils, Forms, JvxFDualLst, Consts, JvxTConst, JvxVCLUtils;

{ TJvxDualListDialog }

constructor TJvxDualListDialog.Create(AOwner: TComponent);
begin
...
destructor TJvxDualListDialog.Destroy;
begin
  List1.Free;
  List2.Free;
...
function TJvxDualListDialog.GetTitle: string;
begin
  Result := FTitle;
end;

procedure TJvxDualListDialog.SetTitle(const ATitle: string);
begin
  FTitle := ATitle;
end;

procedure TJvxDualListDialog.SetList1(Value: TStrings);
begin
  FList1.Assign(Value);
end;

procedure TJvxDualListDialog.SetList2(Value: TStrings);
begin
  FList2.Assign(Value);
end;

function TJvxDualListDialog.IsLabel1Custom: Boolean;
begin
  Result := CompareStr(Label1Caption, LoadStr(SDualListSrcCaption)) <> 0;
end;

function TJvxDualListDialog.IsLabel2Custom: Boolean;
begin
  Result := CompareStr(Label2Caption, LoadStr(SDualListDestCaption)) <> 0;
end;

function TJvxDualListDialog.IsOkBtnCustom: Boolean;
begin
  Result := CompareStr(OkBtnCaption, ResStr(SOKButton)) <> 0;
end;

function TJvxDualListDialog.IsCancelBtnCustom: Boolean;
begin
  Result := CompareStr(CancelBtnCaption, ResStr(SCancelButton)) <> 0;
end;

function TJvxDualListDialog.IsHelpBtnCustom: Boolean;
begin
  Result := CompareStr(HelpBtnCaption, ResStr(SHelpButton)) <> 0;
end;

function TJvxDualListDialog.Execute: Boolean;
var
  Form: TJvxDualListForm;
begin
  Form := TJvxDualListForm.Create(Application);
  try
    with Form do begin
      Ctl3D := Self.Ctl3D;
{$IFDEF WIN32}
