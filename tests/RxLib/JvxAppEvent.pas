...
The Original Code is: JvxAppEvent.PAS, released on 2002-07-04.
...
uses SysUtils, {$IFDEF WIN32} Windows, {$ELSE} WinTypes, WinProcs, {$ENDIF}
  Messages, Classes, Graphics, Controls, Forms{, JvxComponent}
...
type
  TJvxAppEvents = class(TComponent)
  private
...
uses JvxAppUtils, JvxVCLUtils;

{ TJvxAppEventList }

type
  TJvxAppEventList = class(TObject)
  private
    FAppEvents: TList;
    FHooked: Boolean;
    FOnActivate: TNotifyEvent;
...
{$IFDEF Delphi4_Up}
    ...
    FOnShortCut: TShortCutEvent;
{$ENDIF}
    procedure AddEvents(App: TJvxAppEvents);
    procedure RemoveEvents(App: TJvxAppEvents);
...
constructor TJvxAppEventList.Create;
begin
  inherited Create;
  FAppEvents := TList.Create;
end;

destructor TJvxAppEventList.Destroy;
begin
  ClearEvents;
  FAppEvents.Free;
  inherited Destroy;
...
procedure TJvxAppEventList.ClearEvents;
begin
...
procedure TJvxAppEventList.AddEvents(App: TJvxAppEvents);
begin
...
procedure TJvxAppEventList.RemoveEvents(App: TJvxAppEvents);
begin
...
procedure TJvxAppEventList.DoActivate(Sender: TObject);
var
  I: Integer;
begin
  for I := FAppEvents.Count - 1 downto 0 do begin
    if Assigned(TJvxAppEvents(FAppEvents[I]).FOnActivate) then
      TJvxAppEvents(FAppEvents[I]).FOnActivate(Sender);
    if not TJvxAppEvents(FAppEvents[I]).Chained then Exit;
  end;
  if Assigned(FOnActivate) then FOnActivate(Sender);
end;

procedure TJvxAppEventList.DoDeactivate(Sender: TObject);
var
  I: Integer;
begin
  for I := FAppEvents.Count - 1 downto 0 do begin
    if Assigned(TJvxAppEvents(FAppEvents[I]).FOnDeactivate) then
      TJvxAppEvents(FAppEvents[I]).FOnDeactivate(Sender);
    if not TJvxAppEvents(FAppEvents[I]).Chained then Exit;
  end;
  if Assigned(FOnDeactivate) then FOnDeactivate(Sender);
end;

procedure TJvxAppEventList.DoException(Sender: TObject; E: Exception);
var
  I: Integer;
  Handled: Boolean;
begin
  Handled := False;
  for I := FAppEvents.Count - 1 downto 0 do begin
    if Assigned(TJvxAppEvents(FAppEvents[I]).FOnException) then begin
      TJvxAppEvents(FAppEvents[I]).FOnException(Sender, E);
      Handled := True;
    end;
    if not TJvxAppEvents(FAppEvents[I]).Chained then begin
...
procedure TJvxAppEventList.DoIdle(Sender: TObject; var Done: Boolean);
var
  I: Integer;
begin
  for I := FAppEvents.Count - 1 downto 0 do begin
    if Assigned(TJvxAppEvents(FAppEvents[I]).FOnIdle) then
      TJvxAppEvents(FAppEvents[I]).FOnIdle(Sender, Done);
    if not TJvxAppEvents(FAppEvents[I]).Chained then Exit;
  end;
  if Assigned(FOnIdle) then FOnIdle(Sender, Done);
end;

function TJvxAppEventList.DoHelp(Command: Word; Data: Longint;
  var CallHelp: Boolean): Boolean;
var
  I: Integer;
begin
  Result := False;
  for I := FAppEvents.Count - 1 downto 0 do begin
    if Assigned(TJvxAppEvents(FAppEvents[I]).FOnHelp) then
      Result := TJvxAppEvents(FAppEvents[I]).FOnHelp(Command, Data, CallHelp);
    if not TJvxAppEvents(FAppEvents[I]).Chained then Exit;
  end;
  if Assigned(FOnHelp) then Result := FOnHelp(Command, Data, CallHelp);
end;

procedure TJvxAppEventList.DoHint(Sender: TObject);
var
  I: Integer;
begin
  for I := FAppEvents.Count - 1 downto 0 do begin
    if Assigned(TJvxAppEvents(FAppEvents[I]).FOnHint) then
      TJvxAppEvents(FAppEvents[I]).FOnHint(Sender);
    if not TJvxAppEvents(FAppEvents[I]).Chained then Exit;
  end;
  if Assigned(FOnHint) then FOnHint(Sender);
end;

procedure TJvxAppEventList.DoMessage(var Msg: TMsg; var Handled: Boolean);
var
  I: Integer;
begin
  for I := FAppEvents.Count - 1 downto 0 do begin
    if Assigned(TJvxAppEvents(FAppEvents[I]).FOnMessage) then
      TJvxAppEvents(FAppEvents[I]).FOnMessage(Msg, Handled);
    if not TJvxAppEvents(FAppEvents[I]).Chained or Handled then Exit;
  end;
  if Assigned(FOnMessage) then FOnMessage(Msg, Handled);
end;

procedure TJvxAppEventList.DoMinimize(Sender: TObject);
var
  I: Integer;
begin
  for I := FAppEvents.Count - 1 downto 0 do begin
    if Assigned(TJvxAppEvents(FAppEvents[I]).FOnMinimize) then
      TJvxAppEvents(FAppEvents[I]).FOnMinimize(Sender);
    if not TJvxAppEvents(FAppEvents[I]).Chained then Exit;
  end;
  if Assigned(FOnMinimize) then FOnMinimize(Sender);
end;

procedure TJvxAppEventList.DoRestore(Sender: TObject);
var
  I: Integer;
begin
  for I := FAppEvents.Count - 1 downto 0 do begin
    if Assigned(TJvxAppEvents(FAppEvents[I]).FOnRestore) then
      TJvxAppEvents(FAppEvents[I]).FOnRestore(Sender);
    if not TJvxAppEvents(FAppEvents[I]).Chained then Exit;
  end;
  if Assigned(FOnRestore) then FOnRestore(Sender);
end;

procedure TJvxAppEventList.DoShowHint(var HintStr: string; var CanShow: Boolean;
  var HintInfo: THintInfo);
var
  I: Integer;
begin
  for I := FAppEvents.Count - 1 downto 0 do begin
    if Assigned(TJvxAppEvents(FAppEvents[I]).FOnShowHint) then
      TJvxAppEvents(FAppEvents[I]).FOnShowHint(HintStr, CanShow, HintInfo);
    if not TJvxAppEvents(FAppEvents[I]).Chained then Exit;
  end;
  if Assigned(FOnShowHint) then FOnShowHint(HintStr, CanShow, HintInfo);
end;

procedure TJvxAppEventList.DoActiveControlChange(Sender: TObject);
var
  I: Integer;
begin
  for I := FAppEvents.Count - 1 downto 0 do begin
    if Assigned(TJvxAppEvents(FAppEvents[I]).FOnActiveControlChange) then
      TJvxAppEvents(FAppEvents[I]).FOnActiveControlChange(Sender);
    if not TJvxAppEvents(FAppEvents[I]).Chained then Exit;
  end;
  if Assigned(FOnActiveControlChange) then FOnActiveControlChange(Sender);
end;

procedure TJvxAppEventList.DoActiveFormChange(Sender: TObject);
var
  I: Integer;
begin
  for I := FAppEvents.Count - 1 downto 0 do begin
    if Assigned(TJvxAppEvents(FAppEvents[I]).FOnActiveFormChange) then
      TJvxAppEvents(FAppEvents[I]).FOnActiveFormChange(Sender);
    if not TJvxAppEvents(FAppEvents[I]).Chained then Exit;
  end;
...
procedure TJvxAppEventList.DoActionExecute(Action: TBasicAction;
  var Handled: Boolean);
var
  I: Integer;
begin
  for I := FAppEvents.Count - 1 downto 0 do begin
    if Assigned(TJvxAppEvents(FAppEvents[I]).FOnActionExecute) then
      TJvxAppEvents(FAppEvents[I]).FOnActionExecute(Action, Handled);
    if not TJvxAppEvents(FAppEvents[I]).Chained or Handled then Exit;
  end;
  if Assigned(FOnActionExecute) then FOnActionExecute(Action, Handled);
end;

procedure TJvxAppEventList.DoActionUpdate(Action: TBasicAction;
  var Handled: Boolean);
var
  I: Integer;
begin
  for I := FAppEvents.Count - 1 downto 0 do begin
    if Assigned(TJvxAppEvents(FAppEvents[I]).FOnActionUpdate) then
      TJvxAppEvents(FAppEvents[I]).FOnActionUpdate(Action, Handled);
    if not TJvxAppEvents(FAppEvents[I]).Chained or Handled then Exit;
  end;
  if Assigned(FOnActionUpdate) then FOnActionUpdate(Action, Handled);
end;

procedure TJvxAppEventList.DoShortCut(var Msg: TWMKey; var Handled: Boolean);
var
  I: Integer;
begin
  for I := FAppEvents.Count - 1 downto 0 do begin
    if Assigned(TJvxAppEvents(FAppEvents[I]).FOnShortCut) then
      TJvxAppEvents(FAppEvents[I]).FOnShortCut(Msg, Handled);
    if not TJvxAppEvents(FAppEvents[I]).Chained or Handled then Exit;
  end;
...
const
  AppList: TJvxAppEventList = nil;

{ TJvxAppEvents }

constructor TJvxAppEvents.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  if AppList = nil then AppList := TJvxAppEventList.Create;
  FChained := True;
...
destructor TJvxAppEvents.Destroy;
begin
  if not (csDesigning in ComponentState) then
...
procedure TJvxAppEvents.Loaded;
begin
  inherited Loaded;
  UpdateAppProps;
end;

function TJvxAppEvents.GetCanvas: TCanvas;
begin
  if FCanvas = nil then FCanvas := TCanvas.Create;
  Result := FCanvas;
end;

procedure TJvxAppEvents.PaintIcon;
var
  PS: TPaintStruct;
begin
...
procedure TJvxAppEvents.SettingsChanged;
begin
  if Assigned(FOnSettingsChanged) then FOnSettingsChanged(Self);
end;

function TJvxAppEvents.MessageHook(var Msg: TMessage): Boolean;
begin
  Result := False;
  case Msg.Msg of
    WM_WININICHANGE:
...
procedure TJvxAppEvents.SetUpdateFormatSettings(Value: Boolean);
begin
...
function TJvxAppEvents.GetUpdateMetricSettings: Boolean;
begin
  if (csDesigning in ComponentState) then Result := FUpdateMetricSettings
  else Result := Application.UpdateMetricSettings;
end;

procedure TJvxAppEvents.SetUpdateMetricSettings(Value: Boolean);
begin
...
function TJvxAppEvents.GetMouseDragThreshold: Integer;
begin
  if (csDesigning in ComponentState) or (Mouse = nil) then
...
procedure TJvxAppEvents.SetMouseDragImmediate(Value: Boolean);
begin
...
procedure TJvxAppEvents.SetMouseDragThreshold(Value: Integer);
begin
...
procedure TJvxAppEvents.SetHintShortCuts(Value: Boolean);
begin
...
procedure TJvxAppEvents.SetBiDiMode(Value: TBiDiMode);
begin
  FBiDiMode := Value;
  if not (csDesigning in ComponentState) then
...
procedure TJvxAppEvents.SetNonBiDiKeyboard(const Value: string);
begin
...
procedure TJvxAppEvents.UpdateAppProps;
begin
