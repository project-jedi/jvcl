...
The Original Code is: JvxDataConv.PAS, released on 2002-07-04.
...
uses SysUtils, {$IFDEF WIN32} Windows, {$ELSE} WinTypes, WinProcs, {$ENDIF}
  Messages, Classes, Graphics, Controls, Forms, Dialogs, JvxDateUtil{, JvxComponent};
...
{ TJvxDateTimeFormat }
  ...
  TJvxDateTimeFormat = class(TPersistent)
  private
    FAMString: string[7];
    FPMString: string[7];
...
{ TJvxConverter }
  ...
  TJvxConverter = class(TComponent)
  private
    { Private declarations }
    FData: String;
    FTextValues: array[Boolean] of string[15];
    FDataType: TDataType;
    FDateTimeFormat: TJvxDateTimeFormat;
    FFloatFormat: TFloatFormat;
    FPrecision, FDigits: Integer;
    FRaiseOnError: Boolean;
    FOnChange: TNotifyEvent;
    procedure SetDataType(Value: TDataType);
    procedure SeTJvxDateTimeFormat(Value: TJvxDateTimeFormat);
    function GeTJvxDateTimeFormat: TJvxDateTimeFormat;
  ...
  published
    ...
    property DateTimeFormat: TJvxDateTimeFormat read GeTJvxDateTimeFormat write SeTJvxDateTimeFormat;
...
constructor TJvxDateTimeFormat.Create;
begin
  inherited Create;
  ResetDefault;
end;

destructor TJvxDateTimeFormat.Destroy;
begin
  inherited Destroy;
end;

procedure TJvxDateTimeFormat.ResetDefault;
begin
...
procedure TJvxDateTimeFormat.Assign(Source: TPersistent);
begin
  if Source is TJvxDateTimeFormat then begin
    FAMString := TJvxDateTimeFormat(Source).AMString;
    FPMString := TJvxDateTimeFormat(Source).PMString;
    FDateOrder := TJvxDateTimeFormat(Source).DateOrder;
    FTimeFormat := TJvxDateTimeFormat(Source).TimeFormat;
    FTimeSeparator := TJvxDateTimeFormat(Source).TimeSeparator;
    FDateSeparator := TJvxDateTimeFormat(Source).DateSeparator;
    FLongDate := TJvxDateTimeFormat(Source).LongDate;
    FFourDigitYear := TJvxDateTimeFormat(Source).FourDigitYear;
    FLeadingZero := TJvxDateTimeFormat(Source).LeadingZero;
    Exit;
  end;
  inherited Assign(Source);
end;

function TJvxDateTimeFormat.GetAMString: string;
begin
  Result := FAMString;
end;

procedure TJvxDateTimeFormat.SetAMString(const Value: string);
begin
  if Value = '' then FAMString := TimeAMString
  else FAMString := Value;
end;

function TJvxDateTimeFormat.GetPMString: string;
begin
  Result := FPMString;
end;

procedure TJvxDateTimeFormat.SetPMString(const Value: string);
begin
  if Value = '' then FPMString := TimePMString
  else FPMString := Value;
end;

function TJvxDateTimeFormat.GetDateMask: string;
var
...
function TJvxDateTimeFormat.GetTimeMask: string;
var
  S: array[1..3] of string[7];
  Separator: string[3];
  AMPM: string[16];
...
function TJvxDateTimeFormat.GetMask: string;
begin
...
constructor TJvxConverter.Create(AOwner: TComponent);
begin
  ...
  FPrecision := 15;
  FDigits := 2;
  FDateTimeFormat := TJvxDateTimeFormat.Create;
  FTextValues[False] := 'False';
  FTextValues[True] := 'True';
  FRaiseOnError := False;
end;

destructor TJvxConverter.Destroy;
begin
  FDataType := dtString;
...
procedure TJvxConverter.Change;
begin
  if Assigned(FOnChange) then FOnChange(Self);
end;

function TJvxConverter.GetString: string;
begin
  Result := FData;
end;

procedure TJvxConverter.SetString(const Value: string);
begin
  FData := Value;
end;

function TJvxConverter.GeTJvxDateTimeFormat: TJvxDateTimeFormat;
begin
  Result := FDateTimeFormat;
end;

procedure TJvxConverter.SeTJvxDateTimeFormat(Value: TJvxDateTimeFormat);
begin
  FDateTimeFormat.Assign(Value);
end;

function TJvxConverter.GetBoolValues(Index: Integer): string;
begin
  Result := FTextValues[Boolean(Index)];
end;

procedure TJvxConverter.SetBoolValues(Index: Integer; const Value: string);
begin
  FTextValues[Boolean(Index)] := Value;
end;

function TJvxConverter.BoolToStr(Value: Boolean): string;
begin
  Result := GetBoolValues(Integer(Value));
end;

function TJvxConverter.FloatToString(Value: Double): string;
begin
  Result := FloatToStrF(Value, FloatFormat, Precision, Digits);
end;

function TJvxConverter.DateTimeToString(Value: TDateTime): string;
begin
  case FDataType of
...
procedure TJvxConverter.SetDataType(Value: TDataType);
begin
...
function TJvxConverter.IsValidChar(Ch: Char): Boolean;
begin
  case FDataType of
...
procedure TJvxConverter.CheckDataType;
begin
  case FDataType of
...
function TJvxConverter.GetAsBoolean: Boolean;
var
  S: string[15];
begin
  S := GetString;
...
function TJvxConverter.GetDateTime: TDateTime;
var
  S: string;
  I: Integer;
...
function TJvxConverter.GetAsDateTime: TDateTime;
begin
  try
    Result := GetDateTime;
  except
...
function TJvxConverter.GetAsDate: TDateTime;
var
  Year, Month, Day: Word;
begin
  try
...
function TJvxConverter.GetAsTime: TDateTime;
var
...
function TJvxConverter.GetAsFloat: Double;
begin
  try
    case FDataType of
...
function TJvxConverter.GetAsInteger: Longint;
begin
  Result := Round(GetAsFloat);
end;

function TJvxConverter.GetAsString: string;
begin
  case FDataType of
...
procedure TJvxConverter.SetAsBoolean(Value: Boolean);
begin
  SetAsString(BoolToStr(Value));
end;

procedure TJvxConverter.SetAsDateTime(Value: TDateTime);
begin
  SetAsString(DateTimeToStr(Value));
end;

procedure TJvxConverter.SetAsDate(Value: TDateTime);
begin
  SetAsDateTime(Value);
end;

procedure TJvxConverter.SetAsTime(Value: TDateTime);
begin
  SetAsDateTime(Value);
end;

procedure TJvxConverter.SetAsFloat(Value: Double);
begin
...
procedure TJvxConverter.SetAsInteger(Value: Longint);
begin
  if FDataType = dtInteger then SetAsString(IntToStr(Value))
  else SetAsFloat(Value);
end;

procedure TJvxConverter.SetAsString(const Value: string);
var
  S: string;
begin
  S := GetString;
