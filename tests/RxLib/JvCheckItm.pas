...
uses {$IFDEF WIN32} Windows, {$ELSE} WinTypes, WinProcs, {$ENDIF} Messages,
  ...
  Dialogs, JvCtrls, ExtCtrls, RTLConsts, DesignIntf, DesignEditors, VCLEditors,
  CheckLst;
...
begin
  ...
  try
    ...
    if Comp is TJvCheckListBox then begin
      CheckList.AllowGrayed := TJvCheckListBox(Comp).AllowGrayed;
      CheckList.Sorted := TJvCheckListBox(Comp).Sorted;
      CheckList.CheckKind := TJvCheckListBox(Comp).CheckKind;
    end;
