...
The Original Code is: JvxDBUtils.PAS, released on 2002-07-04.
...
{ TJvxLocateObject }
  ...
  TJvxLocateObject = class(TObject)
  private
    FDataSet: TDataSet;
    FLookupField: TField;
    FLookupValue: string;
...
type
  TCreateLocateObject = function: TJvxLocateObject;
const
  CreateLocateObject: TCreateLocateObject = nil;
function CreateLocate(DataSet: TDataSet): TJvxLocateObject;
...
uses Forms, Controls, Dialogs, Consts, DBConsts, JvxDConst, JvxVCLUtils, JvxFileUtil,
  JvxAppUtils, JvxStrUtils, JvxMaxMin, {$IFNDEF Delphi3_Up} JvxBdeUtils, {$ENDIF}
  {$IFNDEF WIN32} JvxStr16, {$ENDIF} JvxDateUtil;
...
procedure TJvxLocateObject.SetDataSet(Value: TDataSet);
begin
  ActiveChanged;
  FDataSet := Value;
end;

function TJvxLocateObject.LocateFull: Boolean;
begin
  Result := False;
...
function TJvxLocateObject.LocateKey: Boolean;
begin
  Result := False;
end;

function TJvxLocateObject.FilterApplicable: Boolean;
begin
{$IFDEF Delphi3_Up}
...
function TJvxLocateObject.LocateFilter: Boolean;
{$IFDEF WIN32}
var
  SaveCursor: TCursor;
...
procedure TJvxLocateObject.CheckFieldType(Field: TField);
begin
end;

function TJvxLocateObject.Locate(const KeyField, KeyValue: string;
...
function TJvxLocateObject.UseKey: Boolean;
begin
  Result := False;
end;

procedure TJvxLocateObject.ActiveChanged;
begin
end;

function TJvxLocateObject.MatchesLookup(Field: TField): Boolean;
var
  Temp: string;
begin
  Temp := Field.AsString;
...
begin
  ...
  else Result := TJvxLocateObject.Create;
