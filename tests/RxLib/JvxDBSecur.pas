...
The Original Code is: JvxDBSecur.PAS, released on 2002-07-04.
...
uses SysUtils, {$IFDEF WIN32} Windows, {$ELSE} WinTypes, WinProcs, {$ENDIF}
  ...
  JvxLogin, JvxLoginDlg, JvxChPswDlg;
...
{ TJvxDBSecurity }
  ...
  TJvxDBSecurity = class(TJvxCustomLogin)
  private
    FDatabase: TDatabase;
...
constructor TJvxDBSecurity.Create(AOwner: TComponent);
begin
...
destructor TJvxDBSecurity.Destroy;
begin
  //DisposeStr(FLoginNameField);
  inherited Destroy;
end;

procedure TJvxDBSecurity.Notification(AComponent: TComponent; Operation: TOperation);
begin
  inherited Notification(AComponent, Operation);
  if (Operation = opRemove) and (AComponent = Database) then Database := nil;
end;

procedure TJvxDBSecurity.Loaded;
begin
  inherited Loaded;
...
procedure TJvxDBSecurity.SetDatabase(Value: TDatabase);
begin
...
procedure TJvxDBSecurity.SetUsersTableName(const Value: TFileName);
begin
  if FUsersTableName <> Value then FUsersTableName := Value;
end;

function TJvxDBSecurity.GetLoginNameField: string;
begin
  Result := FLoginNameField;
end;

procedure TJvxDBSecurity.SetLoginNameField(const Value: string);
begin
  FLoginNameField := Value;
end;

function TJvxDBSecurity.DoCheckUser(UsersTable: TTable; const UserName,
...
function TJvxDBSecurity.DoLogin(var UserName: string): Boolean;
var
...
function TJvxDBSecurity.ChangePassword: Boolean;
begin
