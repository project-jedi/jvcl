...
{ TJvSplitter }
  ...
  TJvSplitter = class(TCustomPanel)
  private
...
procedure TJvSplitter.Loaded;
begin
  inherited Loaded;
  UpdateState;
end;

procedure TJvSplitter.DefineProperties(Filer: TFiler); { for backward compatibility }
begin
  inherited DefineProperties(Filer);
  Filer.DefineProperty('LimitOffset', ReadOffset, WriteOffset, False);
end;

procedure TJvSplitter.ReadOffset(Reader: TReader);
var
  I: Integer;
begin
...
procedure TJvSplitter.WriteOffset(Writer: TWriter);
begin
  Writer.WriteInteger(FTopLeftLimit);
end;

procedure TJvSplitter.UpdateState;
begin
  inherited Cursor := Cursor;
end;

function TJvSplitter.FindControl: TControl;
var
  P: TPoint;
  I: Integer;
begin
...
procedure TJvSplitter.CheckPosition(var X, Y: Integer);
begin
...
procedure TJvSplitter.StartInverseRect;
var
  R: TRect;
  W: Integer;
begin
...
procedure TJvSplitter.EndInverseRect(X, Y: Integer; AllowChange,
  Apply: Boolean);
const
  DecSize = 3;
var
...
procedure TJvSplitter.MoveInverseRect(X, Y: Integer; AllowChange: Boolean);
var
  P: TPoint;
  NoDrop: Boolean;
begin
...
procedure TJvSplitter.ShowInverseRect(X, Y: Integer; Mode: TInverseMode);
var
  P: TPoint;
  MaxRect: TRect;
  Horiz: Boolean;
...
procedure TJvSplitter.DrawSizingLine(Split: TPoint);
var
  P: TPoint;
begin
  if FForm <> nil then begin
...
function TJvSplitter.GetStyle: TSplitterStyle;
begin
  Result := spUnknown;
...
procedure TJvSplitter.SetAlign(Value: TAlign);
begin
...
function TJvSplitter.GetAlign: TAlign;
begin
  Result := inherited Align;
end;

function TJvSplitter.GetCursor: TCursor;
begin
  Result := crDefault;
  case GetStyle of
...
procedure TJvSplitter.SetControlFirst(Value: TControl);
begin
...
procedure TJvSplitter.SetControlSecond(Value: TControl);
begin
...
procedure TJvSplitter.Notification(AComponent: TComponent; AOperation: TOperation);
begin
...
procedure TJvSplitter.Changed;
begin
  if Assigned(FOnPosChanged) then FOnPosChanged(Self);
end;

procedure TJvSplitter.Changing(X, Y: Integer; var AllowChange: Boolean);
begin
  if Assigned(FOnPosChanging) then FOnPosChanging(Self, X, Y, AllowChange);
end;

procedure TJvSplitter.StopSizing(X, Y: Integer; Apply: Boolean);
var
  AllowChange: Boolean;
begin
...
procedure TJvSplitter.ControlKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TJvSplitter.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
...
procedure TJvSplitter.MouseMove(Shift: TShiftState; X, Y: Integer);
var
  AllowChange: Boolean;
begin
...
procedure TJvSplitter.MouseUp(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
  StopSizing(X, Y, True);
