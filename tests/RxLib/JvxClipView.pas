...
The Original Code is: JvxClipView.PAS, released on 2002-07-04.
...
{ TJvxCustomClipboardViewer }
  ...
  TJvxCustomClipboardViewer = class(TScrollBox)
  private
  ...
  TJvxClipboardViewer = class(TJvxCustomClipboardViewer)
  published
{$IFDEF Delphi4_Up}
    property Anchors;
    property BiDiMode;
...
uses Grids, JvxClipIcon, JvxMaxMin, JvxTConst, {$IFNDEF WIN32} JvxStr16, {$ENDIF}
  JvxVCLUtils;
...
type
  TJvxPaletteGrid = class(TDrawGrid)
  private
...
constructor TJvxPaletteGrid.Create(AOwner: TComponent);
begin
...
destructor TJvxPaletteGrid.Destroy;
begin
  if FPalette <> 0 then DeleteObject(FPalette);
  inherited Destroy;
end;

procedure TJvxPaletteGrid.UpdateSize;
var
  Rows: Integer;
begin
...
function TJvxPaletteGrid.GetPalette: HPALETTE;
begin
  if FPalette <> 0 then Result := FPalette
  else Result := inherited GetPalette;
end;

procedure TJvxPaletteGrid.SetPalette(Value: HPALETTE);
var
  I: Integer;
...
function TJvxPaletteGrid.CellColor(ACol, ARow: Longint): TColor;
var
  PalIndex: Integer;
begin
...
procedure TJvxPaletteGrid.DrawSquare(CellColor: TColor; CellRect: TRect;
...
function TJvxPaletteGrid.SelectCell(ACol, ARow: Longint): Boolean;
begin
  Result := ((ACol = 0) and (ARow = 0)) or (CellColor(ACol, ARow) <> clNone);
end;

procedure TJvxPaletteGrid.DrawCell(ACol, ARow: Longint; ARect: TRect;
...
procedure TJvxPaletteGrid.WMSize(var Message: TWMSize);
begin
  inherited;
  UpdateSize;
end;
...
constructor TJvxCustomClipboardViewer.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
...
procedure TJvxCustomClipboardViewer.ForwardMessage(var Message: TMessage);
begin
  if FWndNext <> 0 then
    with Message do SendMessage(FWndNext, Msg, WParam, LParam);
end;

procedure TJvxCustomClipboardViewer.CreateWnd;
begin
  inherited CreateWnd;
...
procedure TJvxCustomClipboardViewer.DestroyWindowHandle;
begin
...
procedure TJvxCustomClipboardViewer.CreatePaintControl;
var
  Icon: TIcon;
...
begin
  ...
  case Format of
    ...
    cvPalette:
      begin
        FPaintControl := TJvxPaletteGrid.Create(Self);
        with TJvxPaletteGrid(FPaintControl) do
        try
          BorderStyle := bsNone;
          Parent := Self;
          Ctl3D := False;
...
function TJvxCustomClipboardViewer.GetClipboardFormatNames(Index: Integer): string;
begin
  Result := '';
...
procedure TJvxCustomClipboardViewer.Change;
begin
  if Assigned(FOnChange) then FOnChange(Self);
end;

procedure TJvxCustomClipboardViewer.WMSize(var Message: TMessage);
begin
  inherited;
...
procedure TJvxCustomClipboardViewer.WMChangeCBChain(var Message: TWMChangeCBChain);
begin
...
procedure TJvxCustomClipboardViewer.WMNCDestroy(var Message: TWMNCDestroy);
begin
  if FChained then begin
    ChangeClipboardChain(Handle, FWndNext);
    FChained := False;
...
procedure TJvxCustomClipboardViewer.WMDrawClipboard(var Message: TMessage);
var
  Format: Word;
begin
...
procedure TJvxCustomClipboardViewer.WMDestroyClipboard(var Message: TMessage);
begin
  FViewFormat := cvEmpty;
  Change;
  CreatePaintControl;
...
function TJvxCustomClipboardViewer.IsEmptyClipboard: Boolean;
begin
  Result := (Clipboard.FormatCount = 0);
end;

procedure TJvxCustomClipboardViewer.SetViewFormat(Value: TClipboardViewFormat);
var
  Format: Word;
begin
...
function TJvxCustomClipboardViewer.GetDrawFormat: TClipboardViewFormat;
...
class function TJvxCustomClipboardViewer.CanDrawFormat(ClipboardFormat: Word): Boolean;
begin
  Result := ClipboardFormatToView(ClipboardFormat) <> cvUnknown;
end;

function TJvxCustomClipboardViewer.ValidFormat(Format: TClipboardViewFormat): Boolean;
begin
