...
The Original Code is: JvxSelDSFrm.PAS, released on 2002-07-04.
...
uses Windows, SysUtils, Messages, Classes, Graphics, Controls, Forms, Dialogs, DB, StdCtrls,
  RTLConsts, DesignIntf, DesignEditors, VCLEditors, JvxDsgn;

type

{ TJvxSelectDataSetForm }

  TJvxSelectDataSetForm = class(TForm)
    GroupBox: TGroupBox;
...
{ TJvxMemDataSetEditor }

  TJvxMemDataSetEditor = class(TComponentEditor)
  private
...
uses DbConsts, TypInfo, JvxVCLUtils, JvxStrUtils, JvxLConst,
...
begin
  Result := nil;
  with TJvxSelectDataSetForm.Create(Application) do
  try
...
procedure TJvxMemDataSetEditor.BorrowStructure;
var
  DataSet: TDataSet;
  I: Integer;
  Caption: string;
...
function TJvxMemDataSetEditor.UniqueName(Field: TField): string;
const
...
procedure TJvxMemDataSetEditor.ExecuteVerb(Index: Integer);
begin
  case Index of
{$IFDEF Delphi5_Up}
...
function TJvxMemDataSetEditor.GetVerb(Index: Integer): string;
begin
  case Index of
...
function TJvxMemDataSetEditor.GetVerbCount: Integer;
begin
  Result := 2;
end;
...
procedure TJvxSelectDataSetForm.AddDataSet(const S: string);
begin
  if (S <> '') and (S <> FExclude) then DataSetList.Items.Add(S);
end;

procedure TJvxSelectDataSetForm.FillDataSetList(ExcludeDataSet: TDataSet);
{$IFNDEF WIN32}
var
  I: Integer;
...
procedure TJvxSelectDataSetForm.DataSetListDblClick(Sender: TObject);
begin
  if DataSetList.ItemIndex >= 0 then ModalResult := mrOk;
end;

procedure TJvxSelectDataSetForm.DataSetListKeyPress(Sender: TObject;
  var Key: Char);
begin
