...
The Original Code is: JvxPicClip.PAS, released on 2002-07-04.
...
uses Messages, Classes, Controls, Windows, RTLConsts, Graphics{, JvxComponent};
...
uses SysUtils, JvxVCLUtils, Consts, JvxConst;

{ TJvxPicClip }

constructor TJvxPicClip.Create(AOwner: TComponent);
begin
...
destructor TJvxPicClip.Destroy;
begin
  FOnChange := nil;
...
procedure TJvxPicClip.Assign(Source: TPersistent);
begin
  if Source is TJvxPicClip then begin
    with TJvxPicClip(Source) do begin
      Self.FRows := Rows;
      Self.FCols := Cols;
      Self.FMasked := Masked;
...
{$IFDEF WIN32}
type
  TJvxHack = class(TImageList);
{$ENDIF}

procedure TJvxPicClip.AssignTo(Dest: TPersistent);
{$IFDEF WIN32}
var
  I: Integer;
...
procedure TJvxPicClip.Changed;
begin
  if Assigned(FOnChange) then FOnChange(Self);
end;

function TJvxPicClip.GetIsEmpty: Boolean;
begin
  Result := (Picture.Graphic = nil) or Picture.Graphic.Empty;
end;

function TJvxPicClip.GetCount: Integer;
begin
  if IsEmpty then Result := 0
  else Result := Cols * Rows;
end;

procedure TJvxPicClip.Draw(Canvas: TCanvas; X, Y, Index: Integer);
var
  Image: TGraphic;
begin
...
procedure TJvxPicClip.DrawCenter(Canvas: TCanvas; Rect: TRect; Index: Integer);
var
  X, Y: Integer;
begin
...
procedure TJvxPicClip.LoadBitmapRes(Instance: THandle; ResID: PChar);
var
  Bmp: TBitmap;
begin
...
procedure TJvxPicClip.CheckIndex(Index: Integer);
begin
...
function TJvxPicClip.GetIndex(Col, Row: Cardinal): Integer;
begin
  Result := Col + (Row * Cols);
  if (Result >= Cols * Rows) or IsEmpty then Result := -1;
end;

function TJvxPicClip.GetCell(Col, Row: Cardinal): TBitmap;
begin
  Result := GetGraphicCell(GetIndex(Col, Row));
end;

function TJvxPicClip.GetGraphicCell(Index: Integer): TBitmap;
begin
  CheckIndex(Index);
...
function TJvxPicClip.GetDefaultMaskColor: TColor;
begin
  Result := clOlive;
...
function TJvxPicClip.GetHeight: Integer;
begin
  Result := Picture.Height div FRows;
end;

function TJvxPicClip.GetWidth: Integer;
begin
  Result := Picture.Width div FCols;
end;

function TJvxPicClip.IsMaskStored: Boolean;
begin
  Result := MaskColor <> GetDefaultMaskColor;
end;

procedure TJvxPicClip.SetMaskColor(Value: TColor);
begin
