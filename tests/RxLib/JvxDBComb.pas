...
The Original Code is: JvxDBComb.PAS, released on 2002-07-04.
...
{ TJvxCustomDBComboBox }

  TJvxCustomDBComboBox = class(TCustomComboBox)
  private
...
{ TJvxDBComboBox }
  ...
  TJvxDBComboBox = class(TJvxCustomDBComboBox)
  private
    FValues: TStrings;
...
constructor TJvxCustomDBComboBox.Create(AOwner: TComponent);
begin
...
destructor TJvxCustomDBComboBox.Destroy;
begin
{$IFDEF WIN32}
  FPaintControl.Free;
{$ENDIF}
...
procedure TJvxCustomDBComboBox.Loaded;
begin
  inherited Loaded;
  if (csDesigning in ComponentState) then DataChange(Self);
end;

procedure TJvxCustomDBComboBox.Notification(AComponent: TComponent;
...
procedure TJvxCustomDBComboBox.CreateWnd;
begin
  inherited CreateWnd;
  SetEditReadOnly;
end;

procedure TJvxCustomDBComboBox.DataChange(Sender: TObject);
begin
...
procedure TJvxCustomDBComboBox.UpdateData(Sender: TObject);
begin
  FDataLink.Field.Text := ComboText;
end;

procedure TJvxCustomDBComboBox.SetComboText(const Value: string);
var
  I: Integer;
  Redraw: Boolean;
begin
...
function TJvxCustomDBComboBox.GetComboText: string;
var
  I: Integer;
begin
...
procedure TJvxCustomDBComboBox.Change;
begin
  FDataLink.Edit;
  inherited Change;
  FDataLink.Modified;
...
procedure TJvxCustomDBComboBox.Click;
begin
  FDataLink.Edit;
  inherited Click;
  FDataLink.Modified;
...
procedure TJvxCustomDBComboBox.DropDown;
begin
{$IFNDEF WIN32}
  FDataLink.Edit;
{$ENDIF}
...
function TJvxCustomDBComboBox.GetDataSource: TDataSource;
begin
  Result := FDataLink.DataSource;
end;

procedure TJvxCustomDBComboBox.SetDataSource(Value: TDataSource);
begin
{$IFDEF Delphi4_Up}
...
function TJvxCustomDBComboBox.GetDataField: string;
begin
  Result := FDataLink.FieldName;
end;

procedure TJvxCustomDBComboBox.SetDataField(const Value: string);
begin
  FDataLink.FieldName := Value;
end;

function TJvxCustomDBComboBox.GetReadOnly: Boolean;
begin
  Result := FDataLink.ReadOnly;
end;

procedure TJvxCustomDBComboBox.SetReadOnly(Value: Boolean);
begin
  FDataLink.ReadOnly := Value;
end;

function TJvxCustomDBComboBox.GetField: TField;
begin
  Result := FDataLink.Field;
end;

procedure TJvxCustomDBComboBox.KeyDown(var Key: Word; Shift: TShiftState);
begin
...
procedure TJvxCustomDBComboBox.KeyPress(var Key: Char);
begin
...
procedure TJvxCustomDBComboBox.EditingChange(Sender: TObject);
begin
  SetEditReadOnly;
end;

procedure TJvxCustomDBComboBox.SetEditReadOnly;
begin
...
procedure TJvxCustomDBComboBox.WndProc(var Message: TMessage);
begin
...
procedure TJvxCustomDBComboBox.ComboWndProc(var Message: TMessage; ComboWnd: HWnd;
  ComboProc: Pointer);
begin
  if not (csDesigning in ComponentState) then
    case Message.Msg of
...
procedure TJvxCustomDBComboBox.CMExit(var Message: TCMExit);
begin
  try
...
{$IFDEF WIN32}
procedure TJvxCustomDBComboBox.CMGetDatalink(var Message: TMessage);
begin
  Message.Result := Longint(FDataLink);
end;

procedure TJvxCustomDBComboBox.WMPaint(var Message: TWMPaint);
var
  S: string;
  R: TRect;
  P: TPoint;
...
function TJvxCustomDBComboBox.GetPaintText: string;
begin
  if FDataLink.Field <> nil then Result := FDataLink.Field.Text
  else Result := '';
end;

procedure TJvxCustomDBComboBox.SetItems(const Value: TStrings);
begin
...
{$IFNDEF WIN32}
function TJvxCustomDBComboBox.GetStyle: TComboBoxStyle;
begin
  Result := inherited Style;
end;
{$ENDIF}

procedure TJvxCustomDBComboBox.SetStyle(Value: TComboBoxStyle);
begin
{$IFDEF WIN32}
...
{$IFDEF Delphi4_Up}
function TJvxCustomDBComboBox.UseRightToLeftAlignment: Boolean;
begin
  Result := DBUseRightToLeftAlignment(Self, Field);
end;

function TJvxCustomDBComboBox.ExecuteAction(Action: TBasicAction): Boolean;
begin
  Result := inherited ExecuteAction(Action) or (FDataLink <> nil) and
    FDataLink.ExecuteAction(Action);
end;

function TJvxCustomDBComboBox.UpdateAction(Action: TBasicAction): Boolean;
begin
...
constructor TJvxDBComboBox.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
...
destructor TJvxDBComboBox.Destroy;
begin
...
procedure TJvxDBComboBox.ValuesChanged(Sender: TObject);
begin
  if FEnableValues then DataChange(Self);
end;

function TJvxDBComboBox.GetPaintText: string;
var
  I: Integer;
begin
  Result := '';
...
function TJvxDBComboBox.GetComboText: string;
var
  I: Integer;
begin
...
procedure TJvxDBComboBox.SetComboText(const Value: string);
var
  I: Integer;
  Redraw: Boolean;
begin
...
procedure TJvxDBComboBox.SetEnableValues(Value: Boolean);
begin
...
procedure TJvxDBComboBox.SetValues(Value: TStrings);
begin
  FValues.Assign(Value);
end;

procedure TJvxDBComboBox.SetStyle(Value: TComboboxStyle);
begin
