...
uses SysUtils, Messages, Classes, Controls, Forms, Dialogs, StdCtrls,
  ExtCtrls, DB, DBTables, JvxDBLists, JvxLogin, JvxBdeUtils;
...
type
  ...
  TJvxDBLoginDialog = class
  private
    FDialog: TJvxLoginForm;
    FMode: TDialogMode;
...
uses {$IFDEF WIN32} Windows, Registry, BDE, {$ELSE} WinTypes, WinProcs,
  DbiTypes, {$ENDIF} IniFiles, Graphics, JvxAppUtils, JvxDConst, Consts,
  JvxVCLUtils, JvxConst;
...
constructor TJvxDBLoginDialog.Create(DialogMode: TDialogMode; DatabaseSelect: Boolean);
begin
  inherited Create;
  FMode := DialogMode;
...
destructor TJvxDBLoginDialog.Destroy;
begin
  FDialog.Free;
  inherited Destroy;
end;

procedure TJvxDBLoginDialog.OkBtnClick(Sender: TObject);
var
  Ok: Boolean;
...
procedure TJvxDBLoginDialog.FormShow(Sender: TObject);
var
  S: string;
begin
  if (FMode in [dmAppLogin, dmDBLogin]) and FSelectDatabase then begin
    with TJvxBDEItems.Create(FDialog) do
    try
{$IFDEF WIN32}
...
function TJvxDBLoginDialog.ExecuteAppLogin: Boolean;
var
  Ini: TObject;
begin
  try
...
function TJvxDBLoginDialog.ExecuteDbLogin(LoginParams: TStrings): Boolean;
{$IFDEF WIN32}
var
...
function TJvxDBLoginDialog.ExecuteUnlock: Boolean;
begin
...
function TJvxDBLoginDialog.Execute(LoginParams: TStrings): Boolean;
var
  SaveCursor: TCursor;
begin
...
function TJvxDBLoginDialog.GetUserName: string;
begin
...
function TJvxDBLoginDialog.CheckDatabaseChange: Boolean;
begin
...
procedure TJvxDBLoginDialog.FillParams(LoginParams: TStrings);
begin
...
procedure TJvxDBLoginDialog.Login(Database: TDatabase; LoginParams: TStrings);
begin
  FillParams(LoginParams);
end;

function TJvxDBLoginDialog.GetUserInfo: Boolean;
var
  Table: TTable;
begin
...
function TJvxDBLoginDialog.CheckUser(Table: TTable): Boolean;
begin
...
function TJvxDBLoginDialog.CheckUnlock: Boolean;
begin
...
procedure OnLoginDialog(Database: TDatabase; LoginParams: TStrings;
  AttemptNumber: Integer; ShowDBName: Boolean);
var
  Dlg: TJvxDBLoginDialog;
begin
  Dlg := TJvxDBLoginDialog.Create(dmDBLogin, False);
  try
...
var
  Dlg: TJvxDBLoginDialog;
begin
  Dlg := TJvxDBLoginDialog.Create(dmUnlock, False);
  try
...
var
  Dlg: TJvxDBLoginDialog;
begin
  Dlg := TJvxDBLoginDialog.Create(dmAppLogin, SelectDatabase);
  try
