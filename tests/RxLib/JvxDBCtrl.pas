...
The Original Code is: JvxDBCtrl.PAS, released on 2002-07-04.
...
uses 
  ...
  StdCtrls, Mask, IniFiles, JvxToolEdit, DB, DBGrids, 
  {$IFNDEF Delphi3_Up} DBTables, {$ENDIF}
  JvxPlacemnt, JvxDateUtil, DBCtrls, JvxCtrls, JvxCurrEdit;
...
{ TJvxDBComboEdit }
  ...
  TJvxDBComboEdit = class(TJvxCustomComboEdit)
  private
...
{ TJvxDBDateEdit }
  ...
  TJvxDBDateEdit = class(TJvxCustomDateEdit)
  private
    FDataLink: TFieldDataLink;
{$IFDEF WIN32}
    FCanvas: TControlCanvas;
...
{ TJvxDBCalcEdit }
  ...
  TJvxDBCalcEdit = class(TJvxCustomCalcEdit)
  private
    FDataLink: TFieldDataLink;
...
{ TJvxDBStatusLabel }
  ...
  TJvxDBStatusLabel = class(TJvxCustomLabel)
  private
    FDataLink: TDataLink;
    FDataSetName: String;
...
uses SysUtils, JvxStrUtils, Dialogs, ExtCtrls, DbConsts, JvxAppUtils, JvxVCLUtils,
  JvxDBUtils, {$IFNDEF Delphi3_Up} JvxBdeUtils, {$ENDIF} JvxPickDate, JvxCalc, JvxMaxMin,
  JvxDConst;
...
procedure GridInvalidateRow(Grid: TJvxDBGrid; Row: Longint);
var
  I: Longint;
begin
...
begin
  Result := BookmarksCompare(TJvxDBGrid(FGrid).Datalink.Dataset,
    Item1, Item2);
end;
...
begin
  ...
  Result := TJvxDBGrid(FGrid).Datalink.Dataset.GetBookmark;
end;
...
var
  I: Longint;
begin
  with TJvxDBGrid(FGrid).Datalink.Dataset do begin
    DisableControls;
    try
...
begin
  Result := False;
  with TJvxDBGrid(FGrid).DataLink.Dataset do
  try
    CheckBrowseMode;
...
begin
  ...
  ListChanged;
  TJvxDBGrid(FGrid).InvalidateRow(TJvxDBGrid(FGrid).Row);
  GridInvalidateRow(TJvxDBGrid(FGrid), TJvxDBGrid(FGrid).Row);
end;
...
constructor TJvxDBGrid.Create(AOwner: TComponent);
var
  Bmp: TBitmap;
begin
...
function TJvxDBGrid.GetImageIndex(Field: TField): Integer;
var
...
function TJvxDBGrid.ActiveRowSelected: Boolean;
var
{$IFDEF WIN32}
  Index: Integer;
{$ELSE}
...
function TJvxDBGrid.HighlightCell(DataCol, DataRow: Integer;
...
procedure TJvxDBGrid.SelectAll;
var
  ABookmark: TBookmark;
begin
  if MultiSelect and DataLink.Active then begin
...
procedure TJvxDBGrid.UnselectAll;
begin
...
procedure TJvxDBGrid.GotoSelection(Index: Longint);
begin
...
{$IFNDEF WIN32}
function TJvxDBGrid.GetIndicatorOffset: Byte;
begin
  Result := 0;
...
procedure TJvxDBGrid.LayoutChanged;
var
  ACol: Longint;
begin
  ACol := Col;
...
{$IFDEF WIN32}
procedure TJvxDBGrid.ColWidthsChanged;
var
  ACol: Longint;
begin
  ACol := Col;
...
function TJvxDBGrid.CreateEditor: TInplaceEdit;
begin
  Result := inherited CreateEditor;
  TEdit(Result).OnChange := EditChanged;
end;

function TJvxDBGrid.GetTitleOffset: Byte;
{$IFDEF Delphi4_Up}
var
  I, J: Integer;
{$ENDIF}
...
procedure TJvxDBGrid.SetColumnAttributes;
begin
  inherited SetColumnAttributes;
  SetFixedCols(FFixedCols);
end;

procedure TJvxDBGrid.SetFixedCols(Value: Integer);
var
  FixCount, I: Integer;
begin
...
function TJvxDBGrid.GetFixedCols: Integer;
begin
...
{$IFDEF Delphi4_Up}
function TJvxDBGrid.CalcLeftColumn: Integer;
begin
...
procedure TJvxDBGrid.KeyDown(var Key: Word; Shift: TShiftState);
var
...
procedure TJvxDBGrid.SetShowGlyphs(Value: Boolean);
begin
...
procedure TJvxDBGrid.SetRowsHeight(Value: Integer);
begin
...
function TJvxDBGrid.GetRowsHeight: Integer;
begin
...
function TJvxDBGrid.GetOptions: TDBGridOptions;
begin
...
procedure TJvxDBGrid.SetOptions(Value: TDBGridOptions);
var
...
procedure TJvxDBGrid.LinkActive(Value: Boolean);
begin
  SelectedRows.LinkActive(Value);
  inherited LinkActive(Value);
end;

function TJvxDBGrid.GetFixedColor: TColor;
begin
  Result := inherited TitleColor;
end;

procedure TJvxDBGrid.SetFixedColor(Value: TColor);
begin
...
procedure TJvxDBGrid.ColumnMoved(FromIndex, ToIndex: Longint);
begin
...
procedure TJvxDBGrid.SetTitleButtons(Value: Boolean);
begin
...
procedure TJvxDBGrid.SetMultiSelect(Value: Boolean);
begin
...
function TJvxDBGrid.GetStorage: TJvxFormPlacement;
begin
  Result := FIniLink.Storage;
end;

procedure TJvxDBGrid.SetStorage(Value: TJvxFormPlacement);
begin
  FIniLink.Storage := Value;
end;

function TJvxDBGrid.AcquireFocus: Boolean;
begin
  Result := True;
...
function TJvxDBGrid.CanEditShow: Boolean;
var
  F: TField;
begin
...
procedure TJvxDBGrid.GetCellProps(Field: TField; AFont: TFont;
...
procedure TJvxDBGrid.DoTitleClick(ACol: Longint; AField: TField);
begin
  if Assigned(FOnTitleBtnClick) then FOnTitleBtnClick(Self, ACol, AField);
end;

procedure TJvxDBGrid.CheckTitleButton(ACol, ARow: Longint; var Enabled: Boolean);
var
  Field: TField;
begin
...
procedure TJvxDBGrid.DisableScroll;
begin
  Inc(FDisableCount);
end;
...
procedure TJvxDBGrid.EnableScroll;
begin
...
function TJvxDBGrid.ScrollDisabled: Boolean;
begin
  Result := FDisableCount <> 0;
end;

procedure TJvxDBGrid.Scroll(Distance: Integer);
{$IFNDEF Delphi3_Up}
var
  IndicatorRect: TRect;
{$ENDIF}
...
function TJvxDBGrid.DoMouseWheelDown(Shift: TShiftState; MousePos: TPoint): Boolean;
begin
  Result := False;
...
function TJvxDBGrid.DoMouseWheelUp(Shift: TShiftState; MousePos: TPoint): Boolean;
begin
  Result := False;
...
procedure TJvxDBGrid.EditChanged(Sender: TObject);
begin
  if Assigned(FOnEditChange) then FOnEditChange(Self);
end;

procedure TJvxDBGrid.TopLeftChanged;
begin
...
procedure TJvxDBGrid.StopTracking;
begin
...
procedure TJvxDBGrid.TrackButton(X, Y: Integer);
var
  Cell: TGridCoord;
  NewPressed: Boolean;
  I, Offset: Integer;
...
procedure TJvxDBGrid.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
var
  Cell: TGridCoord;
...
procedure TJvxDBGrid.MouseMove(Shift: TShiftState; X, Y: Integer);
begin
  if FTracking then TrackButton(X, Y);
  inherited MouseMove(Shift, X, Y);
end;

procedure TJvxDBGrid.MouseUp(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
var
  Cell: TGridCoord;
  ACol: Longint;
...
{$IFDEF WIN32}
procedure TJvxDBGrid.WMRButtonUp(var Message: TWMMouse);
begin
...
procedure TJvxDBGrid.WMCancelMode(var Message: TMessage);
begin
  StopTracking;
  inherited;
end;
...
procedure TJvxDBGrid.WMChar(var Msg: TWMChar);
  ...
  begin
    ...
    if (Form <> nil) and TForm(Form).KeyPreview and
      TJvxHack(Form).DoKeyPress(Msg) then Exit;
    with Msg do begin
...
procedure TJvxDBGrid.KeyPress(var Key: Char);
begin
...
procedure TJvxDBGrid.DefaultDataCellDraw(const Rect: TRect; Field: TField;
  State: TGridDrawState);
begin
...
{$IFDEF WIN32}
function TJvxDBGrid.GetMasterColumn(ACol, ARow: Longint): TColumn;
begin
...
procedure TJvxDBGrid.DrawCell(ACol, ARow: Longint; ARect: TRect;
...
{$IFDEF WIN32}
procedure TJvxDBGrid.DrawColumnCell(const Rect: TRect; DataCol: Integer;
  Column: TColumn; State: TGridDrawState);
{$ELSE}
procedure TJvxDBGrid.DrawDataCell(const Rect: TRect; Field: TField;
  State: TGridDrawState);
{$ENDIF}
var
  I: Integer;
...
procedure TJvxDBGrid.MouseToCell(X, Y: Integer; var ACol, ARow: Longint);
var
  Coord: TGridCoord;
begin
...
procedure TJvxDBGrid.SaveColumnsLayout(IniFile: TObject;
...
procedure TJvxDBGrid.RestoreColumnsLayout(IniFile: TObject;
  const Section: string);
type
  TColumnInfo = record
    Column: TColumn;
...
procedure TJvxDBGrid.SaveLayoutReg(IniFile: TRegIniFile);
begin
  InternalSaveLayout(IniFile, '');
end;

procedure TJvxDBGrid.RestoreLayoutReg(IniFile: TRegIniFile);
begin
...
procedure TJvxDBGrid.InternalSaveLayout(IniFile: TObject;
  const Section: string);
begin
...
procedure TJvxDBGrid.InternalRestoreLayout(IniFile: TObject;
  const Section: string);
begin
...
procedure TJvxDBGrid.SaveLayout(IniFile: TIniFile);
begin
  InternalSaveLayout(IniFile, '');
end;

procedure TJvxDBGrid.RestoreLayout(IniFile: TIniFile);
begin
  InternalRestoreLayout(IniFile, '');
end;

procedure TJvxDBGrid.IniSave(Sender: TObject);
var
  Section: string;
begin
...
procedure TJvxDBGrid.IniLoad(Sender: TObject);
var
  Section: string;
begin
  if (Name <> '') and (FIniLink.IniObject <> nil) then begin
...
procedure ResetMaxLength(DBEdit: TJvxDBComboEdit);
var
  F: TField;
begin
  with DBEdit do
...
constructor TJvxDBComboEdit.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
{$IFDEF WIN32}
...
destructor TJvxDBComboEdit.Destroy;
begin
  FDataLink.Free;
  FDataLink := nil;
{$IFDEF WIN32}
...
procedure TJvxDBComboEdit.Loaded;
begin
  inherited Loaded;
  ResetMaxLength(Self);
...
procedure TJvxDBComboEdit.Notification(AComponent: TComponent;
...
procedure TJvxDBComboEdit.KeyDown(var Key: Word; Shift: TShiftState);
begin
  inherited KeyDown(Key, Shift);
...
procedure TJvxDBComboEdit.KeyPress(var Key: Char);
begin
  inherited KeyPress(Key);
...
function TJvxDBComboEdit.EditCanModify: Boolean;
begin
  Result := FDataLink.Edit;
end;

procedure TJvxDBComboEdit.Reset;
begin
  FDataLink.Reset;
  SelectAll;
end;

procedure TJvxDBComboEdit.SetFocused(Value: Boolean);
begin
...
procedure TJvxDBComboEdit.Change;
begin
  FDataLink.Modified;
  inherited Change;
end;

function TJvxDBComboEdit.GetDataSource: TDataSource;
begin
  Result := FDataLink.DataSource;
end;

procedure TJvxDBComboEdit.SetDataSource(Value: TDataSource);
begin
{$IFDEF Delphi4_Up}
...
function TJvxDBComboEdit.GetDataField: string;
begin
  Result := FDataLink.FieldName;
end;

procedure TJvxDBComboEdit.SetDataField(const Value: string);
begin
  if not (csDesigning in ComponentState) then ResetMaxLength(Self);
  FDataLink.FieldName := Value;
end;

function TJvxDBComboEdit.GetReadOnly: Boolean;
begin
  Result := FDataLink.ReadOnly;
end;

procedure TJvxDBComboEdit.SetReadOnly(Value: Boolean);
begin
  FDataLink.ReadOnly := Value;
end;

function TJvxDBComboEdit.GetField: TField;
begin
  Result := FDataLink.Field;
end;

procedure TJvxDBComboEdit.DataChange(Sender: TObject);
begin
...
procedure TJvxDBComboEdit.EditingChange(Sender: TObject);
begin
  inherited ReadOnly := not FDataLink.Editing;
end;

procedure TJvxDBComboEdit.UpdateData(Sender: TObject);
begin
  ValidateEdit;
  FDataLink.Field.Text := Text;
end;

procedure TJvxDBComboEdit.WMPaste(var Message: TMessage);
begin
  FDataLink.Edit;
  inherited;
end;

procedure TJvxDBComboEdit.WMCut(var Message: TMessage);
begin
  FDataLink.Edit;
  inherited;
end;

procedure TJvxDBComboEdit.CMEnter(var Message: TCMEnter);
begin
  SetFocused(True);
  inherited;
{$IFDEF Delphi3_Up}
...
procedure TJvxDBComboEdit.CMExit(var Message: TCMExit);
begin
  try
...
{$IFDEF WIN32}
procedure TJvxDBComboEdit.WMPaint(var Message: TWMPaint);
var
  S: string;
begin
...
procedure TJvxDBComboEdit.CMGetDataLink(var Message: TMessage);
begin
...
{$IFDEF Delphi4_Up}
function TJvxDBComboEdit.UseRightToLeftAlignment: Boolean;
begin
  Result := DBUseRightToLeftAlignment(Self, Field);
end;

function TJvxDBComboEdit.ExecuteAction(Action: TBasicAction): Boolean;
begin
  Result := inherited ExecuteAction(Action) or (FDataLink <> nil) and
    FDataLink.ExecuteAction(Action);
end;

function TJvxDBComboEdit.UpdateAction(Action: TBasicAction): Boolean;
begin
...
constructor TJvxDBDateEdit.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
{$IFDEF WIN32}
  ControlStyle := ControlStyle + [csReplicatable];
...
destructor TJvxDBDateEdit.Destroy;
begin
  FDataLink.Free;
  FDataLink := nil;
{$IFDEF WIN32}
...
procedure TJvxDBDateEdit.AfterPopup(Sender: TObject; var Date: TDateTime;
  var Action: Boolean);
begin
...
procedure TJvxDBDateEdit.Notification(AComponent: TComponent;
  Operation: TOperation);
begin
  inherited Notification(AComponent, Operation);
  if (Operation = opRemove) and (FDataLink <> nil) and
...
procedure TJvxDBDateEdit.KeyDown(var Key: Word; Shift: TShiftState);
begin
  inherited KeyDown(Key, Shift);
...
procedure TJvxDBDateEdit.KeyPress(var Key: Char);
begin
  inherited KeyPress(Key);
  if (Key in [#32..#255]) and (FDataLink.Field <> nil) and
...
function TJvxDBDateEdit.EditCanModify: Boolean;
begin
  Result := FDataLink.Edit;
end;

procedure TJvxDBDateEdit.Reset;
begin
  FDataLink.Reset;
  SelectAll;
end;

procedure TJvxDBDateEdit.Change;
begin
  if not Formatting then FDataLink.Modified;
  inherited Change;
end;

function TJvxDBDateEdit.GetDataSource: TDataSource;
begin
  Result := FDataLink.DataSource;
end;

procedure TJvxDBDateEdit.SetDataSource(Value: TDataSource);
begin
{$IFDEF Delphi4_Up}
  if not (FDataLink.DataSourceFixed and (csLoading in ComponentState)) then
{$ENDIF}
...
function TJvxDBDateEdit.GetDataField: string;
begin
  Result := FDataLink.FieldName;
end;

procedure TJvxDBDateEdit.SetDataField(const Value: string);
begin
  FDataLink.FieldName := Value;
end;

function TJvxDBDateEdit.GetReadOnly: Boolean;
begin
  Result := FDataLink.ReadOnly;
end;

procedure TJvxDBDateEdit.SetReadOnly(Value: Boolean);
begin
  FDataLink.ReadOnly := Value;
end;

function TJvxDBDateEdit.GetField: TField;
begin
  Result := FDataLink.Field;
end;

procedure TJvxDBDateEdit.UpdateMask;
begin
  UpdateFormat;
  UpdatePopup;
  DataChange(nil);
...
procedure TJvxDBDateEdit.DataChange(Sender: TObject);
begin
  if FDataLink.Field <> nil then begin
...
procedure TJvxDBDateEdit.EditingChange(Sender: TObject);
begin
  inherited ReadOnly := not FDataLink.Editing;
...
procedure TJvxDBDateEdit.UpdateData(Sender: TObject);
var
  D: TDateTime;
begin
  ValidateEdit;
...
{$IFDEF WIN32}
procedure TJvxDBDateEdit.CMGetDataLink(var Message: TMessage);
begin
  Message.Result := Integer(FDataLink);
end;

procedure TJvxDBDateEdit.WMPaint(var Message: TWMPaint);
var
  S: string;
begin
...
procedure TJvxDBDateEdit.AcceptValue(const Value: Variant);
begin
...
procedure TJvxDBDateEdit.ApplyDate(Value: TDateTime);
begin
  FDataLink.Edit;
  inherited ApplyDate(Value);
end;

procedure TJvxDBDateEdit.WMPaste(var Message: TMessage);
begin
  FDataLink.Edit;
  inherited;
end;

procedure TJvxDBDateEdit.WMCut(var Message: TMessage);
begin
  FDataLink.Edit;
  inherited;
end;

procedure TJvxDBDateEdit.CMEnter(var Message: TCMEnter);
begin
  inherited;
end;

procedure TJvxDBDateEdit.CMExit(var Message: TCMExit);
begin
  try
...
{$IFDEF Delphi4_Up}
function TJvxDBDateEdit.UseRightToLeftAlignment: Boolean;
begin
  Result := DBUseRightToLeftAlignment(Self, Field);
end;

function TJvxDBDateEdit.ExecuteAction(Action: TBasicAction): Boolean;
begin
  Result := inherited ExecuteAction(Action) or (FDataLink <> nil) and
    FDataLink.ExecuteAction(Action);
end;

function TJvxDBDateEdit.UpdateAction(Action: TBasicAction): Boolean;
begin
  Result := inherited UpdateAction(Action) or (FDataLink <> nil) and
    FDataLink.UpdateAction(Action);
end;
...
constructor TJvxDBCalcEdit.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
{$IFDEF WIN32}
  ControlStyle := ControlStyle + [csReplicatable];
...
destructor TJvxDBCalcEdit.Destroy;
begin
  FDataLink.Free;
  FDataLink := nil;
  inherited Destroy;
...
procedure TJvxDBCalcEdit.Notification(AComponent: TComponent;
  Operation: TOperation);
begin
  inherited Notification(AComponent, Operation);
  if (Operation = opRemove) and (FDataLink <> nil) and
...
procedure TJvxDBCalcEdit.KeyDown(var Key: Word; Shift: TShiftState);
begin
  inherited KeyDown(Key, Shift);
  if not ReadOnly and ((Key = VK_DELETE) or ((Key = VK_INSERT)
...
procedure TJvxDBCalcEdit.KeyPress(var Key: Char);
begin
  inherited KeyPress(Key);
  case Key of
    ^H, ^V, ^X, #32..#255:
...
function TJvxDBCalcEdit.IsValidChar(Key: Char): Boolean;
begin
...
procedure TJvxDBCalcEdit.UpdatePopup;
var
  Precision: Byte;
begin
...
function TJvxDBCalcEdit.EditCanModify: Boolean;
begin
  Result := FDataLink.Edit;
end;

{$IFDEF WIN32}
function TJvxDBCalcEdit.GetDisplayText: string;
var
  E: Extended;
begin
  if (csPaintCopy in ControlState) and (FDatalink.Field <> nil) then begin
...
procedure TJvxDBCalcEdit.Reset;
begin
  FDataLink.Reset;
  inherited Reset;
end;

procedure TJvxDBCalcEdit.Change;
begin
  if not Formatting then FDataLink.Modified;
  inherited Change;
end;

function TJvxDBCalcEdit.GetDataSource: TDataSource;
begin
  Result := FDataLink.DataSource;
end;

procedure TJvxDBCalcEdit.SetDataSource(Value: TDataSource);
begin
...
function TJvxDBCalcEdit.GetDataField: string;
begin
  Result := FDataLink.FieldName;
end;

procedure TJvxDBCalcEdit.SetDataField(const Value: string);
begin
...
procedure TJvxDBCalcEdit.SetDefaultParams(Value: Boolean);
begin
...
procedure TJvxDBCalcEdit.UpdateFieldParams;
begin
  if FDatalink.Field <> nil then begin
...
function TJvxDBCalcEdit.GetReadOnly: Boolean;
begin
  Result := FDataLink.ReadOnly;
end;

procedure TJvxDBCalcEdit.SetReadOnly(Value: Boolean);
begin
  FDataLink.ReadOnly := Value;
end;

function TJvxDBCalcEdit.GetField: TField;
begin
  Result := FDataLink.Field;
end;

procedure TJvxDBCalcEdit.DataChange(Sender: TObject);
begin
  if FDefaultParams then UpdateFieldParams;
  if FDataLink.Field <> nil then begin
    if FDataLink.Field.IsNull then begin
...
procedure TJvxDBCalcEdit.EditingChange(Sender: TObject);
begin
  inherited ReadOnly := not FDataLink.Editing;
end;

procedure TJvxDBCalcEdit.UpdateFieldData(Sender: TObject);
begin
...
{$IFDEF WIN32}
procedure TJvxDBCalcEdit.CMGetDataLink(var Message: TMessage);
begin
  Message.Result := Integer(FDataLink);
end;

procedure TJvxDBCalcEdit.AcceptValue(const Value: Variant);
begin
  if VarIsNull(Value) or VarIsEmpty(Value) then FDataLink.Field.Clear
...
procedure TJvxDBCalcEdit.WMPaste(var Message: TMessage);
begin
  FDataLink.Edit;
  inherited;
end;

procedure TJvxDBCalcEdit.WMCut(var Message: TMessage);
begin
  FDataLink.Edit;
  inherited;
end;

procedure TJvxDBCalcEdit.CMEnter(var Message: TCMEnter);
begin
  inherited;
end;

procedure TJvxDBCalcEdit.CMExit(var Message: TCMExit);
begin
  try
    CheckRange;
    FDataLink.UpdateRecord;
...
{$IFDEF Delphi4_Up}
function TJvxDBCalcEdit.UseRightToLeftAlignment: Boolean;
begin
  Result := DBUseRightToLeftAlignment(Self, Field);
end;

function TJvxDBCalcEdit.ExecuteAction(Action: TBasicAction): Boolean;
begin
  Result := inherited ExecuteAction(Action) or (FDataLink <> nil) and
    FDataLink.ExecuteAction(Action);
end;

function TJvxDBCalcEdit.UpdateAction(Action: TBasicAction): Boolean;
begin
  Result := inherited UpdateAction(Action) or (FDataLink <> nil) and
    FDataLink.UpdateAction(Action);
end;
...
type
  TJvxStatusDataLink = class(TDataLink)
  private
    FLabel: TJvxDBStatusLabel;
  protected
    ...
    procedure LayoutChanged; override;
  public
    constructor Create(ALabel: TJvxDBStatusLabel);
    destructor Destroy; override;
  end;

constructor TJvxStatusDataLink.Create(ALabel: TJvxDBStatusLabel);
begin
  inherited Create;
  FLabel := ALabel;
end;

destructor TJvxStatusDataLink.Destroy;
begin
  FLabel := nil;
  inherited Destroy;
end;

procedure TJvxStatusDataLink.ActiveChanged;
begin
  DataSetChanged;
end;

procedure TJvxStatusDataLink.DataSetScrolled(Distance: Integer);
begin
  if (FLabel <> nil) and (FLabel.Style = lsRecordNo) then
    FLabel.UpdateStatus;
end;

procedure TJvxStatusDataLink.EditingChanged;
begin
  if (FLabel <> nil) and (FLabel.Style <> lsRecordSize) then
    FLabel.UpdateStatus;
end;

procedure TJvxStatusDataLink.DataSetChanged;
begin
  if (FLabel <> nil) then FLabel.UpdateData;
end;

procedure TJvxStatusDataLink.LayoutChanged;
begin
  if (FLabel <> nil) and (FLabel.Style <> lsRecordSize) then
    DataSetChanged; { ??? }
end;
...
constructor TJvxDBStatusLabel.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  ShadowSize := 0;
  Layout := tlCenter;
  ...
  FDataLink := TJvxStatusDataLink.Create(Self);
  FStyle := lsState;
...
destructor TJvxDBStatusLabel.Destroy;
begin
  FDataLink.Free;
  FDataLink := nil;
...
procedure TJvxDBStatusLabel.Loaded;
begin
  inherited Loaded;
  UpdateData;
end;

function TJvxDBStatusLabel.GetDefaultFontColor: TColor;
begin
...
function TJvxDBStatusLabel.GetLabelCaption: string;
begin
...
function TJvxDBStatusLabel.GetDatasetState: TDataSetState;
begin
...
procedure TJvxDBStatusLabel.SetName(const Value: TComponentName);
begin
  inherited SetName(Value);
  if (csDesigning in ComponentState) then Invalidate;
end;

procedure TJvxDBStatusLabel.SetCaptions(Value: TStrings);
begin
  FCaptions.Assign(Value);
end;

function TJvxDBStatusLabel.GetStatusKind(State: TDataSetState): TDBStatusKind;
begin
{$IFDEF WIN32}
...
function TJvxDBStatusLabel.GetCaption(State: TDataSetState): string;
const
...
procedure TJvxDBStatusLabel.Paint;
var
  GlyphOrigin: TPoint;
begin
  inherited Paint;
...
procedure TJvxDBStatusLabel.CaptionsChanged(Sender: TObject);
begin
  TStringList(FCaptions).OnChange := nil;
  try
...
procedure TJvxDBStatusLabel.UpdateData;
...
procedure TJvxDBStatusLabel.UpdateStatus;
begin
...
procedure TJvxDBStatusLabel.Notification(AComponent: TComponent;
  Operation: TOperation);
begin
  inherited Notification(AComponent, Operation);
  if (Operation = opRemove) and (FDataLink <> nil) and
...
function TJvxDBStatusLabel.GetDataSetName: string;
begin
  Result := FDataSetName;
  if not (csDesigning in ComponentState) then begin
...
procedure TJvxDBStatusLabel.SetDataSetName(Value: string);
begin
  FDataSetName := Value;
  Invalidate;
end;

function TJvxDBStatusLabel.GetDataSource: TDataSource;
begin
  Result := FDataLink.DataSource;
end;

procedure TJvxDBStatusLabel.SetDataSource(Value: TDataSource);
begin
{$IFDEF Delphi4_Up}
  if not (FDataLink.DataSourceFixed and (csLoading in ComponentState)) then
{$ENDIF}
...
procedure TJvxDBStatusLabel.SetEditColor(Value: TColor);
begin
...
procedure TJvxDBStatusLabel.SetGlyphAlign(Value: TGlyphAlign);
begin
...
procedure TJvxDBStatusLabel.SetShowOptions(Value: TDBLabelOptions);
begin
...
procedure TJvxDBStatusLabel.SetCalcCount(Value: Boolean);
begin
...
procedure TJvxDBStatusLabel.SetStyle(Value: TDBLabelStyle);
begin
