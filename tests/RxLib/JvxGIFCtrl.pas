...
The Original Code is: JvxGIFCtrl.PAS, released on 2002-07-04.
...
uses Messages, {$IFDEF WIN32} Windows, {$ELSE} WinTypes, WinProcs, {$ENDIF}
  ...
  JvxAnimate, JvxGIF, JvxTimer;

type

{ TJvxGIFAnimator }

  TJvxGIFAnimator = class(TJvxImageControl)
  private
    FAnimate: Boolean;
    FImage: TGIFImage;
    FTimer: TJvxTimer;
...
uses JvxVCLUtils, JvxMaxMin, JvxGraph;
...
constructor TJvxGIFAnimator.Create(AOwner: TComponent);
begin
...
destructor TJvxGIFAnimator.Destroy;
begin
  Destroying;
  FOnStart := nil;
  FOnStop := nil;
...
procedure TJvxGIFAnimator.AdjustSize;
begin
...
{$IFDEF Delphi4_Up}
function TJvxGIFAnimator.CanAutoSize(var NewWidth, NewHeight: Integer): Boolean;
begin
  Result := True;
...
function TJvxGIFAnimator.GetDelayTime(Index: Integer): Cardinal;
begin
...
function TJvxGIFAnimator.GetFrameBitmap(Index: Integer;
...
function TJvxGIFAnimator.GetPalette: HPALETTE;
begin
  Result := 0;
  if not FImage.Empty then Result := FImage.Palette;
end;

procedure TJvxGIFAnimator.ImageChanged(Sender: TObject);
begin
  Lock;
  try
    FCacheIndex := -1;
...
procedure TJvxGIFAnimator.SetImage(Value: TGIFImage);
begin
  Lock;
  try
    FImage.Assign(Value);
...
procedure TJvxGIFAnimator.SetCenter(Value: Boolean);
begin
...
procedure TJvxGIFAnimator.SetStretch(Value: Boolean);
begin
...
procedure TJvxGIFAnimator.SetTransparent(Value: Boolean);
begin
...
procedure TJvxGIFAnimator.SetFrameIndex(Value: Integer);
begin
...
procedure TJvxGIFAnimator.DoPaintImage;
var
  Frame: TBitmap;
  Dest: TRect;
  TransColor: TColor;
...
procedure TJvxGIFAnimator.Paint;
begin
  PaintImage;
  if FImage.Transparent or FImage.Empty then
    PaintDesignRect;
...
procedure TJvxGIFAnimator.TimerDeactivate;
var
  F: TCustomForm;
begin
  SetAnimate(False);
...
procedure TJvxGIFAnimator.TimerExpired(Sender: TObject);
begin
{$IFDEF Delphi3_Up}
...
procedure TJvxGIFAnimator.Change;
begin
  if Assigned(FOnChange) then FOnChange(Self);
end;

procedure TJvxGIFAnimator.FrameChanged;
begin
  if Assigned(FOnFrameChanged) then FOnFrameChanged(Self);
end;

procedure TJvxGIFAnimator.Stop;
begin
  if Assigned(FOnStop) then FOnStop(Self);
end;

procedure TJvxGIFAnimator.Start;
begin
  if Assigned(FOnStart) then FOnStart(Self);
end;

{$IFNDEF Delphi4_Up}
procedure TJvxGIFAnimator.SetAutoSize(Value: Boolean);
begin
...
{$IFDEF Delphi3_Up}
procedure TJvxGIFAnimator.SetAsyncDrawing(Value: Boolean);
begin
...
procedure TJvxGIFAnimator.SetAnimate(Value: Boolean);
begin
...
procedure TJvxGIFAnimator.WMSize(var Message: TWMSize);
begin
  inherited;
{$IFNDEF Delphi4_Up}
  AdjustSize;
