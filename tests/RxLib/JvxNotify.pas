...
The Original Code is: JvxNotify.PAS, released on 2002-07-04.
...
uses Windows, SysUtils, Classes, Messages, ExtCtrls{, JvxComponent};
...
type
  ...
  TJvxNotifyThread = class;

{ TJvxFolderMonitor }

  TJvxFolderMonitor = class(TComponent)
  private
    FNotifyThread: TJvxNotifyThread;
...
{ TJvxNotifyThread }
  ...
  TJvxNotifyThread = class(TThread)
  private
...
function CreateNotifyThread(const FolderName: string; WatchSubtree: Boolean;
  Filter: TFileChangeFilters): TJvxNotifyThread;
...
uses Forms, JvxVCLUtils, JvxFileUtil;
...
constructor TJvxNotifyThread.Create(const FolderName: string;
...
destructor TJvxNotifyThread.Destroy;
begin
  FOnChange := nil;
  StopWaiting;
  inherited Destroy;
...
procedure TJvxNotifyThread.Terminate;
begin
  inherited Terminate;
  StopWaiting;
end;

procedure TJvxNotifyThread.CallOnChange;
begin
  if Assigned(FOnChange) then FOnChange(Self);
end;

procedure TJvxNotifyThread.DoChange;
begin
  if Assigned(FOnChange) then Synchronize(CallOnChange);
end;

procedure TJvxNotifyThread.DoTerminate;
begin
  if FNotifyHandle <> INVALID_HANDLE_VALUE then
...
procedure TJvxNotifyThread.Execute;
var
...
procedure TJvxNotifyThread.StopWaiting;
begin
...
function CreateNotifyThread(const FolderName: string; WatchSubtree: Boolean;
  Filter: TFileChangeFilters): TJvxNotifyThread;
begin
  Result := TJvxNotifyThread.Create(FolderName, WatchSubtree, Filter);
  try
...
constructor TJvxFolderMonitor.Create(AOwner: TComponent);
begin
...
destructor TJvxFolderMonitor.Destroy;
begin
...
procedure TJvxFolderMonitor.Loaded;
begin
  inherited Loaded;
  try
...
function TJvxFolderMonitor.GetActive: Boolean;
begin
  Result := FNotifyThread <> nil;
end;

procedure TJvxFolderMonitor.SetActive(Value: Boolean);
begin
...
procedure TJvxFolderMonitor.SetFilter(Value: TFileChangeFilters);
var
...
procedure TJvxFolderMonitor.SetMonitorSubtree(Value: Boolean);
begin
...
procedure TJvxFolderMonitor.SetFolderName(const Value: string);
begin
...
procedure TJvxFolderMonitor.FreeNotifyThread;
begin
...
procedure TJvxFolderMonitor.ResetNotifyThread(Activate: Boolean);
begin
  FreeNotifyThread;
...
function TJvxFolderMonitor.GetDelayTime: Cardinal;
begin
  if FDelayTimer <> nil then
...
procedure TJvxFolderMonitor.SetDelayTime(Value: Cardinal);
begin
...
procedure TJvxFolderMonitor.ThreadNotification(Sender: TObject);
begin
...
procedure TJvxFolderMonitor.Timer(Sender: TObject);
begin
  FDelayTimer.Free;
  FDelayTimer := nil;
  Changed;
...
procedure TJvxFolderMonitor.Changed;
begin
  if Assigned(FOnChange) then FOnChange(Self);
end;
