...
The Original Code is: JvxAniFile.PAS, released on 2002-07-04.
...
type
  TJvxIconFrame = class(TPersistent)
  private
    FIcon: TIcon;
    FIsIcon: Boolean;
    FTag: TAniTag;
...
{ TJvxAnimatedCursorImage }

  TANINAME = array[0..255] of Char;

  TJvxAnimatedCursorImage = class(TPersistent)
  private
    FHeader: TAniHeader;
    FTitle: TANINAME;
    FCreator: TANINAME;
    ...
    function GetFrame(Index: Integer): TJvxIconFrame;
  ...
  public
    ...
    property Frames[Index: Integer]: TJvxIconFrame read GetFrame;
...
uses Consts, JvxVCLUtils, JvxMaxMin, JvxGraph, JvxIcoList, JvxClipIcon;
...
constructor TJvxIconFrame.Create(Index: Integer; Jiff: Longint);
begin
  inherited Create;
  FSeq := Index;
  FJiffRate := Jiff;
...
destructor TJvxIconFrame.Destroy;
begin
  if FIcon <> nil then FIcon.Free;
  inherited Destroy;
end;

procedure TJvxIconFrame.Assign(Source: TPersistent);
begin
  if Source is TJvxIconFrame then begin
    with TJvxIconFrame(Source) do begin
...
constructor TJvxAnimatedCursorImage.Create;
begin
  inherited Create;
  FIcons := TList.Create;
end;

destructor TJvxAnimatedCursorImage.Destroy;
begin
  NewImage;
  FIcons.Free;
  inherited Destroy;
...
procedure TJvxAnimatedCursorImage.Clear;
begin
  NewImage;
end;

procedure TJvxAnimatedCursorImage.NewImage;
var
  I: Integer;
begin
  for I := 0 to FIcons.Count - 1 do TJvxIconFrame(FIcons[I]).Free;
  FIcons.Clear;
...
procedure TJvxAnimatedCursorImage.RiffReadError;
begin
  raise EReadError.Create(ResStr(SReadError));
end;

function TJvxAnimatedCursorImage.GetTitle: string;
begin
  Result := StrPas(FTitle);
end;

function TJvxAnimatedCursorImage.GetCreator: string;
begin
  Result := StrPas(FCreator);
end;

function TJvxAnimatedCursorImage.GetIconCount: Integer;
begin
  Result := FIcons.Count;
end;

function TJvxAnimatedCursorImage.GetIcon(Index: Integer): TIcon;
begin
  Result := TJvxIconFrame(FIcons[Index]).FIcon;
end;

function TJvxAnimatedCursorImage.GetFrame(Index: Integer): TJvxIconFrame;
begin
  Result := TJvxIconFrame(FIcons[Index]);
end;

function TJvxAnimatedCursorImage.GetDefaultRate: Longint;
begin
  Result := Max(0, Min((FHeader.jifRate * 100) div 6, High(Result)));
end;

procedure TJvxAnimatedCursorImage.Assign(Source: TPersistent);
var
  I: Integer;
  Frame: TJvxIconFrame;
begin
  ...
  else if Source is TJvxAnimatedCursorImage then begin
    NewImage;
    try
      with TJvxAnimatedCursorImage(Source) do begin
        ...
        for I := 0 to FIcons.Count - 1 do begin
          Frame := TJvxIconFrame.Create(-1, FHeader.jifRate);
          try
            Frame.Assign(TJvxIconFrame(FIcons[I]));
...
procedure TJvxAnimatedCursorImage.AssignTo(Dest: TPersistent);
var
  I: Integer;
begin
  ...
  else if Dest is TJvxIconList then begin
    TJvxIconList(Dest).BeginUpdate;
    try
      TJvxIconList(Dest).Clear;
      for I := 0 to IconCount - 1 do TJvxIconList(Dest).Add(Icons[I]);
    finally
      TJvxIconList(Dest).EndUpdate;
    end;
  end
  else inherited AssignTo(Dest);
end;

function TJvxAnimatedCursorImage.ReadCreateIcon(Stream: TStream; ASize: Longint;
  var HotSpot: TPoint; var IsIcon: Boolean): TIcon;
type
...
procedure TJvxAnimatedCursorImage.ReadAniStream(Stream: TStream);
var
...
begin
  ...
  while ReadTag(Stream, @Tag) do begin
    if Tag.ckID = FOURCC_anih then begin
      ...
      for I := 0 to FHeader.cSteps - 1 do begin
        Frame := TJvxIconFrame.Create(I, FHeader.jifRate);
        FIcons.Add(Frame);
      end;
    end
      ...
      if iRate < FIcons.Count then
        TJvxIconFrame(FIcons[iRate]).FJiffRate := Temp;
      Inc(iRate);
    end
      ...
      if iSeq < FIcons.Count then
        TJvxIconFrame(FIcons[iSeq]).FSeq := Temp;
      Inc(iSeq);
    end
      ...
      if (Tag.ckID = FOURCC_fram) then begin
        while (cbChunk >= SizeOf(Tag)) do begin
          ...
          if (Tag.ckID = FOURCC_icon) then begin
            ...
            bFound := False;
            for I := 0 to FIcons.Count - 1 do begin
              if TJvxIconFrame(FIcons[I]).FSeq = iFrame then begin
                TJvxIconFrame(FIcons[I]).FIcon := Icon;
                TJvxIconFrame(FIcons[I]).FTag := Tag;
                TJvxIconFrame(FIcons[I]).FHotSpot := HotSpot;
                TJvxIconFrame(FIcons[I]).FIsIcon := IsIcon;
                bFound := True;
              end;
            end;
  ...
  for I := FIcons.Count - 1 downto 0 do begin
    if TJvxIconFrame(FIcons[I]).FIcon = nil then begin
      TJvxIconFrame(FIcons[I]).Free;
      FIcons.Delete(I);
    end;
  end;
...
procedure TJvxAnimatedCursorImage.ReadStream(Size: Longint; Stream: TStream);
var
  Data: TMemoryStream;
begin
...
procedure TJvxAnimatedCursorImage.WriteStream(Stream: TStream;
  WriteSize: Boolean);
begin
  NotImplemented;
end;

procedure TJvxAnimatedCursorImage.LoadFromStream(Stream: TStream);
begin
  ReadStream(Stream.Size - Stream.Position, Stream);
end;

procedure TJvxAnimatedCursorImage.SaveToStream(Stream: TStream);
begin
  WriteStream(Stream, False);
end;

procedure TJvxAnimatedCursorImage.LoadFromFile(const Filename: string);
var
  Stream: TStream;
begin
...
procedure TJvxAnimatedCursorImage.Draw(ACanvas: TCanvas; const ARect: TRect);
begin
  if FIcons.Count > 0 then
    DrawRealSizeIcon(ACanvas, Icons[0], ARect.Left, ARect.Top);
end;

procedure TJvxAnimatedCursorImage.AssignToBitmap(Bitmap: TBitmap; BackColor: TColor;
  DecreaseColors, Vertical: Boolean);
var
  I: Integer;
  Temp: TBitmap;
