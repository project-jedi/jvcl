...
uses TypInfo, JvDBLists, JvLConst, JvDBQBE, JvDBFilter, JvDBIndex, JvDBPrgrss, 
  JvLogin, JvDBSecur, JvQuery, JvVCLUtils, JvDbExcpt, JvDsgn,
...
{ Data aware components and controls }
  RegisterComponents(LoadStr(srRXDBAware), [TJvQuery, TJvSQLScript,
    TJvMemoryTable, TJvQBEQuery, TJvDBFilter, TJvDBIndexCombo, TJvDBProgress, 
    TJvDBSecurity]);
{$IFDEF RX_MIDAS}
{ MIDAS components }
  RegisterComponents(LoadStr(srRXDBAware), [TJvRemoteLogin]);
  RegisterNonActiveX([TJvRemoteLogin], axrComponentOnly);
{$ENDIF}
{ Database lists }
  RegisterComponents(LoadStr(srRXDBAware), [TJvBDEItems, TJvDatabaseItems,
    TJvTableItems]);
{$IFNDEF CBUILDER}
 {$IFDEF USE_OLD_DBLISTS}
  RegisterComponents(LoadStr(srRXDBAware), [TJvDatabaseList, TJvLangDrivList,
