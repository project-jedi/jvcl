...
The Original Code is: JvxVCLUtils.PAS, released on 2002-07-04.
...
type
  TJvxScreenCanvas = class(TCanvas)
  private
    FDeviceContext: HDC;
  protected
...
{ TJvxMetafileCanvas }
  ...
  TJvxMetafileCanvas = class(TCanvas)
  private
    FMetafile: TMetafile;
  public
...
{ TJvxResourceStream }
  ...
  TJvxResourceStream = class(THandleStream)
  private
    FStartPos: LongInt;
    FEndPos: LongInt;
  protected
...
Uses SysUtils, Messages, JvxMaxMin, Consts, JvxConst, {$IFDEF COMPILER35_UP} SysConst, {$ENDIF}
  {$IFDEF WIN32} CommCtrl, {$ELSE} JvxStr16, {$ENDIF} JvxCConst;
...
type
  TJvxParentControl = class(TWinControl);
...
{$ENDIF}
    ...
    try
      ...
      with TJvxParentControl(Control.Parent) do begin
...
begin
  ...
  WriteText(TJvxHack(Control).Canvas, ARect, MinOffs, H, S, Align, WordWrap,
    ARightToLeft);
end;
...
begin
  ...
  TJvxHack(Control).Canvas.Draw(Rect.Left, Rect.Top, Bmp);
end;

{ TJvxScreenCanvas }

destructor TJvxScreenCanvas.Destroy;
begin
  FreeHandle;
  inherited Destroy;
end;

procedure TJvxScreenCanvas.CreateHandle;
begin
  if FDeviceContext = 0 then FDeviceContext := GetDC(0);
  Handle := FDeviceContext;
end;

procedure TJvxScreenCanvas.FreeHandle;
begin
...
procedure TJvxScreenCanvas.SetOrigin(X, Y: Integer);
var
  FOrigin: TPoint;
begin
...
(*
  ...
  with TJvxMetafileCanvas.Create(MyMetafile, 0) do
  try
    Brush.Color := clRed;
    Ellipse(0,0,100,100);
    ...
...
constructor TJvxMetafileCanvas.Create(AMetafile: TMetafile; ReferenceDevice: HDC);
var
  Temp: HDC;
begin
  inherited Create;
...
destructor TJvxMetafileCanvas.Destroy;
var
  Temp: HDC;
...
constructor TJvxResourceStream.Create(Instance: THandle; const ResName: string;
  ResType: PChar);
var
...
constructor TJvxResourceStream.CreateFromID(Instance: THandle; ResID: Integer;
  ResType: PChar);
begin
  CreateFromPChar(Instance, MakeIntResource(ResID), ResType);
end;

constructor TJvxResourceStream.CreateFromPChar(Instance: THandle; ResName,
  ResType: PChar);
var
  ResInfo: THandle;
  Handle: Integer;
...
destructor TJvxResourceStream.Destroy;
begin
  if Handle >= 0 then FileClose(Handle);
  inherited Destroy;
end;

function TJvxResourceStream.Write(const Buffer; Count: Longint): Longint;
begin
  raise EStreamError.CreateRes(SWriteError);
end;

function TJvxResourceStream.Seek(Offset: Longint; Origin: Word): Longint;
begin
  case Origin of
    soFromBeginning:
