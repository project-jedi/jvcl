...
The Original Code is: JvxBdeUtils.PAS, released on 2002-07-04.
...
{$ENDIF}
  ...
  TJvxDBLocate = class(TJvxLocateObject)
  private
{$IFNDEF WIN32}
...
{ TJvxCloneDataset }
  ...
  TJvxCloneDataset = class(TBDEDataSet)
  private
...
{ TJvxCloneDbDataset }
  ...
  TJvxCloneDbDataset = class(TDBDataSet)
  private
    FSourceHandle: HDBICur;
    FReadOnly: Boolean;
    procedure SetReadOnly(Value: Boolean);
...
function CreateDbLocate: TJvxLocateObject;
{$IFNDEF WIN32}
...
uses Forms, Controls, Dialogs, Consts, DBConsts, JvxDConst, JvxVCLUtils,
  JvxFileUtil, JvxAppUtils, JvxStrUtils, JvxMaxMin, {$IFNDEF WIN32} JvxStr16, {$ENDIF}
  {$IFDEF Delphi3_Up} BDEConst, DBCommon, {$ENDIF} JvxDateUtil;
...
procedure TJvxCloneDataset.SetSourceHandle(ASourceHandle: HDBICur);
begin
...
function TJvxCloneDataset.CreateHandle: HDBICur;
begin
  Check(DbiCloneCursor(FSourceHandle, FReadOnly, False, Result));
end;

procedure TJvxCloneDataset.SetReadOnly(Value: Boolean);
begin
  CheckInactive;
  FReadOnly := Value;
end;
...
procedure TJvxCloneDbDataset.InitFromDataSet(Source: TDBDataSet; Reset: Boolean);
begin
...
procedure TJvxCloneDbDataset.SetSourceHandle(ASourceHandle: HDBICur);
begin
  if ASourceHandle <> FSourceHandle then begin
    Close;
    FSourceHandle := ASourceHandle;
...
function TJvxCloneDbDataset.CreateHandle: HDBICur;
begin
  Check(DbiCloneCursor(FSourceHandle, FReadOnly, False, Result));
end;

procedure TJvxCloneDbDataset.SetReadOnly(Value: Boolean);
begin
  CheckInactive;
  FReadOnly := Value;
end;
...
procedure TJvxCloneTable.InitFromTable(SourceTable: TTable; Reset: Boolean);
begin
...
procedure TJvxCloneTable.SetSourceHandle(ASourceHandle: HDBICur);
begin
  if ASourceHandle <> FSourceHandle then begin
    Close;
    FSourceHandle := ASourceHandle;
...
procedure TJvxCloneTable.SetReadOnly(Value: Boolean);
begin
  CheckInactive;
  FReadOnly := Value;
end;

function TJvxCloneTable.CreateHandle: HDBICur;
begin
  Check(DbiCloneCursor(FSourceHandle, FReadOnly, False, Result));
end;
...
begin
  Result := TJvxDBLocate(pDBLocate).RecordFilter(RecBuf, RecNo);
end;
{$ENDIF WIN32}

destructor TJvxDBLocate.Destroy;
begin
{$IFNDEF WIN32}
  DropFilter;
{$ENDIF}
...
procedure TJvxDBLocate.CheckFieldType(Field: TField);
var
  Locale: TLocale;
begin
...
function TJvxDBLocate.UseKey: Boolean;
var
  I: Integer;
begin
  Result := False;
...
function TJvxDBLocate.LocateKey: Boolean;
var
  Clone: TJvxCloneTable;
...
begin
  try
    ...
    if TTable(DataSet).IndexFieldNames = LookupField.FieldName then
      ...
      Clone := TJvxCloneTable.Create(DataSet);
      with Clone do
      try
        ReadOnly := True;
...
function TJvxDBLocate.FilterApplicable: Boolean;
begin
...
function TJvxDBLocate.LocateCallback: Boolean;
var
  Clone: TJvxCloneDbDataset;
begin
  Result := False;
  try
    TBDEDataSet(DataSet).CheckBrowseMode;
    Clone := TJvxCloneDbDataset.Create(DataSet);
    with Clone do
    try
      ReadOnly := True;
...
procedure TJvxDBLocate.RecordFilter(DataSet: TDataSet; var Accept: Boolean);
begin
  Accept := MatchesLookup(DataSet.FieldByName(LookupField.FieldName));
end;

function TJvxDBLocate.LocateFilter: Boolean;
var
  SaveCursor: TCursor;
begin
...
procedure TJvxDBLocate.BuildFilterHeader(var Rec);
const
...
procedure TJvxDBLocate.BuildFilterTree;
var
  Temp: PChar;
  Rec: TFilterRec;
begin
...
procedure TJvxDBLocate.FreeTree;
begin
  if FTree <> nil then FreeMem(FTree, FTreeSize);
  FTree := nil;
  FTreeSize := 0;
...
procedure TJvxDBLocate.CheckFilterKind;
var
...
procedure TJvxDBLocate.ActivateFilter;
begin
  CheckFilterKind;
...
procedure TJvxDBLocate.DeactivateFilter;
begin
  DbiDeactivateFilter(DataSet.Handle, FFilterHandle);
end;

procedure TJvxDBLocate.DropFilter;
begin
...
function TJvxDBLocate.RecordFilter(RecBuf: Pointer; RecNo: Longint): Smallint;
var
  Accept: Boolean;
begin
  try
...
procedure TJvxDBLocate.ChangeBookmark;
begin
  if Bookmark <> nil then DataSet.FreeBookmark(Bookmark);
  Bookmark := DataSet.GetBookmark;
end;

procedure TJvxDBLocate.ActiveChanged;
begin
  DropFilter;
end;
...
initialization
  JvxDBUtils.CreateLocateObject := CreateDbLocate;
{$IFDEF WIN32}
finalization
  ReleaseSaveIndexies;
{$ELSE}
