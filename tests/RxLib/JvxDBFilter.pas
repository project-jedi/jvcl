...
The Original Code is: JvxDBFilter.PAS, released on 2002-07-04.
...
uses SysUtils, Windows, Messages, Classes, Controls, Forms,
  Graphics, Menus, StdCtrls, ExtCtrls, Bde, DB, DBTables{, JvxComponent};
{$ELSE}
...
uses {$IFNDEF WIN32} DBIErrs, DBIProcs, JvxStr16, {$ENDIF} DBConsts, Dialogs,
  {$IFDEF Delphi3_Up} DbCommon, {$ENDIF} JvxDConst, JvxVCLUtils, JvxDBUtils, JvxBdeUtils;
...
constructor TJvxFilterExpr.Create(DataSet: TDataSet; Options: TDBFilterOptions);
begin
  FDataSet := DataSet;
  FOptions := Options;
end;

destructor TJvxFilterExpr.Destroy;
var
  Node: PExprNode;
begin
...
function TJvxFilterExpr.FieldFromNode(Node: PExprNode): TField;
begin
...
function TJvxFilterExpr.GetExprData(Pos, Size: Integer): PChar;
begin
{$IFDEF WIN32}
...
function TJvxFilterExpr.GetFilterData(Root: PExprNode): PCANExpr;
begin
...
function TJvxFilterExpr.NewCompareNode(Field: TField; Operator: CanOp;
  const Value: string): PExprNode;
var
...
function TJvxFilterExpr.NewNode(Kind: TExprNodeKind; Operator: CanOp;
  const Data: string; Left, Right: PExprNode): PExprNode;
begin
  New(Result);
...
function TJvxFilterExpr.PutCompareNode(Node: PExprNode): Integer;
const
...
function TJvxFilterExpr.PutConstNode(DataType: Integer; Data: PChar;
  Size: Integer): Integer;
begin
...
function TJvxFilterExpr.PutConstStr(const Value: string): Integer;
var
...
function TJvxFilterExpr.PutData(Data: PChar; Size: Integer): Integer;
begin
...
function TJvxFilterExpr.PutExprNode(Node: PExprNode): Integer;
const
...
function TJvxFilterExpr.PutFieldNode(Field: TField): Integer;
var
  Buffer: array[0..255] of Char;
begin
...
function TJvxFilterExpr.PutNode(NodeType: NodeClass; OpType: CanOp;
  OpCount: Integer): Integer;
var
  Size: Integer;
begin
...
procedure TJvxFilterExpr.SetNodeOp(Node, Index, Data: Integer);
begin
...
var
  Root: PExprNode;
begin
  FFilter := TJvxFilterExpr.Create(DataSet, Options);
  FText := Text;
  FSourcePtr := Text;
  NextToken;
  Root := ParseExpr;
...
type
  TJvxFilterDataLink = class(TDataLink)
  private
    FFilter: TJvxDBFilter;
  protected
    procedure ActiveChanged; override;
  public
    constructor Create(Filter: TJvxDBFilter);
    destructor Destroy; override;
  end;

constructor TJvxFilterDataLink.Create(Filter: TJvxDBFilter);
begin
  inherited Create;
  FFilter := Filter;
end;

destructor TJvxFilterDataLink.Destroy;
begin
  FFilter := nil;
  inherited Destroy;
end;

procedure TJvxFilterDataLink.ActiveChanged;
begin
...
begin
  Result := TJvxDBFilter(pDBFilter).RecordFilter(RecBuf, RecNo);
end;
{$ENDIF WIN32}

{ TJvxDBFilter }

constructor TJvxDBFilter.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  FDataLink := TJvxFilterDataLink.Create(Self);
...
destructor TJvxDBFilter.Destroy;
begin
...
function TJvxDBFilter.GetDataSource: TDataSource;
begin
  Result := FDataLink.DataSource;
end;

procedure TJvxDBFilter.SetDataSource(Value: TDataSource);
var
  DSChange: Boolean;
begin
...
procedure TJvxDBFilter.Notification(AComponent: TComponent;
...
function TJvxDBFilter.CreateExprFilter: hDBIFilter;
begin
  Result := nil;
...
function TJvxDBFilter.CreateFuncFilter: hDBIFilter;
var
  FuncPriority: Word;
begin
...
{$IFDEF WIN32}
  Check(DbiAddFilter((FDataLink.DataSet as TBDEDataSet).Handle, Longint(Self),
    FuncPriority, False, nil, PFGENFilter(@TJvxDBFilter.RecordFilter),
    Result));
{$ELSE}
...
procedure TJvxDBFilter.SetFilterHandle(var Filter: HDBIFilter;
  Value: HDBIFilter);
var
  Info: FilterInfo;
begin
...
procedure TJvxDBFilter.DropFilters;
begin
...
procedure TJvxDBFilter.ActivateFilters;
begin
...
procedure TJvxDBFilter.DeactivateFilters;
begin
  if (FFuncHandle <> nil) then
...
function TJvxDBFilter.RecordFilter(RecBuf: Pointer; RecNo: Longint): Smallint;
var
  ACanModify: Boolean;
  Buffers: PBufferList;
{$IFDEF Delphi4_Up}
...
procedure TJvxDBFilter.FilterChanged(Sender: TObject);
begin
  RecreateExprFilter;
end;

procedure TJvxDBFilter.SetOnFiltering(const Value: TFilterEvent);
begin
...
procedure TJvxDBFilter.RecreateFuncFilter;
var
  Filter: HDBIFilter;
begin
...
procedure TJvxDBFilter.RecreateExprFilter;
var
  Filter: HDBIFilter;
begin
...
procedure TJvxDBFilter.SetFilter(Value: TStrings);
begin
  FFilter.Assign(Value);
end;

procedure TJvxDBFilter.SetOptions(Value: TDBFilterOptions);
begin
...
procedure TJvxDBFilter.SetLogicCond(Value: TFilterLogicCond);
begin
  FLogicCond := Value;
end;

procedure TJvxDBFilter.SetPriority(Value: Word);
begin
...
function TJvxDBFilter.GetFilterText: PChar;
var
  BufLen: Word;
  I: Integer;
  StrEnd: PChar;
...
procedure TJvxDBFilter.DestroyTree;
begin
...
procedure TJvxDBFilter.BeforeDataPost(DataSet: TDataSet);
begin
  ReadCaptureControls;
  ReleaseCapture;
  Activate;
...
procedure TJvxDBFilter.BeforeDataChange(DataSet: TDataSet);
begin
  FilterError(SCaptureFilter);
end;

procedure TJvxDBFilter.BeforeDataCancel(DataSet: TDataSet);
begin
  ReleaseCapture;
end;

function TJvxDBFilter.BuildTree: Boolean;
var
  Expr: PChar;
  I: Integer;
begin
...
procedure TJvxDBFilter.DoActivate;
begin
  if Assigned(FOnActivate) then FOnActivate(Self);
end;

procedure TJvxDBFilter.DoDeactivate;
begin
  if Assigned(FOnDeactivate) then FOnDeactivate(Self);
end;

procedure TJvxDBFilter.SetActive(Value: Boolean);
var
  Bookmark: TBookmark;
begin
...
procedure TJvxDBFilter.Activate;
begin
  SetActive(True);
end;

procedure TJvxDBFilter.Deactivate;
begin
  SetActive(False);
end;

procedure TJvxDBFilter.SetCapture;
begin
...
procedure TJvxDBFilter.ReleaseCapture;
begin
...
procedure TJvxDBFilter.ReadCaptureControls;
const
...
procedure TJvxDBFilter.UpdateFuncFilter;
begin
...
procedure TJvxDBFilter.ActiveChanged;
var
  WasActive: Boolean;
begin
