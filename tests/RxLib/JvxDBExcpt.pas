...
The Original Code is: JvxDbExcpt.PAS, released on 2002-07-04.
...
TJvxBdeErrorDlg based on sample form             
...
var
  DbEngineErrorDlg: TJvxBdeErrorDlg;
...
uses {$IFDEF WIN32} Windows, BDE, {$ELSE} WinProcs, WinTypes, DbiErrs,
  JvxStr16, {$ENDIF} Consts, JvxDConst, JvxCConst, JvxVCLUtils;
...
begin
  ...
  DbEngineErrorDlg := TJvxBdeErrorDlg.Create(Application);
end;

{ TJvxBdeErrorDlg }

procedure TJvxBdeErrorDlg.ShowException(Sender: TObject; E: Exception);
begin
...
procedure TJvxBdeErrorDlg.ShowError;
var
  BDEError: TDbError;
  S: string;
  I: Integer;
...
procedure TJvxBdeErrorDlg.SetShowDetails(Value: Boolean);
begin
  DisableAlign;
  try
    if Value then begin
...
procedure TJvxBdeErrorDlg.GetErrorMsg(Error: TDBError; var Msg: string);
begin
...
procedure TJvxBdeErrorDlg.FormCreate(Sender: TObject);
begin
{$IFNDEF WIN32}
  BorderIcons := [];
{$ENDIF}
...
procedure TJvxBdeErrorDlg.FormDestroy(Sender: TObject);
begin
  Application.OnException := FPrevOnException;
end;

procedure TJvxBdeErrorDlg.FormShow(Sender: TObject);
var
  S: string;
  ErrNo: Integer;
begin
...
procedure TJvxBdeErrorDlg.DetailsBtnClick(Sender: TObject);
begin
  SetShowDetails(not Details);
end;

procedure TJvxBdeErrorDlg.BackClick(Sender: TObject);
begin
  Dec(CurItem);
  ShowError;
end;

procedure TJvxBdeErrorDlg.NextClick(Sender: TObject);
begin
  Inc(CurItem);
  ShowError;
end;
