...
The Original Code is: JvxIcoLEdit.PAS, released on 2002-07-04.
...
uses {$IFDEF WIN32} Windows, {$ELSE} WinTypes, WinProcs, {$ENDIF}
  Messages, Classes, Graphics, Forms, Controls, Dialogs, Buttons, JvxIcoList,
  RTLConsts, DesignIntf, DesignEditors, VCLEditors, StdCtrls, ExtCtrls, JvxPlacemnt, {$IFDEF Delphi3_Up} ExtDlgs, {$ELSE}
  ImagPrvw, {$ENDIF} Menus, JvxSpeedbar;
...
{ TIconListDialog }
  ...
  TIconListDialog = class(TForm)
    ...
    Label3: TLabel;
    IdxLabel: TLabel;
    JvxSpeedbar: TJvxSpeedBar;
    Load: TJvxSpeedItem;
    LoadAni: TJvxSpeedItem;
    Delete: TJvxSpeedItem;
    Clear: TJvxSpeedItem;
    Copy: TJvxSpeedItem;
    Paste: TJvxSpeedItem;
...
procedure EditIconList(IconList: TJvxIconList);

implementation

uses TypInfo, SysUtils, Clipbrd, Consts, JvxClipIcon, JvxVCLUtils, JvxAppUtils,
  JvxConst, JvxLConst, JvxMaxMin, JvxAniFile;
...
begin
  ...
  try
    ...
    Editor.Icons.Assign(TJvxIconList(Pointer(GetOrdValue)));
    ...
    if (Res = mrOk) and Editor.Modified then begin
      TJvxIconList(Pointer(GetOrdValue)).Assign(Editor.Icons);
      Designer.Modified;
    end;
  finally
    Editor.Free;
...
var
  List: TJvxIconList;
begin
  List := TJvxIconList(Pointer(GetOrdValue));
