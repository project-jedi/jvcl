...
The Original Code is: JvxHints.PAS, released on 2002-07-04.
...
type
  ...
  TJvxHintWindow = class(THintWindow)
  private
    FSrcImage: TBitmap;
    FImage: TBitmap;
    FPos: THintPos;
...
uses SysUtils, JvxVCLUtils, JvxAppUtils, JvxMaxMin;
...
begin
  ...
  RegisterHintWindow(TJvxHintWindow);
end;
...
constructor TJvxHintWindow.Create(AOwner: TComponent);
begin
...
destructor TJvxHintWindow.Destroy;
begin
  FSrcImage.Free;
  FImage.Free;
  inherited Destroy;
...
procedure TJvxHintWindow.CreateParams(var Params: TCreateParams);
begin
...
{$IFDEF Delphi3_Up}
procedure TJvxHintWindow.WMNCPaint(var Message: TMessage);
begin
end;
{$ENDIF}

procedure TJvxHintWindow.WMEraseBkgnd(var Message: TMessage);
begin
  Message.Result := 1;
end;

function TJvxHintWindow.CreateRegion(Shade: Boolean): HRgn;
var
  R: TRect;
  W, TileOffs: Integer;
  Tail, Dest: HRgn;
...
procedure TJvxHintWindow.FillRegion(Rgn: HRgn; Shade: Boolean);
begin
  if Shade then begin
...
procedure TJvxHintWindow.Paint;
var
  R: TRect;
  FShadeRgn, FRgn: HRgn;
...
procedure TJvxHintWindow.ActivateHint(Rect: TRect; const AHint: string);
var
  R: TRect;
  ScreenDC: HDC;
  P: TPoint;
...
function TJvxHintWindow.CalcHintRect(MaxWidth: Integer; const AHint: string;
  AData: Pointer): TRect;
const
  Flag: array[TAlignment] of Longint = (DT_LEFT, DT_RIGHT, DT_CENTER);
var
...
{$IFDEF Delphi3_Up}
procedure TJvxHintWindow.ActivateHintData(Rect: TRect; const AHint: string;
  AData: Pointer);
begin
