...
The Original Code is: JvxClock.PAS, released on 2002-07-04.
...
uses Windows, SysUtils, Messages, Classes, Graphics, Controls,
    Forms, StdCtrls, ExtCtrls, Menus, JvxTimer, RTLConsts;

type
  TShowClock = (scDigital, scAnalog);
  TPaintMode = (pmPaintAll, pmHandPaint);

  TJvxClockTime = packed record
    Hour, Minute, Second: Word;
  end;

  TJvxGetTimeEvent = procedure (Sender: TObject; var ATime: TDateTime) of object;
...
{ TJvxClock }
  ...
  private
    ...
    FDotsColor: TColor;
    FAlarmWait: Boolean;
    FDisplayTime: TJvxClockTime;
    FClockRect: TRect;
    FClockRadius: Longint;
    FClockCenter: TPoint;
    FOnGetTime: TJvxGetTimeEvent;
    FOnAlarm: TNotifyEvent;
    procedure TimerExpired(Sender: TObject);
    procedure GetTime(var T: TJvxClockTime);
...
{$ENDIF}
    ...
    property OnGetTime: TJvxGetTimeEvent read FOnGetTime write FOnGetTime;
    property OnClick;
    property OnDblClick;
...
function HourHandPos(T: TJvxClockTime): Integer;
begin
...
procedure TJvxClock.CMCtl3DChanged(var Message: TMessage);
begin
  inherited;
  if ShowMode = scAnalog then Invalidate;
end;

procedure TJvxClock.CMTextChanged(var Message: TMessage);
begin
  { Skip this message, no repaint }
end;

procedure TJvxClock.CMFontChanged(var Message: TMessage);
begin
  inherited;
  Invalidate;
...
procedure TJvxClock.WMTimeChange(var Message: TMessage);
begin
  inherited;
  Invalidate;
  CheckAlarm;
...
function TJvxClock.FormatSettingsChange(var Message: TMessage): Boolean;
begin
  Result := False;
  case Message.Msg of
    WM_WININICHANGE:
...
procedure TJvxClock.GetTime(var T: TJvxClockTime);
var
  MSec: Word;
begin
  with T do
