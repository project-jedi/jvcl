...
uses Classes, Controls, StdCtrls, JvCtrls;
...
begin
  ...
  else if List is TJvCustomListBox then
    Result := TJvCustomListBox(List).Items
  else Result := nil;
end;
...
begin
  if List is TCustomListBox then
    ...
    Result := TJvCustomListBox(List).Selected[Index]
  else Result := False;
end;
...
begin
  if List is TCustomListBox then
    ...
    TJvCustomListBox(List).Selected[Index] := Value;
end;
...
begin
  if List is TCustomListBox then
    ...
    Result := TJvCustomListBox(List).ItemIndex
  else Result := LB_ERR;
end;
...
begin
  if List is TCustomListBox then
    ...
    Result := TJvCustomListBox(List).Canvas
  else Result := nil;
end;
{$ENDIF}
...
begin
  if List is TCustomListBox then
    ...
    TJvCustomListBox(List).ItemIndex := Index;
end;
...
begin
  if List is TCustomListBox then
    ...
    Result := TJvCheckListBox(List).MultiSelect
  else Result := False;
end;
...
begin
  if List is TCustomListBox then
    ...
    Result := TJvCustomListBox(List).SelCount
  else Result := 0;
end;
...
begin
  if List is TCustomListBox then
    ...
    Result := TJvCustomListBox(List).ItemAtPos(Pos, Existing)
  else Result := LB_ERR;
end;
...
begin
  if List is TCustomListBox then
    ...
    Result := TJvCustomListBox(List).ItemRect(Index)
    ...
    try
      ...
      while I < BoxItems(SrcList).Count do begin
        if BoxGetSelected(SrcList, I) then begin
          ...
          if (SrcList is TJvCheckListBox) and (DstList is TJvCheckListBox) then
          begin
            TJvCheckListBox(DstList).State[NewIndex] :=
              TJvCheckListBox(SrcList).State[I];
          end;
...
begin
  if Source <> List then
    Accept := (Source is TWinControl) or (Source is TJvCustomListBox)
  else begin
