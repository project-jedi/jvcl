...
The Original Code is: JvxProps.PAS, released on 2002-07-04.
...
uses SysUtils, Classes, Graphics, Controls, Forms, TypInfo, JvxVCLUtils;

type

{ TJvxPropInfoList }

  TJvxPropInfoList = class(TObject)
  private
    FList: PPropList;
    FCount: Integer;
    FSize: Integer;
  ...
  public
    ...
    procedure Intersect(List: TJvxPropInfoList);
...
{ TJvxPropsStorage }
  ...
  TJvxPropsStorage = class(TObject)
  private
    FObject: TObject;
    FOwner: TComponent;
    FPrefix: string;
...
{$ENDIF}
  ...
  protected
    ...
    function CreateStorage: TJvxPropsStorage; virtual;
  public
...
uses {$IFDEF WIN32} Windows, {$ELSE} WinTypes, WinProcs, JvxStr16, {$ENDIF}
  Consts, JvxStrUtils;
...
constructor TJvxPropInfoList.Create(AObject: TObject; Filter: TTypeKinds);
begin
...
destructor TJvxPropInfoList.Destroy;
begin
  if FList <> nil then FreeMem(FList, FSize);
end;

function TJvxPropInfoList.Contains(P: PPropInfo): Boolean;
var
  I: Integer;
begin
...
function TJvxPropInfoList.Find(const AName: string): PPropInfo;
var
  I: Integer;
begin
  for I := 0 to FCount - 1 do
...
procedure TJvxPropInfoList.Delete(Index: Integer);
begin
  Dec(FCount);
...
function TJvxPropInfoList.Get(Index: Integer): PPropInfo;
begin
  Result := FList^[Index];
end;

procedure TJvxPropInfoList.Intersect(List: TJvxPropInfoList);
var
  I: Integer;
begin
...
function TJvxPropsStorage.GetItemName(const APropName: string): string;
begin
  Result := Prefix + APropName;
end;

procedure TJvxPropsStorage.LoadAnyProperty(PropInfo: PPropInfo);
var
  S, Def: string;
begin
  try
...
procedure TJvxPropsStorage.StoreAnyProperty(PropInfo: PPropInfo);
var
  S: string;
begin
  if PropInfo <> nil then begin
...
function TJvxPropsStorage.StoreIntegerProperty(PropInfo: PPropInfo): string;
begin
  Result := IntToStr(GetOrdProp(FObject, PropInfo));
end;

function TJvxPropsStorage.StoreCharProperty(PropInfo: PPropInfo): string;
begin
  Result := Char(GetOrdProp(FObject, PropInfo));
end;

function TJvxPropsStorage.StoreEnumProperty(PropInfo: PPropInfo): string;
begin
  Result := GetEnumName(GetPropType(PropInfo), GetOrdProp(FObject, PropInfo));
end;

function TJvxPropsStorage.StoreFloatProperty(PropInfo: PPropInfo): string;
const
{$IFDEF WIN32}
...
function TJvxPropsStorage.StoreStringProperty(PropInfo: PPropInfo): string;
begin
  Result := GetStrProp(FObject, PropInfo);
end;

{$IFDEF WIN32}
function TJvxPropsStorage.StoreLStringProperty(PropInfo: PPropInfo): string;
begin
  Result := GetStrProp(FObject, PropInfo);
end;

function TJvxPropsStorage.StoreWCharProperty(PropInfo: PPropInfo): string;
begin
  Result := Char(GetOrdProp(FObject, PropInfo));
end;

function TJvxPropsStorage.StoreVariantProperty(PropInfo: PPropInfo): string;
begin
...
{$IFDEF Delphi4_Up}
function TJvxPropsStorage.StoreInt64Property(PropInfo: PPropInfo): string;
begin
  Result := IntToStr(GetInt64Prop(FObject, PropInfo));
end;
{$ENDIF}

function TJvxPropsStorage.StoreSetProperty(PropInfo: PPropInfo): string;
var
  TypeInfo: PTypeInfo;
  W: Cardinal;
  I: Integer;
...
function TJvxPropsStorage.StoreStringsProperty(PropInfo: PPropInfo): string;
var
  List: TObject;
  I: Integer;
  SectName: string;
...
function TJvxPropsStorage.StoreComponentProperty(PropInfo: PPropInfo): string;
var
  Comp: TComponent;
  RootName: string;
begin
...
function TJvxPropsStorage.StoreClassProperty(PropInfo: PPropInfo): string;
var
  Saver: TJvxPropsStorage;
  I: Integer;
  Obj: TObject;
  ...
  begin
    with Saver do begin
      ...
      Props := TJvxPropInfoList.Create(AObject, tkProperties);
      try
...
procedure TJvxPropsStorage.LoadIntegerProperty(const S: string; PropInfo: PPropInfo);
begin
  SetOrdProp(FObject, PropInfo, StrToIntDef(S, 0));
end;

procedure TJvxPropsStorage.LoadCharProperty(const S: string; PropInfo: PPropInfo);
begin
  SetOrdProp(FObject, PropInfo, Integer(S[1]));
end;

procedure TJvxPropsStorage.LoadEnumProperty(const S: string; PropInfo: PPropInfo);
var
  I: Integer;
  EnumType: PTypeInfo;
begin
...
procedure TJvxPropsStorage.LoadFloatProperty(const S: string; PropInfo: PPropInfo);
begin
...
{$IFDEF Delphi4_Up}
procedure TJvxPropsStorage.LoadInt64Property(const S: string; PropInfo: PPropInfo);
begin
...
{$IFDEF WIN32}
procedure TJvxPropsStorage.LoadLStringProperty(const S: string; PropInfo: PPropInfo);
begin
  SetStrProp(FObject, PropInfo, S);
end;

procedure TJvxPropsStorage.LoadWCharProperty(const S: string; PropInfo: PPropInfo);
begin
  SetOrdProp(FObject, PropInfo, Longint(S[1]));
end;

procedure TJvxPropsStorage.LoadVariantProperty(const S: string; PropInfo: PPropInfo);
begin
  SetVariantProp(FObject, PropInfo, S);
end;
{$ENDIF}

procedure TJvxPropsStorage.LoadStringProperty(const S: string; PropInfo: PPropInfo);
begin
  SetStrProp(FObject, PropInfo, S);
end;

procedure TJvxPropsStorage.LoadSetProperty(const S: string; PropInfo: PPropInfo);
const
  Delims = [' ', ',', '[', ']'];
var
  TypeInfo: PTypeInfo;
...
procedure TJvxPropsStorage.LoadStringsProperty(const S: string; PropInfo: PPropInfo);
var
  List: TObject;
  Temp: TStrings;
  I, Cnt: Integer;
...
procedure TJvxPropsStorage.LoadComponentProperty(const S: string; PropInfo: PPropInfo);
{$IFDEF WIN32}
var
...
procedure TJvxPropsStorage.LoadClassProperty(const S: string; PropInfo: PPropInfo);
var
  Loader: TJvxPropsStorage;
  I: Integer;
{$IFDEF WIN32}
  Cnt: Integer;
  Recreate: Boolean;
...
procedure TJvxPropsStorage.StoreProperties(PropList: TStrings);
var
  I: Integer;
  Props: TJvxPropInfoList;
begin
...
procedure TJvxPropsStorage.LoadProperties(PropList: TStrings);
var
  I: Integer;
  Props: TJvxPropInfoList;
begin
...
function TJvxPropsStorage.CreateInfoList(AComponent: TComponent; StoredList: TStrings): TStrings;
var
  I: Integer;
  Obj: TComponent;
  Props: TJvxPropInfoList;
  ...
  try
    ...
    for I := 0 to StoredList.Count - 1 do begin
      ...
      if Result.IndexOf(Obj.Name) < 0 then begin
        Props := TJvxPropInfoList.Create(Obj, tkProperties);
        try
...
procedure TJvxPropsStorage.FreeInfoLists(Info: TStrings);
var
  I: Integer;
begin
...
procedure TJvxPropsStorage.LoadObjectsProps(AComponent: TComponent; StoredList: TStrings);
var
  Info: TStrings;
  I, Idx: Integer;
  Props: TJvxPropInfoList;
    ...
    for I := 0 to StoredList.Count - 1 do begin
      if ParseStoredItem(StoredList[I], CompName, PropName) then begin
        ...
        if Idx >= 0 then begin
          ...
          Props := TJvxPropInfoList(Info.Objects[Idx]);
...
procedure TJvxPropsStorage.StoreObjectsProps(AComponent: TComponent; StoredList: TStrings);
var
  Info: TStrings;
  I, Idx: Integer;
  Props: TJvxPropInfoList;
...
function TJvxPropsStorage.CreateStorage: TJvxPropsStorage;
begin
  Result := TJvxPropsStorage.Create;
end;

function TJvxPropsStorage.ReadString(const ASection, Item, Default: string): string;
begin
  if Assigned(FOnReadString) then Result := FOnReadString(ASection, Item, Default)
  else Result := '';
end;

procedure TJvxPropsStorage.WriteString(const ASection, Item, Value: string);
begin
  if Assigned(FOnWriteString) then FOnWriteString(ASection, Item, Value);
end;

procedure TJvxPropsStorage.EraseSection(const ASection: string);
begin
