...
uses Messages, {$IFDEF WIN32} Windows, {$ELSE} WinTypes, WinProcs, {$ENDIF}
  SysUtils, Classes, Graphics, Controls, Forms, StdCtrls, Menus, JvxTimer;

type

{ TJvxImageControl }

  TJvxImageControl = class(TGraphicControl)
  private
    FDrawing: Boolean;
...
{ TJvxAnimatedImage }
  ...
  TJvxAnimatedImage = class(TJvxImageControl)
  private
    FActive: Boolean;
    FGlyph: TBitmap;
    FImageWidth: Integer;
...
uses JvxConst, {$IFDEF Delphi3_Up} JvxHook, {$ENDIF} JvxVCLUtils;
...
type
  TJvxHackBitmap = class(TBitmap)
  protected
    procedure Draw(ACanvas: TCanvas; const Rect: TRect); override;
  end;

procedure TJvxHackBitmap.Draw(ACanvas: TCanvas; const Rect: TRect);
begin
...
begin
  if Hooked then Exit;
  Index := FindVirtualMethodIndex(TJvxHack, @TJvxHack.Draw);
  SetVirtualMethodAddress(TBitmap, Index, @TJvxHackBitmap.Draw);
  Hooked := True;
end;
...
constructor TJvxImageControl.Create(AOwner: TComponent);
begin
...
destructor TJvxImageControl.Destroy;
begin
{$IFDEF Delphi3_Up}
...
procedure TJvxImageControl.Lock;
begin
{$IFDEF Delphi3_Up}
...
procedure TJvxImageControl.Unlock;
begin
{$IFDEF Delphi3_Up}
...
procedure TJvxImageControl.PaintImage;
var
  Save: Boolean;
begin
...
procedure TJvxImageControl.WMPaint(var Message: TWMPaint);
var
  DC, MemDC: HDC;
...
procedure TJvxImageControl.PaintDesignRect;
begin
...
procedure TJvxImageControl.DoPaintControl;
var
  DC: HDC;
begin
{$IFDEF Delphi3_Up}
...
function TJvxImageControl.DoPaletteChange: Boolean;
var
...
procedure TJvxImageControl.PictureChanged;
begin
...
constructor TJvxAnimatedImage.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
...
destructor TJvxAnimatedImage.Destroy;
begin
  Destroying;
  FOnFrameChanged := nil;
  FOnStart := nil;
...
procedure TJvxAnimatedImage.Loaded;
begin
  inherited Loaded;
  ResetImageBounds;
  UpdateInactive;
...
function TJvxAnimatedImage.GetPalette: HPALETTE;
begin
  Result := 0;
  if not FGlyph.Empty then Result := FGlyph.Palette;
end;

procedure TJvxAnimatedImage.ImageChanged(Sender: TObject);
begin
  Lock;
  try
...
procedure TJvxAnimatedImage.UpdateInactive;
begin
...
function TJvxAnimatedImage.TransparentStored: Boolean;
begin
...
procedure TJvxAnimatedImage.SetOpaque(Value: Boolean);
begin
...
procedure TJvxAnimatedImage.SetTransparentColor(Value: TColor);
begin
...
procedure TJvxAnimatedImage.SetOrientation(Value: TGlyphOrientation);
begin
...
procedure TJvxAnimatedImage.SetGlyph(Value: TBitmap);
begin
  Lock;
  try
    FGlyph.Assign(Value);
...
procedure TJvxAnimatedImage.SetStretch(Value: Boolean);
begin
...
procedure TJvxAnimatedImage.SetCenter(Value: Boolean);
begin
...
procedure TJvxAnimatedImage.SetGlyphNum(Value: Integer);
begin
...
procedure TJvxAnimatedImage.SetInactiveGlyph(Value: Integer);
begin
...
procedure TJvxAnimatedImage.SetNumGlyphs(Value: Integer);
begin
  Lock;
  try
    FNumGlyphs := Value;
...
procedure TJvxAnimatedImage.DefineBitmapSize;
begin
  Lock;
  try
    FNumGlyphs := 1;
...
procedure TJvxAnimatedImage.ResetImageBounds;
begin
...
procedure TJvxAnimatedImage.AdjustSize;
begin
...
procedure TJvxAnimatedImage.DoPaintImage;
var
  BmpIndex: Integer;
...
procedure TJvxAnimatedImage.Paint;
begin
  PaintImage;
...
procedure TJvxAnimatedImage.TimerExpired(Sender: TObject);
begin
{$IFDEF Delphi3_Up}
...
procedure TJvxAnimatedImage.FrameChanged;
begin
  if Assigned(FOnFrameChanged) then FOnFrameChanged(Self);
end;

procedure TJvxAnimatedImage.Stop;
begin
  if not (csReading in ComponentState) then
    if Assigned(FOnStop) then FOnStop(Self);
end;

procedure TJvxAnimatedImage.Start;
begin
  if not (csReading in ComponentState) then
...
{$IFNDEF Delphi4_Up}
procedure TJvxAnimatedImage.SetAutoSize(Value: Boolean);
begin
...
{$IFDEF Delphi4_Up}
function TJvxAnimatedImage.CanAutoSize(var NewWidth, NewHeight: Integer): Boolean;
begin
  Result := True;
...
procedure TJvxAnimatedImage.SetInterval(Value: Cardinal);
begin
  FTimer.Interval := Value;
end;

function TJvxAnimatedImage.GetInterval: Cardinal;
begin
  Result := FTimer.Interval;
end;

procedure TJvxAnimatedImage.SetActive(Value: Boolean);
begin
...
{$IFDEF Delphi3_Up}
procedure TJvxAnimatedImage.SetAsyncDrawing(Value: Boolean);
begin
...
procedure TJvxAnimatedImage.WMSize(var Message: TWMSize);
begin
  inherited;
{$IFNDEF Delphi4_Up}
  AdjustSize;
