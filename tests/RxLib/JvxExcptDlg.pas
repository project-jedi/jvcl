...
The Original Code is: JvxExcptDlg.PAS, released on 2002-07-04.
...
{$ELSE WIN32}
  WinProcs, WinTypes, ToolHelp, JvxStr16,
{$ENDIF WIN32}
  Consts, JvxCConst, JvxStrUtils, JvxVCLUtils;
...
begin
  ...
  RxErrorDialog := TJvxErrorDialog.Create(Application);
end;

{ TJvxErrorDialog }

procedure TJvxErrorDialog.ShowException(Sender: TObject; E: Exception);
begin
...
procedure TJvxErrorDialog.ErrorInfo(var LogicalAddress: Pointer;
...
procedure TJvxErrorDialog.ShowError;
var
  S, ModuleName: string;
  P: Pointer;
begin
...
procedure TJvxErrorDialog.SetShowDetails(Value: Boolean);
begin
  DisableAlign;
  try
    if Value then begin
...
procedure TJvxErrorDialog.GetErrorMsg(var Msg: string);
var
  I: Integer;
begin
  I := Pos(CRLF, Msg);
...
{$IFDEF WIN32}
procedure TJvxErrorDialog.WMHelp(var Message: TWMHelp);
var
  AppHelpFile: string;
begin
...
procedure TJvxErrorDialog.FormCreate(Sender: TObject);
begin
{$IFDEF WIN32}
...
procedure TJvxErrorDialog.FormDestroy(Sender: TObject);
begin
  Application.OnException := FPrevOnException;
end;

procedure TJvxErrorDialog.FormShow(Sender: TObject);
var
  S: string;
{$IFDEF WIN32}
  ExStyle: Longint;
...
procedure TJvxErrorDialog.DetailsBtnClick(Sender: TObject);
begin
  SetShowDetails(not Details);
end;

procedure TJvxErrorDialog.FormKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
{$IFDEF WIN32}
var
  Info: THelpInfo;
