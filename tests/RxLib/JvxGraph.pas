...
The Original Code is: JvxGraph.PAS, released on 2002-07-04.
...
uses {$IFDEF WIN32} Windows, {$ELSE} WinTypes, WinProcs, {$ENDIF}
  SysUtils, Classes, Graphics, JvxVCLUtils;
...
type
  TJvxGradient = class(TPersistent)
  private
    FStartColor: TColor;
    FEndColor: TColor;
...
constructor TJvxGradient.Create;
begin
  inherited Create;
...
procedure TJvxGradient.Assign(Source: TPersistent);
begin
  if Source is TJvxGradient then begin
    with TJvxGradient(Source) do begin
...
procedure TJvxGradient.Changed;
begin
  if Assigned(FOnChange) then FOnChange(Self);
end;

procedure TJvxGradient.Draw(Canvas: TCanvas; Rect: TRect);
begin
  GradientFillRect(Canvas, Rect, FStartColor, FEndColor, FDirection,
    FStepCount);
end;

procedure TJvxGradient.SetStartColor(Value: TColor);
begin
...
procedure TJvxGradient.SetEndColor(Value: TColor);
begin
...
procedure TJvxGradient.SetDirection(Value: TFillDirection);
begin
...
procedure TJvxGradient.SetStepCount(Value: Byte);
begin
...
procedure TJvxGradient.SetVisible(Value: Boolean);
begin
