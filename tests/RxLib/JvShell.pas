...
procedure TJvTrayIcon.Notification(AComponent: TComponent;
...
procedure TJvTrayIcon.SendCancelMode;
var
  F: TForm;
begin
...
function TJvTrayIcon.CheckMenuPopup(X, Y: Integer): Boolean;
begin
  Result := False;
...
function TJvTrayIcon.CheckDefaultMenuItem: Boolean;
{$IFDEF WIN32}
var
  Item: TMenuItem;
  I: Integer;
...
function TJvTrayIcon.GetActiveIcon: TIcon;
begin
  Result := FIcon;
...
function TJvTrayIcon.GetAnimated: Boolean;
begin
  Result := FAnimated;
end;
...
procedure TJvTrayIcon.Show;
begin
  Active := True;
end;

procedure TJvTrayIcon.Hide;
begin
  Active := False;
end;

procedure TJvTrayIcon.SetShowDesign(Value: Boolean);
begin
...
procedure TJvTrayIcon.SetInterval(Value: Word);
begin
...
{$IFDEF USE_TIMER}
procedure TJvTrayIcon.Timer(Sender: TObject);
{$ELSE}
procedure TJvTrayIcon.Timer;
{$ENDIF}
begin
...
procedure TJvTrayIcon.SetHint(const Value: string);
begin
...
procedure TJvTrayIcon.UpdateNotifyData;
var
  Ico: TIcon;
begin
...
procedure TJvTrayIcon.ChangeIcon;
var
  Ico: TIcon;
begin
...
procedure TJvTrayIcon.MouseMove(Shift: TShiftState; X, Y: Integer);
begin
  if Assigned(FOnMouseMove) then FOnMouseMove(Self, Shift, X, Y);
end;

procedure TJvTrayIcon.MouseDown(Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  if Assigned(FOnMouseDown) then FOnMouseDown(Self, Button, Shift, X, Y);
end;

procedure TJvTrayIcon.MouseUp(Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  if Assigned(FOnMouseUp) then FOnMouseUp(Self, Button, Shift, X, Y);
end;

procedure TJvTrayIcon.DblClick;
begin
...
procedure TJvTrayIcon.WndProc(var Message: TMessage);
