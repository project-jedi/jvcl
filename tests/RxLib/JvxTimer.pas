...
The Original Code is: JvxTimer.PAS, released on 2002-07-04.
...
uses {$IFDEF WIN32} Windows, {$ELSE} WinTypes, WinProcs, {$ENDIF}
  Messages, SysUtils, Classes, Controls{, JvxComponent};

type

{ TJvxTimer }

  TJvxTimer = class(TComponent)
  private
    FEnabled: Boolean;
    FInterval: Cardinal;
...
type
  ...
  public
    constructor Create(Timer: TJvxTimer; Enabled: Boolean);
  end;

constructor TJvxTimerThread.Create(Timer: TJvxTimer; Enabled: Boolean);
begin
  FOwner := Timer;
...
procedure TJvxTimerThread.HandleException;
begin
...
procedure TJvxTimerThread.Execute;
...
constructor TJvxTimer.Create(AOwner: TComponent);
begin
...
{$IFDEF WIN32}
  ...
  FTimerThread := TJvxTimerThread.Create(Self, False);
{$ELSE}
  FWindowHandle := AllocateHWnd(WndProc);
{$ENDIF}
end;

destructor TJvxTimer.Destroy;
begin
  Destroying;
  FEnabled := False;
  FOnTimer := nil;
{$IFDEF WIN32}
  {TJvxTimerThread(FTimerThread).FOwner := nil;}
...
procedure TJvxTimer.WndProc(var Msg: TMessage);
begin
  with Msg do
...
procedure TJvxTimer.UpdateTimer;
begin
{$IFDEF WIN32}
  if FThreaded then begin
    ...
    TJvxTimerThread(FTimerThread).FInterval := FInterval;
...
procedure TJvxTimer.SetEnabled(Value: Boolean);
begin
...
procedure TJvxTimer.SetInterval(Value: Cardinal);
begin
...
procedure TJvxTimer.SetThreaded(Value: Boolean);
begin
...
procedure TJvxTimer.SetThreadPriority(Value: TThreadPriority);
begin
...
procedure TJvxTimer.Synchronize(Method: TThreadMethod);
begin
  if (FTimerThread <> nil) then begin
    with TJvxTimerThread(FTimerThread) do begin
      if Suspended or Terminated then Method
      else TJvxTimerThread(FTimerThread).Synchronize(Method);
    end;
  end
  else Method;
end;
...
procedure TJvxTimer.SetOnTimer(Value: TNotifyEvent);
begin
