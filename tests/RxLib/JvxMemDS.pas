...
The Original Code is: JvxMemDS.PAS, released on 2002-07-04.
...
uses Windows, SysUtils, Classes, Controls, DB, JvxDBUtils, Variants;
...
type
  ...
  TJvxMemoryRecord = class;
  TLoadMode = (lmCopy, lmAppend);
  TCompareRecords = function (Item1, Item2: TJvxMemoryRecord): Integer of object;

  TJvxMemoryData = class(TDataSet)
  private
    FRecordPos: Integer;
    FRecordSize: Integer;
    ...
    function AddRecord: TJvxMemoryRecord;
    function InsertRecord(Index: Integer): TJvxMemoryRecord;
    function FindRecordID(ID: Integer): TJvxMemoryRecord;
    ...
    function GetMemoryRecord(Index: Integer): TJvxMemoryRecord;
  ...
  protected
    procedure AssignMemoryRecord(Rec: TJvxMemoryRecord; Buffer: PChar);
    function GetActiveRecBuf(var RecBuf: PChar): Boolean; virtual;
    procedure InitFieldDefsFromFields;
    procedure RecordToBuffer(Rec: TJvxMemoryRecord; Buffer: PChar);
    procedure SetMemoryRecordData(Buffer: PChar; Pos: Integer); virtual;
    procedure SetAutoIncFields(Buffer: PChar); virtual;
    function CompareRecords(Item1, Item2: TJvxMemoryRecord): Integer; virtual;
...
{$ENDIF}
    ...
    property Records[Index: Integer]: TJvxMemoryRecord read GetMemoryRecord;
  public
...
{ TJvxMemBlobStream }
  ...
  TJvxMemBlobStream = class(TStream)
  private
    FField: TBlobField;
    FDataSet: TJvxMemoryData;
    FBuffer: PChar;
...
{ TJvxMemoryRecord }
  ...
  TJvxMemoryRecord = class(TPersistent)
  private
    FMemoryData: TJvxMemoryData;
    FID: Integer;
    FData: Pointer;
    FBlobs: Pointer;
    function GetIndex: Integer;
    procedure SetMemoryData(Value: TJvxMemoryData; UpdateParent: Boolean);
  protected
    procedure SetIndex(Value: Integer); virtual;
  public
    constructor Create(MemoryData: TJvxMemoryData); virtual;
    constructor CreateEx(MemoryData: TJvxMemoryData; UpdateParent: Boolean); virtual;
    destructor Destroy; override;
    property MemoryData: TJvxMemoryData read FMemoryData;
...
uses Forms, DbConsts {$IFDEF Delphi5_Up}, ComObj {$ENDIF};
...
const
  ftBlobTypes = [ftBlob, ftMemo, ftGraphic, ftFmtMemo, ftParadoxOle,
    ftDBaseOle, ftTypedBinary {$IFDEF Delphi5_Up}, ftOraBlob, ftOraClob {$ENDIF}];
  ...
  ftSupported = [ftString, ftSmallint, ftInteger, ftWord, ftBoolean, ftFloat,
    ...
    ftVarBytes {$IFDEF Delphi4_Up}, ftADT, ftFixedChar, ftWideString,
    ftLargeint {$ENDIF} {$IFDEF Delphi5_Up}, ftVariant, ftGuid {$ENDIF}] + 
    ftBlobTypes;
...
constructor TJvxMemoryRecord.Create(MemoryData: TJvxMemoryData);
begin
  CreateEx(MemoryData, True);
end;

constructor TJvxMemoryRecord.CreateEx(MemoryData: TJvxMemoryData;
...
destructor TJvxMemoryRecord.Destroy;
begin
  SetMemoryData(nil, True);
  inherited Destroy;
end;

function TJvxMemoryRecord.GetIndex: Integer;
begin
  if FMemoryData <> nil then Result := FMemoryData.FRecords.IndexOf(Self)
  else Result := -1;
end;

procedure TJvxMemoryRecord.SetMemoryData(Value: TJvxMemoryData; UpdateParent: Boolean);
var
  I: Integer;
  DataSize: Integer;
begin
...
procedure TJvxMemoryRecord.SetIndex(Value: Integer);
var
  CurIndex: Integer;
begin
  CurIndex := GetIndex;
...
constructor TJvxMemoryData.Create(AOwner: TComponent);
begin
...
destructor TJvxMemoryData.Destroy;
begin
  inherited Destroy;
  FreeIndexList;
  ClearRecords;
...
function TJvxMemoryData.GetCapacity: Integer;
begin
  if FRecords <> nil then Result := FRecords.Capacity
  else Result := 0;
end;

procedure TJvxMemoryData.SetCapacity(Value: Integer);
begin
  if FRecords <> nil then FRecords.Capacity := Value;
end;

function TJvxMemoryData.AddRecord: TJvxMemoryRecord;
begin
  Result := TJvxMemoryRecord.Create(Self);
end;

function TJvxMemoryData.FindRecordID(ID: Integer): TJvxMemoryRecord;
var
  I: Integer;
begin
  for I := 0 to FRecords.Count - 1 do begin
    Result := TJvxMemoryRecord(FRecords[I]);
    if Result.ID = ID then Exit;
  end;
  Result := nil;
end;

function TJvxMemoryData.InsertRecord(Index: Integer): TJvxMemoryRecord;
begin
  Result := AddRecord;
  Result.Index := Index;
end;

function TJvxMemoryData.GetMemoryRecord(Index: Integer): TJvxMemoryRecord;
begin
  Result := TJvxMemoryRecord(FRecords[Index]);
end;
...
function TJvxMemoryData.BCDToCurr(BCD: Pointer; var Curr: Currency): Boolean;
begin
  Move(BCD^, Curr, SizeOf(Currency));
  Result := True;
end;

function TJvxMemoryData.CurrToBCD(const Curr: Currency; BCD: Pointer; Precision,
...
procedure TJvxMemoryData.InitFieldDefsFromFields;
var
  I: Integer;
  Offset: Word;
begin
...
function TJvxMemoryData.FindFieldData(Buffer: Pointer; Field: TField): Pointer;
var
  Index: Integer;
begin
{$IFDEF Delphi4_Up}
...
function TJvxMemoryData.CalcRecordSize: Integer;
var
  I: Integer;
begin
  Result := 0;
...
procedure TJvxMemoryData.InitBufferPointers(GetProps: Boolean);
begin
...
procedure TJvxMemoryData.ClearRecords;
begin
...
function TJvxMemoryData.AllocRecordBuffer: PChar;
begin
...
procedure TJvxMemoryData.FreeRecordBuffer(var Buffer: PChar);
begin
  if BlobFieldCount > 0 then
...
procedure TJvxMemoryData.ClearCalcFields(Buffer: PChar);
begin
  FillChar(Buffer[FRecordSize], CalcFieldsSize, 0);
end;

procedure TJvxMemoryData.InternalInitRecord(Buffer: PChar);
var
  I: Integer;
begin
...
procedure TJvxMemoryData.InitRecord(Buffer: PChar);
begin
...
function TJvxMemoryData.GetCurrentRecord(Buffer: PChar): Boolean;
begin
  Result := False;
...
procedure TJvxMemoryData.RecordToBuffer(Rec: TJvxMemoryRecord; Buffer: PChar);
var
  I: Integer;
begin
...
function TJvxMemoryData.GetRecord(Buffer: PChar; GetMode: TGetMode;
...
function TJvxMemoryData.GetRecordSize: Word;
begin
  Result := FRecordSize;
end;

function TJvxMemoryData.GetActiveRecBuf(var RecBuf: PChar): Boolean;
begin
  case State of
    dsBrowse:
...
function TJvxMemoryData.GetFieldData(Field: TField; Buffer: Pointer): Boolean;
var
  RecBuf, Data: PChar;
{$IFDEF Delphi5_Up}
  VarData: Variant;
...
begin
  ...
  if Field.FieldNo > 0 then begin
    ...
    if Data <> nil then begin
      ...
      Inc(Data);
      if Field.DataType in [ftString {$IFDEF Delphi4_Up}, ftFixedChar,
        ftWideString {$ENDIF} {$IFDEF Delphi5_Up}, ftGuid {$ENDIF}] then
...
procedure TJvxMemoryData.SetFieldData(Field: TField; Buffer: Pointer);
var
  RecBuf, Data: PChar;
{$IFDEF Delphi5_Up}
  VarData: Variant;
...
procedure TJvxMemoryData.SetFiltered(Value: Boolean);
begin
...
procedure TJvxMemoryData.SetOnFilterRecord(const Value: TFilterRecordEvent);
begin
  if Active then begin
    CheckBrowseMode;
...
function TJvxMemoryData.RecordFilter: Boolean;
var
...
function TJvxMemoryData.GetBlobData(Field: TField; Buffer: PChar): TMemBlobData;
begin
  Result := PMemBlobArray(Buffer + FBlobOfs)[Field.Offset];
end;

procedure TJvxMemoryData.SetBlobData(Field: TField; Buffer: PChar;
  Value: TMemBlobData);
begin
...
procedure TJvxMemoryData.CloseBlob(Field: TField);
begin
...
function TJvxMemoryData.CreateBlobStream(Field: TField; Mode: TBlobStreamMode): TStream;
begin
  Result := TJvxMemBlobStream.Create(Field as TBlobField, Mode);
end;

{ Bookmarks }

function TJvxMemoryData.BookmarkValid(Bookmark: TBookmark): Boolean;
begin
  Result := FActive and (TBookmarkData(Bookmark^) > Low(Integer)) and
    (TBookmarkData(Bookmark^) <= FLastID);
end;

function TJvxMemoryData.CompareBookmarks(Bookmark1, Bookmark2: TBookmark): Integer;
begin
...
procedure TJvxMemoryData.GetBookmarkData(Buffer: PChar; Data: Pointer);
begin
  Move(PMemBookmarkInfo(Buffer + FBookmarkOfs)^.BookmarkData, Data^,
    SizeOf(TBookmarkData));
end;

procedure TJvxMemoryData.SetBookmarkData(Buffer: PChar; Data: Pointer);
begin
  Move(Data^, PMemBookmarkInfo(Buffer + FBookmarkOfs)^.BookmarkData,
    SizeOf(TBookmarkData));
end;

function TJvxMemoryData.GetBookmarkFlag(Buffer: PChar): TBookmarkFlag;
begin
  Result := PMemBookmarkInfo(Buffer + FBookmarkOfs)^.BookmarkFlag;
end;

procedure TJvxMemoryData.SetBookmarkFlag(Buffer: PChar; Value: TBookmarkFlag);
begin
  PMemBookmarkInfo(Buffer + FBookmarkOfs)^.BookmarkFlag := Value;
end;

procedure TJvxMemoryData.InternalGotoBookmark(Bookmark: TBookmark);
var
  Rec: TJvxMemoryRecord;
  SavePos: Integer;
  Accept: Boolean;
begin
...
procedure TJvxMemoryData.InternalSetToRecord(Buffer: PChar);
begin
  InternalGotoBookmark(@PMemBookmarkInfo(Buffer + FBookmarkOfs)^.BookmarkData);
end;

procedure TJvxMemoryData.InternalFirst;
begin
  FRecordPos := -1;
end;

procedure TJvxMemoryData.InternalLast;
begin
...
procedure TJvxMemoryData.AssignMemoryRecord(Rec: TJvxMemoryRecord; Buffer: PChar);
var
  I: Integer;
begin
...
procedure TJvxMemoryData.SetMemoryRecordData(Buffer: PChar; Pos: Integer);
var
  Rec: TJvxMemoryRecord;
begin
  if State = dsFilter then Error(SNotEditing);
...
procedure TJvxMemoryData.SetAutoIncFields(Buffer: PChar);
var
  I, Count: Integer;
  Data: PChar;
begin
...
procedure TJvxMemoryData.InternalAddRecord(Buffer: Pointer; Append: Boolean);
var
  RecPos: Integer;
  Rec: TJvxMemoryRecord;
begin
...
procedure TJvxMemoryData.InternalDelete;
var
  Accept: Boolean;
begin
...
procedure TJvxMemoryData.InternalPost;
var
  RecPos: Integer;
begin
...
procedure TJvxMemoryData.OpenCursor(InfoQuery: Boolean);
begin
...
procedure TJvxMemoryData.InternalOpen;
begin
...
procedure TJvxMemoryData.InternalClose;
begin
  ClearRecords;
  FAutoInc := 1;
  BindFields(False);
...
procedure TJvxMemoryData.InternalHandleException;
begin
  Application.HandleException(Self);
end;

procedure TJvxMemoryData.InternalInitFieldDefs;
begin
end;

function TJvxMemoryData.IsCursorOpen: Boolean;
begin
  Result := FActive;
end;
...
function TJvxMemoryData.GetRecordCount: Integer;
begin
  Result := FRecords.Count;
end;

function TJvxMemoryData.GetRecNo: Integer;
begin
  CheckActive;
  UpdateCursorPos;
...
procedure TJvxMemoryData.SetRecNo(Value: Integer);
begin
...
function TJvxMemoryData.IsSequenced: Boolean;
begin
  Result := not Filtered;
end;

function TJvxMemoryData.Locate(const KeyFields: string;
...
procedure TJvxMemoryData.EmptyTable;
begin
  if Active then begin
    CheckBrowseMode;
    ClearRecords;
...
procedure TJvxMemoryData.CopyStructure(Source: TDataSet);
...
function TJvxMemoryData.LoadFromDataSet(Source: TDataSet; RecordCount: Integer;
  Mode: TLoadMode): Integer;
var
...
function TJvxMemoryData.SaveToDataSet(Dest: TDataSet; RecordCount: Integer): Integer;
var
  MovedCount: Integer;
begin
  Result := 0;
...
procedure TJvxMemoryData.SortOnFields(const FieldNames: string;
{$IFDEF Delphi4_Up}
  CaseInsensitive: Boolean = True; Descending: Boolean = False);
{$ELSE}
  CaseInsensitive, Descending: Boolean);
...
procedure TJvxMemoryData.QuickSort(L, R: Integer; Compare: TCompareRecords);
var
  I, J: Integer;
  P: TJvxMemoryRecord;
begin
...
function TJvxMemoryData.CompareRecords(Item1, Item2: TJvxMemoryRecord): Integer;
var
  Data1, Data2: PChar;
  F: TField;
  I: Integer;
...
function TJvxMemoryData.GetIsIndexField(Field: TField): Boolean;
begin
...
procedure TJvxMemoryData.CreateIndexList(const FieldNames: string);
var
  Pos: Integer;
  F: TField;
begin
...
procedure TJvxMemoryData.FreeIndexList;
begin
  FIndexList.Free;
  FIndexList := nil;
end;
...
constructor TJvxMemBlobStream.Create(Field: TBlobField; Mode: TBlobStreamMode);
begin
  FMode := Mode;
  FField := Field;
  FDataSet := FField.DataSet as TJvxMemoryData;
...
destructor TJvxMemBlobStream.Destroy;
begin
...
function TJvxMemBlobStream.GetBlobFromRecord(Field: TField): TMemBlobData;
var
  Rec: TJvxMemoryRecord;
  Pos: Integer;
begin
...
function TJvxMemBlobStream.Read(var Buffer; Count: Longint): Longint;
begin
  Result := 0;
...
function TJvxMemBlobStream.Write(const Buffer; Count: Longint): Longint;
var
  Temp: TMemBlobData;
begin
  Result := 0;
...
function TJvxMemBlobStream.Seek(Offset: Longint; Origin: Word): Longint;
begin
  case Origin of
    0: FPosition := Offset;
...
procedure TJvxMemBlobStream.Truncate;
begin
  if FOpened and FCached and (FMode <> bmRead) then begin
...
function TJvxMemBlobStream.GetBlobSize: Longint;
begin
  Result := 0;
  if FOpened then
    if FCached then
