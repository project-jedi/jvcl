...
The Original Code is: JvxGrids.PAS, released on 2002-07-04.
...
uses {$IFDEF WIN32} Windows, Registry, {$ELSE} WinTypes, WinProcs, {$ENDIF}
  ...
  Grids, JvxConst, IniFiles, JvxPlacemnt;
...
uses SysUtils, JvxVCLUtils, JvxMaxMin, Consts, JvxAppUtils;
...
type
  TJvxPopupListbox = class;

  TJvxInplaceEdit = class(TInplaceEdit)
  private
    FAlignment: TAlignment;
    FButtonWidth: Integer;
    FPickList: TJvxPopupListbox;
  ...
  protected
    ...
    property PickList: TJvxPopupListbox read FPickList;
  public
...
{ TJvxPopupListbox }
  ...
  TJvxPopupListbox = class(TCustomListbox)
  private
    FSearchText: string;
...
procedure TJvxPopupListbox.CreateParams(var Params: TCreateParams);
begin
...
procedure TJvxPopupListbox.CreateWnd;
begin
  inherited CreateWnd;
...
procedure TJvxPopupListbox.KeyPress(var Key: Char);
var
  TickCount: Integer;
begin
  case Key of
...
procedure TJvxPopupListbox.MouseUp(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
  inherited MouseUp(Button, Shift, X, Y);
  TJvxInplaceEdit(Owner).CloseUp((X >= 0) and (Y >= 0) and (X < Width) and
    (Y < Height));
end;

{ TJvxInplaceEdit }

constructor TJvxInplaceEdit.Create(Owner: TComponent);
begin
...
procedure TJvxInplaceEdit.CreateParams(var Params: TCreateParams);
const
...
procedure TJvxInplaceEdit.BoundsChanged;
var
  R: TRect;
begin
...
procedure TJvxInplaceEdit.CloseUp(Accept: Boolean);
var
  ListValue: string;
begin
...
procedure TJvxInplaceEdit.DoDropDownKeys(var Key: Word; Shift: TShiftState);
begin
  case Key of
    VK_UP, VK_DOWN:
...
procedure TJvxInplaceEdit.DropDown;
const
  MaxListCount = 8;
var
  P: TPoint;
...
procedure TJvxInplaceEdit.KeyDown(var Key: Word; Shift: TShiftState);
begin
...
procedure TJvxInplaceEdit.ListMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
procedure TJvxInplaceEdit.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
...
procedure TJvxInplaceEdit.MouseMove(Shift: TShiftState; X, Y: Integer);
var
  ListPos: TPoint;
...
procedure TJvxInplaceEdit.MouseUp(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
var
  WasPressed: Boolean;
begin
...
procedure TJvxInplaceEdit.PaintWindow(DC: HDC);
const
  LeftOffs = 3;
var
  R: TRect;
...
procedure TJvxInplaceEdit.SetAlignment(Value: TAlignment);
begin
...
procedure TJvxInplaceEdit.SetEditStyle(Value: TInplaceEditStyle);
begin
    ...
    esPickList:
      begin
        if FPickList = nil then begin
          FPickList := TJvxPopupListbox.Create(Self);
...
procedure TJvxInplaceEdit.StopTracking;
begin
  if FTracking then begin
    TrackButton(-1, -1);
    FTracking := False;
...
procedure TJvxInplaceEdit.TrackButton(X,Y: Integer);
var
  NewState: Boolean;
  R: TRect;
begin
...
procedure TJvxInplaceEdit.UpdateContents;
var
...
procedure TJvxInplaceEdit.CMCancelMode(var Message: TCMCancelMode);
begin
  if (Message.Sender <> Self) and (Message.Sender <> FActiveList) then
    CloseUp(False);
end;

procedure TJvxInplaceEdit.WMCancelMode(var Message: TMessage);
begin
  StopTracking;
  inherited;
end;

procedure TJvxInplaceEdit.WMKillFocus(var Message: TMessage);
begin
{$IFDEF Delphi3_Up}
...
procedure TJvxInplaceEdit.WMLButtonDblClk(var Message: TWMLButtonDblClk);
begin
  with Message do
...
procedure TJvxInplaceEdit.WMPaint(var Message: TWMPaint);
begin
  PaintHandler(Message);
end;

procedure TJvxInplaceEdit.WMSetCursor(var Message: TWMSetCursor);
var
  P: TPoint;
begin
  GetCursorPos(P);
...
procedure TJvxInplaceEdit.WndProc(var Message: TMessage);
begin
  case Message.Msg of
...
function TJvxDrawGrid.GetStorage: TJvxFormPlacement;
begin
  Result := FIniLink.Storage;
end;

procedure TJvxDrawGrid.SetStorage(Value: TJvxFormPlacement);
begin
...
begin
  ...
  begin
    if (InplaceEditor <> nil) and InplaceEditor.HandleAllocated and
      InplaceEditor.Visible then TJvxInplaceEdit(InplaceEditor).Change;
...
begin
{$IFDEF WIN32}
  Result := TJvxInplaceEdit.Create(Self);
{$ELSE}
