...
The Original Code is: JvDBUKPost.PAS, released on 2002-07-03.
...
uses
  Windows, Messages, Classes, Controls, Dialogs, DBCtrls, Sysutils, JVCLVer;

type
  TJvDBUKPostCodeEdit = class(TDBEdit)
  private
...
constructor TJvDBUKPostCodeEdit.Create(AOwner: TComponent);
begin
...
procedure TJvDBUKPostCodeEdit.KeyPress(var Key: Char);
var IsAlpha: boolean;
  IsNumber: boolean;
  IsSpace: boolean;
  IsBackSpace: boolean;
...
function TJvDBUKPostCodeEdit.CheckValidPostCode(BlankIsOK: boolean): boolean;
begin
  Result := IsUkpostcode(text, BlankIsOK);
end;

function TJvDBUKPostCodeEdit.CheckValidShortPostCode(BlankIsOK: boolean): boolean;
begin
  Result := IsUkShortPostcode(text, BlankISOK);
end;

procedure TJvDBUKPostCodeEdit.CMExit(var Message: TCMExit);
var IsValid: boolean;
begin
...
function TJvDBUKPostCodeEdit.IsUKShortPostCode(S: string; BlankIsOK: boolean): boolean;
var
  P: string;
begin
...
function TJvDBUKPostCodeEdit.IsUKPostCode(S: string; BlankIsOK: boolean): boolean;
var
  P: string;
begin
  P := getPostCodeFormat(S);
...
function TJvDBUKPostCodeEdit.GetPostCodeFormat(s: string): string;
var
  P: string;
  l: integer;
  i: integer;
