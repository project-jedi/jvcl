...
//    procedure CMMouseLeave(var Message: TMessage); message CM_MOUSELEAVE;
  ...
  protected
    ...
    property ShowASCIISymbols: Boolean read FShowASCIISymbols write SetShowASCIISymbols;
    ...
    property ShowASCIISymbolsDescription: Boolean read FShowASCIISymbolsDescription write SetShowASCIISymbolsDescription;

    // Determines if the CharPanel will be automatically hidden when the control loses focus
    property AutoHidePanel: Boolean read FAutoHidePanel write FAutoHidePanel;
...
type
  ...
  private
    ...
    function  IsASCIISymbol(AChar: WideChar): Boolean; inline;
    function  IsCustomRange: Boolean; inline;
  protected
...
//procedure TJvCustomCharMap.CMMouseLeave(var Message: TMessage);
//begin
//  inherited;
//  PanelVisible := False;
//end;
  ...
  try
    ...
    begin
      ...
      if FShowASCIISymbols and (Cardinal(AChar) <= cASCIISymbolsRange) then
        WideDrawText(Canvas, cASCIISymbols[Cardinal(AChar)], ARect,
          DT_SINGLELINE or DT_CENTER or DT_VCENTER or DT_NOPREFIX)
      else if (FCharRange.FFilter = ufCustomRange1) and Assigned(FOnGetCustomText) then begin
        CharText := AChar;
        FOnGetCustomText(Self, AChar, CharText);
        WideDrawText(Canvas, CharText, ARect,
          DT_SINGLELINE or DT_CENTER or DT_VCENTER or DT_NOPREFIX);
      end
      else
...
begin
  if Value <> FShowASCIISymbols then begin
...
begin
  if Value <> FShowASCIISymbolsDescription then begin
    FShowASCIISymbolsDescription := Value;
    if FShowASCIISymbols and TCharZoomPanel(FCharPanel).Visible then begin
      TCharZoomPanel(FCharPanel).InitSizeNeeded;
      TCharZoomPanel(FCharPanel).Invalidate;
    end;
  end;
...
begin
  inherited;
  if not FAutoHidePanel then Exit;
...
begin
  ...
  if TJvCustomCharMap(Parent).IsValidChar(AChar) then begin
    if IsASCIISymbol(AChar) then begin
      WideDrawText(Canvas, cASCIISymbols[Cardinal(AChar)], R,
        DT_SINGLELINE or DT_CENTER or DT_VCENTER or DT_NOPREFIX);
      Canvas.Font.Height := 18;
      if TJvCustomCharMap(Parent).FShowASCIISymbolsDescription then begin
        R.TopLeft := Point(6, R.Height - Canvas.Font.Height);
        WideDrawText(Canvas, cASCIISymbolsDesc[Cardinal(AChar)], R,
          DT_SINGLELINE or DT_LEFT or DT_VCENTER or DT_NOPREFIX);
      end;
    end
    else if IsCustomRange then begin
      CharText := AChar;
      ...
      WideDrawText(Canvas, CharText, R,
        DT_SINGLELINE or DT_CENTER or DT_VCENTER or DT_NOPREFIX);
    end
    else
